...
typedef union
{
    struct
    {
        ...
        u8 local_terminate;
        u8 terminate_reason;
        u8 terminate_pending; //When receiving a terminate packet, wait 3 interval  before officially disconnecting
    };
...
typedef enum
{
    LINK_LEVEL_ENC_OFF                                      = 0x00,
    LINK_LEVEL_ENC_ON_E0_BR_EDR                             = 0x01,
    LINK_LEVEL_ENC_ON_AES_CCM_LE                            = 0x01,
    LINK_LEVEL_ENC_ON_AES_CCM_BR_EDR                        = 0x02,
    LINK_LEVEL_ENC_PFS_ON_AES_CCM_LE                        = 0x03,
    OPPORTUNISTIC_LINK_LEVEL_ENC_PFS_ON_AES_CCM_LE          = 0x04,
} enc_enable_type_t;
...
_attribute_aligned_(4) typedef struct
{
    ...
    chn_sound_capabilities_t csRemoteSupCap;
#endif
    ...
    u16 acl_snif_task_cnt;
    u16 acl_snif_1stRxOk_cnt;
#endif

#if (LL_FEATURE_ENABLE_FRAME_SPACE_UPDATE)
    fsu_hci_param_t fsu_param;
#endif

#if (LL_FEATURE_ENABLE_HIGHER_DATA_THROUGHPUT)
    hdt_param_t    hdtParam;
#endif

} st_ll_conn_t;

typedef ble_sts_t(*ble_host_cs_handler)(st_ll_conn_t *pAclConn, u8 opcode, u8 *raw);
typedef ble_host_cs_handler(*host_cs_ctrl_callback)(uint8_t code);
extern host_cs_ctrl_callback ll_chn_sounding_get_ctrl_package_handler_cb;
...
#if (LL_FEATURE_ENABLE_FRAME_SPACE_UPDATE)
typedef ble_sts_t (*ll_fsu_ctrl_handler_t)(st_ll_conn_t *, u8, u8 *);
extern ll_fsu_ctrl_handler_t ll_fsu_ctrl_handler;
#endif
...
void rf_txWait_txSettle_handle(u8 acl_role);
...
static inline u8 blt_llms_get_connEffectiveMaxTxTime_by_connIdx(u16 conn_idx)
{
#if (LL_FEATURE_ENABLE_LE_DATA_LENGTH_EXTENSION)
    return blms[conn_idx].ext_data.connEffectiveMaxTxTime;
#else
    (void)conn_idx;
    return LL_PDU_TIME_1M(27);
#endif
}
...
static inline u8 blt_llms_get_connEffectiveMaxRxTime_by_connIdx(u16 conn_idx)
{
#if (LL_FEATURE_ENABLE_LE_DATA_LENGTH_EXTENSION)
    return blms[conn_idx].ext_data.connEffectiveMaxRxTime;
#else
    (void)conn_idx;
    return LL_PDU_TIME_1M(27);
#endif
...
void blt_ll_sendPhyUpdateInd_V2(u16 connHandle);
...
_always_inline int blt_acl_checkSleepReady(void)
{
    // Check if the RX FIFO queue is empty (read pointer == write pointer)
    // and there are no pending events (connect, disconnect, connection update, or PHY update)
    if (blt_rxfifo.rptr == blt_rxfifo.wptr &&
        // No connection event
        !blmsParam.connectEvt_mask &&
        // No disconnect event
        !blmsParam.disconnEvt_mask &&
        // No connection update event
        !blmsParam.conupdtEvt_mask &&
        // No PHY update event
        !blmsParam.phyupdtEvt_mask) {
