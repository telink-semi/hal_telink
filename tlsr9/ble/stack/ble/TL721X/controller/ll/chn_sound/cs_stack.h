...
#ifndef LOCK_AGC_LEVEL
    #define LOCK_AGC_LEVEL 0
#endif

#define AGC_LEVEL 4

#ifndef GOOGLE_LR20_CALI_EN
    #define GOOGLE_LR20_CALI_EN 0
#endif
...
//#if(Google_SRS), if Google_SRS default close when release SDK, delete it to save ram size
    u8 srs_filteredChnArray[72];
    u8 srs_chn_en_num;
    u8 srs_chn_en;
    u16 rfu2;
//#endif
...
typedef int (*set_ant_param_cb)(cs_config_t *csCfg, u8 sync_ant_value);
typedef int (*set_ant_permutation_cb)(unsigned int val);


typedef struct {
    set_ant_param_cb        set_ant_param_func;
    set_ant_permutation_cb  set_ant_permu_func;

    u32 ant_ctl_default;
    u32 ant_ctrl_seq_base;

} antenna_control_func_t;
...
slip_window_step_t *blt_cs_getNextSlipWindow(void);
...
int  blt_cs_subevent_post(unsigned char procedure_end_flag);
...
#define DMA_CS_RFRX_DMA_LEN(p)     (p[DMA_CS_RFRX_OFFSET_DMA_LEN] + (p[DMA_CS_RFRX_OFFSET_DMA_LEN + 1] << 8))
    ...
    #define CS_RX_HD_EXT_LEN        52
#else
    #define CS_RX_HD_EXT_LEN        44
#endif
#define MODE0_REFLETOR_DMA_LEN         (CS_RX_HD_EXT_LEN + 5)

#if (CHIP_TYPE == CHIP_TYPE_TL721X)||(CHIP_TYPE == CHIP_TYPE_TL322X)
    #define DMA_CS_RFRX_IQ_DATA_LEN(p) (DMA_CS_RFRX_DMA_LEN(p) - CS_RX_HD_EXT_LEN)
#else
    #define DMA_CS_RFRX_IQ_DATA_LEN(p) (DMA_CS_RFRX_DMA_LEN(p) - CS_RX_HD_EXT_LEN)
#endif
...
typedef struct __attribute__((packed))
{
    ...
    u8                rsvd[1];                 //53 SW //61
#if (CHIP_TYPE == CHIP_TYPE_TL721X)||(CHIP_TYPE == CHIP_TYPE_TL322X)
    u8 step_RttSSPos[2]; //only sounding sequence use it. for 96 bit, max 2 mark position. only two type:32 bit and 96 bit.
    u8 step_RttSeq[16];  //max 128bit = 16B; either sounding sequence or random sequence.//when subevent start, for(jumpStep){calculate}
#endif
} cs_rx_para_t;
...
typedef struct CS_BUFF_FLAG            cs_rx_flag;

#define CS_FIELD_MODE_MASK                   0x03
...
#if (CHIP_TYPE == CHIP_TYPE_TL721X)||(CHIP_TYPE == CHIP_TYPE_TL322X)
    #define CS_TX_PATH_DELAY_CS_1M                 3
    #define CS_TX_PATH_DELAY_CS_2M                 2
    #define CS_TX_LL_DEALY_CS_1M                   2
    #define CS_TX_LL_DEALY_CS_2M                   2
#else
    #define CS_TX_PATH_DELAY_CS_1M                 3
    #define CS_TX_PATH_DELAY_CS_2M                 2
    #define CS_TX_LL_DEALY_CS_1M                   2
...
#define CS_RFRXEN_MODE_1M_EARLY_US             5            //5us invalid tone
#define CS_RFRXEN_MODE_2M_EARLY_US             5

#define CS_RF_TX_1M_PACKET_EARLY_US_MODE0      (CS_TX_PATH_DELAY_CS_1M + CS_TX_LL_DEALY_CS_1M + CS_RF_RX_1M_EXTRA_PREAMBLE_US)       //8us(1 byte extra preamble) + 5us(1M PHY TXLLDLY + TXPATHDLY)
#define CS_RF_TX_1M_PACKET_EARLY_US_MODE1      (CS_TX_PATH_DELAY_CS_1M + CS_TX_LL_DEALY_CS_1M )         //mode1 no need extra 1byte preamble
#define CS_RF_TX_1M_TONE_EARLY_US              (CS_TX_PATH_DELAY_CS_1M + CS_TX_LL_DEALY_CS_1M )         //5us(1M PHY TXLLDLY + TXPATHDLY)

#define CS_RF_TX_2M_PACKET_EARLY_US_MODE0      (CS_TX_PATH_DELAY_CS_2M + CS_TX_LL_DEALY_CS_2M)
#define CS_RF_TX_2M_PACKET_EARLY_US_MODE1      (CS_TX_PATH_DELAY_CS_2M + CS_TX_LL_DEALY_CS_2M)
#define CS_RF_TX_2M_TONE_EARLY_US              (CS_TX_PATH_DELAY_CS_2M + CS_TX_LL_DEALY_CS_2M) //todo

#define CS_MODE0_RX_EARLY_US_1M                20//enough receive margin for mode0.
...
#define MODE1_FINE_RTT        1
#define MAX_MODE1_SEQ_BITS    (128)
#define MAX_MODE1_SEQ_US      (MAX_MODE1_SEQ_BITS)

#define IQ_CAL_MODE1_AA_ONLY  (4*(CS_1M_PACKET_AA_ONLY_US  + CS_RF_RX_1M_WINDOW_EXTEND_US))
#define IQ_CAL_START          (4*(CS_RFRXEN_MODE_1M_EARLY_US)*2)
#define MODE1_MEDIAN_POS      (161)
#define MODE1_SEARCH_RANGE    (6)

static inline void get_pnSeq(int *pnSeq, unsigned char *data, int pnSeqLen)
{
    for(int i = 0; i < pnSeqLen; i++)
    {
        pnSeq[i] = (data[i>>3] >> (i & 7)) & 1;
...
extern u8 CS_tx_early_1M_phy[3];
extern u8 CS_tx_early_2M_phy[3];
...
extern antenna_control_func_t ant_ctrl_cfg;
