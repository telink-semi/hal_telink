 ...
 */
_attribute_ram_code_com_sec_noinline_ unsigned char adc_get_irq_status_dma(void)
{
 ...
 * @note      The code is placed in the ram code section, in order to shorten the time.
 */
_attribute_ram_code_com_sec_noinline_ void adc_clr_irq_status_dma(void)
{
    /*
 ...
 * @note    The reference voltage value of ADC Vref decreases with time, which can be avoided by calling this interface.
 */
_attribute_ram_code_com_sec_noinline_ void adc_anti_aging_mode_ramcode_for_asm(void)
{
    //The following logic is equivalent to adc_reset()
    reg_rst3 &= (~FLD_RST3_SARADC);
    reg_rst3 |= FLD_RST3_SARADC;
