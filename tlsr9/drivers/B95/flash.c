 ...
 *  If tRES1 is greater than 150us, this flash model cannot be used, because the chip hardware boot program only waits for 150us.
	...
	P25Q80SU	  0x4b		0x146085	  PUYA		8us		16ms
	P25Q16SU	  0x4b		0x156085	  PUYA		8us		16ms
 */
unsigned int flash_support_mid[] = {0x146085, 0x156085};
const unsigned int FLASH_CNT = sizeof(flash_support_mid)/sizeof(*flash_support_mid);



_attribute_data_retention_sec_ flash_handler_t flash_read_page = flash_read_data;
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_text_sec_ unsigned int flash_read_mid_dev(mspi_slave_device_num_e device_num)
{
...
{
	return flash_read_mid_dev(SLAVE0);
}
	...
	*flash_mid = flash_read_mid(device_num);
	...
	for(i=0; i<FLASH_CNT; i++){
		if(flash_support_mid[i] == *flash_mid){
			flash_read_uid(device_num, ((FLASH_READ_UID_CMD_GD_PUYA_ZB_TH>>16)&0xff), (unsigned char *)flash_uid);
			break;
		}
	}
	if(i == FLASH_CNT){
