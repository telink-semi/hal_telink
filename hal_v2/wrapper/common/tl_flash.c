/**************************************************************************************************
 * Copyright (c) 2023-2026 Telink Semiconductor (Shanghai) Co., Ltd. ("TELINK")
 * All rights reserved.
 *
...
uint16_t flash_protection_lock_select(uint32_t addr)
{
	...
	unsigned int flash_lockBlock_cmd =
		flash_change_app_lock_block_to_flash_lock_block(FLASH_LOCK_FW_LOW_1M);
	...
	if(addr == FLASH_1M_ADR_OFFSET) {
		...
		flash_lockBlock_cmd =
			flash_change_app_lock_block_to_flash_lock_block(app_lockBlock);
...
{
	/*
	 * suppose we will operate lock area, it will do ota,
	 * will unlock first and not lock again until it will reboot.
	 */
	if(offset < FLASH_ADR_OFFSET_SELECT){
		flash_unlock();
	}
