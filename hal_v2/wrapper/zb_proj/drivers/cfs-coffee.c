 ...
 * @file    cfs-coffee.c
 *
 * @brief   This is the source file for cfs-coffee
 *
 * @author  Zigbee Group
 * @date    2021
 *
...
/*---------------------------------------------------------------------------*/
extern void cfs_flash_write(u32 addr, u32 len, u8 *buf);
extern void cfs_flash_read(u32 addr, u32 len, u8 *buf);

static void write_header(struct file_header *hdr, coffee_page_t page)
{
    hdr->flags |= HDR_FLAG_VALID;
    COFFEE_WRITE((u8*)hdr, (u32)sizeof(*hdr), (u32)page * COFFEE_PAGE_SIZE);
}

/*---------------------------------------------------------------------------*/
static void read_header(struct file_header *hdr, coffee_page_t page)
{
    COFFEE_READ((u8*)hdr, (u32)sizeof(*hdr), (u32)page * COFFEE_PAGE_SIZE);
#if DEBUG
...
/*---------------------------------------------------------------------------*/
static void collect_garbage(int mode)
{
    uint16_t sector;
...
/*---------------------------------------------------------------------------*/
static int remove_by_page(coffee_page_t page, int remove_log, int close_fds, int gc_allowed)
{
    struct file_header hdr;
    int i;
...
static int get_record_index(coffee_page_t log_page, uint16_t search_records, uint16_t region)
{
    ...
    batch_size = search_records > COFFEE_LOG_TABLE_LIMIT ? COFFEE_LOG_TABLE_LIMIT : search_records;
    processed = 0;
    match_index = -1;

    {
        ...
        while (processed < search_records && match_index < 0) {
            ...
            COFFEE_READ((u8*)&indices, sizeof(indices[0]) * batch_size, base);
...
static int read_log_page(struct file_header *hdr, int16_t record_count, struct log_param *lp)
{
    ...
    base += lp->offset;
    COFFEE_READ((u8*)lp->buf, lp->size, base);
...
static int merge_log(coffee_page_t file_page, int extend)
{
    ...
    do {
        ...
        if (n < 0) {
            ...
            COFFEE_WRITE((u8*)buf, n, absolute_offset(new_file->page, offset));
            offset += n;
        }
    } while(n != 0);
...
static int find_next_record(struct file *file, coffee_page_t log_page, int log_records)
{
...

    {
        ...
        for (processed = 0; processed < log_records; processed += batch_size) {
            ...
            COFFEE_READ((u8*)&indices, batch_size * sizeof(indices[0]),
...

    {
        ...
        if ((lp->offset > 0 || lp->size != log_record_size) &&
           read_log_page(&hdr, log_record, &lp_out) < 0) {
            COFFEE_READ((u8*)copy_buf, sizeof(copy_buf),
        ...
        ++region;
        COFFEE_WRITE((u8*)&region, sizeof(region),
                     (offset + log_record * sizeof(region)));

        offset += log_records * sizeof(region);
        COFFEE_WRITE((u8*)copy_buf, sizeof(copy_buf),
  ...
  /* Attempt to extend the file if we try to write past the end. */
    ...
    if (FILE_MODIFIED(file) || fdp->offset < file->end) {
        ...
        if (need_dummy_write) {
             ...
             */
            COFFEE_WRITE((u8*)dummy, 1, absolute_offset(file->page, fdp->offset - 1));
        }
    } else {
#endif /* COFFEE_MICRO_LOGS */
        ...
        COFFEE_WRITE((u8*)buf, size, absolute_offset(file->page, fdp->offset));
        fdp->offset += size;
#if COFFEE_MICRO_LOGS
    }
#endif /* COFFEE_MICRO_LOGS */
...
int cfs_coffee_configure_log(const char *filename, unsigned log_size, unsigned log_record_size)
{
    ...
    if (log_record_size == 0 ||
        log_record_size > COFFEE_PAGE_SIZE ||
