...
void app_Init_CS_Multi_ANT_Switch(void){
	...
	rf_cs_ant_switch_pin_init(ant_switch_ctrl,
		sizeof(ant_switch_ctrl)/sizeof(rf_cs_ant_switch_ctrl));
}
#endif
/**
...
void app_channel_sounding_init(void)
{
	...
	blc_ll_initCsTransportRxFifo(app_cs_transport_rx_buf, CS_TRANSPORT_RX_FIFO_SIZE,
		CS_TRANSPORT_RX_FIFO_NUM);
	...
	blc_cs_initStepDRBGInfo(app_cs_stepDRBGInfoBuffer,
		CS_STEP_DRBG_INFO_SIZE, APP_CS_CONFIG_NUM);

#if (ANTENNA_SWITCHING_AUTO_EN)
	/* Initialize multi-antennas */
	blc_cs_antenna_switch_config_init(&ant_cfg);
	rf_cs_ant_switch_pin_init(ant_switch_ctrl,
		sizeof(ant_switch_ctrl)/sizeof(rf_cs_ant_switch_ctrl));
#endif
	...
	/*
	 * Init algorithm-4 estimate buffer.
	 * return 0:success; -1:buffer null; -2:tag number invalid
	 */
