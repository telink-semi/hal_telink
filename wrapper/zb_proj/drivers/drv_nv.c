...
static const struct device *rand_device =	DEVICE_DT_GET(DT_CHOSEN(zephyr_entropy));
...
bool flash_writeWithCheck(u32 addr, u32 len, u8 *buf)
{
    ...
    flash_write(flash_device,addr, buf, len);

    while (toalLen > 0) {
        wLen = (toalLen > copyLen) ? copyLen : toalLen;
         flash_read(flash_device,sAddr, pTemp, wLen);
...
static nv_sts_t nv_sect_info_check(u8 sect, u16 id)
{
    ...
    flash_read(flash_device,sectStartAddr, (u8*)&s, sizeof(nv_sect_info_t));
...
static nv_sts_t nv_index_read_op(u16 id, u8 itemId, u16 *itemLen, u8 opSect, u32 totalItemNum, u16 *idxNo, bool lenCheck)
{
    ...
    while (idxTotalNum > 0) {
        ...
        flash_read(flash_device,idxStartAddr, (u8 *)idxInfo, readIdxNum * sizeof(nv_info_idx_t));
...
nv_sts_t nv_sector_read(u16 id, u8 sectTotalNum, nv_sect_info_t *sectInfo)
{
    ...
    for (i = 0; i < sectTotalNum; i++) {
        flash_read(flash_device,moduleStartAddr, (u8 *)&s, sizeof(nv_sect_info_t));
...
nv_sts_t nv_write_item(u8 single, u16 id, u8 itemId, u8 opSect, u16 opItemIdx, u16 len, u8 *buf, bool isFlashCopy)
{
    ...
    if (opItemIdx == 0) {
        ...
        flash_read(flash_device,idxStartAddr + (opItemIdx - 1)*sizeof(nv_info_idx_t), (u8*)&idxInfo, sizeof(nv_info_idx_t));

        /* check item idex */
        flash_read(flash_device,idxInfo.offset, (u8 *)&hdr, sizeof(itemHdr_t));
    ...
    if (isFlashCopy) {
        ...
        flash_read(flash_device,sAddr, (u8 *)&hdr, sizeof(itemHdr_t));
        ...
        flash_write(flash_device,payloadAddr, buf, len);
        ...
        while (tLen > 0) {
            ...
            flash_read(flash_device,payloadAddr, vCheck, unitLen);
...
nv_sts_t nv_flashReadByIndex(u8 id, u8 itemId, u8 opSect, u16 opIdx, u16 len, u8 *buf)
{
    ...
    flash_read(flash_device,idxStartAddr + opIdx * sizeof(nv_info_idx_t), (u8 *)&idx, sizeof(nv_info_idx_t));
    if (!ITEM_VALID_FLAG_CHECK(idx.usedState)) {
        return NV_ITEM_NOT_FOUND;
    }

    itemHdr_t hdr;
    flash_read(flash_device,idx.offset, (u8*)&hdr, sizeof(itemHdr_t));
    ...
    if (lenMatch && (ITEM_HDR_VALID_CHECK(hdr.used)) && hdr.itemId == itemId) {
        flash_read(flash_device,idx.offset + sizeof(itemHdr_t), buf, realLen);
        ...
        if (hdr.used == ITEM_HDR_FIELD_VALID_CHECKSUM) {
            ...
            if (realLen < hdr.size) {
                ...
                while (leftSize > 0) {
                    ...
                    flash_read(flash_device,readOft, pTemp, opLen);
...
nv_sts_t nv_itemDeleteByIndex(u8 id, u8 itemId, u8 opSect, u16 opIdx)
{
    ...
    flash_write(flash_device,idxStartAddr + opIdx * sizeof(nv_info_idx_t) + staOffset, &(idx.usedState), 1);
    return ret;
}
...
static void nv_exceptionDataHandler(bool single, u8 opSect, u16 id, u8 itemId, u16 len)
{
    ...
    s16 opIdx = 0;
    nv_info_idx_t idx;
    u8 *idxTbl = (u8 *)malloc(idxTotalNum);
    opIdx = idxTotalNum - 1;
    bool add = 0;
    u32 idxStartAddr = MODULE_IDX_START(id, opSect);
...
static nv_sts_t nv_flashWriteNewHandler(bool forceChgSec, u8 single, u16 id, u8 itemId, u16 len, u8 *buf)
{
    ...
    if (ret != NV_SUCC) {
        ...
        for (s32 j = 0; j < NV_SECTOR_SIZE(id) / FLASH_SECTOR_SIZE; j++) {
            flash_erase(flash_device,moduleStartAddr + j * FLASH_SECTOR_SIZE,FLASH_SECTOR_SIZE);
        }
    } else {
        opSect = sectInfo.opSect;
    }
    ...
    if (forceChgSec) {
        ...
        if (ret == NV_SUCC) {
            flash_read(flash_device,idxStartAddr + wItemIdx * sizeof(nv_info_idx_t), (u8 *)idxInfo , sizeof(nv_info_idx_t));
    ...
    if (sectorUpdate) {
        ...
        for (s32 k = 0; k < nv_realSectNum; k++) {
            // flash_erase(flash_device,moduleStartAddr + opSect * FLASH_SECTOR_SIZE,FLASH_SECTOR_SIZE);
            flash_erase(flash_device,eraseAddr,FLASH_PAGE_SIZE);
            ...
            while (idxTotalNum > 0 && search) {
                ...
                for (i = 0; i < readIdxNum; i++) {
                    ...
                    if (ITEM_VALID_FLAG_CHECK(idxInfo[i].usedState) &&
                        ...
                        flash_read(flash_device,idxInfo[i].offset, (u8 *)&hdr, sizeof(itemHdr_t));
                        if (hdr.used == ITEM_HDR_FIELD_VALID_CHECKCRC) {
                            u32 crcIdx = 0xffffffff;
...
nv_sts_t nv_nwkFrameCountSearch(u8 id, u8 opSect, u32 *frameCount, u32 *validAddr)
{
    ...
    *validAddr = FRAMECOUNT_PAYLOAD_START(opSect);
    ...
    for (i = 0; i < pageUsed - 1; i++) {
        flash_read(flash_device,(u32)opPageAddr, (u8 *)checkValid, 4);
    ...
    while (checkSize) {
        ...
        flash_read(flash_device,(u32)opPageAddr, (u8 *)checkValid, readSize);
        if (checkValid[0] != 0xffffffff) {
...
nv_sts_t nv_nwkFrameCountSaveToFlashHandler(bool forceChgSec, u8 *errSect, u32 frameCount)
{
    ...
    if (forceChgSec) {
        ...
        for (s32 k = 0; k < nv_realSectNum; k++) {
            ...
            flash_erase(flash_device,eraseAddr,FLASH_SECTOR_SIZE);
            eraseAddr += FLASH_SECTOR_SIZE;
        }
    } else {
        ret = nv_sector_read(id, MODULE_SECTOR_NUM, &sectInfo);
    ...
    if (ret == NV_SUCC) {
        *frameCount = lastFrmCnt;
        ...
        if (wAddr > FRAMECOUNT_PAYLOAD_START(opSect) + 8) {
            u32 pCnt[2];
            flash_read(flash_device,wAddr-8, (u8 *)pCnt, 8);
...
bool nv_facrotyNewRstFlagCheck(void)
{
    ...
    flash_read(flash_device,CFG_FACTORY_RST_CNT, &flag, 1);
...
{
    flash_erase(flash_device,CFG_FACTORY_RST_CNT,1);
}


const u8 startIEEEAddr[] = {0x38, 0xc1, 0xa4};
void generateMacAddr(u8 *outAddr)
{
    u8 addr[8];
    flash_read(flash_device,CFG_MAC_ADDRESS, addr, 8);

    if (ZB_IEEE_ADDR_IS_INVALID(addr)) {
        if (!drv_get_primary_ieee_addr(addr)) {//no primary ieee addr
            unsigned int t0 = clock_time();
            u32 jitter = 0;
            do {
                // jitter = drv_u32Rand() % 0x0fff;
                entropy_get_entropy(rand_device, (u8 *)&jitter, sizeof(jitter));
                jitter %= 0x0fff;
            } while(jitter == 0);
            while(!clock_time_exceed(t0, jitter));
            entropy_get_entropy(rand_device, (u8 *)&addr, 5);
            // drv_generateRandomData(addr, 5);
            memcpy(addr + 5, startIEEEAddr, 3);
        }

        flash_write(flash_device,CFG_MAC_ADDRESS, addr + 2, 6);
        flash_write(flash_device,CFG_MAC_ADDRESS + 6, addr, 2);

        u8 buf[8];
        flash_read(flash_device,CFG_MAC_ADDRESS, buf, 8);
        ...
        if ((addr[3] == 0x38 && addr[4] == 0xC1 && addr[5] == 0xA4) || \
            ...
            flash_read(flash_device,CFG_MAC_ADDRESS, addr + 2, 6);
            flash_read(flash_device,CFG_MAC_ADDRESS + 6, addr, 2);
        }
    }

    ZB_IEEE_ADDR_COPY(outAddr, addr);
}