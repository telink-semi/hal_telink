...
int app_le_adv_report_event_handle(u8 *p)
{
    ...
    if ((central_auto_connect || user_manual_pairing) && adv_name_match) {
        ...
        if (status == BLE_SUCCESS) { //create connection success
            tlkapi_send_string_data(APP_LOG_EN, "[APP] create connection success", pa->mac, 6);
        }
    }
...
int app_host_event_callback(u32 h, u8 *para, int n)
{
    ...
    #if (ACL_CENTRAL_SMP_ENABLE)
        if (dev_char_get_conn_role_by_connhandle(p->connHandle) == ACL_ROLE_CENTRAL) {
            snif_main_node_connection_setup(p->connHandle, ACL_ROLE_CENTRAL);
            if (central_smp_pending == p->connHandle) {
                central_smp_pending = 0;
            }
        }
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    blc_hci_le_setEventMask_cmd(HCI_LE_EVT_MASK_ENHANCED_CONNECTION_COMPLETE | HCI_LE_EVT_MASK_ADVERTISING_REPORT | HCI_LE_EVT_MASK_CONNECTION_UPDATE_COMPLETE);
