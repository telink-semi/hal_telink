...
static u16 ring_buf_read(ring_buf_t *ring_buf, u16 length, u8 *buffer)
{
    ...
    if (ring_buf->read_index > ring_buf->write_index) {
        ...
        blc_app_memory_copy(buffer, &ring_buf->buffer[ring_buf->read_index], available, sizeof(ringBuf), 0x14280000 | __LINE__);
    ...
    if (ring_buf->read_index < ring_buf->write_index) {
        ...
        blc_app_memory_copy(buffer, &ring_buf->buffer[ring_buf->read_index], available, sizeof(ringBuf), 0x14290000 | __LINE__);
        ring_buf->read_index = (ring_buf->read_index + available) % ring_buf->size;
        remaining -= available;
        buffer += available;
    }
...
/**
 * @brief       uart1 receive or tx data irq handler.
 * @param[in]   none.
 * @return      none.
 */
...
void app_parse_loop(void)
{
    ...
    if (shellRecvCmdBuf[shellRecvCmdBufIdx] == '\0') {
        ...
        blc_app_memory_set(shellRecvCmdBuf, 0, shellRecvCmdBufIdx + 1, sizeof(shellRecvCmdBuf), 0x15650000 | __LINE__);
