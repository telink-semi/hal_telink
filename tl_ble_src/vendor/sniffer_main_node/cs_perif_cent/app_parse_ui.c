...
/**
 * @brief       advertising enable or disable
 ...
 * @return      none.
 */
static void app_parse_ui_adv(char *argv[], int argc, void *user_data)
{
    (void)user_data;

    if (argc == 0) {
        app_parse_printf("adv <enable|disable>\r\n");
        return;
    }

    if (strcasecmp(argv[0], "enable") == 0) {
        if (blc_ll_setAdvEnable(BLC_ADV_ENABLE) == BLE_SUCCESS){
            app_parse_printf("advertising start!\r\n");
        }
        else {
            app_parse_printf("advertising enable failed\r\n");
        }
    }
    else if (strcasecmp(argv[0], "disable") == 0) {
        if (blc_ll_setAdvEnable(BLC_ADV_DISABLE) == BLE_SUCCESS){
            app_parse_printf("advertising stop!\r\n");
        }
        else {
            app_parse_printf("advertising disable failed\r\n");
        }
    }
    else {
        app_parse_printf("adv not support [%s]", argv[0]);
        app_parse_printf("adv <enable|disable>\r\n");
    }
}
...
int app_parse_foundAdv(u8 *p)
{
    ...
    if (scan_filter_enable) {
        if (memcmp(pa->mac, filter_scan_addr, 6)) {
            return 0;
        }
    }
    ...
    if (index) {
        ...
        while (len) {
            ...
            if (pdata[1] == DT_COMPLETE_LOCAL_NAME) {
                ...
                blc_app_memory_copy(completeName, (pdata) + 2, adLen - 1, sizeof(completeName), 0x15230000 | __LINE__);
        ...
        app_parse_printf("[%02d] %s %s RSSI:%d name:%s\r\n",
                         index,
...
static void app_parse_ui_cs(char *argv[], int argc, void *user_data)
{
    ...
    if (strcasecmp(argv[0], "rst") == 0) {
         ...
         */
        p->Channel_Map[0]         = 0x0;  //   0xfc;// 0xfc;// 0~7
        p->Channel_Map[1]         = 0x0;  //0xff;// 8~15
        p->Channel_Map[2]         = 0x0;  //0x7f;// 16~23
        p->Channel_Map[3]         = 0x0;  //0xf0;//0xf0;// 24~31
        p->Channel_Map[4]         = 0x0;  //0xff,//0xff;// 32~39
        ...
        if (cs_para_ret == BLE_SUCCESS) {
            ...
            app_parse_printf("cs procedureEnable param error: 0x%x\r\n", cs_para_ret);
        }
