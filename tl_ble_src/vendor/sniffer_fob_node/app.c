...
#define WAKEUP_STATUS_PAD_TIMER (WAKEUP_STATUS_TIMER | WAKEUP_STATUS_PAD)

#if (APP_CONTROL_PM_ENABLE)
_attribute_ble_data_retention_ u32 app_pm_leg_adv_start_tick = 0;
_attribute_ble_data_retention_ u32 app_pm_sleep_enter_flag = 0;

void app_fob_node_sleep_process(void);
#endif
...
const u8    tbl_advData[] = {
    ...
    7, DT_COMPLETE_LOCAL_NAME,                 'c', 's', '_', 'T','a','g',
#else
    7, DT_COMPLETE_LOCAL_NAME,                 'c', 's', '_', 'F','o','b',
#endif
    2, DT_FLAGS,                               0x05, // BLE limited discoverable mode and BR/EDR not supported
    3, DT_INCOMPLETE_LIST_16BIT_SERVICE_UUID,  0x5B, 0x18, // incomplete list of 0x185B(Ranging))
};
...
int app_disconnect_event_handle(u8 *p)
{
    ...
    dev_char_info_delete_by_connhandle(pDisConn->connHandle);
        ...
        {
            #if (APP_CS_THREE_POINT_POSITIONING_TAG_EN)
                if (pDisConn->reason == HCI_ERR_RESERVED2) {
                    /* set to past time for go to sleep immediately */
                    app_pm_leg_adv_start_tick = (clock_time() - APP_CONTROL_ENTER_PM_TIMEOUT_MS*SYSTEM_TIMER_TICK_1MS)|1;
                }
                else {
                    app_pm_leg_adv_start_tick = clock_time()|1;
                }
            #else
                app_pm_leg_adv_start_tick = clock_time()|1;
            #endif
    ...
    if (h & HCI_FLAG_EVENT_BT_STD) //Controller HCI event
    {
        ...
        } else if (evtCode == HCI_EVT_LE_META)         //LE Event
        {
            ...
            //------hci le event: LE CS Subevent Result event-------------------
                ...
                memcpy(tempBuff + 3, &pCsSubevent->Subevent_Code, n);
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    tlkapi_debug_init();
    blc_debug_addStackLog(STK_LOG_SMP_LTK | STK_LOG_HCI_CS | STK_LOG_LL_CS);
#endif
...
//////////////////////////// User Configuration for BLE application ////////////////////////////
    ...
    app_channel_sounding_init();
        ...
        app_pm_leg_adv_start_tick = clock_time()|1;
    #endif
//////////////////////////// User Configuration for BLE application  End ////////////////////////////

    #if (APP_CS_THREE_POINT_POSITIONING_TAG_EN)
        tlkapi_send_string_data(APP_FOB_LOG_EN, "[APP][FOB] APP_CS_THREE_POINT_POSITIONING_TAG_EN=1", 0, 0);
    #endif

    #if (APP_CONTROL_PM_ENABLE)
        tlkapi_printf(APP_FOB_LOG_EN, "[APP][FOB] sniffer_fob_node init finished:M%dS%d, enter_sleep_timeout:%dms\n", ACL_CENTRAL_MAX_NUM, ACL_PERIPHR_MAX_NUM, APP_CONTROL_ENTER_PM_TIMEOUT_MS);
    #else
        tlkapi_printf(APP_FOB_LOG_EN, "[APP][FOB] sniffer_fob_node init finished:M%dS%d\n", ACL_CENTRAL_MAX_NUM, ACL_PERIPHR_MAX_NUM);
    #endif
    tlkapi_send_string_data(APP_LOG_EN, "    MAC", mac_public, 6);
    tlkapi_printf(APP_LOG_EN, "    Build time: %s %s\r\n", __DATE__, __TIME__);
}
...
_attribute_ram_code_ void user_init_deepRetn(void)
{
    ...
    #if (APP_CONTROL_PM_ENABLE)
        if(app_pm_sleep_enter_flag){
            blc_ll_setAdvEnable(BLC_ADV_ENABLE);  //ADV enable
            app_pm_leg_adv_start_tick = clock_time()|1;
            app_pm_sleep_enter_flag = 0;

            tlkapi_send_string_data(APP_LOG_EN, "[APP][PM] user_deep_retention_exit", 0, 0);
        }
    #endif
...
#if (APP_CONTROL_PM_ENABLE)
/**
 * @brief      application layer fob node sleep process
 * @param[in]  none
 * @return     none.
 */
void app_fob_node_sleep_process(void)
{
    if(app_pm_leg_adv_start_tick && clock_time_exceed(app_pm_leg_adv_start_tick, APP_CONTROL_ENTER_PM_TIMEOUT_MS*1000))
    {
        blc_ll_setAdvEnable(BLC_ADV_DISABLE); //ADV disable

        if(blc_ll_isBleTaskIdle())
        {
            tlkapi_send_string_data(APP_FOB_LOG_EN, "[APP][PM] user_sleep_enter", 0, 0);
            ...
            app_pm_sleep_enter_flag = 1;

            user_sleep_enter(BLT_EV_FLAG_SLEEP_ENTER, NULL, 0);

            u32 wakeup_src;
            #if (APP_CS_THREE_POINT_POSITIONING_TAG_EN)
                wakeup_src = cpu_sleep_wakeup(DEEPSLEEP_MODE, PM_WAKEUP_PAD, 0);
                //wakeup_src = cpu_sleep_wakeup(DEEPSLEEP_MODE, PM_WAKEUP_PAD|PM_WAKEUP_TIMER, clock_time()+APP_CONTROL_ENTER_PM_TIMEOUT_MS*SYSTEM_TIMER_TICK_1MS);//for test
            #else
                #if (PM_DEEPSLEEP_RETENTION_ENABLE)
                    wakeup_src = cpu_sleep_wakeup(DEEPSLEEP_MODE_RET_SRAM_LOW96K, PM_WAKEUP_PAD, 0);
                #else
                    wakeup_src = cpu_sleep_wakeup(SUSPEND_MODE, PM_WAKEUP_PAD, 0);
                #endif
                //wakeup_src = cpu_sleep_wakeup(DEEPSLEEP_MODE, PM_WAKEUP_PAD, 0);
            #endif

            if(wakeup_src & STATUS_ENTER_SUSPEND){
                rf_drv_ble_init();//mandatory
            }

            user_suspend_exit(BLT_EV_FLAG_SUSPEND_EXIT, (u8 *)&wakeup_src, 1);

            if(((wakeup_src & WAKEUP_STATUS_PAD_TIMER) == WAKEUP_STATUS_PAD) || (wakeup_src == STATUS_GPIO_ERR_NO_ENTER_PM))
            {
                user_gpio_early_wakeup(BLT_EV_FLAG_GPIO_EARLY_WAKEUP, NULL, 0);
            }
            ...
            tlkapi_send_string_data(APP_FOB_LOG_EN, "[APP][PM] user_suspend_exit", 0, 0);
        }
    }
}
#endif
