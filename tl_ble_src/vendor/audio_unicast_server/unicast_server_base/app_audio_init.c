...
static blc_adv_tmapRole_t advTmapRole = {
    ...
    .tmapRole = BLC_TMAP_ROLE_CALL_TERMINAL | BLC_TMAP_ROLE_UNICAST_MEDIA_RECEIVER,
        ...
        #define MY_DEV_NAME_SIZE   32

        #define MY_DEV_NAME_HEADER "leHdst"

typedef struct
{
    blc_adv_ltv_t ltv;
    u8            completeName[MY_DEV_NAME_SIZE];
} my_adv_completeName_t;

my_adv_completeName_t my_advDefCompleteName = {
    .ltv.type = DT_COMPLETE_LOCAL_NAME,
};
    #endif //end of #if (SIHUI_DEBUG_AUDIO)
...
void app_ext_adv_init(void)
{
    ...
    #if (SIHUI_DEBUG_AUDIO)
    char myDeviceName[MY_DEV_NAME_SIZE] = "";
    sprintf(myDeviceName, MY_DEV_NAME_HEADER "-%02X%02X%02X%02X%02X%02X", bltMac.macAddress_public[5], bltMac.macAddress_public[4], bltMac.macAddress_public[3], bltMac.macAddress_public[2], bltMac.macAddress_public[1], bltMac.macAddress_public[0]);

    strncpy(my_advDefCompleteName.completeName, myDeviceName, MY_DEV_NAME_SIZE);
    my_advDefCompleteName.ltv.len = strlen(myDeviceName) + 1;
    blc_svc_setDeviceName(myDeviceName);

    blc_adv_ltv_t *adv_ltvs[] = {
        (blc_adv_ltv_t *)&advDefFlags,
        (blc_adv_ltv_t *)&my_advDefCompleteName,
