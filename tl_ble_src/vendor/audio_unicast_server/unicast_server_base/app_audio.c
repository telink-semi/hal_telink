...
static int app_audio_prfEvtCb(u16 aclHandle, int evtID, u8 *pData, u16 dataLen)
{
    ...
    app_volume_event_callback(aclHandle, evtID, pData, dataLen);
    return 0;
}
...
static void app_event_acl_disconnect(u16 aclHandle, blc_prf_aclDisconnEvt_t *pEvt)
{
    ...
    extern unsigned short app_audio_output_buffer[];
    extern unsigned short app_audio_input_buffer[];
    u32                   irq = irq_disable();
    memset((u8 *)app_audio_output_buffer, 0, 2 * APP_AUDIO_OUTPUT_BUFFER_SIZE);
    memset((u8 *)app_audio_input_buffer, 0, 2 * APP_AUDIO_INPUT_BUFFER_SIZE);
    app_list_free();
    timer_stop(TIMER0);
    irq_restore(irq);
...
{
    blc_vcs_volume_state_t *vol = blc_vcss_getVolState(appCtrl.aclHandle);
    tlkapi_printf(APP_LOG_EN, "app_audio_volume_up volume:%d", vol->volSetting);
    if (vol->volSetting == 255) {
        return;
    }
    vol->volSetting = vol->volSetting + 15;
    if (BLE_SUCCESS == blc_vcss_updateVolSetting(appCtrl.aclHandle, vol->volSetting)) {
        app_audio_volume_set(vol->volSetting);
    }
}
...
{
    blc_vcs_volume_state_t *vol = blc_vcss_getVolState(appCtrl.aclHandle);
    tlkapi_printf(APP_LOG_EN, "app_audio_volume_down volume:%d", vol->volSetting);
    if (vol->volSetting == 0) {
        return;
    }
    vol->volSetting = vol->volSetting - 15;
...
{
    blc_vcs_volume_state_t *vol = blc_vcss_getVolState(appCtrl.aclHandle);
    tlkapi_printf(APP_LOG_EN, "app_audio_mute mute: %d", vol->mute);
    if (vol->mute) {
        return;
    }
    if (BLE_SUCCESS == blc_vcss_updateMuteState(appCtrl.aclHandle, TRUE)) {
        app_audio_mute_set(true);
    }
}
...
{
    blc_vcs_volume_state_t *vol = blc_vcss_getVolState(appCtrl.aclHandle);
    tlkapi_printf(APP_LOG_EN, "app_audio_unmute mute: %d", vol->mute);
    if (!vol->mute) {
        return;
    }
    if (BLE_SUCCESS == blc_vcss_updateMuteState(appCtrl.aclHandle, FALSE)) {
        app_audio_mute_set(false);
    }
}
...
{
    int       i;
    static u8 app_audio_vol = 150;
    for (i = 0; i < sample; i++) {
        p[i] = p[i] * app_audio_vol / 255;
        if (app_audio_vol < appCtrl.volume) {
            app_audio_vol++;
        } else if (app_audio_vol > appCtrl.volume) {
            app_audio_vol--;
        }
    }
}
