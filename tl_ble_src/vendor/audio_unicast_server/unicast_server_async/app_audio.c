...
static const app_audio_evtCb_t unicastCb[] = {
    ...
    {(audio_event_enum)PRF_EVTID_ACL_CONNECT,             (void *)app_event_acl_connect        },
    {(audio_event_enum)PRF_EVTID_ACL_DISCONNECT,          (void *)app_event_acl_disconnect     },
    {(audio_event_enum)AUDIO_EVT_CIS_CONNECT,             (void *)app_event_cis_connect        },
    {(audio_event_enum)AUDIO_EVT_CIS_DISCONNECT,          (void *)app_event_cis_disconnect     },
    {(audio_event_enum)AUDIO_EVT_CIS_REQUEST,             (void *)app_event_cis_request        },
    {(audio_event_enum)PRF_EVTID_SMP_SECURITY_DONE,       (void *)app_event_security_done      },
    /* Event for Client SDP */
    {(audio_event_enum)PRF_EVTID_CLIENT_SDP_FOUND,        (void *)app_event_audio_sdp_found    },
    {(audio_event_enum)PRF_EVTID_CLIENT_SDP_FAIL,         (void *)app_event_audio_sdp_not_found},
    {(audio_event_enum)PRF_EVTID_CLIENT_ALL_SDP_OVER,     (void *)app_event_audio_sdp_over     },
    /* Event for BAP Unicast Server */
    {(audio_event_enum)AUDIO_EVT_BAPUS_CODEC_CONFIGURED,  (void *)app_ep_codec_configured      },
    {(audio_event_enum)AUDIO_EVT_BAPUS_QOS_CONFIGURED,    (void *)app_ep_qos_configured        },
    {(audio_event_enum)AUDIO_EVT_BAPUS_ENABLING,          (void *)app_ep_enabling              },
    {(audio_event_enum)AUDIO_EVT_BAPUS_RECEIVE_STREAMING, (void *)app_ep_receive_streaming     },
    {(audio_event_enum)AUDIO_EVT_BAPUS_SEND_STREAMING,    (void *)app_ep_send_streaming        },
    {(audio_event_enum)AUDIO_EVT_BAPUS_DISABLING,         (void *)app_ep_disabling             },
    {(audio_event_enum)AUDIO_EVT_BAPUS_RELEASING,         (void *)app_ep_releasing             },
};
...
static int app_audio_prfEvtCb(u16 aclHandle, int evtID, u8 *pData, u16 dataLen)
{
    for (u32 i = 0; i < ARRAY_SIZE(unicastCb); i++) {
        if (unicastCb[i].id == (audio_event_enum)evtID) {
...
static void app_event_acl_connect(u16 aclHandle, blc_prf_aclConnEvt_t *pEvt)
{
    (void)aclHandle;                                                               //unused, remove warning
    ...
    app_tone_play(TLK_TONE_CONNECTED);
 ...
 * @return     none.
 */
_attribute_ram_code_ void app_audio_handler(void)
{
...
_attribute_ram_code_ void app_audio_tone_handle_task(void)
{
    ...
    if (!appCtrl.is_tone_codec_cfg) {
        ...
        appCtrl.tone_len = 120 * sizeof(tcodec_type);
        ...
        tlk_codec_config(TLK_CODEC_OUTPUT, TLK_CODEC_FREQ_24000, TLK_CODEC_1_CHANNEL, TLK_CODEC_LINE, (u8 *)app_audio_output_buffer, 2 * APP_AUDIO_OUTPUT_BUFFER_SIZE);
        #else
        tlk_codec_config(TLK_CODEC_OUTPUT, TLK_CODEC_FREQ_24000, TLK_CODEC_2_CHANNEL, TLK_CODEC_LINE, (u8 *)app_audio_output_buffer, 2 * APP_AUDIO_OUTPUT_BUFFER_SIZE);
        #endif
    ...
    appCtrl.tone_len = tlk_tone_get_sample((int16_t *)appCtrl.tone_buff, 120 * sizeof(tcodec_type), 24000);
