...
int app_le_enhanced_connection_complete_event_handle(u8 *p)
{
    ...
    tlkapi_printf(APP_LOG_EN, "### ACL Connected ###");
    if (IS_ASYNC_LEA_LINK(pConnEvt->connHandle)) {
        tlkapi_send_string_data(APP_LOG_EN, "[APP][LEA]connnect complete", (u8 *)&pConnEvt->connHandle, 2);
        appAsyncState       = APP_ASYNC_STATE_CONNECT;
        appCtrl.asyncHandle = pConnEvt->connHandle;
        app_led_state_sync(SYS_STATE_SYNC);
        if (appCtrl.leaRole == ACL_ROLE_PERIPHERAL) {
            tlkapi_send_string_data(APP_LOG_EN, "[APP][LEA]adv disable", 0, 0);
            blc_ll_setExtAdvEnable(BLC_ADV_DISABLE, ADV_HANDLE1, 0, 0); //connect success, disable the adv.
        } else {
            tlkapi_send_string_data(APP_LOG_EN, "[APP][LEA]scan disable", 0, 0);
    ...
    if (pConnEvt->status == BLE_SUCCESS) {
        ...
        if (dev_char_get_conn_role_by_connhandle(pConnEvt->connHandle) == ACL_ROLE_PERIPHERAL) {
            app_led_state_sync(SYS_STATE_PAIR);
        }
    #endif
    }
    return 0;
...
int app_disconnect_event_handle(u8 *p)
{
    ...
    tlkapi_send_string_data(APP_LOG_EN, "[APP][EVT]Disconnection reason", &pDisConn->reason, 1);
    tlkapi_send_string_data(APP_LOG_EN, "[APP][EVT]Disconnection handle", (u8 *)&pDisConn->connHandle, 2);

    if (IS_ASYNC_LEA_LINK(pDisConn->connHandle)) {
        appCtrl.asyncHandle = 0;
        appAsyncState       = APP_ASYNC_STATE_IDLE;
        if (appCtrl.leaRole == ACL_ROLE_PERIPHERAL) {
            tlkapi_send_string_data(APP_LOG_EN, "[APP][LEA]adv enable", 0, 0);
            blc_ll_setExtAdvEnable(BLC_ADV_ENABLE, ADV_HANDLE1, 0, 0); //connect success, disable the adv.
        } else {
            tlkapi_send_string_data(APP_LOG_EN, "[APP][LEA]scan enable", 0, 0);
...
int app_controller_event_callback(u32 h, u8 *p, int n)
{
    ...
    if (h & HCI_FLAG_EVENT_BT_STD) //Controller HCI event
    {
        ...
        } else if (evtCode == HCI_EVT_LE_META)         //LE Event
        {
            ...
            {
                app_le_ext_adv_report_event_handle(p);
            }
    ...
    #if (TLK_LED_ENABLE)
                app_led_state_sync(SYS_STATE_CONNECTED);
    #endif
            }
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    u8 leaCfg[17];
    flash_read_page(LEA_ROLE_4M_FLASH, 1, leaCfg);
    flash_read_page(LEA_SIRK_4M_FLASH, 16, leaCfg + 1);
    smemcpy(appCtrl.sirkCfg, leaCfg + 1, 16);
    //0x20 is central,other is peripheral role
    appCtrl.leaRole = (leaCfg[0] == 0x20 ? ACL_ROLE_CENTRAL : ACL_ROLE_PERIPHERAL);
    tlkapi_printf(APP_LOG_EN, "lea role:%d", appCtrl.leaRole);
    ...
    #if (ACL_CENTRAL_MAX_NUM || ACL_PERIPHR_MAX_NUM)
    ...
    blc_hci_le_setEventMask_cmd(HCI_LE_EVT_MASK_CONNECTION_COMPLETE | HCI_LE_EVT_MASK_ENHANCED_CONNECTION_COMPLETE | HCI_LE_EVT_MASK_EXTENDED_ADVERTISING_REPORT | HCI_LE_EVT_MASK_CONNECTION_UPDATE_COMPLETE | HCI_LE_EVT_MASK_CIS_ESTABLISHED | HCI_LE_EVT_MASK_CIS_REQUESTED);
