...
void app_async_init(void)
{
    if(appCtrl.leaRole == ACL_ROLE_PERIPHERAL)
    {
        ...
        blc_ll_setExtAdvParam( ADV_HANDLE1,         ADV_EVT_PROP_EXTENDED_CONNECTABLE_UNDIRECTED,                   ADV_INTERVAL_30MS,          ADV_INTERVAL_30MS,
...
//              BLT_APP_STR_LOG("[APP]adv para\n", adv_data->data, adv_data->length-1);
        ...
        if(connIndex)
        {
            u32 status = blc_ll_extended_createConnection( INITIATE_FP_ADV_SPECIFY, OWN_ADDRESS_PUBLIC, pExtAdvInfo->address_type, pExtAdvInfo->address, INIT_PHY_1M, \
