...
#include "algorithm/audio_alg/hybrid/gcc10/tlka_hybrid_alg_api.h"
...
void                    app_codec_send_process(void)
{
    ...
    if (appCtrl.mic_reset) {
        ...
        alg_spk_rptr = ((spk_rptr / 2) + APP_AUDIO_OUTPUT_BUFFER_SIZE - 348) % APP_AUDIO_OUTPUT_BUFFER_SIZE; //ALG_AEC_DELAY 360 = 4 * 160 (4 * frames offset) - 160 (mic_rptr offset) - 120 (codec_offset)
...
_attribute_ram_code_ void app_timer0_irq_proc(void)
{
    if (appCtrl.spk_reset) {
        appCtrl.spk_reset = 0;
    }
    ...
    if (offset < 4 || offset > 48) {
        tlkapi_printf(1, "get offset error:0x%x,0x%x,0x%x", writeOffset, readOffset, offset);
        //        APP_DBG_CHN_4_HIGH;

        if (tlk_codec_output_setWriteOffset(readOffset + 26) != TLK_CODEC_SUCCESS) {
            app_list_free();
