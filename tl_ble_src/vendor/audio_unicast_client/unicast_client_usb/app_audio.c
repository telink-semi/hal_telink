...
static const app_audio_evtCb_t unicastCb[] = {
    ...
    {AUDIO_EVT_VCSC_CHANGED_VOLUME_STATE,   (void *)app_vcsc_changedVolState     },
    {AUDIO_EVT_VOCSC_CHANGED_VOLUME_OFFSET, (void *)app_vocsc_changedVolOffset   },
};
...
static void app_event_audio_sdp_over(u16 aclHandle, blc_prf_sdpOverEvt_t *pEvt)
{
    ...
    for (int i = 0; i < audChnInfo.sinkASEsPerSvr; i++) {
        ...
        if (audioRet != AUDIO_ESUCC) {
            ...
            appCtrl.aclParam[acl_index].source.location   = audChnInfo.sinkAudLocAlloc[i];
    ...
    for (int i = 0; i < audChnInfo.srcASEsPerSvr; i++) {
        ...
        if (audioRet != AUDIO_ESUCC) {
            ...
            appCtrl.aclParam[acl_index].sink.location   = audChnInfo.srcAudLocAlloc[i];
...
static int app_vcsc_changedVolState(u16 connHandle, u8 *pData, u16 dataLen)
{
    ...
    tlkapi_printf(APP_LOG_EN, "Conn:%x, volume(min:0, max:255) is %d, muteSate:%s\r\n", connHandle, volState->volumeSetting, volState->mute ? "Mute" : "Unmute");

    static bool mute   = FALSE;
    static u8   volume = 150;

    /* Synchronize information to other servers */
    for (int i = 0; i < ACL_CENTRAL_MAX_NUM; i++) {
        if (conn_dev_list[i].conn_handle == connHandle) {
            continue;
        }
        // index is from 0 to "ACL_CENTRAL_MAX_NUM - 1"
        if (volState->volumeSetting != volume) {
            blc_vcsc_writeSetAbsoluteVol(conn_dev_list[i].conn_handle, volState->volumeSetting);
        } else if (volState->mute != mute) {
            if (volState->mute) {
                blc_vcsc_writeMute(conn_dev_list[i].conn_handle);
            } else {
                blc_vcsc_writeUnmute(conn_dev_list[i].conn_handle);
            }
        }
    }
    volume = volState->volumeSetting;
    mute   = volState->mute;
    return 0;
}
...
static int app_vocsc_changedVolOffset(u16 connHandle, u8 *pData, u16 dataLen)
{
    ...
    tlkapi_printf(APP_LOG_EN, "Conn:%x, vocs index is %d volume offset(min:-255, max:255) is %d,\r\n", connHandle, volOffset->vocsIndex, volOffset->volumeOffset);
