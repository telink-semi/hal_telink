    ...
    if (appCtrl.aclParam[0].sink.sS) {
        ...
        if (pPkt != NULL) {
            ...
            if (ret_lc3 != LC3DEC_OK) {
                ...
                pRaw.renderPoint = pPkt->timestamp + appCtrl.aclParam[0].sink.pD * SYSTEM_TIMER_TICK_1US + AUDIO_UNICAST_CLIENT_MAX_TRANSPORT_LATENCY * SYSTEM_TIMER_TICK_1MS; //tick
                app_list_add(&pRaw);
            }
            APP_DBG_CHN_3_LOW;
        }
