            ...
            while (index < pExtAdvInfo->data_length) {
                ...
                if (adv_data->type == DT_APPEARANCE) {
                    ...
                    if (ServiceUUID == SERVICE_UUID_AUDIO_STREAM_CONTROL) {
                        app_adv_announcement_t *pAnno = (app_adv_announcement_t *)&adv_data->data[2];
                        if (pAnno->announcement_type == BLC_AUDIO_TARGETED_ANNOUNCEMENT && (pAnno->available_audio_context | (BLC_AUDIO_CONTEXT_TYPE_CONVERSATIONAL | BLC_AUDIO_CONTEXT_TYPE_MEDIA))) {
