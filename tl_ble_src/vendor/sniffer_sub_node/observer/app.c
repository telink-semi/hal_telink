...
int app_le_adv_report_event_handle(u8 *p)
{
    ...
    extern u8 mac_table[REMOTE_DEVICE_MAX_NUM][6];
    for (int i = 0; i < REMOTE_DEVICE_MAX_NUM; i++) {
        if (!memcmp(pa->mac, mac_table[i], 6)) {
            user_sniffer_scan_rssi_report((0x80 | i), rssi + 110);
            tlkapi_printf(APP_LOG_EN, "[APP][EVT]  Adv RSSI:%d\n", rssi);
            tlkapi_send_string_data(APP_LOG_EN, "[APP][EVT] Adv report", &pa->mac[0], pa->len + 7);
            return 0;
        }
    }
    #endif
