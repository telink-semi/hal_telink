    ...
    if (h & HCI_FLAG_EVENT_BT_STD) //Controller HCI event
    {
        ...
        if (evtCode == HCI_EVT_LE_META) //LE Event
        {
            ...
            //------HCI LE event: LE CS Subevent Result event-------------------------------
                ...
                snif_sub_node_cs_procedure_subevent_result_event(p);
            }
            //------HCI LE event: LE CS Subevent Result Continue event-------------------------------
                ...
                snif_sub_node_cs_procedure_subevent_result_continue_event(p);
            }
        }
    }
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    tlkapi_debug_customize_usb_id(0x125);
    tlkapi_debug_init();
    blc_debug_addStackLog(STK_LOG_HCI_CS | STK_LOG_LL_CS | STK_LOG_ALGO_CS);
    #endif
    ...
    tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] monitor CS central peripheral demo, BLE lib version(Hex)", lib_ver, 5);
