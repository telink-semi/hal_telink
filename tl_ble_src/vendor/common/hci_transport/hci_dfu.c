...
void DFU_FwDataCmdHandler(u8 *pParam, u32 len)
{
    ...
    if(DFU_Crc32Calc(DFU_CRC_INIT_VALUE, pParam, len) != 0x00000000){
        ...
        {
            u8 bootFlag = 0;
            if(dfuCb.fwOffset == 0 && dataLen >= DFU_FW_FLAG_OFFSET){//TODO: max_payload must >= 8 Bytes
                bootFlag = pParam[DFU_FW_FLAG_OFFSET];
                pParam[DFU_FW_FLAG_OFFSET] = 0xff;
            }
            /* Write data to flash. */
            ...
            if(dfuCb.fwOffset == 0){
                buf[DFU_FW_FLAG_OFFSET] = bootFlag;
            }
...
void DFU_CmdHandler(u8 *pHciTrPkt, u32 len)
{
    ...
    {
        if(paramLen != 6){
            Hci_SendCmdCmplStatusEvt(opcode, HCI_ERR_INVALID_HCI_CMD_PARAMS);
            return;
        }
        u8 *addr = pPkt;
        extern void blc_ll_setBdAddr(u8 bdaddr[6]);
        blc_ll_setBdAddr(addr);

        u32 tmp = 0;
        flash_read_page(flash_sector_mac_address, 4, (u8 *)&tmp);
        if(tmp != 0xFFFFFFFF){
            flash_erase_sector(flash_sector_mac_address);
        }
        flash_write_page(flash_sector_mac_address, BLE_ADDR_LEN, addr);

        Hci_SendCmdCmplStatusEvt(opcode, HCI_SUCCESS);
        break;
    }
    case HCI_OPCODE_VS_SET_TX_PWR:
    {
        s8 power = pPkt[0];

        if(paramLen != 1 || power < 0 || power > 11){
            Hci_SendCmdCmplStatusEvt(opcode, HCI_ERR_INVALID_HCI_CMD_PARAMS);
            return;
        }
    ...
    case HCI_OPCODE_VS_READ_TX_PWR:
    {
        extern u8 hci_findTxPowerIndex(void);
        u8 txPower = hci_findTxPowerIndex();
        u8 buf[2]={0};
        u8 *p = buf;
        UINT8_TO_BSTREAM(p, HCI_SUCCESS);
        UINT8_TO_BSTREAM(p, txPower);
