...
static void app_codec_process(void) //config lc3
{
    if (appCtrl.aclParam.sink.codecOp == APP_CONFIG_CODEC) {
        ...
        if (appCtrl.aclParam.sink.blocks > 1) {
            ...
            int lc3Ret = lc3dec_decode_init_bap(0, codecSettings[paramIndex].frequencyValue, codecSettings[paramIndex].durationValue, codecSettings[paramIndex].frameOctets);
            if (lc3Ret != LC3DEC_OK) {
                tlkapi_printf(APP_LOG_EN, "lc3 decode init fail:0x%x", lc3Ret);
                return;
            }
    ...
    if (appCtrl.aclParam.source.codecOp == APP_CONFIG_CODEC) {
        ...
        if (appCtrl.aclParam.source.blocks > 1) {
            ...
            int lc3Ret = lc3enc_encode_init_bap(0, codecSettings[paramIndex].frequencyValue, codecSettings[paramIndex].durationValue, codecSettings[paramIndex].frameOctets);
            if (lc3Ret != LC3DEC_OK) {
                tlkapi_printf(APP_LOG_EN, "lc3  encode init fail:0x%x", lc3Ret);
                return;
            }
