...
int app_le_adv_report_event_handle(u8 *p)
{
    ...
    if(central_auto_connect || user_manual_pairing){
        ...
        tlkapi_printf(APP_CONTR_EVT_LOG_EN,"[APP][EVT] le_createConnection mac:%02X %02X %02X %02X %02X %02X",\
...
//          if(central_auto_connect || user_manual_pairing)
            {
                tlkapi_printf(APP_CONTR_EVT_LOG_EN,"[APP][EVT] le_createConnection mac:%02X %02X %02X %02X %02X %02X",\
                        pExtAdvInfo->address[0], pExtAdvInfo->address[1], pExtAdvInfo->address[2],\
                        pExtAdvInfo->address[3], pExtAdvInfo->address[4], pExtAdvInfo->address[5]);
...
//  tlkapi_send_string_data(APP_CONTR_EVT_LOG_EN, " Num_Responses", &pPasdr->Num_Responses, 1);
    ...
    for(int i=0; i < pPasdr->Num_Responses; i++){
        ...
        tlkapi_printf(dataStatus != 0xFF && APP_CONTR_EVT_LOG_EN, "[APP][EVT] SE%d-TS%d-TP%d-RSSI%d-RS%d-DS%X",
                pPasdr->Subevent, pPasdr->Tx_Status, txPower, RSSI, responseSlot, dataStatus);
