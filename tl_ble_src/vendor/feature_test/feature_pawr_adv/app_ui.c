...
//  u8 key_buf[8] = {0,0,0,0,0,0,0,0};
    ...
    else if(kb_event.cnt == 1)
    {
        if(key0 >= CR_VOL_UP )  //volume up/down
        {
            ...
            tlkapi_printf(APP_LOG_EN,"le_createConnection_v2 mac:%02X %02X %02X %02X %02X %02X",\
                    ESL_deviceDB[subevent_create_conn].address[0],ESL_deviceDB[subevent_create_conn].address[1],ESL_deviceDB[subevent_create_conn].address[2],\
                    ESL_deviceDB[subevent_create_conn].address[3],ESL_deviceDB[subevent_create_conn].address[4],ESL_deviceDB[subevent_create_conn].address[5]);
            ...
            blc_ll_extended_createConnection_v2(ADV_HANDLE0, ESL_deviceDB[subevent_create_conn].set, /* adv_handle, subevent */
                                                ...
                                                ESL_deviceDB[subevent_create_conn].type, ESL_deviceDB[subevent_create_conn].address, /* peerAdrType, *peerAddr */
                                                0, /* init_phys */
                                                0, 0, 0, 0, 0, /* scanInter_0, scanWindow_0, conn_min_0, conn_max_0, timeout_0 */
