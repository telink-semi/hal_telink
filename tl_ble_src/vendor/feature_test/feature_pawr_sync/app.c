...
int app_le_connection_complete_event_handle(u8 *p)
{
    ...
    tlkapi_printf(APP_PAWR_EVT_LOG_EN,"[APP][EVT] le_connection_complete connHandle:%04X mac:%02X %02X %02X %02X %02X %02X",\
...
int app_le_periodic_adv_sync_established_event_handle(u8 *p)
{
    ...
    tlkapi_printf(APP_PAWR_EVT_LOG_EN,"[APP][EVT]periodic_adv_sync_established syncHandle:%04X mac:%02X %02X %02X %02X %02X %02X",\
...
int app_le_periodic_adv_sync_transfer_received_event_handle(u8 *p)
{
    ...
    tlkapi_printf(APP_PAWR_EVT_LOG_EN,"[APP][EVT] periodic_adv_sync_established syncHandle:%04X mac:%02X %02X %02X %02X %02X %02X",\
                                        pPastEvt->syncHandle,pPastEvt->advAddr[0],pPastEvt->advAddr[1],pPastEvt->advAddr[2],\
                                        pPastEvt->advAddr[3],pPastEvt->advAddr[4],pPastEvt->advAddr[5]);
...
int app_le_periodic_adv_report_event_handle(u8 *p)
{
    ...
    u8 status  = blc_hci_le_setPAwRsync_rspData( pPdaReport->syncHandle, req_pawrEvtCnt, req_subevent,
                                             rsp_subevent, rsp_slotIdx, RSP_DATA_LEN, rsp_dataBuff);
    if(status != 0){
