...
void key_change_proc(void)
{
    ...
    else if (kb_event.cnt == 1)
    {
        if (key0 >= CR_VOL_UP) //volume up/down
        {
            ...
            /* Here is just Telink Demonstration effect. Cause the demo board has limited
             * key to use, when Vol+/Vol- key pressed, we send media key "Vol+" or "Vol-"
             * to central for all peripheral role in connection. For users, you should known
             * that this is not a good method, you should manage your device and GATT data
             * transfer according to  conn_dev_list[]
             */
        ...
        else
        {
            ...
            if (key0 == BTN_PAIR) {         //Manual pair triggered by Key Press
                ...
                tlkapi_send_string_data(APP_PAIR_LOG_EN, "[UI][PAIR] Unpair", 0, 0);
                /* Here is just Telink Demonstration effect. Cause the demo board has limited
                 * key to use, only one "un_pair" key is available. When "un_pair" key pressed,
                 * we will choose and un_pair one device in connection state.
                 */
                    ...
                    if (!central_disconnect_connhandle) //if one central un_pair disconnection flow not finish, here new un_pair not accepted
                    {
                        ...
                        for (int i = 0; i < ACL_CENTRAL_MAX_NUM; i++) //peripheral index is from 0 to "ACL_CENTRAL_MAX_NUM - 1"
                        {
                            if (conn_dev_list[i].conn_state) {
                                ...
                                tlkapi_send_string_data(APP_PAIR_LOG_EN, "[UI][PAIR] Unpair begin", &central_unpair_enable, 2);
                                break;
                            }
                        }
                    }
...
void proc_central_role_unpair(void)
{
    ...
    if (central_unpair_enable) {
        ...
        if (dev_char_info) {                                                                        //un_pair device in still in connection state
            struct ble_hci_lc_disconnect_cp dis_conn_cmd;
            dis_conn_cmd.conn_handle = central_unpair_enable;
            dis_conn_cmd.reason = HCI_ERR_REMOTE_USER_TERM_CONN;
            if (ble_host_hci_disconnect(&dis_conn_cmd) == BLE_HOST_HCI_ERR_SUCCESS) {
