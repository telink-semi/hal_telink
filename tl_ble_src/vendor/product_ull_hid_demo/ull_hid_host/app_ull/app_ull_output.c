...
static int app_output_report_data_timeout(void* arg);

static struct soft_timer outputReportTimer = {
    .next = NULL,
    .cb = app_output_report_data_timeout,
    .arg = NULL,
};
...
void app_ullhid_report_data(char *argv[], int argc, void *user_data)
{
    ...
    {
        app_parse_printf("No Output Report Type in Hybrid Mode.\r\n");
        return ;
    }

    app_parse_printf("report Output Report ID is %d\r\n", ullhidParam.reportID);

    timer_set_cap_tick(TIMER0, sys_clk.pclk*ullhidParam.cisSduInterval);

    outputReportTimer.timer = app_parse_str2n(argv[0]);

    soft_timer_add(&outputReportTimer);
