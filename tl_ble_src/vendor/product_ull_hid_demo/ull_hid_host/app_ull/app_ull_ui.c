...
struct ull_param_check suppParam[] = {
    {1000, 5}, {1000, 10}, {1000, 15},
    {1250, 4}, {1250, 8}, {1250, 12},
    {2000, 5}, {2000, 10},
    {2500, 4}, {2500, 8}, {2500, 12},
    {3000, 5},
    {3750, 4},
    {4000, 5},
    {5000, 1}, {5000, 2}, {5000, 4},
};
...
static void app_ullhid_show_device(char *argv[], int argc, void *user_data)
{
    ...
    else
    {
        ...
        else
        {
            for(int i=0; i<g_deviceNum; i++)
            {
                app_parse_printf("[%d]:Address[%s]:%s, name:%s\r\n", i+1, g_device[i].addressType?"random":"public",
                        addr_to_str(g_device[i].address), g_device[i].name);
            }
        }
    }
}
...
static void app_ullhid_conn_device(char *argv[], int argc, void *user_data)
{
    ...
    g_scanEn = 0;
    u8 status = blc_ll_extended_createConnection( INITIATE_FP_ADV_SPECIFY, OWN_ADDRESS_PUBLIC,
            g_device[index-1].addressType, g_device[index-1].address,
            INIT_PHY_1M,
