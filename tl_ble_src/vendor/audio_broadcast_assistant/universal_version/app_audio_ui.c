...
/**
 * @brief       Assistant UI add source by PAST into sink device.
 * @param[in]   argv: parse input parameter pointer.
 * @param[in]   argc: parse input parameter size.
 * @param[in]   user_data: command input data, default NULL.
 * @return      none.
 */
static void app_audio_ui_add_source_past(char *argv[], int argc, void *user_data)
{
    if (argc < 3) {
        app_parse_printf("add-source-past <conn_idx> <source_idx> <bis_sync> [broadcast_key]\r\n");
        return;
    }
    ...
    if (!blc_bapba_startPAST(connHandle, (blc_audio_source_head_t *)sourceInfo)) {
        return app_parse_printf("PAST procedure start Fail\r\n");
    }
...
static const parse_fun_list_t assistantParse[] = {
    ...
    {"add-source-past", app_audio_ui_add_source_past},
