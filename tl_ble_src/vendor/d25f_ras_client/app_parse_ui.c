...
static u32 scan_start_tick = 0;
#define SCAN_TIMEOUT_SECONDS 2

extern void ble_host_hci_set_leg_scan(void);
...
static u8 app_findAdvInfo(u8 addrType, u8 address[6])
{
    if (advCnt == MAX_ADV_INFO_NUM) {
        ...
        //close scan when advInfoTable is full.
        struct ble_hci_le_set_scan_enable_cp leg_scan_enable={0};
        leg_scan_enable.enable            = BLC_SCAN_DISABLE;
        leg_scan_enable.filter_duplicates = DUP_FILTER_DISABLE;
        ble_host_hci_le_set_scan_enable(&leg_scan_enable);
        app_parse_printf("Found %d devices, scan stop\r\n", MAX_ADV_INFO_NUM);
        scan_start_tick = 0;
        return 0;
    }

    if(addrType != OWN_ADDRESS_PUBLIC) {
        return 0;
    }
...
int app_parse_foundAdv(u8 *p)
{
    ...
    if (reconn_en) {
        if (blc_smp_searchBondingPeripheralDevice_by_PeerMacAddress(pa->adr_type, pa->mac)) {
            ...
            cmdParam.min_conn_itvl  = CONN_INTERVAL_10MS;
            cmdParam.max_conn_itvl  = CONN_INTERVAL_10MS;
            cmdParam.conn_latency   = 0;
            cmdParam.tmo            = CONN_TIMEOUT_4S;
            cmdParam.min_ce         = 0;
            cmdParam.max_ce         = 0xFFFF;
            ble_host_hci_le_create_connection(&cmdParam);
        }
        return 0;
    }
...
int app_createACLConn(int index)
{
    ...
    cmdParam.scan_window    = SCAN_INTERVAL_100MS;
    cmdParam.filter_policy  = INITIATE_FP_ADV_SPECIFY;
    cmdParam.peer_addr_type = advInfoTable[index - 1].addrType;
    memcpy(cmdParam.peer_addr, advInfoTable[index - 1].address, 6);
    cmdParam.own_addr_type  = OWN_ADDRESS_PUBLIC;
    cmdParam.min_conn_itvl  = CONN_INTERVAL_10MS;
    cmdParam.max_conn_itvl  = CONN_INTERVAL_10MS;
    cmdParam.conn_latency   = 0;
...
static void app_parse_ui_cs(char *argv[], int argc, void *user_data)
{
    ...
    if (strcasecmp(argv[0], "rst") == 0) {
        ...
        //sub mode
        create_config_cmd.sub_mode_type = app_parse_str2n(argv[4]);
        if (create_config_cmd.sub_mode_type == 0xff) {
            create_config_cmd.min_main_mode_steps = 0xfe;
            create_config_cmd.max_main_mode_steps = 0xfe;
        } else {
            create_config_cmd.min_main_mode_steps = 2;
            create_config_cmd.max_main_mode_steps = 4;
        }

        create_config_cmd.conn_handle = cs_app_ctrl.cs_ctrl[0].connhandle;
        create_config_cmd.config_id = 0;
        create_config_cmd.create_context = 1;
        create_config_cmd.main_mode_type = main_mode;
        create_config_cmd.main_mode_repetition = app_parse_str2n(argv[5]) & 0x03;
        create_config_cmd.mode_0_steps = app_parse_str2n(argv[3]) & 0x03;
        create_config_cmd.role = app_parse_str2n(argv[1]) ? CS_CONFIG_REFLECTOR_ROLE : CS_CONFIG_INITIATOR_ROLE;
        create_config_cmd.rtt_type = app_parse_str2n(argv[6]);; //RTT CS Access Address only timing
        create_config_cmd.cs_sync_phy = 1; //0x01: LE CS SYNC 1M PHY; 0x02: LE CS SYNC 2M PHY
        memcpy(&create_config_cmd.channel_map[0], &cs_chn_map_all[0], 10);
        ...
        create_config_cmd.companion_signal_enable = 0; //Companion Signal disabled

        int ret = ble_host_hci_le_cs_create_config(&create_config_cmd);
        if(ret != BLE_HOST_HCI_ERR_SUCCESS){
            app_parse_printf("cs create config error: 0x%x\r\n", ret);
            return;
        }
        ...
        set_proc_para_cmd.conn_handle = cs_app_ctrl.cs_ctrl[0].connhandle;
        set_proc_para_cmd.config_id = cs_app_ctrl.cs_ctrl[0].config_id;
        set_proc_para_cmd.max_procedure_len = appCsParam.cs_max_procedure_len;
        set_proc_para_cmd.min_procedure_interval = appCsParam.cs_procedure_interval;
        set_proc_para_cmd.max_procedure_interval = appCsParam.cs_procedure_interval;
        set_proc_para_cmd.max_procedure_count = max_proc_cnt;
        set_proc_para_cmd.min_subevent_len[0] = appCsParam.cs_subevent_len & 0xFF;
        set_proc_para_cmd.min_subevent_len[1] = (appCsParam.cs_subevent_len >> 8) & 0xFF;
        set_proc_para_cmd.min_subevent_len[2] = (appCsParam.cs_subevent_len >> 16) & 0xFF;

        set_proc_para_cmd.max_subevent_len[0] = appCsParam.cs_subevent_len & 0xFF;
        set_proc_para_cmd.max_subevent_len[1] = (appCsParam.cs_subevent_len >> 8) & 0xFF;
        set_proc_para_cmd.max_subevent_len[2] = (appCsParam.cs_subevent_len >> 16) & 0xFF;
        ...
        int ret_para = ble_host_hci_le_cs_set_procedure_parameters(&set_proc_para_cmd);
        app_parse_printf("cs set cs procedure param\r\n");

        if (ret_para == BLE_HOST_HCI_ERR_SUCCESS) {
            struct ble_hci_le_cs_proc_enable_cp proc_enable_cmd;
            proc_enable_cmd.conn_handle = cs_app_ctrl.cs_ctrl[0].connhandle;
            proc_enable_cmd.config_id = cs_app_ctrl.cs_ctrl[0].config_id;
            proc_enable_cmd.enable  = 1;

            int ret_en = ble_host_hci_le_cs_procedure_enable(&proc_enable_cmd);
            if(ret_en != BLE_HOST_HCI_ERR_SUCCESS){
                 app_parse_printf("cs procedure enable failed: 0x%x\r\n", ret_en);
                 return;
             }
...
void app_parse_ui_loop(void)
{
    ...
    if(scan_start_tick && clock_time_exceed(scan_start_tick, SCAN_TIMEOUT_SECONDS*1000*1000)){
        scan_start_tick = 0;
        struct ble_hci_le_set_scan_enable_cp leg_scan_enable={0};
        leg_scan_enable.enable            = BLC_SCAN_DISABLE;
        leg_scan_enable.filter_duplicates = DUP_FILTER_DISABLE;
        ble_host_hci_le_set_scan_enable(&leg_scan_enable);
        app_parse_printf("scan timeout!\r\n");
    }
}
#endif
