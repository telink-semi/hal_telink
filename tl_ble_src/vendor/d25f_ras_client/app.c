...
static int app_acl_connected_event_callback(u16 aclHandle, u8 *pEvent, u16 dataLen)
{
    (void) aclHandle;
    (void) dataLen;
    ...
    app_parse_printf("Connected to %s success!\r\n", addr_to_str((u8*)pAclConnEvt->PeerAddr));
#endif
    return 0;
}

static int app_acl_disconnected_event_callback(u16 aclHandle, u8 *pEvent, u16 dataLen)
{
    (void) aclHandle;
    (void) dataLen;
...
static int app_acl_connect_security_done_callback(u16 aclHandle, u8 *pEvent, u16 dataLen)
{
    (void) pEvent;
    (void) dataLen;
...
static int app_acl_connect_interval_update_callback(u16 aclHandle, u8 *pEvent, u16 dataLen)
{
    (void) aclHandle;
    (void) dataLen;
...
static int app_client_sdp_found_callback(u16 aclHandle, u8 *pEvent, u16 dataLen)
{
    (void) dataLen;
...
static int app_client_sdp_not_found_callback(u16 aclHandle, u8 *pEvent, u16 dataLen)
{
    (void) dataLen;
...
static int app_client_all_sdp_over_callback(u16 aclHandle, u8 *pEvent, u16 dataLen)
{
    (void)pEvent;
    (void) dataLen;
...
_attribute_ram_code_ int app_le_adv_report_callback(u16 connHandle, u8 *pEvent, u16 dataLen)
{
    (void) connHandle;
    (void) dataLen;
    ...
    //s8 rssi = pa->data[pa->len];
#if UI_CONTROL_ENABLE
        app_parse_foundAdv((u8*)pEvent);
        return 0;
#endif
    //tlkapi_send_string_data(APP_CONTR_EVT_LOG_EN, "[APP][EVT] Adv report", p, pa->len+12);
//    static volatile u32 tickReport = 0;
//    if(clock_time_exceed(tickReport, 3000*1000))
//    {
//        tickReport = clock_time();
//        tlkapi_send_string_data(APP_CONTR_EVT_LOG_EN, "[APP][EVT] Adv report", (u8 *)pa, pa->len+12);
//    }
#if 0
    if(memcmp(pa->mac,(u8[]){0x13, 0x00, 0x00, 0x11, 0x11, 0x11},6)) {
       return 0;
    }
 ...
 */
_attribute_ble_data_retention_ app_prf_evtCb_t profile_event[] = {
    ...
    {PRF_EVTID_LE_CS_READ_REMOTE_SUPPORTED_CAPABILITIES_COMPLETE, app_le_processReadRemoteSupCapCompleteEvt},
    {PRF_EVTID_LE_CS_READ_REMOTE_FAE_TABLE_COMPLETE, app_le_processRemoteFAE_tableEvt},
    {PRF_EVTID_LE_CS_CONFIG_COMPLETE, app_le_csConfigCompleteEvt},
    {PRF_EVTID_LE_CS_SECURITY_ENABLE_COMPLETE, app_le_csSecurityEnableCompleteEvt},
    {PRF_EVTID_LE_CS_PROCEDURE_ENABLE_COMPLETE, app_le_csProcedureEnableCompleteEvt},
    {PRF_EVTID_LE_CS_SUBEVENT_RESULT, app_le_csSubeventResultEvt},
    {PRF_EVTID_LE_CS_SUBEVENT_RESULT_CONTINUE, app_le_csSubeventResultContinueEvt},
...
{
    blc_prf_initialModule(app_prf_eventCb, hostMallocBuffer1, HOST_MALLOC_BUFF_SIZE_1);
