...
def create_accessary_feature_profile(name):
    ...
    acjsonData = '"ringTheTag": ' + dev['RING_THE_TAG'] +', ' \
                ...
                + '"powerSaving": ' + dev['POWER_SAVING_MODE'] +', ' \
                + '"nfc": ' + dev['NFC']
    ...
    f.write(jsonData)
    f.close()
    print(" * The accessory profile is created!")
    print("  - %s\n" % (os.path.realpath(name + '.json')))
...
if __name__ == '__main__':
    ...
    #create_accessary_feature_profile(dev['NAME']+ '_' + nowDate.strftime("%m_%d_%H%M"))