...
#define FDB_STR_KV_VALUE_MAX_SIZE                1100
#endif
...
//        FDB_INFO("get_kv_from_cache %x %d\n",db->kv_cache_table[i].addr,name_crc);
...
//    FDB_INFO("read_kv %x\r\n",kv->addr.start);
    ...
    uint32_t calc_crc321 = 0;
    uint32_t calc_crc322 = 0;
    ...
    if (calc_crc32 != kv_hdr.crc32) {
        ...
        FDB_INFO(" %x %x %x %x %d %d %d %d\n", calc_crc32, calc_crc321,calc_crc322,kv_hdr.crc32,name_len,kv_hdr.value_len,kv->len);
...
//    FDB_INFO("kv_iterator sec_addr %x\r\n",sec_addr);
    /* search all sectors */
    do {
...
#endif /* FDB_KV_USING_CACHE */
//    FDB_INFO("get_kv_from_cache fail\n");
...
static fdb_err_t create_kv_blob(fdb_kvdb_t db, kv_sec_info_t sector, const char *key, const void *value, size_t len)
{
    ...
    if (kv_addr != FAILED_ADDR || (kv_addr = new_kv(db, sector, kv_hdr.len)) != FAILED_ADDR) {
        ...
        if (result == FDB_NO_ERR) {
            ...
            while (align_remain--) {
             FDB_INFO("kv crc FDB_WG_ALIGN2 !\r\n");
                kv_hdr.crc32 = fdb_calc_crc32(kv_hdr.crc32, &ff, 1);
            }
//            FDB_INFO("kv crc  0x%x %x %x %x %d %d %d %d\n", kv_addr, calc_crc321 ,calc_crc322 ,kv_hdr.crc32,kv_hdr.name_len,kv_hdr.value_len,kv_hdr.len,*((uint8_t *)value));
...
//        uint8_t  temp_data_buf[2];
//        _fdb_flash_read((fdb_db_t) db,kv_addr + KV_HDR_DATA_SIZE + FDB_WG_ALIGN(kv_hdr.name_len),temp_data_buf,1);
//        FDB_INFO("kv data3  %d %d %d %d %x\r\n",temp_data_buf[0],*((uint8_t *)value),FDB_WG_ALIGN(kv_hdr.name_len),KV_HDR_DATA_SIZE,kv_addr);
        /* write value */
        if (result == FDB_NO_ERR) {
...
//        _fdb_flash_read((fdb_db_t) db,kv_addr + KV_HDR_DATA_SIZE + FDB_WG_ALIGN(kv_hdr.name_len),temp_data_buf,1);
//        FDB_INFO("kv data1  %d %d\r\n",temp_data_buf[0],*((uint8_t *)value));
