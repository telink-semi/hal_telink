...
_attribute_ble_data_retention_  TaskHandle_t hButtonTask = NULL;
/**
...
void vApplicationIdleHook(void)
{
    ...
    if(APP_isDeepRetnFlag == pdTRUE)
    {
        ...
        if( eTaskGetState( hButtonTask ) == eSuspended )
        {
            DBG_CHN10_HIGH;
            DBG_CHN10_LOW;
            vTaskResume(hButtonTask);
        }
...
//    if (battery_get_detect_enable() && clock_time_exceed(lowBattDet_tick, 1000)) {
...
void app_BleTaskCreate(void)
{
    ...
    ret = xTaskCreate(ble_task, "tble", 3072, (void *)0, (tskIDLE_PRIORITY + 10), &hBleTask);
