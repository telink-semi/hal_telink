...
int app_le_adv_report_event_handle(u8 *p)
{
    ...
    if (central_auto_connect || user_manual_pairing) {
        ...
        u8 status = blc_ll_createConnection(SCAN_INTERVAL_100MS, SCAN_WINDOW_100MS, INITIATE_FP_ADV_SPECIFY, pa->adr_type, pa->mac, OWN_ADDRESS_PUBLIC, CONN_INTERVAL_31P25MS, CONN_INTERVAL_31P25MS, 0, CONN_TIMEOUT_4S, 0, 0xFFFF);
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    tlkapi_send_string_data(APP_LOG_EN, "[APP][INI]adv_scan_onoff init", 0, 0);
}
...
void key_change_proc(void)
{
    ...
    {
        tlkapi_send_string_data(APP_KEY_LOG_EN, "two key press", 0, 0);
    } else if (kb_event.cnt == 1) {
        if (key0 >= CR_VOL_UP) {
            if (acl_conn_periphr_num > 0) {
               ...
               * */
                ...
                if (key0 == CR_VOL_UP) {
                    blc_ll_setAdvEnable(BLC_ADV_ENABLE);  //ADV enable
                    tlkapi_send_string_data(APP_KEY_LOG_EN, "[UI][KEY]BLC_ADV_ENABLE", 0, 0);
                } else if (key0 == CR_VOL_DN) {
                    blc_ll_setAdvEnable(BLC_ADV_DISABLE); //ADV enable
                    tlkapi_send_string_data(APP_KEY_LOG_EN, "[UI][KEY]BLC_ADV_DISABLE", 0, 0);
                }
            }
        } else {
...
void main_adv_scan_onoff(void)
{
    #if 0
        #define SCAN_STATE_INIT 0
        #define SCAN_STATE_WORK 1
        #define SCAN_STATE_STOP 2

    static u32 scan_status = 0;  //0 1  2
    static u32 scan_on_tick = 0; //
    static u32 rand_us = 0;

    switch(scan_status)
    {
      case SCAN_STATE_INIT:
        scan_on_tick = clock_time() | 1;
        u32 rand_value = rand();
        rand_us = (50 + (rand_value&0x3f)) *20000;  //(50 ~ (50+63))*10 mS
         tlkapi_printf(APP_LOG_EN,"SCAN_STATE_RUN_INIT");
        scan_status = SCAN_STATE_STOP;//
        break;
      case SCAN_STATE_STOP:
        if(clock_time_exceed(scan_on_tick, rand_us))
        {
          scan_on_tick = clock_time() | 1;
          blc_ll_setScanEnable (BLC_SCAN_DISABLE, DUP_FILTER_DISABLE);
          gpio_write(GPIO_LED_BLUE,0);
          gpio_write(GPIO_LED_RED,0);
          tlkapi_printf(APP_LOG_EN,"BLC_SCAN_DISABLE");
          u32 rand_value = rand();
          rand_us = (50 + (rand_value&0x3f)) *20000;  //(50 ~ (50+63))*10 mS
          scan_status = SCAN_STATE_WORK;//
        }
        break;
      case SCAN_STATE_WORK:
        if(clock_time_exceed(scan_on_tick, rand_us))
        {
          scan_on_tick = clock_time() | 1;
          blc_ll_setScanEnable (BLC_SCAN_ENABLE, DUP_FILTER_DISABLE);
          tlkapi_printf(APP_LOG_EN,"BLC_SCAN_ENABLE");
    ...
    #if 0
        #define ADV_STATE_INIT 0
        #define ADV_STATE_WORK 1
        #define ADV_STATE_STOP 2

    static u32 adv_status = 0;  //0 1  2
    static u32 adv_on_tick = 0; //
    static u32 rand_us = 0;

    switch(adv_status)
    {
      case ADV_STATE_INIT:
        adv_on_tick = clock_time() | 1;
        u32 rand_value = rand();
        rand_us = (50 + (rand_value&0x3f)) *20000;  //(50 ~ (50+63))*10 mS
         tlkapi_printf(APP_LOG_EN,"ADV_STATE_RUN_INIT");
        adv_status = ADV_STATE_STOP;//
        break;
      case ADV_STATE_STOP:
        if(clock_time_exceed(adv_on_tick, rand_us))
        {
          adv_on_tick = clock_time() | 1;
      ...
      case ADV_STATE_WORK:
        if(clock_time_exceed(adv_on_tick, rand_us))
        {
          ...
          gpio_write(GPIO_LED_BLUE,1);
          tlkapi_printf(APP_LOG_EN,"BLC_ADV_ENABLE");
          u32 rand_value = rand();
          rand_us = (50 + (rand_value&0x3f)) *20000;  //(50 ~ (50+63))*10 mS
          adv_status = ADV_STATE_STOP;//
        }
