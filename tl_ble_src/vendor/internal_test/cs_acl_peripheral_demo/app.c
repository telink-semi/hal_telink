...
const u8    tbl_advData[] = {
     13, DT_COMPLETE_LOCAL_NAME,                'c', 's', '_', 'r', 'e', 'f', 'l', 'e', 'c', 't', 'o','r',
...
int app_le_connection_complete_event_handle(u8 *p)
{
    ...
    if (pConnEvt->status == BLE_SUCCESS) {
        ...
        if (pConnEvt->role == ACL_ROLE_PERIPHERAL) {
            bls_l2cap_requestConnParamUpdate(pConnEvt->connHandle, CONN_INTERVAL_100MS, CONN_INTERVAL_100MS, 0, CONN_TIMEOUT_4S);   // 1 second
        }
    }
...
int my_usb_audio_debug(unsigned char *p, int len)
{
    ...
    {
    case 0:
        blc_ll_test_setCsParam(1);//1 for slave
...
//////////////////////////// BLE stack Initialization  Begin //////////////////////////////////
    ...
    blc_cs_registerRasProfileControlServer(NULL);
