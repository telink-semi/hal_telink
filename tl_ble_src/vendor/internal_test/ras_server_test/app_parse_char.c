...
static int hex_to_int(u8 *input)
{
    if((*input >= 'A') && (*input <= 'F'))
    {
        return (*input) - 'A' + 10;
...
int hex_byte_to_int(u8 *input)
{
    ...
    nibble_hi = hex_to_int(input);
    nibble_low = hex_to_int(input+1);

    if((nibble_hi <0 ) || nibble_low < 0)
    {
        return -1;
    }
    else
    {
        return nibble_hi * 16 + nibble_low;
    }
}

int hex_string_to_bytes(u8 * buffer, char *string, u16 hard_limit)
{
    u16 length;
    int byte;
    u16 i;
    ...
    if((length%2!=0) || (length > hard_limit))
    {
        return -2;
    }
