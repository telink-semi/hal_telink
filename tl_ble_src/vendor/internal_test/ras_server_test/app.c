...
int app_le_connection_complete_event_handle(u8 *p)
{
    ...
    if (pConnEvt->status == BLE_SUCCESS) {
        ...
        if (pConnEvt->role == ACL_ROLE_PERIPHERAL) {
            bls_l2cap_requestConnParamUpdate(pConnEvt->connHandle, CONN_INTERVAL_15MS, CONN_INTERVAL_15MS, 20, CONN_TIMEOUT_4S); // 1 second
        }
    }
...
/* SMP Initialization */
    ...
    #if (RAS_OOB)
        blc_smp_setSecurityLevel_periphr(Authenticated_LE_Secure_Connection_Pairing_with_Encryption);
        blc_smp_setSecurityParameters_periphr(Bondable_Mode, 1, LE_Secure_Connection, 1, 0, IO_CAPABILITY_NO_INPUT_NO_OUTPUT);
        blc_smp_setEcdhDebugMode_periphr(non_debug_mode);
    #else
