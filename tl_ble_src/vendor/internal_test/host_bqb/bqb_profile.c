...
u8 defaultAttr3Val[100] = {0x15, 0x16, 0x17, 0x18, 0x19, 0x1a,};
...
u8 app_disocvery_char_uuid_cb(u16 connHandle, gatt_attr_t *attr, struct gattc_sdp_cfg *params)
{
    ...
    tlkapi_printf(APP_GATTC_LOG_EN, "Found characteristic , attHandle is %x properties is %x, value Handle  is %x UUID is %s",
            characteristic->attrHdl, characteristic->properties, characteristic->valueHdl
            , hex_to_str(characteristic->uuid.uuidVal.u, characteristic->uuid.uuidLen)
    );
...
//      app_disocvery_char_uuid(connHandle, params->startHdl, params->endHdl);
    ...
    tlkapi_printf(APP_GATTC_LOG_EN, "discovery include ending, startHandle is %x endingHanlde is %x",
            params->startHdl, params->endHdl);

    gatt_include_t *include = (gatt_include_t *)attr->user_data;
    tlkapi_printf(APP_GATTC_LOG_EN, "Found include ,startHandle is %x endingHanlde is %x UUID is %s",
            include->startHdl, include->endHdl, hex_to_str(include->uuid.uuidVal.u, include->uuid.uuidLen)
    );
...
u8 app_discovery_primary_service_cb(u16 connHandle, gatt_attr_t *attr, struct gattc_sdp_cfg *params)
{
    ...
    tlkapi_printf(APP_GATTC_LOG_EN, "discovery primary service, startHandle is %x endingHanlde is %x UUID is %s",
            attr->handle, prim_service->endHdl, hex_to_str(prim_service->uuid->uuidVal.u, prim_service->uuid->uuidLen));
