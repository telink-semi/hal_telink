...
static int app_bapbs_remoteScanStoppedCb(u16 connHandle, u8 *pData);
static int app_bapbs_remoteScanStartedCb(u16 connHandle, u8 *pData);
static int app_bapbs_initCodec(u16 connHandle, u8 *pData);
static int app_bapbs_syncBig(u16 connHandle, u8 *pData);
static int app_vcss_changedVolState(u16 connHandle, u8 *pData);
...
static const app_audio_evtCb_t unicastCb[] = {
    ...
    {AUDIO_EVT_BAPBS_REMOTE_SCAN_STOPPED, (void *)app_bapbs_remoteScanStoppedCb},
    {AUDIO_EVT_BAPBS_REMOTE_SCAN_STARTED, (void *)app_bapbs_remoteScanStartedCb},
    {AUDIO_EVT_BAPBS_BIS_SINK_INIT_CODEC, (void *)app_bapbs_initCodec          },
    {AUDIO_EVT_BAPBS_BIS_SINK_SYNC_BIG,   (void *)app_bapbs_syncBig            },
    /* Event for VCS Server */
    {AUDIO_EVT_VCSS_CHANGED_VOLUME_STATE, (void *)app_vcss_changedVolState     },
};
...
static void app_event_acl_connect(u16 aclHandle, blc_prf_aclConnEvt_t *pEvt)
{
    ...
    if (advRet != BLE_SUCCESS) {
        tlkapi_printf(APP_LOG_EN, "close CIS adv fail, adv handle:0x%x", ADV_HANDLE0);
    }

    advRet = blc_ll_setExtAdvEnable(BLC_ADV_DISABLE, ADV_HANDLE1, 0, 0); //connect success, disable the adv.
    if (advRet != BLE_SUCCESS) {
        tlkapi_printf(APP_LOG_EN, "close BIS adv fail, adv handle:0x%x", ADV_HANDLE1);
    }
}
...
static void app_event_acl_disconnect(u16 aclHandle, blc_prf_aclDisconnEvt_t *pEvt)
{
    ...
    ble_sts_t advRet = blc_ll_setExtAdvEnable(BLC_ADV_ENABLE, ADV_HANDLE1, 0, 0);
    if (advRet != BLE_SUCCESS) {
        tlkapi_printf(APP_LOG_EN, "open BIS adv fail, adv handle:0x%x", ADV_HANDLE1);
    }
}
...
static void app_event_cis_disconnect(u16 aclHandle, blc_audio_cisDisconnEvt_t *pEvt)
{
    ...
    ble_sts_t advRet = blc_ll_setExtAdvEnable(BLC_ADV_ENABLE, ADV_HANDLE0, 0, 0);
    if (advRet != BLE_SUCCESS) {
        tlkapi_printf(APP_LOG_EN, "open cis adv fail, adv handle:0x%x", ADV_HANDLE0);
    }
...
{
    if (appSinkVcpState.volume == 255) {
        return;
    }

    if (appSinkVcpState.volume + 20 >= 255) {
        appSinkVcpState.volume = 255;
    } else {
        appSinkVcpState.volume += 20;
    }

    blc_vcss_updateVolSetting(appCtrl.aclHandle, appSinkVcpState.volume);
    app_changeVolState();
}
...
{
    if (appSinkVcpState.volume == 0) {
        return;
    }

    if (appSinkVcpState.volume - 20 <= 0) {
...
void app_send_mute(void)
{
    ...
    blc_vcss_updateMuteState(appCtrl.aclHandle, appSinkVcpState.mute);
    app_changeVolState();
}
...
static int app_bapbs_remoteScanStoppedCb(u16 connHandle, u8 *pData)
{
    tlkapi_printf(APP_LOG_EN, "BIS sink remote scan stop:0x%x\n", connHandle);
    return 0;
}
...
static int app_bapbs_remoteScanStartedCb(u16 connHandle, u8 *pData)
{
    tlkapi_printf(APP_LOG_EN, "BIS sink remote scan start:0x%x\n", connHandle);
    return 0;
}
...
static int app_bapbs_initCodec(u16 connHandle, u8 *pData)
{
    ...
    tlkapi_printf(APP_LOG_EN, "Bis sink init codec, PresentationDelay:%dus bisNumber is %d\n", initCodecEvt->presentationDelay, initCodecEvt->bisNum);

    initCodecEvt->presentationDelay = 50 * 1000;
...
static int app_bapbs_syncBig(u16 connHandle, u8 *pData)
{
    ...
    tlkapi_printf(APP_LOG_EN, "BIG Sync state:%d, bigHandle is 0x%02x, bis number is %d, iosInterval is %.2fms\n", bigSyncState->state, bigSyncState->bigHandle, bigSyncState->numBis, bigSyncState->isoInterval * 1.25);
    tlkapi_printf(APP_LOG_EN, "bis Handle 1 is 0x%02x, bis Handle 2 is 0x%02x\n", bigSyncState->bisHandles[0], bigSyncState->bisHandles[1]);
    if (bigSyncState->state == BIG_SYNCED) {
    #if (TLK_LED_ENABLE)
        tlk_led_pattern_set(LED3_ID, LED_PATTERN_LED_BREATH, DEFAULT_PATTERN);
    #endif
        app_codec_setBigSyncState(BIG_SYNCED, bigSyncState->numBis, bigSyncState->bisHandles);
    } else {
    #if (TLK_LED_ENABLE)
        tlk_led_pattern_set(LED3_ID, LED_PATTERN_LED_FLASH_FAST, DEFAULT_PATTERN);
    #endif
...
static int app_vcss_changedVolState(u16 connHandle, u8 *pData)
{
    ...
    tlkapi_printf(APP_LOG_EN, "vol setting data is:0x%x, mute state is %s", volStateChange->volumeSetting, volStateChange->mute ? "muted" : "mute");
