...
int app_le_enhanced_connection_complete_event_handle(u8 *p)
{
    ...
    if (pConnEvt->status == BLE_SUCCESS) {
        ...
        if (dev_char_get_conn_role_by_connhandle(pConnEvt->connHandle) == ACL_ROLE_PERIPHERAL) {
            ...
            tlk_led_pattern_set(LED4_ID, LED_PATTERN_LED_FLASH_FAST, DEFAULT_PATTERN);
...
int app_disconnect_event_handle(u8 *p)
{
    ...
    if (dev_char_get_conn_index_by_connhandle(pDisConn->connHandle) != 0xFF) { //ACL handle
        ...
        #if (TLK_LED_ENABLE)
            if (appSinkInfo.bigSyncState != BIG_SYNCED) {
                tlk_led_pattern_set(LED3_ID, LED_PATTERN_LED_FLASH_SLOW, DEFAULT_PATTERN);
            }
            tlk_led_pattern_set(LED4_ID, LED_PATTERN_LED_FLASH_SLOW, DEFAULT_PATTERN);
    ...
    #if (TLK_LED_ENABLE)
        tlk_led_pattern_set(LED4_ID, LED_PATTERN_LED_FLASH_FAST, DEFAULT_PATTERN); // cis disconnect
...
int app_controller_event_callback(u32 h, u8 *p, int n)
{
    if (h & HCI_FLAG_EVENT_BT_STD) //Controller HCI event
    {
        ...
        } else if (evtCode == HCI_EVT_LE_META)         //LE Event
        {
    ...
    #if (TLK_LED_ENABLE)
                tlk_led_pattern_set(LED4_ID, LED_PATTERN_LED_BREATH, DEFAULT_PATTERN);
            ...
            } else if (subEvt_code == HCI_LE_EVT_MASK_BIG_SYNC_ESTABLISHED) {
                hci_le_bigSyncLostEvt_t *pEvt = (hci_le_bigSyncLostEvt_t *)p;
                BLT_APP_LOG("BIG Sync ok, bigHandle:0x%x", pEvt->bigHandle);
            } else if (subEvt_code == HCI_LE_EVT_MASK_BIG_SYNC_LOST) {
                hci_le_bigSyncLostEvt_t *pEvt = (hci_le_bigSyncLostEvt_t *)p;
...
_attribute_no_inline_ void user_init_normal(void)
{
    ...
    blc_hci_le_setEventMask_cmd(
        HCI_LE_EVT_MASK_CONNECTION_COMPLETE | HCI_LE_EVT_MASK_ENHANCED_CONNECTION_COMPLETE | HCI_LE_EVT_MASK_ADVERTISING_REPORT | HCI_LE_EVT_MASK_CONNECTION_UPDATE_COMPLETE | HCI_LE_EVT_MASK_CIS_ESTABLISHED | HCI_LE_EVT_MASK_CIS_REQUESTED
