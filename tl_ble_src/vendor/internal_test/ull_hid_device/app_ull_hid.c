...
void app_initial_ull_hid_device(void)
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        app_parse_printf("Report ID:%d, Type:%s, Power Saving Confirmation:%s, Repetition:%s\n", properties->hybridModeReport[i].reportID, properties->hybridModeReport[i].reportType == 0x00 ? "Input" : "Output", temp[properties->hybridModeReport[i].powerSavingCfm], temp[properties->hybridModeReport[i].repetition]);
    }
}
