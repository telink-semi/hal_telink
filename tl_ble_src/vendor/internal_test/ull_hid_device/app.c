...
int app_le_enhanced_connection_complete_event_handle(u8 *p)
{
    ...
    if(pConnEvt->status == BLE_SUCCESS)
    {
        ...
        tlkapi_printf(APP_CONTR_EVT_LOG_EN, "ACL connection interval is %.2fms\n", pConnEvt->connInterval*1.25);
    ...
    if (h &HCI_FLAG_EVENT_BT_STD)       //Controller HCI event
    {
        ...
        else if(evtCode == HCI_EVT_LE_META)  //LE Event
        {
            ...
            else if (subEvt_code == HCI_SUB_EVT_LE_CONNECTION_UPDATE_COMPLETE)  // connection update
            {
                ...
                tlkapi_printf(APP_CONTR_EVT_LOG_EN, "ACL connection interval update, new interval is %.2fms\n", pUpdateEvt->connInterval*1.25);
            }
...
//////////////////////////// BLE stack Initialization  Begin //////////////////////////////////
...
#if (ADV_USE_EXT_MODE)
    blc_ll_setExtAdvParam( ADV_HANDLE0,         ADV_EVT_PROP_LEGACY_CONNECTABLE_SCANNABLE_UNDIRECTED,                   ADV_INTERVAL_30MS,          ADV_INTERVAL_30MS,
     ...
     */
    u8  tbl_advData[] = {
         13, DT_COMPLETE_LOCAL_NAME,                'T', 'L', 'S', 'R', '9', '2', '1', '7', '-', 'U', 'L', 'L',
         2,  DT_FLAGS,                              0x05,                   // BLE limited discoverable mode and BR/EDR not supported
         3, DT_COMPLETE_LIST_16BIT_SERVICE_UUID,    U16_TO_BYTES(SERVICE_UUID_ULL_HID)
    };
...
#else
    u8  tbl_advData[] = {
         8, DT_COMPLETE_LOCAL_NAME,                 'U', 'L', 'L', '-', 'H', 'I', 'D',
         2,  DT_FLAGS,                              0x05,                   // BLE limited discoverable mode and BR/EDR not supported
         3, DT_COMPLETE_LIST_16BIT_SERVICE_UUID,    U16_TO_BYTES(SERVICE_UUID_ULL_HID)
    };
    u8  tbl_scanRsp[] = {
