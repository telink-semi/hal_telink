...
u32 smp_encProcess_done_tick = 0;
u16 ras_connectionhandle     = 0;
...
int app_le_connection_complete_event_handle(u8 *p)
{
    ...
    ras_connectionhandle                     = pConnEvt->connHandle;
...
int app_host_event_callback(u32 h, u8 *para, int n)
{
    ...
    {
        smp_encProcess_done_tick = clock_time() | 1;
...
int my_usb_audio_debug(unsigned char *p, int len)
{
    ...
    switch (p[1]) {
    case 0:
        blc_ll_test_setCsParam(0); //0 for master
        tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] cs test set param", 0, 0);
        break;
    case 1:
        blc_ll_test_startCs();
        tlkapi_send_string_data(APP_LOG_EN, "[APP][INI] cs test start", 0, 0);
        break;
...
static void app_ras_control_point_cb(u16 connHandle, att_err_t err)
{
    if (err == ATT_SUCCESS) {
        tlk_printf("write control point value is success\r\n");
    } else {
        tlk_printf("write control point error, ATT error code is 0x%x", err);
    }
}
...
_attribute_no_inline_ void main_loop(void)
{
    ...
    if (smp_encProcess_done_tick && clock_time_exceed(smp_encProcess_done_tick, 1000000)) {
        tlk_printf("start control point");
        u8 controlCmd[2]         = {0x11, 0x22};
        smp_encProcess_done_tick = 0;

        ble_sts_t status = 0;
        status           = blc_rasc_writeControlPoint(ras_connectionhandle, controlCmd, sizeof(controlCmd), app_ras_control_point_cb);

        if (status == BLE_SUCCESS) {
            tlk_printf("send success");
        }
    }
