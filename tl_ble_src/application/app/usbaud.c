...
void usbaud_set_speaker_vol_res(u16 vol)
{
    speaker_vol_res = vol;
}
...
{
    return speaker_setting.vol_cur;
}
...
void usbaud_handle_set_feature_mute(int id, int mute)
{
    int b = (id >> 8) & 15;
    if ((id & 0xff) < 2) {
        b |= BIT(4);
    ...
    //  my_dump_str_u32s (1, "usbaud_handle_set_feature_mute", id, mute, usbaud_mute, b);
}

unsigned char usbaud_handle_get_feature_mute(int id)
{
    int b = (id >> 8) & 15;
    if ((id & 0xff) < 2) {
        b |= BIT(4);
    ...
    //  my_dump_str_u32s (1, "usbaud_handle_get_feature_mute", id, b, usbaud_mute, b);

    return (usbaud_mute >> b) & 1;
}

// return -1 on fail,  0 on success
int usbaud_handle_set_speaker_cmd(int req, int type, int id)
{
    if (type == AUDIO_FEATURE_MUTE) {
        speaker_setting.mute = usbhw_read_ctrl_ep_data();
        usbaud_handle_set_feature_mute(id, speaker_setting.mute);
    } else if (type == AUDIO_FEATURE_VOLUME) {
        u16 val = usbhw_read_ctrl_ep_u16();
        if (req == AUDIO_REQ_SetCurrent) {
            usbaud_set_speaker_vol(val);
        } else if (req == AUDIO_REQ_SetResolution) {
            usbaud_set_speaker_vol_res(val);
        }
    } else {
        return -1;
    }
...
// return -1 on fail,  0 on success
int usbaud_handle_get_speaker_cmd(int req, int type, int id)
{
    if (type == AUDIO_FEATURE_MUTE) {
        usbhw_write_ctrl_ep_data(usbaud_handle_get_feature_mute(id));
