 ...
 * @param[out]  sp_key_idx          - index of secure port key.
 ...
 * @return      0:success     other:error
 */
unsigned int hmac_sha512_dma_init(HMAC_SHA512_DMA_CTX *ctx, const unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes, HASH_CALLBACK callback)
{
...
{
    return hmac_dma(HASH_SHA512, key, sp_key_idx, key_bytes, msg, msg_bytes, mac, callback);
}
