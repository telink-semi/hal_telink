...
void rf_emi_tx_burst_loop(rf_mode_e rf_mode, unsigned char pkt_type)
{
    ...
    if ((rf_mode == RF_MODE_BLE_1M_NO_PN) || (rf_mode == RF_MODE_BLE_2M_NO_PN)) //ble
    {
        ...
        rf_start_stx((void *)emi_ble_tx_packet, rf_bb_timer_get_tick() + 10);
        ...
        rf_start_stx((void *)emi_zigbee_tx_packet, rf_bb_timer_get_tick() + 10);
        while (!(rf_get_irq_status(FLD_RF_IRQ_TX)));
        rf_clr_irq_status(FLD_RF_IRQ_TX);
        ...
        rf_start_stx((void *)Private_TPLL_tx_packet, rf_bb_timer_get_tick() + 10);
        while (!(rf_get_irq_status(FLD_RF_IRQ_TX)));
        rf_clr_irq_status(FLD_RF_IRQ_TX);
