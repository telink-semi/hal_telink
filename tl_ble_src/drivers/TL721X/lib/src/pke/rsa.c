...
unsigned int RSA_Get_E2(unsigned int e[], unsigned int fai_n[], unsigned int bitLen)
{
    ...
    {
        j = 32;
    }                                //now j express e or phi_n's bit length of the highest word 
    j--;                             //now j express e or phi_n's second highest bit at the position of the highest word
    i = wordLen - 1;                 //i express highest word position

    if(j==0)                         //j=0, second highest word 
    {
        i--;
        j=32;
    }
 ...
 * @param[in]   aBitLen          - unsigned int big even integer fai_n.
