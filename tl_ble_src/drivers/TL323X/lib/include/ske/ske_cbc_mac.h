 ...
 */
    unsigned int ske_lp_cbc_mac_update(ske_cbc_mac_ctx_t *ctx, const unsigned char *msg, unsigned int msg_bytes);
 ...
 */
    unsigned int ske_lp_cbc_mac(ske_alg_e alg, ske_mac_e mac_action, ske_padding_e padding, const unsigned char *key, unsigned short sp_key_idx, const unsigned char *msg,
                                unsigned int msg_bytes, unsigned char *mac, unsigned char mac_bytes);
 ...
 * @param[in]       msg_bytes            - byte length of msg, can not be 0
 * @param[out]      tmp_out              - temporary output ciphertext, ske_lp need to output, it occupies the same blocks as the
 ...
 */
    unsigned int ske_lp_dma_cbc_mac_update_including_last_block(ske_cbc_mac_dma_ctx_t *ctx, unsigned int *msg, unsigned int msg_bytes, unsigned int *tmp_out, unsigned int *mac,
                                                                SKE_CALLBACK callback);
    /**
 ...
 * @param[in]       sp_key_idx           - index of secure port key, (sp_key_idx & 0x7FFF) must be in [1,SKE_MAX_KEY_IDX],
 * @param[in]       msg                  - message
 * @param[in]       msg_bytes            - byte length of message, can not be 0
 * @param[out]      tmp_out              - temporary output ciphertext, ske_lp need to output, it occupies the same blocks as the
 ...
 */
    unsigned int ske_lp_dma_cbc_mac(ske_alg_e alg, const unsigned char *key, unsigned short sp_key_idx, unsigned int *msg, unsigned int msg_bytes, unsigned int *tmp_out,
