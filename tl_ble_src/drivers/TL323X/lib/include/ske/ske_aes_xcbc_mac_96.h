 ...
 *           mac is input, return value SKE_SUCCESS means the mac is valid, otherwise mac is invalid
 */
    unsigned int ske_lp_aes_xcbc_mac_96(ske_mac_e mac_action, const unsigned char *key, unsigned short sp_key_idx, unsigned char *msg, unsigned int msg_bytes,
 ...
 */
    unsigned int ske_lp_dma_aes_xcbc_mac_96(const unsigned char *key, unsigned short sp_key_idx, unsigned int *msg, unsigned int msg_bytes, unsigned int *tmp_out,
                                            unsigned int *mac, SKE_CALLBACK callback);
#endif
