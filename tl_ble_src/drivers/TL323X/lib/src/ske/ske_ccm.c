...
/**
 * @brief           Common initialization for ske_lp_ccm_init() and ske_lp_dma_ccm_init()
 ...
 */
unsigned int ske_lp_ccm_pre_init(ske_ccm_ctx_t *ctx, ske_crypto_e crypto, const unsigned char *nonce, unsigned char M, unsigned char L, unsigned int aad_bytes,
...
/**
 * @brief           Prepare B1 block without input aad (if aad exists)
 * @param[in]       ctx                  - ske_ccm_ctx_t context pointer
 * @note            Please make sure all parameters are valid
 */
void ske_lp_ccm_pre_B1(ske_ccm_ctx_t *ctx)
{
    unsigned char tmp[4];
 ...
 *        5. aad_bytes and c_bytes could not be zero at the same time due to hardware implementationnnn
 */
 ...
 * @note
 *        1. this function must be called after calling ske_lp_ccm_init()
 *        2. if there is no aad, this function could be omitte
 */
...
unsigned int ske_lp_ccm_final(ske_ccm_ctx_t *ctx, unsigned char *mac)
{
    ...
    // memset_(ctx, 0, sizeof(ske_ccm_ctx_t));
 ...
 *           otherwise, key is from secure port, and (sp_key_idx & 0x7FFF) must be in [1,SKE_MAX_KEY_IDX]
 *        4. valid M is {4,6,8,10,12,14,16}, and valid L is {2,3,4,5,6,7,8},
 *        5. aad_bytes and c_bytes could not be zero at the same time due to hardware implementationnnnn
