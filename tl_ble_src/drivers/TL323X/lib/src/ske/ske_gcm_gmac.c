 ...
 */
unsigned int ske_lp_gcm_init(ske_gcm_ctx_t *ctx, ske_alg_e alg, ske_crypto_e crypto, const unsigned char *key, unsigned short sp_key_idx, unsigned char *iv, unsigned int iv_bytes,
                             unsigned int aad_bytes, unsigned int c_bytes, unsigned int mac_bytes)
{
    unsigned int tmp[4];
 ...
 *        1. this function must be called after calling ske_lp_gcm_init()
 *        2. if there is no aad, this function could be omitted
 *        3. please make sure aad is integra
 */
...
unsigned int ske_lp_gcm_final(ske_gcm_ctx_t *ctx, unsigned char *mac)
{
    ...
    // memset_(ctx, 0, sizeof(ske_gcm_ctx_t));
