 ...
 *
 *******************************************************************************************************/
#include "lib/include/ske/ske_cfb.h"
...
{
    return ske_lp_init(alg, SKE_MODE_CFB, crypto, key, sp_key_idx, iv);
}
 ...
 * @return          SKE_SUCCESS if successful, otherwise error code
 */
unsigned int ske_lp_cfb_crypto(ske_alg_e alg, ske_crypto_e crypto, const unsigned char *key, unsigned short sp_key_idx, unsigned char *iv, unsigned char *in, unsigned char *out,
                               unsigned int bytes)
{
    return ske_lp_crypto(alg, SKE_MODE_CFB, crypto, key, sp_key_idx, iv, in, out, bytes);
}
...
{
    return ske_lp_dma_init(alg, SKE_MODE_CFB, crypto, key, sp_key_idx, iv);
}
 ...
 * @return          SKE_SUCCESS if successful, otherwise error code
 */
unsigned int ske_lp_dma_cfb_crypto(ske_alg_e alg, ske_crypto_e crypto, const unsigned char *key, unsigned short sp_key_idx, unsigned char *iv, unsigned int *in, unsigned int *out,
                                   unsigned int words, SKE_CALLBACK callback)
{
    return ske_lp_dma_crypto(alg, SKE_MODE_CFB, crypto, key, sp_key_idx, iv, in, out, words, callback);
}
#endif
#endif
