 ...
 * @param[in]       e                    - e value, 8 words, little-endian
 * @param[in]       k                    - random number k, 8 words, little-endian
 * @param[in]       dA                   - private key, 8 words, little-endian
 * @param[out]      r                    - Signature r, 8 words, little-endian
 * @param[in]       s                    - Signature s, 8 words, little-endian
 ...
 */
FLAG_STATIC unsigned int sm2_sign_with_k(const unsigned int e[8], const unsigned int k[8], const unsigned int dA[8], unsigned int r[8], unsigned int s[8])
{
    ...
    ret = uint32_integer_check(k, sm2_curve->eccp_n, SM2_WORD_LEN, SM2_ZERO_ALL, SM2_INTEGER_TOO_BIG, SM2_SUCCESS);
    if (SM2_SUCCESS != ret)
    {
        return ret;
    }
    ...
    // tmp1 = x1 mod n
    if (uint32_big_num_cmp(tmp1, SM2_WORD_LEN, sm2_curve->eccp_n, SM2_WORD_LEN) >= 0)
    {
    ...
    // r = e + x1 mod n
    ret = pke_modadd(sm2_curve->eccp_n, e, tmp1, r, SM2_WORD_LEN);
    if (PKE_SUCCESS != ret)
    {
        return ret;
    }
    ...
    // tmp1 = r + k mod n
    ret = pke_modadd(sm2_curve->eccp_n, r, k, tmp1, SM2_WORD_LEN);
    if (PKE_SUCCESS != ret)
    {
        return ret;
    }
    ...
    ret = pke_load_modulus_and_pre_monts(sm2_curve->eccp_n, sm2_curve->eccp_n_h, sm2_curve->eccp_n_n0, SM2_BIT_LEN);
#else
...
// tmp1 =  r*dA mod n
    ...
    // tmp1 =  (k - r*dA) mod n
    ret = pke_modsub(sm2_curve->eccp_n, k, tmp1, tmp1, SM2_WORD_LEN);
    if (PKE_SUCCESS != ret)
    {
        return ret;
    }
    ...
    // tmp2 = (1+dA)^(-1) mod n
    ret = pke_modinv(sm2_curve->eccp_n, tmp2, tmp2, SM2_WORD_LEN, SM2_WORD_LEN);
    if (PKE_SUCCESS != ret)
    {
        return ret;
    }
...
#endif
    if (uint32_big_num_cmp(e, SM2_WORD_LEN, sm2_curve->eccp_n, SM2_WORD_LEN) >= 0)
    {
...
#endif
    ret = uint32_integer_check(dA, g_sm2p256v1_n_minus_1, SM2_WORD_LEN, SM2_ZERO_ALL, SM2_INTEGER_TOO_BIG, SM2_SUCCESS);
    if (SM2_SUCCESS != ret)
    {
        return ret;
    }
...
#else
    reverse_byte_array(&pubKey[1], (unsigned char *)(&tmp[2u * SM2_WORD_LEN]), SM2_BYTE_LEN);
...
#endif
    ret = uint32_integer_check(r, sm2_curve->eccp_n, SM2_WORD_LEN, SM2_ZERO_ALL, SM2_INTEGER_TOO_BIG, SM2_SUCCESS);
    if (SM2_SUCCESS != ret)
    {
        goto END;
    }
...
// make sure s in [1, n-1]
...
#endif
    ret = uint32_integer_check(s, sm2_curve->eccp_n, SM2_WORD_LEN, SM2_ZERO_ALL, SM2_INTEGER_TOO_BIG, SM2_SUCCESS);
    if (SM2_SUCCESS != ret)
    {
        goto END;
    }
    ...
    // t = (r+s) mod n
    ret = pke_modadd(sm2_curve->eccp_n, r, s, t, SM2_WORD_LEN);
    if (PKE_SUCCESS != ret)
    {
        goto END;
    }
...
#ifdef SM2_HIGH_SPEED
    ret = eccp_pointMul_Shamir_safe((eccp_curve_t *)sm2_curve, s, (unsigned int *)sm2p256v1_Gx, (unsigned int *)sm2p256v1_Gy, t, tmp + (2u * SM2_WORD_LEN),
                                    tmp + (3u * SM2_WORD_LEN), tmp, NULL);
#else
    //[s]G
...
// e = e mod n
    ...
    // tmp = x1 mod n
    if (uint32_big_num_cmp(tmp, SM2_WORD_LEN, sm2_curve->eccp_n, SM2_WORD_LEN) >= 0)
    {
    ...
    // tmp = e + x1 mod n
    ret = pke_modadd(sm2_curve->eccp_n, e, tmp, tmp, SM2_WORD_LEN);
    if (PKE_SUCCESS != ret)
    {
        goto END;
    }
    ...
    // cmp
    if (0 != uint32_big_num_cmp(tmp, SM2_WORD_LEN, r, SM2_WORD_LEN))
    {
 ...
 */
static unsigned int sm2_encrypt_with_k(const unsigned char *M, unsigned int m_len, const unsigned int *k, const unsigned int *pubkey_x, const unsigned int *pubkey_y,
                                       sm2_cipher_order_e order, unsigned char *C, unsigned int *c_len)
{
    ...
    hash_node_t hash_node[3]; // since M and C may point the same address, please
                              // do not initialize hash_node here.

    if ((NULL == M) || (NULL == k) || (NULL == pubkey_x) || (NULL == pubkey_y) || (NULL == C) || (NULL == c_len))
    {
        return SM2_BUFFER_NULL;
    }
    else if (0u == m_len)
    ...
    C3 = &C[1u + (2u * SM2_BYTE_LEN) + ((SM2_C1C2C3 == order) ? m_len : 0u)];

    // not support M and C crossing, but support M = C
    if (M > C)
    {
        if ((C + m_len + 1u + (3u * SM2_BYTE_LEN)) > M)
        {
            return SM2_INPUT_INVALID;
        }
        else
...
// get x2||y2
    ...
    hash_node[0].msg_len = SM2_BYTE_LEN;
    hash_node[1].msg_addr = (const unsigned char *)M;
    hash_node[1].msg_len = m_len;
    hash_node[2].msg_addr = (unsigned char *)(&xy[SM2_WORD_LEN]);
    hash_node[2].msg_len = SM2_BYTE_LEN;
    ...
    // get C2
    hash_node[0].msg_len = SM2_BYTE_LEN << 1;
    hash_node[1].msg_addr = (unsigned char *)counter;
    hash_node[1].msg_len = 4u;
    ret = ansi_x9_63_kdf_node_with_xor_in(HASH_SM3, hash_node, 2u, counter, M, C2, m_len, 1u);
    if (HASH_SUCCESS == ret)
    {
        c_len[0] = m_len + 1u + (3u * SM2_BYTE_LEN);
                         ...
                         unsigned int *c_len)
{
    ...
    if (NULL == rand_k)
    {
        do
        {
            ...
            ret = sm2_encrypt_with_k(M, m_len, k, pubkey_x, pubkey_y, order, C, c_len);
        } while ((SM2_ZERO_ALL == ret) || (SM2_INTEGER_TOO_BIG == ret));
    }
    else
    {
...
unsigned int sm2_decrypt(const unsigned char *C, unsigned int c_len, const unsigned char priKey[32], sm2_cipher_order_e order, unsigned char *M, unsigned int *m_len)
{
    ...
    const unsigned char *C2, *C3;
    unsigned int ret;

    hash_node_t hash_node[3];

    if ((NULL == C) || (NULL == priKey) || (NULL == M) || (NULL == m_len))
    {
        return SM2_BUFFER_NULL;
    }
    else if (c_len <= (1u + (3u * SM2_BYTE_LEN))) // 97 = 1+3*ECCP_BYTELEN
    {
        return SM2_INPUT_INVALID;
    }
    else if (order > SM2_C1C2C3)
    ...
    temLen = c_len - 1u - (3u * SM2_BYTE_LEN);
 ...
 * @param[in]       k_len                - byte length of output key, should be less than (2^32 - 1)bit
 *                                         @param[out]  KA[k_len]      - output key
 ...
 */
static unsigned int sm2_exchangekey_s_check_input(sm2_exchange_role_e role, const unsigned char *dA, const unsigned char *PB, const unsigned char *rA, const unsigned char *RA,
                                                  const unsigned char *RB, const unsigned char *ZA, const unsigned char *ZB, unsigned int k_len, const unsigned char *KA)
{
    unsigned int ret = SM2_INPUT_INVALID;
...
/**
 * @brief           SM2 Key Exchange helper function
 * @param[in]       role                 - SM2_Role_Sponsor - sponsor, SM2_Role_Responsor - responsor
 * @param[in]       hash_node            - Hash node structure pointer
 * @param[in]       counter              - Pointer to 4-byte counter in node array
 ...
 * @param[in]       tmp                  - Temporary buffer
 * @param[in]       k                    - Random number
 ...
 */
FLAG_STATIC unsigned int sm2_exchangekey_get_S1_SA(sm2_exchange_role_e role, hash_node_t *hash_node, unsigned char *counter, const unsigned char *ZA, const unsigned char *ZB,
                                                   const unsigned char *RA, const unsigned char *RB, unsigned int *tmp, unsigned int *k, unsigned char *S1, unsigned char *SA,
                                                   unsigned int k_len, unsigned char *KA)
{
    unsigned int ret = SM2_SUCCESS;
    ...
    hash_node[1].msg_len = SM2_BYTE_LEN;
    hash_node[2].msg_len = SM2_BYTE_LEN;
    hash_node[3].msg_addr = (unsigned char *)counter;
    hash_node[3].msg_len = 4u;

    // KA
    ret = ansi_x9_63_kdf_node(HASH_SM3, hash_node, 4u, counter, KA, k_len, NULL, 0u);
    if (HASH_SUCCESS != ret)
    {
        goto END;
    }
    ...
    if ((NULL != S1) && (NULL != SA))
    {
        ...
        hash_node[1].msg_len = SM2_BYTE_LEN;
        hash_node[2].msg_len = SM2_BYTE_LEN;
        hash_node[3].msg_len = SM2_BYTE_LEN << 1;
        hash_node[4].msg_len = SM2_BYTE_LEN << 1;

        ret = hash_node_steps(HASH_SM3, hash_node, 5u, (unsigned char *)k);
        if (HASH_SUCCESS != ret)
        {
            goto END;
        }
        ...
        hash_node[0].msg_len = SM2_BYTE_LEN + 1u;
        hash_node[1].msg_addr = (unsigned char *)k;
        hash_node[1].msg_len = SM2_BYTE_LEN;
        if (SM2_Role_Sponsor == role)
        {
 ...
 */
unsigned int sm2_exchangekey(sm2_exchange_role_e role, const unsigned char *dA, const unsigned char *PB, const unsigned char *rA, const unsigned char *RA, const unsigned char *RB,
                             const unsigned char *ZA, const unsigned char *ZB, unsigned int k_len, unsigned char *KA, unsigned char *S1, unsigned char *SA)
{
    unsigned char counter[4] = {0, 0, 0, 1};
    unsigned int x1[SM2_WORD_LEN], t1[SM2_WORD_LEN], tmp[SM2_WORD_LEN << 2];
    hash_node_t hash_node[5];
    unsigned int ret;

    ret = sm2_exchangekey_s_check_input(role, dA, PB, rA, RA, RB, ZA, ZB, k_len, KA);
    if (PKE_SUCCESS == ret)
    {
#ifdef PKE_BIG_ENDIAN
...
#endif
        ret = uint32_integer_check(t1, g_sm2p256v1_n_minus_1, SM2_WORD_LEN, SM2_ZERO_ALL, SM2_INTEGER_TOO_BIG, SM2_SUCCESS);
        if (SM2_SUCCESS != ret)
        {
            goto END;
        }
...
// make sure dA in [1, n-2]
...
#endif
        ret = uint32_integer_check(x1, g_sm2p256v1_n_minus_1, SM2_WORD_LEN, SM2_ZERO_ALL, SM2_INTEGER_TOO_BIG, SM2_SUCCESS);
        if (SM2_SUCCESS != ret)
        {
            goto END;
        }
        ...
        ret = pke_modadd(sm2_curve->eccp_n, t1, x1, t1, SM2_WORD_LEN);
        if (PKE_SUCCESS != ret)
        {
            goto END;
        }
...
// get PB point and verify
        ...
        ret = eccp_pointMul_Shamir_safe((eccp_curve_t *)sm2_curve, t1, tmp + (2u * SM2_WORD_LEN), tmp + (3u * SM2_WORD_LEN), x1, tmp, tmp + SM2_WORD_LEN, tmp, tmp + SM2_WORD_LEN);
#else
...
// xU||yU
        ...
        ret = sm2_exchangekey_get_S1_SA(role, hash_node, counter, ZA, ZB, RA, RB, tmp, t1, S1, SA, k_len, KA);
    }
