...
{
#if 0
    unsigned int a_u32[RSA_MAX_WORD_LEN];
    unsigned int e_u32[RSA_MAX_WORD_LEN];
    unsigned int n_u32[RSA_MAX_WORD_LEN];
    unsigned int e_len = get_byte_len(eBitLen);
    ...
    reverse_byte_array(e, (unsigned char *)e_u32, e_len);
    reverse_byte_array(a, (unsigned char *)a_u32, n_len);
    reverse_byte_array(n, (unsigned char *)n_u32, n_len);

    ret = RSA_ModExp(a_u32, e_u32, n_u32, a_u32, eBitLen, nBitLen);
    if(RSA_SUCCESS == ret)
    {
        reverse_byte_array((unsigned char *)a_u32, out, n_len);
    }
    else
    {}
...
{
    unsigned int a_u32[RSA_MAX_WORD_LEN];
    unsigned int p_u32[RSA_MAX_WORD_LEN >> 1];
    unsigned int q_u32[RSA_MAX_WORD_LEN >> 1];
    unsigned int dp_u32[RSA_MAX_WORD_LEN >> 1];
    unsigned int dq_u32[RSA_MAX_WORD_LEN >> 1];
    unsigned int u_u32[RSA_MAX_WORD_LEN >> 1];

    unsigned int p_len = get_byte_len(nBitLen >> 1);
    unsigned int p_wlen = get_word_len(nBitLen >> 1);
    unsigned int n_len = get_byte_len(nBitLen);
    unsigned int n_wlen = get_word_len(nBitLen);
    unsigned int ret;
    ...
    reverse_byte_array(p, (unsigned char *)p_u32, p_len);
    reverse_byte_array(q, (unsigned char *)q_u32, p_len);
    reverse_byte_array(dp, (unsigned char *)dp_u32, p_len);
    reverse_byte_array(dq, (unsigned char *)dq_u32, p_len);
    reverse_byte_array(u, (unsigned char *)u_u32, p_len);
    reverse_byte_array(a, (unsigned char *)a_u32, n_len);
...
{
    unsigned int e_u32[RSA_MAX_WORD_LEN];
    unsigned int d_u32[RSA_MAX_WORD_LEN];
    unsigned int n_u32[RSA_MAX_WORD_LEN];
    unsigned int e_len = get_byte_len(eBitLen);
    unsigned int n_len = get_byte_len(nBitLen);
    unsigned int ret;
    ...
    if (RSA_SUCCESS == ret)
    {
        reverse_byte_array((unsigned char *)e_u32, e, e_len);
        reverse_byte_array((unsigned char *)d_u32, d, n_len);
        reverse_byte_array((unsigned char *)n_u32, n, n_len);
    }
    else
    {
    }
 ...
 *                  calling the internal rsa_get CRT-Key function. then, it converts the generated
 ...
 *        3. if eBitLen is 2, 5, 17, here makes e as 3, 17, 65537 respectively, otherwise e is random.
 */
unsigned int RSA_GetCRTKey_U8(unsigned char *e, unsigned char *p, unsigned char *q, unsigned char *dp, unsigned char *dq, unsigned char *u, unsigned char *n, unsigned int eBitLen,
                              unsigned int nBitLen)
{
    ...
    {
        reverse_byte_array((unsigned char *)e_u32, e, e_len);
        reverse_byte_array((unsigned char *)p_u32, p, p_len);
        reverse_byte_array((unsigned char *)q_u32, q, p_len);
        reverse_byte_array((unsigned char *)dp_u32, dp, p_len);
        reverse_byte_array((unsigned char *)dq_u32, dq, p_len);
        reverse_byte_array((unsigned char *)u_u32, u, p_len);
        reverse_byte_array((unsigned char *)n_u32, n, n_len);
    }
    else
    {
