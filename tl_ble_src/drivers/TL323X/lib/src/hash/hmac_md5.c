...
{
    return hmac(HASH_MD5, key, sp_key_idx, key_bytes, msg, msg_len, mac);
}
 ...
 */
unsigned int hmac_md5_node_steps(const unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes, const hash_node_t *node, unsigned int node_num, unsigned char *mac)
{
...
{
    return hmac_dma(HASH_MD5, key, sp_key_idx, key_bytes, msg, msg_len, mac, callback);
}
