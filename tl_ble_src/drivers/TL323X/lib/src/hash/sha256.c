...
{
    return hash(HASH_SHA256, msg, msg_len, digest);
}
...
{
    return hash_node_steps(HASH_SHA256, node, node_num, digest);
}
#endif
...
{
    return hash_dma(HASH_SHA256, msg, msg_len, digest, callback);
}
...
{
    return hash_dma_node_steps(HASH_SHA256, node, node_num, digest, callback);
}
#endif
