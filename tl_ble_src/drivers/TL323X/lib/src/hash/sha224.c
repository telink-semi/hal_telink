...
/**
 * @brief           init sha224
 * @param[in]       ctx                  - sha224_ctx_t context pointer
 * @return          HASH_SUCCESS(success), other(error)
 */
unsigned int sha224_init(sha224_ctx_t *ctx)
{
 ...
 */
unsigned int sha224_update(sha224_ctx_t *ctx, const unsigned char *msg, unsigned int msg_len)
{
 ...
 */
unsigned int sha224_final(sha224_ctx_t *ctx, unsigned char *digest)
{
 ...
 *        1. please make sure the digest buffer is sufficient
 */
unsigned int sha224(const unsigned char *msg, unsigned int msg_len, unsigned char *digest)
{
    return hash(HASH_SHA224, msg, msg_len, digest);
}
 ...
 */
unsigned int sha224_node_steps(const hash_node_t *node, unsigned int node_num, unsigned char *digest)
{
 ...
 * @param[in]       ctx                  - sha224_dma_ctx_t context pointer
 * @param[in]       callback             - callback function pointer
 * @return          HASH_SUCCESS(success), other(error)
 */
unsigned int sha224_dma_init(sha224_dma_ctx_t *ctx, hash_callback callback)
{
 ...
 * @param[in]       msg_len            - byte length of the input message, must be a multiple of sha224
 * @return          HASH_SUCCESS(success), other(error)
 * @note
 *        1. please make sure the four parameters are valid, and ctx is initialize
 */
unsigned int sha224_dma_update_blocks(sha224_dma_ctx_t *ctx, unsigned int *msg, unsigned int msg_len)
{
 ...
 *        1. please make sure the four parameters are valid, and ctx is initialize
 */
unsigned int sha224_dma_final(sha224_dma_ctx_t *ctx, unsigned int *remainder_msg, unsigned int remainder_bytes, unsigned int *digest)
{
 ...
 */
unsigned int sha224_dma(unsigned int *msg, unsigned int msg_len, unsigned int *digest, hash_callback callback)
{
    return hash_dma(HASH_SHA224, msg, msg_len, digest, callback);
}
 ...
 */
unsigned int sha224_dma_node_steps(const hash_dma_node_t *node, unsigned int node_num, unsigned int *digest, hash_callback callback)
{
