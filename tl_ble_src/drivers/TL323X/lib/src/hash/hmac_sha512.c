 ...
 *
 *******************************************************************************************************/
#include "lib/include/hash/hmac_sha512.h"
...
{
    return hmac(HASH_SHA512, key, sp_key_idx, key_bytes, msg, msg_len, mac);
}
 ...
 */
unsigned int hmac_sha512_node_steps(const unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes, const hash_node_t *node, unsigned int node_num, unsigned char *mac)
{
    return hmac_node_steps(HASH_SHA512, key, sp_key_idx, key_bytes, node, node_num, mac);
}
#endif
...
{
    return hmac_dma_init(ctx, HASH_SHA512, key, sp_key_idx, key_bytes, callback);
}
...
{
    return hmac_dma(HASH_SHA512, key, sp_key_idx, key_bytes, msg, msg_len, mac, callback);
}
...
{
    return hmac_dma_node_steps(HASH_SHA512, key, sp_key_idx, key_bytes, node, node_num, mac, callback);
}
#endif
