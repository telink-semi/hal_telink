 ...
 * @param[in]       ctx                  - sha512_256_ctx_t context pointer
 * @return          HASH_SUCCESS(success), other(error)
 */
unsigned int sha512_256_init(sha512_256_ctx_t *ctx)
{
 ...
 */
unsigned int sha512_256_update(sha512_256_ctx_t *ctx, const unsigned char *msg, unsigned int msg_len)
{
 ...
 */
unsigned int sha512_256_final(sha512_256_ctx_t *ctx, unsigned char *digest)
{
 ...
 *        1. please make sure the digest buffer is sufficient
 */
unsigned int sha512_256(const unsigned char *msg, unsigned int msg_len, unsigned char *digest)
{
    return hash(HASH_SHA512_256, msg, msg_len, digest);
}

#ifdef SUPPORT_HASH_NODE
/**
 * @brief           input whole message and get its sha512_256 digest(node style)
 ...
 */
unsigned int sha512_256_node_steps(const hash_node_t *node, unsigned int node_num, unsigned char *digest)
{
    return hash_node_steps(HASH_SHA512_256, node, node_num, digest);
}
#endif
 ...
 * @param[in]       ctx                  - sha512_256_ctx_dma_t context pointer
 * @param[in]       callback             - callback function pointer
 * @return          HASH_SUCCESS(success), other(error)
 */
unsigned int sha512_256_dma_init(sha512_256_ctx_dma_t *ctx, hash_callback callback)
{
 ...
 * @param[in]       msg_len            - byte length of the input message, must be a multiple of sha512_256
 * @return          HASH_SUCCESS(success), other(error)
 * @note
 *        1. please make sure the four parameters are valid, and ctx is initialize
 */
unsigned int sha512_256_dma_update_blocks(sha512_256_ctx_dma_t *ctx, unsigned int *msg, unsigned int msg_len)
{
 ...
 *        1. please make sure the four parameters are valid, and ctx is initialize
 */
unsigned int sha512_256_dma_final(sha512_256_ctx_dma_t *ctx, unsigned int *remainder_msg, unsigned int remainder_bytes, unsigned int *digest)
{
 ...
 */
unsigned int sha512_256_dma(unsigned int *msg, unsigned int msg_len, unsigned int *digest, hash_callback callback)
{
    return hash_dma(HASH_SHA512_256, msg, msg_len, digest, callback);
}

#ifdef SUPPORT_HASH_DMA_NODE
/**
 * @brief           input whole message and get its sha512_256 digest(dma node style)
 * @param[in]       node                 - message node pointer
 * @param[in]       node_num             - number of hash nodes, i.e. number of message segments.
 * @param[out]      digest               - sha512_256 digest, 32 bytes
 * @param[in]       callback             - callback function pointer
 ...
 */
unsigned int sha512_256_dma_node_steps(const hash_dma_node_t *node, unsigned int node_num, unsigned int *digest, hash_callback callback)
{
    return hash_dma_node_steps(HASH_SHA512_256, node, node_num, digest, callback);
}
#endif
