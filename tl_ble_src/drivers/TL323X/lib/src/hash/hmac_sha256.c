 ...
 * @param[in]       ctx                  - hmac_sha256_ctx_t context pointer
 * @param[in]       key                  - key
 * @param[in]       sp_key_idx           - index of secure port key
 * @param[in]       key_bytes            - byte length of key, it could be 0
 * @return          HASH_SUCCESS(success), other(error)
 */
unsigned int hmac_sha256_init(hmac_sha256_ctx_t *ctx, const unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes)
{
 ...
 */
unsigned int hmac_sha256_update(hmac_sha256_ctx_t *ctx, const unsigned char *msg, unsigned int msg_len)
{
 ...
 */
unsigned int hmac_sha256_final(hmac_sha256_ctx_t *ctx, unsigned char *mac)
{
 ...
 */
unsigned int hmac_sha256(const unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes, const unsigned char *msg, unsigned int msg_len, unsigned char *mac)
{
    return hmac(HASH_SHA256, key, sp_key_idx, key_bytes, msg, msg_len, mac);
}
 ...
 */
unsigned int hmac_sha256_node_steps(const unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes, const hash_node_t *node, unsigned int node_num, unsigned char *mac)
{
 ...
 * @param[in]       ctx                  - hmac_sha256_dma_ctx_t context pointer
 * @param[in]       key                  - key
 * @param[in]       sp_key_idx           - index of secure port key
 ...
 * @return          HASH_SUCCESS(success), other(error)
 */
unsigned int hmac_sha256_dma_init(hmac_sha256_dma_ctx_t *ctx, const unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes, hash_callback callback)
{
 ...
 */
unsigned int hmac_sha256_dma_update_blocks(hmac_sha256_dma_ctx_t *ctx, unsigned int *msg, unsigned int msg_len)
{
 ...
 *        1. please make sure the three parameters are valid, and ctx is initialize
 */
unsigned int hmac_sha256_dma_final(hmac_sha256_dma_ctx_t *ctx, unsigned int *remainder_msg, unsigned int remainder_bytes, unsigned int *mac)
{
 ...
 * @return          HASH_SUCCESS(success), other(error)
 */
unsigned int hmac_sha256_dma(const unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes, unsigned int *msg, unsigned int msg_len, unsigned int *mac,
                             hash_callback callback)
{
    return hmac_dma(HASH_SHA256, key, sp_key_idx, key_bytes, msg, msg_len, mac, callback);
}
 ...
 */
unsigned int hmac_sha256_dma_node_steps(const unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes, const hash_dma_node_t *node, unsigned int node_num,
