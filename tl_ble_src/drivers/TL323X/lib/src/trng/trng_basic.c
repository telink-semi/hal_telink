 ...
 * @note            
 *        1. this works while i_skip_startup is 0. if i_skip_startup is 1, no need to check this.
 */
 ...
 */
unsigned int trng_ro_entropy_config(uint8_t cfg)
{
 ...
 * @return          TRNG_SUCCESS(success), other(error)
 */
unsigned int trng_ro_sub_entropy_config(uint8_t sn, uint16_t cfg)
{
 ...
 * @return          TRNG_SUCCESS(success), other(error)
 */
unsigned int trng_set_freq(uint8_t freq)
{
 ...
 * @return          TRNG_SUCCESS(success), other(error)
 */
unsigned int get_rand_buffer(uint8_t *rand, unsigned int bytes, GET_RAND_WORDS get_rand_words)
{
    ...
    if (0U != tmp) {
        ...
        if (TRNG_SUCCESS != ret) {
            ...
            if (tmp_len > i) {
                memcpy_(a, (uint8_t *)(&rng_data), i);
                a = &a[i];
                tmp_len -= i;
            } else {
                memcpy_(a, (uint8_t *)(&rng_data), tmp_len);
                goto END;
            }
        }
    } else {
