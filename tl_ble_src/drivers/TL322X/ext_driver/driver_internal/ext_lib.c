...
{
    unsigned char mask = 0x7F; //bit6~bit0
    unsigned char val  = 0x4b; //function select DBG_BB
...
void rf_enable_bb_debug(void)
{
    ...
    //dma_rx
    //REG_ADDR8(0x170003) &= 0xDF;//default 0x0, no need

    //dma_tx
//    REG_ADDR8(0x170003) |= BIT(5);//default 0x0, need
    reg_bb_dbg_sel_h &= 0x0F;       // 140d01[7:4] = 0
    reg_bb_dbg_sel_h |= BIT(0);     // 140d01[0] = 1; if not set, B2/B3 not work.
    reg_bb_dbg_sel_l |= BIT(1);     // 140d00[1] = 1
    reg_bb_dbg_sel_l |= BIT(2);     // 140d00[2] = 1
    reg_bb_dbg_sel_l &= ~BIT(5);    // 140d00[5] = 0

    dbg_bb_set_pin(GPIO_PA0); //tx_en
    dbg_bb_set_pin(GPIO_PA1); //tx_on
    dbg_bb_set_pin(GPIO_PA2); //rx_en
    dbg_bb_set_pin(GPIO_PA4); //hit_sync //access code end
#if 0
