...
#define EMI_TX_FIFO_ADDR REG_BASEBAND_BASE_ADDR + 0x84 //0x17001c
...
void rf_emi_tx_continue_update_data(rf_mode_e rf_mode, rf_power_level_e power_level, signed char rf_chn, unsigned char pkt_type)
{
    ...
    case RF_MODE_PRIVATE_2M:
        rf_set_pri_2M_mode();
        break;
    case RF_MODE_BLE_4M_NO_PN:
        rf_modem_rate_mode(RF_48M_MODEM_RATE);
        rf_set_ble_4M_NO_PN_mode();
        break;
    case RF_MODE_BLE_6M_NO_PN:
        rf_modem_rate_mode(RF_48M_MODEM_RATE);
        rf_set_ble_6M_NO_PN_mode();
        break;
    case RF_MODE_PRIVATE_4M:
        rf_modem_rate_mode(RF_48M_MODEM_RATE);
        rf_set_pri_4M_mode();
        break;
    case RF_MODE_PRIVATE_6M:
        rf_modem_rate_mode(RF_48M_MODEM_RATE);
        rf_set_pri_6M_mode();
        break;
...
void rf_emi_tx_burst_loop(rf_mode_e rf_mode, unsigned char pkt_type)
{
    ...
    if ((rf_mode == RF_MODE_BLE_1M_NO_PN) || (rf_mode == RF_MODE_BLE_2M_NO_PN)||(rf_mode == RF_MODE_BLE_4M_NO_PN) ||(rf_mode == RF_MODE_BLE_6M_NO_PN) ) //ble
    {
    ...
    } else if ((rf_mode == RF_MODE_PRIVATE_1M) || (rf_mode == RF_MODE_PRIVATE_2M)||(rf_mode == RF_MODE_PRIVATE_4M)||(rf_mode == RF_MODE_PRIVATE_6M)) {
        rf_data_len               = EMI_TX_PKT_PAYLOAD + 1;
