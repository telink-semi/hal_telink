...
/**
 * @brief       This function servers to enable rxdll and txdll for high speed mode.
 *              The DLL mode support 96MHz, DLL bypass mode support 64MHz.
 * @return      none 
 */
_attribute_ram_code_sec_noinline_ void mspi_rxdll_txdll_enable(void)
{
    reg_mspi_rxdll_ctrl0 &= (~FLD_MSPI_RXDLL_PD);       //RXDLL_PD=0
    reg_mspi_rxdll_ctrl0 |= FLD_MSPI_RXDLL_LOCK_DET_EN; //FLD_MSPI_RXDLL_LOCK_DET_EN=1
    while ((reg_mspi_dll_status & FLD_MSPI_RXDLL_DET_OUT) != FLD_MSPI_RXDLL_DET_OUT)
        ;                                               //wait RXDLL_DET_OUT=1

    reg_mspi_txdll_ctrl0 &= (~FLD_MSPI_TXDLL_PD);       //TXDLL_PD=0
    reg_mspi_txdll_ctrl0 |= FLD_MSPI_TXDLL_LOCK_DET_EN; //FLD_MSPI_TXDLL_LOCK_DET_EN=1
    while ((reg_mspi_dll_status & FLD_MSPI_TXDLL_DET_OUT) != FLD_MSPI_TXDLL_DET_OUT)
        ;                                               //wait TXDLL_DET_OUT=1

    write_reg8(0x140800, 0x06);

    reg_mspi_txdll_ctrl1 |= FLD_MSPI_TXDLL_MODE; //txdll_mode
    reg_mspi_txdll_ctrl1 |= 0x07;                //txdll_dly_sel

    reg_mspi_rxdll_ctrl1 |= FLD_MSPI_RXDLL_MODE; //rxdll_mode
    reg_mspi_rxdll_ctrl1 |= 0x00;                //rxdll_dly_sel
}

/**
 * @brief       This function servers to disable rxdll and txdll.
 * @return      none
 */
_attribute_ram_code_sec_noinline_ void mspi_rxdll_txdll_disable(void)
{
    reg_mspi_txdll_ctrl1 &= (~FLD_MSPI_TXDLL_MODE); //txdll bypass mode

    reg_mspi_rxdll_ctrl1 &= (~FLD_MSPI_RXDLL_MODE); //rxdll bypass mode
}
