...
void rf_set_ble_125K_mode(void)
{
    ...
    write_reg8(0x17049a, 0x00); //tx_tp_align.//  write_reg8(0x1704c2,0x36);//grx_0.
    write_reg8(0x1704c2, 0x36); //grx_0.
    write_reg8(0x1704c3, 0x48); //grx_1.
    write_reg8(0x1704c4, 0x54); //grx_2.
    write_reg8(0x1704c5, 0x62); //grx_3.
...
{
    unsigned char x                 = 0;
    unsigned char quality_indicator = 0;
    x                               = ((data_len >> 8) & 0xff);
...
/**
  * @brief          This function serves to set PN start byte for RF.
...
{
    rf_set_pn_init_value(config->pn_value);
    rf_set_pn_poly(config->pn_poly);
    rf_set_pn_poly_length(config->rf_pn_length);
    rf_set_pn_start_byte(config->pn_start_byte);
    rf_pn_enable();
}
...
{
    reg_rf_sys_timer_ctrl3 = 0x3;
    reg_bb_timer_irq_mask  = (FLD_BB_TIMER_TIMER_ALIGN | (0x3 << 6)); //Set the bb timer bit width to 32 bits
}
...
{
    reg_bb_timer_cal_irq |= FLD_BB_TIMER_CMD_ALIGN_TRIG;
}
...
{
    if (BB_TIMER == clk) {
        reg_rf_tstimp_ctrl &= ~FLD_RF_R_STIMER_REVERT_EN;
    } else {
        reg_rf_tstimp_ctrl |= FLD_RF_R_STIMER_REVERT_EN;
    }
}
...
/**
 * @brief       This function serves to set rf fpga channel for all mode.The actual channel set by this function is 2402+chn*2.
 * @param[in]   chn
 * @return      none.
 */
...
_attribute_ram_code_sec_ void dbg_bb_set_pin(gpio_pin_e pin)
{
...
_attribute_ram_code_sec_ void rf_enable_bb_debug(void)
{
...
{
    analog_write_reg8(0x10a, analog_read_reg8(0x10a) | 0x80);
}
