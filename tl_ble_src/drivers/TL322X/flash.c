...
#if 1//!defined(MCU_CORE_TL322X_N22) //todo:xh
...
extern void tlk_mailbox_flash_read_write_notify(u8 status);
#endif
...
#else
    #if (defined(HOST_V2_ENABLE))
    tlk_mailbox_flash_read_write_notify(1);
    #endif
    DISABLE_BTB;
    flash_mspi_read_ram(FLASH_DREAD_CMD, addr, buf, len);
    ENABLE_BTB;
    #if (defined(HOST_V2_ENABLE))
    tlk_mailbox_flash_read_write_notify(0);
    #endif
#endif
}
...
/**
 * @brief       This function reads the content from a page to the buf with DDR dual read mode.
 * @param[in]   addr    - slave base address + the access address of flash.The base address of device0 is 0.
 *                      If there are multiple devices, the base address of other devices is determined by the mspi_slave_device_addr_space_config() function.
 * @param[in]   len     - the length(in byte, must be above 0) of content needs to read out from the page.
 * @param[out]  buf     - the start address of the buffer(ram address).
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_text_sec_ void flash_ddr_2read(unsigned long addr, unsigned long len, unsigned char *buf)
{
    DISABLE_BTB;
    mspi_rxdll_txdll_enable();
    mspi_ddr_enable();
    flash_mspi_read_ram(FLASH_DDR_2READ_CMD, addr, buf, len);
    mspi_ddr_disable();
    mspi_rxdll_txdll_disable();
    ENABLE_BTB;
}

/**
 * @brief       This function reads the content from a page to the buf with DDR 4*IO read mode.
 * @param[in]   addr    - slave base address + the access address of flash.The base address of device0 is 0.
 *                      If there are multiple devices, the base address of other devices is determined by the mspi_slave_device_addr_space_config() function.
 * @param[in]   len     - the length(in byte, must be above 0) of content needs to read out from the page.
 * @param[out]  buf     - the start address of the buffer(ram address).
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_text_sec_ void flash_ddr_4read(unsigned long addr, unsigned long len, unsigned char *buf)
{
    DISABLE_BTB;
    mspi_ddr_enable();
    flash_mspi_read_ram(FLASH_DDR_X4READ_CMD, addr, buf, len);
    mspi_ddr_disable();
    DISABLE_BTB;
}
