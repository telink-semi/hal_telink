 ...
 * @param[in]    key            - key.
 * @param[in]    sp_key_idx     - index of secure port key, it could be 0
...
{
    return hmac_node_steps(HASH_MD5, key, sp_key_idx, key_bytes, node, node_num, mac);
}
    #endif
...
unsigned int hmac_md5_dma_final(HMAC_MD5_DMA_CTX *ctx, unsigned int remainder_msg_h, unsigned int remainder_msg_l, unsigned int remainder_bytes, unsigned int mac_h, unsigned int mac_l)
{
 ...
 */
            #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
unsigned int hmac_md5_dma_node_steps(unsigned char *key, unsigned short sp_key_idx, unsigned int key_bytes, HASH_DMA_NODE *node, unsigned int node_num, unsigned int mac_h, unsigned int mac_l, HASH_CALLBACK callback)
{
    return hmac_dma_node_steps(HASH_MD5, key, sp_key_idx, key_bytes, node, node_num, mac_h, mac_l, callback);
}
            #else
...
{
    return hmac_dma_node_steps(HASH_MD5, key, sp_key_idx, key_bytes, node, node_num, mac, callback);
}
            #endif
        #endif
