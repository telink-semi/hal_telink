...
unsigned int ed25519_p[8]   = {0xFFFFFFEDu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0x7FFFFFFFu};
unsigned int ed25519_p_h[8] = {
    0x000005A4u,
    0u,
    0u,
    0u,
...
unsigned int ed25519_p_n0[1] = {0x286BCA1Bu};
unsigned int ed25519_d[8]    = {0x135978A3u, 0x75EB4DCAu, 0x4141D8ABu, 0x00700A4Du, 0x7779E898u, 0x8CC74079u, 0x2B6FFE73u, 0x52036CEEu};
unsigned int ed25519_Gx[8]   = {0x8F25D51Au, 0xC9562D60u, 0x9525A7B2u, 0x692CC760u, 0xFDD6DC5Cu, 0xC0A4E231u, 0xCD6E53FEu, 0x216936D3u};
unsigned int ed25519_Gy[8]   = {0x66666658u, 0x66666666u, 0x66666666u, 0x66666666u, 0x66666666u, 0x66666666u, 0x66666666u, 0x66666666u};
unsigned int ed25519_n[8]    = {0x5CF5D3EDu, 0x5812631Au, 0xA2F79CD6u, 0x14DEF9DEu, 0x00000000u, 0x00000000u, 0x00000000u, 0x10000000u};
unsigned int ed25519_n_h[8]  = {0x449C0F01u, 0xA40611E3u, 0x68859347u, 0xD00E1BA7u, 0x17F5BE65u, 0xCEEC73D2u, 0x7C309A3Du, 0x0399411Bu};
...
unsigned int ed25519_get_pubkey_from_prikey(unsigned char prikey[32], unsigned char pubkey[32])
{
    ...
    SHA512_Hash(prikey, 32, (unsigned char *)h);
...
unsigned int ed25519_sign(Ed25519_MODE mode, unsigned char prikey[32], unsigned char pubkey[32], unsigned char *ctx, unsigned char ctxByteLen, unsigned char *M, unsigned int MByteLen, unsigned char RS[64])
{
    ...
    SHA512_Hash(prikey, Ed25519_BYTE_LEN, (unsigned char *)h);
    ...
    //PH_M
    if (Ed25519_PH == mode) {
        SHA512_Hash(M, MByteLen, (unsigned char *)PH_M);
    ...
    if (Ed25519_DEFAULT != mode) {
        SHA512_Process(sha512_ctx, (unsigned char *)Ed25519_sign_string, sizeof(Ed25519_sign_string));
        // ret = hash_update(sha512_ctx, (unsigned char *)Ed25519_sign_string, sizeof(Ed25519_sign_string));
        // if(HASH_SUCCESS != ret)
        // {
        //     return ret;
        // }
        ...
        SHA512_Process(sha512_ctx, (unsigned char *)&phflag, 1);
        ...
        SHA512_Process(sha512_ctx, (unsigned char *)&ctxByteLen, 1);
        ...
        SHA512_Process(sha512_ctx, ctx, ctxByteLen);
    ...
    //prefix
    SHA512_Process(sha512_ctx, prefix, Ed25519_BYTE_LEN);
    ...
    //PH(M)
    if (Ed25519_PH == mode) {
        SHA512_Process(sha512_ctx, (unsigned char *)PH_M, 64);
        // ret = hash_update(sha512_ctx, (unsigned char *)PH_M, 64);
    } else {
        SHA512_Process(sha512_ctx, M, MByteLen);
    ...
    SHA512_Done(sha512_ctx, (unsigned char *)k);
    ...
    //r = k mod n
    #if defined(PKE_LP)
    ret = pke_mod(&(k[Ed25519_WORD_LEN - 1u]), Ed25519_WORD_LEN + 1u, ed25519->n, ed25519->n_h, ed25519->n_n0, Ed25519_WORD_LEN, h + Ed25519_WORD_LEN);
    #else
    ret = pke_mod(&(k[Ed25519_WORD_LEN - 1u]), Ed25519_WORD_LEN + 1u, ed25519->n, ed25519->n_h, Ed25519_WORD_LEN, h + Ed25519_WORD_LEN);
    #endif
    if (PKE_SUCCESS != ret) {
        return ret;
    } else {
    ...
    //R
    SHA512_Process(sha512_ctx, RS, Ed25519_BYTE_LEN);
    ...
    if (NULL == pubkey) {
        ...
        SHA512_Process(sha512_ctx, (unsigned char *)(k + Ed25519_WORD_LEN), Ed25519_BYTE_LEN);
        // ret = hash_update(sha512_ctx, (unsigned char *)(k+Ed25519_WORD_LEN), Ed25519_BYTE_LEN);
    } else {
        SHA512_Process(sha512_ctx, pubkey, Ed25519_BYTE_LEN);
