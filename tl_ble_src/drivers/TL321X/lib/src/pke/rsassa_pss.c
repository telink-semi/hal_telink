 ...
 * @note
  ...
  @endverbatim
 */
unsigned int emsa_pss_encode_by_msg_digest(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg, unsigned char *salt, unsigned int salt_bytes, unsigned char *msg_digest, unsigned int msg_digest_bytes, unsigned char *em, unsigned int em_bits)
{
 ...
 * @note
  ...
  @endverbatim
 */
unsigned int emsa_pss_verify_by_msg_digest(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg, int32_t salt_bytes, unsigned char *msg_digest, unsigned int msg_digest_bytes, unsigned char *em, unsigned int em_bits)
{
    #if 0
    ...
    //recover DB
    ret = rsa_pkcs1_mgf1_with_xor_in(mgf_hash_alg, em + em_bytes - 1 - msg_digest_bytes, msg_digest_bytes, em, buf, em_bytes - 1 - msg_digest_bytes);
    if (ret != RSA_SUCCESS) {
        goto END;
    } else {
        ;
...
unsigned int rsa_ssa_pss_sign_by_msg_digest(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg, unsigned char *salt, unsigned int salt_bytes, unsigned char *msg_digest, unsigned char *d, unsigned char *n, unsigned int n_bits, unsigned char *signature)
{
    ...
    em[0] = 0;
    ret   = emsa_pss_encode_by_msg_digest(msg_hash_alg, mgf_hash_alg, salt, salt_bytes, msg_digest, msg_digest_bytes, (1 == (n_bits & 7)) ? em + 1 : em, n_bits - 1);
    #else
...
unsigned int rsa_ssa_pss_verify_by_msg_digest(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg, int32_t salt_bytes, unsigned char *msg_digest, unsigned char *e, unsigned int e_bits, unsigned char *n, unsigned int n_bits, unsigned char *signature)
{
    ...
    msg_digest_bytes = hash_get_digest_word_len(msg_hash_alg) << 2;
    ret              = emsa_pss_verify_by_msg_digest(msg_hash_alg, mgf_hash_alg, salt_bytes, msg_digest, msg_digest_bytes, (1 == tmp) ? em + 1 : em, n_bits - 1);
    #else
    ret = emsa_pss_verify_by_msg_digest(msg_hash_alg, mgf_hash_alg, salt_bytes, msg_digest, msg_digest_bytes, em, n_bits - 1);
    #endif
