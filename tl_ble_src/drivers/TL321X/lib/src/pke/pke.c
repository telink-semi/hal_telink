 ...
 * @note
  ...
  @endverbatim
 */
unsigned int pke_modadd_modsub_internal(unsigned int *modulus, unsigned int *a, unsigned int *b,
        unsigned int *out, unsigned int wordLen, unsigned int micro_code)
{
    unsigned int step_bytes, step_words;
    unsigned int ret;
        ...
        unsigned char *out, unsigned int mod_bitLen, unsigned int exp_bitLen, unsigned int calc_pre_monts)
{
    ...
    pke_load_operand_U8((unsigned int *)(rPKE_B(3u,step_bytes)), modulus, mod_byteLen);    //B3 modulus
    if(step_words > mod_wordLen)
    {
        uint32_clear((unsigned int *)(rPKE_B(3u,step_bytes))+mod_wordLen, step_words-mod_wordLen);
        uint32_clear((unsigned int *)(rPKE_B(0u,step_bytes))+mod_wordLen, step_words-mod_wordLen);
    ...
    ret = pke_modexp_check_input((unsigned int *)(rPKE_B(3u,step_bytes)), (unsigned int *)(rPKE_A(1u,step_bytes)),
            (unsigned int *)(rPKE_B(0u,step_bytes)), (unsigned int *)(rPKE_A(0u,step_bytes)), mod_wordLen, exp_wordLen);
    if(PKE_FINISHED == ret)
    {
        goto END;
    }
        ...
        unsigned int *Qx, unsigned int *Qy)
{
    ...
    ret = pke_set_modulus_and_pre_monts(curve->eccp_p, curve->eccp_p_h, curve->eccp_p_n0,
            GET_MAX_LEN(curve->eccp_p_bitLen, curve->eccp_n_bitLen));
    if(PKE_SUCCESS != ret)
    {
        return ret;
    }
