...
#define PM_DEBUG 0
//the code run time for preparation entering and exiting suspend sleep, 116-147(while waiting time). Because B91 fluctuates greatly, we need to reserve more here.
#define SUSPEND_CODE_US   389
...
_attribute_data_retention_sec_ volatile pm_early_wakeup_time_us_s g_pm_longsleep_early_wakeup_time_us = {
...
_attribute_ram_code_sec_optimize_o2_ void pm_wait_xtal_ready(unsigned char all_ramcode_en)
{
    ...
    if (j == g_pm_xtal_stable_loopnum) {
        ...
        analog_write_reg8(PM_ANA_REG_POWER_ON_CLR_BUF0, analog_read_reg8(PM_ANA_REG_POWER_ON_CLR_BUF0) | XTAL_UNSTABLE);
#if (PM_DEBUG)
        while (1);
#endif
...
void pm_update_status_info(void)
{
    ...
    if (analog_38 & BIT(0)) {
        if (analog_39 & BIT(0)) {
            ...
            analog_write_reg8(PM_ANA_REG_WD_CLR_BUF0, analog_38 & 0xfe);
            analog_write_reg8(PM_ANA_REG_POWER_ON_CLR_BUF0, analog_39 | REBOOT_FLAG);
        }
    } else {
        if (!(analog_7f & 0x01)) {
