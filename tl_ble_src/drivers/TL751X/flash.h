 ...
 *      - Since a maximum of 4 flash can be connected, SLAVE0-SLAVE3, access space 64m,
 *        the space size is divided into SLAVE0-SLAVE3 through the interface mspi_slave_device_addr_space_config().
 *        The MSPI module selects which SLAVE to access according to the space divided;
 *        Therefore, addr of the flash interface = flash access location + SLAVE base address.
...
typedef struct{
    ...
    unsigned int  flash_xip_ddr_mode:1;
