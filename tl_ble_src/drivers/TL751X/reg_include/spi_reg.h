...
/**
 * BIT[0:1] data I/O MODE, 2'b00:single  2'b01:dual  2'b10:quad  2'b11:octal.master only.
 ...
 * BIT[3]       enable ddr mode.1: Enable ddr mode, data transmission will be performed on both the rising and falling edges of SCK, that is, one SCK clock can transmit two bits of data, and the transmission rate is doubled. But the actual SPI_CLK is 1/2 of the configuration SPI_CLK.
 */
...
/**
 * BIT[0:1]  spi xip read I/O mode, 2'b00 single,2'b01 dual, 2'b10 quad, 2'b11 octal. XIP ONLY
 ...
 * BIT[20]      enable ddr mode.1: Enable ddr mode, data transmission will be performed on both the rising and falling edges of SCK, that is, one SCK clock can transmit(RD/WR) two bits of data, and the transmission rate is doubled. But the actual SPI_CLK is 1/2 of the configuration SPI_CLK.
...
typedef enum{
    ...
    FLD_SPI_XIP_RD_DDR_MODE      =BIT(20),
 ...
 * i = [0, 15],j = [0, 2].
 */
#define reg_lspi_lcd_lut_data32(i)  REG_ADDR8(LSPI_BASE_ADDR+0x40+(i<<2))
