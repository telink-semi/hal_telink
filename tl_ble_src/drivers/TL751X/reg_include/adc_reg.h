...
#define areg_adc_clk_setting  0x107
enum
{
    FLD_CLK_24M_TO_SAR_EN = BIT(0),
};

#define areg_adc_res_m                  0x16c
enum{
    FLD_ADC_RES_M         =             BIT_RNG(0,1),
    FLD_ADC_EN_DIFF_CHN_M =             BIT(6),
};

#define areg_adc_data_transfer_control  0x172
enum{
    FLD_AUTO_NOT_EN         = BIT(3),
};

#define areg_adc_data_sample_control    0x173
enum{
    FLD_NOT_SAMPLE_ADC_DATA         = BIT(0),//hold mode
    FLD_DWA_EN_O                    = BIT(1),
    FLD_ANA_RD_EN                   = BIT(2),
};
#define areg_adc_sample_clk_div     0x174
enum{
    FLD_ADC_SAMPLE_CLK_DIV  =          BIT_RNG(0,3),
};

#define areg_m_chn_data_valid_status                0x176
enum{
...
#define areg_adc_pga_ctrl               0x17c
enum{
...
enum{
    FLD_TEMP_SENSOR_POWER_DOWN = BIT(7),
};
...
enum{
    FLD_USB_VOLT_ID_CAL_CONFIG = BIT(0),
    FLD_USB_VOLT_ID_EN = BIT(1),
    FLD_M_CH_TRANS_EN = BIT(2),
...
enum{
    ...
    FLD_SOFT_START = BIT(7),//Control the analog side of the adc sampling, write 1 to get 1 sample data.
};
