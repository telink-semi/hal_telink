...
{
    BM_SET(reg_usb1_ep8_fifo_mode, FLD_USB1_ENP8_FIFO_MODE);
}
...
{
    BM_CLR(reg_usb1_ep8_fifo_mode, FLD_USB1_ENP8_FIFO_MODE);
}
...
{
    return ((ep < 7) ? (reg_usb1_ep_ptr(ep)) : (reg_usb1_ep_ptr(ep) | (reg_usb1_ep_ptrh(ep) << 8)));
}
...
static inline void usb1hw_set_ep_addr(usb1_ep_index ep, unsigned short addr)
{
    ...
    if (ep > 6)
    {
        reg_usb1_ep_buf_h_addr(ep) = (addr >> 8) & 0xff;
    }
}
 ...
 * @return    none.
 */
static inline void usb1hw_set_printer_threshold(unsigned char th)
{
    reg_usb1_ep8_send_thres = th;
}
...
{
    unsigned char dat = analog_read_reg8(0x20);
    if (en)
    {
        dat = dat & (~BIT(7));
    }
 ...
 * @return  none.
 */
static inline void usb1_set_pin_en(void)
{
    reg_gpio_func_mux(GPIO_PH0) = reg_gpio_func_mux(GPIO_PH0) & (~BIT_RNG(0, 4));
    reg_gpio_func_mux(GPIO_PH1) = reg_gpio_func_mux(GPIO_PH1) & (~BIT_RNG(0, 4));
    gpio_function_dis(GPIO_PH0 | GPIO_PH1);
    gpio_input_en(GPIO_PH0 | GPIO_PH1); /* DP/DM must set input enable. */
    usb1_dp_pullup_en(1);
}
