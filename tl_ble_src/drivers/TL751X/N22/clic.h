 ...
 *  Header File: clic.h
 */
#ifndef CLIC_INTERRUPT_H
#define CLIC_INTERRUPT_H
#include "core.h"
#include "compiler.h"
...
#define IRQ_GPIO                    43 /* GPIO_IRQ, - gpio_irq_handler */
#define IRQ_GPIO2RISC0              44 /* GPIO2RISC[0]_IRQ, - gpio_risc0_irq_handler */
#define IRQ_GPIO2RISC1              45 /* GPIO2RISC[1]_IRQ, - gpio_risc1_irq_handler */
...
#define IRQ_NPE_SYNC                60 /* NPE_SYNC_IRQ, - npe_sync_irq_handler */
...
#define IRQ_EMQ_IRQ                 64 /* EMQ_IRQ, - emq_irq_handler */
...
/**
 * @brief Interrupt source level. The larger the value, the higher the level. The default level value is 0.
 * @note  An interrupt occurs only when level is greater than threshold.
 */
typedef enum
{
    IRQ_LEV0, /**< 0 indicates that no interrupt is generated. */
    IRQ_LEV1,
    IRQ_LEV2,
    IRQ_LEV3,
} irq_level_e;
 ...
 * @return  none
 */
static inline void clic_init(void)
{
    reg_clic_cfg = ((reg_clic_cfg & (~FLD_CLIC_NLBITS)) | (2 << 1)); /* NLBITS reset value is 0 and can only be set to 2. */
    reg_clic_mth = 0x3f; /* The lower 6 bits of the reg_clic_input_ctl register are 0x3f by default, so set the lower 6 bits of the threshold to 0x3f as well to block interrupts with level 0. */
}
...
{
    reg_clic_attribute(src) |= FLD_CLIC_SHV;
}

/**
 * @brief      This function serves to select non-vector mode for the associated interrupt.
 * @param[in]  src - Interrupt source see @ref irq_source_index.
 * @return     none
 * @note       In non-vector mode, the interrupt service routine is trap_entry().
 */
static inline void clic_interrupt_vector_dis(unsigned int src)
{
    reg_clic_attribute(src) &= (~FLD_CLIC_SHV);
}

/**
 * @brief      This function serves to set CLIC pending.
 * @param[in]  src - Interrupt source see @ref irq_source_index.
 * @return     none
 */
...
/**
 * @brief      This function serves to set interrupt levels, higher-numbered interrupt levels can preempt lower-numbered interrupt levels.
 * @param[in]  src - Interrupt source see @ref irq_source_index.
 * @param[in]  level - Interrupt level see @ref irq_trigger_e.
 * @return     none
 */
static inline void clic_set_level(unsigned int src, irq_level_e level)
{
    reg_clic_input_ctl(src) = ((reg_clic_input_ctl(src) & (~FLD_CLIC_LEVEL)) | (level << 6));
}

/**
 * @brief      This function serves to set priority threshold, only active interrupts with interrupt levels(clic_set_level()) strictly greater than the threshold will cause interrupt.
 * @param[in]  mth - machine mode interrupt level threshold.
 * @return     none
 */
static inline void clic_set_threshold(irq_mth_e mth)
{
    reg_clic_mth = ((reg_clic_mth & (~FLD_CLIC_MTH)) | (mth << 6));
}
