...
static _always_inline void dma_config(dma_chn_e chn ,dma_config_t *config)
{
    ...
    if(chn>>3)
    {
         reg_dma_set|=BIT((dst_req_sel != 0) ? dst_req_sel : src_req_sel);
    }
    else
    {
        reg_dma_set &=(~BIT((dst_req_sel != 0) ? dst_req_sel : src_req_sel));
...
/**
 When the source address/ destination address are not aligned according to burst_size,and DMA burst 4 or 8 are turned on.
 The address will definitely fall in the [1k-burst_size, 1k] interval,There are two treatment options:
 one_k_mode_en-0:Starting from the interval (1k-burst_size, 1k),DMA transmits data according to the unit of 4 bytes(no burst) and the length is burst_size, then continue to transmit in units of burst_size
 one_k_mode_en-1: Regardless of the limit of the 1k boundary,DMA always transmits in units of burst_size
 * @param[in] chn            - DMA channel
 * @param[in] one_k_mode_en  - 0/1
 * @return    none
 * @note      According to whether there is no 1k boundary limit of peripheral module, users cannot configure by themselves.
 ...
 */
static inline void dma_set_size_1k_mode(dma_chn_e chn,unsigned char one_k_mode_en)
{
    reg_dma_cr3(chn) =one_k_mode_en<<7;
}
