...
void usb1hw_init(void)
{
    ...
    BM_SET(reg_rst5, FLD_RST5_USB1);
    BM_SET(reg_clk_en5, FLD_CLK5_USB1_EN);
}
...
{
    /* ep8 fifo mode. */
    if ((USB1_EDP8_IN == ep) && (BM_IS_SET(reg_usb1_ep8_fifo_mode, FLD_USB1_ENP8_FIFO_MODE)))
    {
        ...
        if (data_ptr)
        {
            ...
            for (unsigned char i = 0; (i < 4 - data_ptr) && (i < len); i++)
            {
                ...
                while (reg_usb1_ep8_fifo_mode & FLD_USB1_ENP8_FULL_FLAG)
                {
