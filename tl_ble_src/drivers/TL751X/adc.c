 ...
 * @return     none
 * @attention  ADC digital clock from crystal(48MHz), analog clock from digital clock.
...
/**
 * @brief   This function is used to enable the transmission of data from the adc's M channel to the sar adc rxfifo.
 * @return  none
 * @note    Both adc_m_chn_data_too_fifo_en() and adc_all_chn_data_too_fifo_en() must be called in order for adc code from the m-channel to be transferred to the rxfifo.
 */
static inline void adc_m_chn_data_to_fifo_en(void)
{
    reg_adc_config0 |= FLD_M_CH_TRANS_EN;
}
/**
 * @brief   This function is used to disable the transmission of data from the adc's M channel to the sar adc rxfifo.
 * @return  none
 * @note    Only call adc_m_chn_data_to_fifo_dis() and the adc code for the m-channel will not be transferred to the rxfifo.
 */
static inline void adc_m_chn_data_to_fifo_dis(void)
{
    reg_adc_config0 &= (~FLD_M_CH_TRANS_EN);
}
/**
...
static void adc_set_sample_rate(adc_sample_chn_e chn,adc_sample_freq_e sample_freq)
{
    ...
    adc_set_tsample_cycle(chn,adc_sample_cycle);//24 adc clocks for sample cycle
...
void adc_set_dma_config(dma_chn_e chn)
{
    ...
    /*
     * The RX FIFO is stored in WORD units, and a setting of 0 indicates that a DMA request is sent to the DMA when the data in the RX FIFO is greater than 0 WORD units of data (i.e., 1 WORD).
     */
    adc_set_dma_trig_num(0);//Default value is 0, users should not change.
}
/**
