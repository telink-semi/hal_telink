...
#define aes_base_addr               (0xd4160000)
...
#define reg_embase_addr                 REG_ADDR32(PDZBADDR+0x04)
...
aes_encryption_le(KEY, SKD, ASK);
tlkapi_printf(1, "AES result is %s", hex_to_str(ASK, 16));
    //AES result is 66 c6 c2 27 8e 3b 8e 05 3e 7e a3 26 52 1b ad 99
******************************************************************************************************************************/
...
void aes_encryption_le(u8* key, u8* plaintext, u8 *encrypted_data)
{
    ...
    reg_embase_addr = aes_data_buff;  //set the embase addr //BLE MUST
    ...
    reg_aes_ptr = 0;  //the aes data ptr is base on embase address.

    reg_aes_mode = (FLD_AES_START | AES_ENCRYPT_MODE);
