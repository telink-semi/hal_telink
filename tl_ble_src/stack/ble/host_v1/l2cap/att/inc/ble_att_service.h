 ...
 *
 *  - Write callback(BIT0): When the client want write the attribute value, stack will first call write callback function.
 *      If write callback is not set, stack will check Allow write bit.
 *      If write callback is set, stack will call write callback function.
 *          If write callback function return ATT_SUCCESS, stack will check Allow write bit, and replay response.
 *          If write callback function return value range 1 ot 0xFF, stack will reply ATT_ERROR_RSP with error code that return value.
 ...
 *
 *  This function is called when the client wants to write an attribute value, and ATTS_SET_WRITE_CALLBACK is setted.
 *
 ...
 *
 *  This function is called when the client wants to read an attribute value, and ATTS_SET_READ_CALLBACK is setted.
 *
 *  @param[in] conn_handle Connection handle.
