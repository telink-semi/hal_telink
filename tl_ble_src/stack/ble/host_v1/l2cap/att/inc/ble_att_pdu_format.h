...
struct att_find_information_rsp_format {
    ...
    union { // information data. must least one 16bit uuid.
...
struct att_find_by_type_value_rsp_format {
    ...
    struct attr_handle_group list[1];/** < A list of 1 or more Handle Informations. */
}__attribute__((packed));
...
/**
 *  @brief Definition format fo Handle length Value Tuple.
