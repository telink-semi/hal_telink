...
#define ATT_MAXIMUM_MTU                 517

#define BLE_ATT_DEFAULT_MTU             ATT_MINIMUM_MTU
#define ATT_MTU_CHECK(mtu)              ((mtu) >= ATT_MINIMUM_MTU && (mtu) <= ATT_MAXIMUM_MTU)
...
enum attribute_error_code {
    ...
    ATT_ERR_CCC_DESCRIPTOR_IMPROPERLY_CONFIGURED = 0xFB,
...
struct ble_host_att_process_handle {
    ...
    uint16_t(*att_rx_handler)(const struct ble_host_att_deal_info *,
        const struct ble_att_pdu_format *, uint16_t);
};

/**
 *   @brief Send ATT packet synchronously, only used for att layer ack request packet.
 *
 ...
 */
int ble_host_att_send_packet_sync(struct ble_host_conn *conn,
