...
struct ble_hci_le_ext_create_conn_cp
{
    ...
    struct conn_params conn_params[2];
} __attribute__((packed));
...
typedef void (*hci_transport_layer_process_t)(void);
...
/**
 *   @brief BLE Host HCI command send strategy set.
 *
 *   @param[in] strategy - refer to enum HCI_SEND_STRATEGY,.
 *
 *   @return None.
 */
