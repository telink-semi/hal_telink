...
static uint8_t mcsMediaPlayerNameValue[LE_AUDIO_MCS_PMEDIA_PLAYER_NAME_MAX_SIZE];
_attribute_ble_data_retention_
static uint16_t mcsMediaPlayerNameValueLen;
const uint16_t mcsMediaPlayerNameMaxSize = sizeof(mcsMediaPlayerNameValue);

#if LE_AUDIO_MCS_MEDIA_PLAYER_ICON_OBJECT_ID
_attribute_ble_data_retention_
static uint8_t mcsMediaPlayerIconObjectIDValue[6];
_attribute_ble_data_retention_
static uint16_t mcsMediaPlayerIconObjectIDValueLen;
#endif
...
_attribute_ble_data_retention_
static uint8_t mcsMediaPlayerIconURLValue[LE_AUDIO_MCS_PMEDIA_PLAYER_ICON_URL_MAX_SIZE];
_attribute_ble_data_retention_
static uint16_t mcsMediaPlayerIconURLValueLen;
#endif

static const uint16_t gMcsTrackChangedLen = 0;

_attribute_ble_data_retention_
static uint8_t mcsTrackTitleValue[LE_AUDIO_MCS_TRACK_TITLE_MAX_SIZE];
_attribute_ble_data_retention_
static uint16_t mcsTrackTitleValueLen;
const uint16_t mcsTrackTitleMaxSize = sizeof(mcsTrackTitleValue);

_attribute_ble_data_retention_
static uint32_t mcsTrackDurationValue = 0xFFFFFFFF;
static const uint16_t                   mcsTrackDurationValueLen = 4;

_attribute_ble_data_retention_
static uint32_t mcsTrackPositionValue = 0xFFFFFFFF;
static const uint16_t mcsTrackPositionValueLen = 4;

#if LE_AUDIO_MCS_PLAYBACK_SPEED
_attribute_ble_data_retention_
static char mcsPlaybackSpeedValue = 0;
static const uint16_t mcsPlaybackSpeedValueLen = 1;
#endif

#if LE_AUDIO_MCS_SEEKING_SPEED
_attribute_ble_data_retention_
static char mcsSeekingSpeedValue = 0;
static const uint16_t mcsSeekingSpeedValueLen = 1;
#endif

#if LE_AUDIO_MCS_CURRENT_TRACK_OBJECT_ID
_attribute_ble_data_retention_
static uint8_t mcsCurrentTrackSegmentsObjectIDValue[6];
_attribute_ble_data_retention_
static uint16_t mcsCurrentTrackSegmentsObjectIDValueLen;

_attribute_ble_data_retention_
static uint8_t mcsCurrentTrackObjectIDValue[6];
_attribute_ble_data_retention_
static uint16_t mcsCurrentTrackObjectIDValueLen;

_attribute_ble_data_retention_
static uint8_t mcsNextTrackObjectIDValue[6];
_attribute_ble_data_retention_
static uint16_t mcsNextTrackObjectIDValueLen;

_attribute_ble_data_retention_
static uint8_t mcsParentGroupTrackObjectIDValue[6];
_attribute_ble_data_retention_
static uint16_t mcsParentGroupTrackObjectIDValueLen;

_attribute_ble_data_retention_
static uint8_t mcsCurrentGroupTrackObjectIDValue[6];
_attribute_ble_data_retention_
static uint16_t mcsCurrentGroupTrackObjectIDValueLen;
#endif

#if LE_AUDIO_MCS_PLAYING_ORDER
_attribute_ble_data_retention_
static uint8_t mcsPlayingOrderValue = 0x01;
static const uint16_t mcsPlayingOrderValueLen = 1;
#endif

#if LE_AUDIO_MCS_PLAYING_ORDERS_SUPPORTED
_attribute_ble_data_retention_
static uint16_t mcsPlayingOrderSupportedValue;
static const uint16_t mcsPlayingOrderSupportedValueLen = 2;
#endif

_attribute_ble_data_retention_
static uint8_t mcsMediaStateValue = 0x00;
static const uint16_t mcsMediaStateValueLen = 1;

#if LE_AUDIO_MCS_MEDIA_CONTROL_POINT
_attribute_ble_data_retention_
static uint8_t mcsMediaControlPointValue[5];
_attribute_ble_data_retention_
static uint16_t mcsMediaControlPointValueLen;

_attribute_ble_data_retention_
static uint32_t mcsMediaControlPointSupportedValue;
static const uint16_t mcsMediaControlPointSupportedValueLen = 4;
#endif

#if LE_AUDIO_MCS_SEARCH_RESULTS_OBJECT_ID
_attribute_ble_data_retention_
static uint8_t mcsSearchResultsObjectIDValue[6];
_attribute_ble_data_retention_
static uint16_t mcsSearchResultsObjectIDValueLen;

_attribute_ble_data_retention_
static uint8_t mcsSearchControlPointValue;
static const uint16_t mcsSearchControlPointValueLen = sizeof(mcsSearchControlPointValue);
#endif

_attribute_ble_data_retention_
static uint8_t mcsCCIDValue;
...
static const struct atts_attribute mcsList[] =
{
    ...
    ATTS_CHAR_UUID_ENCR_WRITE_ENTITY_CB(charPropReadNotify, characteristicSearchControlPointUuid, mcsSearchControlPointValue),
    ATTS_COMMON_CCC_DEFINE,
#endif
 ...
 */
_attribute_ble_data_retention_
static struct atts_group svcMcsGroup =
{
    NULL,
    mcsList,
    NULL,
