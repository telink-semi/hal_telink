...
struct svc_csis_SIRK csisSIRKValue;
static const uint16_t csisSIRKValueLen = sizeof(struct svc_csis_SIRK);

#if LE_AUDIO_CSIS_COORDINATED_SET_SIZE
_attribute_ble_data_retention_
uint8_t csisCSSizeValue;
static const uint16_t csisCSSizeValueLen = 1;
#endif

#if LE_AUDIO_CSIS_SET_MEMBER_LOCK
_attribute_ble_data_retention_
uint8_t csisSetMemberLockValue;
static const uint16_t csisSetMemberLockValueLen = 1;
#endif

#if LE_AUDIO_CSIS_SET_MEMBER_RANK
_attribute_ble_data_retention_
uint8_t csisSetMemberRankValue;
static const uint16_t csisSetMemberRankValueLen = 1;
#endif

#if LE_AUDIO_CSIS_COORDINATED_SET_NAME

static uint8_t csisCoordinatedSetNameValue[] = "Telink CSIS Profile";
static const uint16_t csisCoordinatedSetNameValueLen = sizeof(csisCoordinatedSetNameValue);
#endif
...
static const struct atts_attribute csisList[] =
{
    ...
    ATTS_CHAR_UUID_ENCR_READ_ENTITY_CB(charPropReadNotify, characteristicSetIdentityResolvingKeyUuid, csisSIRKValue),
...
#if LE_AUDIO_CSIS_SET_MEMBER_RANK
    //Set Member Rank
    ATTS_CHAR_UUID_ENCR_READ_ENTITY_CB(charPropReadNotify, characteristicSetMemberRankUuid, csisSetMemberRankValue),
#endif
 ...
 */
_attribute_ble_data_retention_
static struct atts_group svcCsisGroup =
{
    NULL,
    csisList,
    NULL,
