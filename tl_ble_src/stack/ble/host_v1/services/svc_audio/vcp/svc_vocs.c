...
#define ATTS_CHAR_VOL_OFFSET_STATE(value)                 \
    ATTS_CHARACTERISTIC_DECLARATIONS(charPropReadNotify), \
        ATTS_CHAR_UUID_DEFINE(ATT_PERMISSIONS_ENCRYPT_READ, \
                                characteristicVolumeOffsetStateUuid, \
                                vocsVolumeOffsetStateValueLen, \
                                VOCS_VOL_OFFSET_STATE_FIX_LEN, value, 0)
...
#define ATTS_CHAR_AUDIO_LOCATION(value)                               \
    ATTS_CHARACTERISTIC_DECLARATIONS(charPropReadWriteWithoutNotify), \
        ATTS_CHAR_UUID_DEFINE(ATT_PERMISSIONS_ENCRYPT_RDWR, \
                                characteristicAudioLocationUuid, \
                                vocsAudioLocationValueLen, \
                                VOCS_AUDIO_LOCATION_FIX_LEN, \
                                value, ATTS_SET_WRITE_CALLBACK | ATTS_SET_ALLOW_WRITE)
...
#define ATTS_CHAR_VOL_OFFSET_CTRL_POINT()                                                                                                             \
    ...
    {                                                                                                                                                 \
        ...
        ATT_16_UUID_LEN, \
        (uint8_t *)(size_t)characteristicVolumeOffsetControlPointUuid, \
...
#define ATTS_CHAR_AUDIO_OUTPUT_DESC(value, len)                       \
    ATTS_CHARACTERISTIC_DECLARATIONS(charPropReadWriteWithoutNotify), \
        ATTS_CHAR_UUID_DEFINE(ATT_PERMISSIONS_ENCRYPT_RDWR, \
                                characteristicAudioOutputDescriptionUuid, \
                                len, VOCS_OUTPUT_DESC_MAX_SIZE, value, \
...
static struct atts_group svcVocsGroup =
{
    NULL,
    vocsList,
    NULL,
