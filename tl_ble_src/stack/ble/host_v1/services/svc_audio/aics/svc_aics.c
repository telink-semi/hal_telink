...
uint8_t aicsAudioInputStateValue[APP_AUDIO_AICS_SERVER_MAX_INSTANCE_NUM][AICS_AUDIO_IN_STATE_FIX_LEN];
_attribute_ble_data_retention_
uint8_t aicsGainSettingPropertiesValue[APP_AUDIO_AICS_SERVER_MAX_INSTANCE_NUM][AICS_GAIN_SET_PROPERTIES_FIX_LEN];
_attribute_ble_data_retention_
uint8_t aicsAudioInputTypeValue[APP_AUDIO_AICS_SERVER_MAX_INSTANCE_NUM];
_attribute_ble_data_retention_
uint8_t aicsAudioInputStatusValue[APP_AUDIO_AICS_SERVER_MAX_INSTANCE_NUM];
_attribute_ble_data_retention_
uint8_t aicsAudioInputDescriptionValue[APP_AUDIO_AICS_SERVER_MAX_INSTANCE_NUM][AICS_INPUT_DESC_MAX_SIZE];
_attribute_ble_data_retention_
uint16_t aicsAudioInputDescriptionValueLen[APP_AUDIO_AICS_SERVER_MAX_INSTANCE_NUM];
#endif

#define ATTS_CHAR_AUDIO_INPUT_STATE(value)                \
    ATTS_CHARACTERISTIC_DECLARATIONS(charPropReadNotify), \
        ATTS_CHAR_UUID_DEFINE(ATT_PERMISSIONS_ENCRYPT_READ, \
                                characteristicAudioInputStateUuid, \
                                aicsAudioInputStateValueLen, \
                                AICS_AUDIO_IN_STATE_FIX_LEN, value, 0)

#define ATTS_CHAR_GAIN_SETTING(value)               \
    ATTS_CHARACTERISTIC_DECLARATIONS(charPropRead), \
        ATTS_CHAR_UUID_DEFINE(ATT_PERMISSIONS_ENCRYPT_READ, \
                                characteristicGainSettingsAttributeUuid, \
                                aicsGainSettingPropertiesValueLen, \
                                AICS_GAIN_SET_PROPERTIES_FIX_LEN, value, 0)

#define ATTS_CHAR_AUDIO_INPUT_TYPE(value)           \
    ATTS_CHARACTERISTIC_DECLARATIONS(charPropRead), \
        ATTS_CHAR_UUID_DEFINE(ATT_PERMISSIONS_ENCRYPT_READ, \
                                characteristicAudioInputTypeUuid, \
                                aicsAudioInputTypeValueLen, \
                                AICS_AUDIO_IN_TYPE_FIX_LEN, value, 0)

#define ATTS_CHAR_AUDIO_INPUT_STATUS(value)               \
    ATTS_CHARACTERISTIC_DECLARATIONS(charPropReadNotify), \
        ATTS_CHAR_UUID_DEFINE(ATT_PERMISSIONS_ENCRYPT_READ, \
                                characteristicAudioInputStatusUuid, \
                                aicsAudioInputStatusValueLen, \
                                AICS_AUDIO_IN_STATUS_FIX_LEN, value, 0)

#define ATTS_CHAR_AUDIO_INPUT_CTRL_POINT()           \
    ATTS_CHARACTERISTIC_DECLARATIONS(charPropWrite), \
    {                                   \
        ATT_PERMISSIONS_ENCRYPT_WRITE, \
        ATT_16_UUID_LEN, \
        (uint8_t *)(size_t)characteristicAudioInputControlPointUuid, \
        NULL, 0, NULL, ATTS_SET_WRITE_CALLBACK \
    }

#define ATTS_CHAR_AUDIO_INPUT_DESC(value, len)                        \
    ATTS_CHARACTERISTIC_DECLARATIONS(charPropReadWriteWithoutNotify), \
        ATTS_CHAR_UUID_DEFINE(ATT_PERMISSIONS_ENCRYPT_RDWR, \
                                characteristicAudioInputDescriptionUuid, \
                                len, AICS_INPUT_DESC_MAX_SIZE, value, \
 ...
 */
_attribute_ble_data_retention_
static struct atts_group svcAicsGroup =
{
    NULL,
    aicsList,
    NULL,
