...
int hci_le_periodicAdvSyncEstablished_evt_v2 (u8 status, u16 syncHandle,u8 advSID, u8 advAddrType, u8 advAddress[6], u8 advPHY,u16 perdAdvItvl, u8 advClkAccuracy,
                                              u8 num_subevent, u8 subevent_intvl, u8 rsp_slot_delay, u8 rsp_slot_spacing)
{
...
int hci_le_readRemoteSupCapComplete_evt(u8 status, u16 connHandle, u8 *data)
{
    ...
    CS_HCI_LOG("R_REM_CAP:%s",hex_to_str(result, sizeof(hci_le_readRemoteSupCapCompleteEvt_t)));
...
int hci_le_readRemoteFAETableComplete_evt(u8 status, u16 connHandle, u8 *table)
{
    ...
    CS_HCI_LOG("R_REM_FAE_TAB:%s",hex_to_str(result, sizeof(hci_le_readRemoteFAETableCompleteEvt_t)));
...
int hci_le_csSecurityEnableComplete_evt(u8 status, u16 connHandle)
{
    ...
    pEvt->Status = status;
    pEvt->Connection_Handle = connHandle;
    CS_HCI_LOG("Sec_En:%s",hex_to_str(result, sizeof(hci_le_csSecurityEnableCompleteEvt_t)));
...
int hci_le_csConfigComplete_evt(u8 status, u16 connHandle, u8 *data)
{
    ...
    pEvt->T_IP1_Time                    = pCsCfg->T_IP1;
    pEvt->T_IP2_Time                    = pCsCfg->T_IP2;
    pEvt->T_FCS_Time                    = pCsCfg->T_FCS;
    pEvt->T_PM_Time                     = pCsCfg->T_PM;
    smemcpy((u8*)pEvt->Channel_Map , (u8*)pCsCfg->Channel_Map, 10);
    CS_HCI_LOG("cfg:%s",hex_to_str(result, sizeof(hci_le_csConfigCompleteEvt_t)));
...
int hci_le_csProcedureEnableComplete_evt(u8 status, u16 connHandle, u8* data)
{
    ...
    pEvt->Procedure_Count               = pCsCfg->procMaxCount;
    CS_HCI_LOG("[HCI][EVT]cs start success");
    CS_HCI_LOG("start_en:%s",hex_to_str(result, sizeof(hci_le_csProcedureEnableCompleteEvt_t)));
...
int hci_le_csSubeventResult_evt(u16 connhandle,u8 config_id,u8* data,u32 data_length){
    (void)config_id;(void)connhandle;
#if (0)
    u32 result_len = 0;
    ...
    CS_HCI_LOG("[HCI][EVT]cs subevent result");
...
#else
//  CS_HCI_LOG("[HCI][EVT]cs subevent result");
//  CS_HCI_LOG("sub_evt_rst:%s",hex_to_str(data, data_length));
    return blc_hci_send_event (HCI_FLAG_EVENT_BT_STD | HCI_EVT_LE_META, data, data_length);
#endif
}

int hci_le_csSubeventResultContinue_evt(u16 connhandle,u8 config_id,u8* data,u32 data_length){
    (void)config_id;(void)connhandle;
#if (0)
    u32 result_len = 0;
    u8 result[255];//length < MAX HCIevent size
    ...
    CS_HCI_LOG("[HCI][EVT]cs subevent result continue");
    CS_HCI_LOG("sub_evt_rst_cont:%s",hex_to_str(result, result_len));
    return blc_hci_send_event (HCI_FLAG_EVENT_BT_STD | HCI_EVT_LE_META, result, result_len);
#else
//  CS_HCI_LOG("[HCI][EVT]cs subevent result continue");
//  CS_HCI_LOG("sub_evt_rst_cont:%s",hex_to_str(data, data_length));
    return blc_hci_send_event (HCI_FLAG_EVENT_BT_STD | HCI_EVT_LE_META, data, data_length);
#endif
}
...
int hci_le_csTestEndComplete_evt(u8 status){
    ...
    pEvt->Status                        = status;
    CS_HCI_LOG("[HCI][EVT]cs test end");
