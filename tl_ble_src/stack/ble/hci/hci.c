...
{
    blt_gap_hci_event_handler = handler; 
}
...
_attribute_ble_data_retention_  u32     hci_eventMask_2 = 0;
...
ble_sts_t   blc_hci_setEventMask_2_cmd(u32 evtMask)
{
    hci_eventMask_2 = evtMask;
    return BLE_SUCCESS;
}
...
_attribute_ram_code_
#endif
ble_sts_t blc_hci_packIsoData(iso_data_packet_t *pIsoDatPkt)
{
    ...
    if(connHandle & BLT_BIS_HANDLE)
    {
        ...
        iso_sdu =(sdu_packet_t*) (sduBisMng.in_fifo_b + (sduBisMng.in_fifo_num  * bis_sel + \
                                (pBis->bisSduIn_wptr & sduBisMng.in_fifo_mask)) * sduBisMng.max_in_fifo_size);
...
//              hci_remoteNateReqComplete_evt (p + 4);
        ...
        ///controller & Baseband commands--OGF(0x03)
            ...
            switch(opcode) {
                ...
                {
                    status = blc_hci_setEventMask_2_cmd(cmdPara[0] | cmdPara[1]<<8 | cmdPara[2]<<16 | cmdPara[3]<<24);
                    eventCode = HCI_EVT_CMD_COMPLETE;
                    resultLen = hci_cmdComplete_evt(1, opcode, HCI_CMD_CBC_OPCODE_OGF, 1, &status, para);
                    break;
                }
            ...
            switch(opcode) {
                case HCI_CMD_READ_LOCAL_VER_INFO:
                {
                    const u8 tbl[9] = {
                            ...
                            hciRevision & 0xff, (hciRevision>>8) & 0xff,    //HCI Revision
...
//core_5.0 begin
                ...
                case HCI_CMD_LE_SET_PERIODIC_ADVERTISING_RESPONSE_DATA:
                {
                    if(blmsParam.prdSyncWr_en){
                        ...
                        status = (u8)blc_hci_le_setPAwRsync_rspData(pCmdPara->sync_handle, pCmdPara->req_event_count, pCmdPara->req_subevt_idx,\
                                                                    pCmdPara->rsp_subevt_idx, pCmdPara->rsp_slot_idx, pCmdPara->rsp_data_len, pCmdPara->rsp_data);
...
//core_5.3 begin
                ...
                case HCI_CMD_LE_CS_READ_LOCAL_SUPPORTED_CAPABILITIES:
                {
                    ...
                    eventCode = HCI_EVT_CMD_COMPLETE;
                    resultLen = hci_cmdComplete_evt(1, opcode, HCI_CMD_LE_OPCODE_OGF,
                            sizeof(hci_le_cs_readLocalSupportedCap_retParam_t), buff, para);
                }
                break;
                ...
                {
                    CS_HCI_LOG("Write_Cache_Remote_Cap:%s", hex_to_str(cmdPara,
                            sizeof(hci_le_cs_writeCachedRemoteSupportedCap_cmdParam_t)));
                    ...
                    eventCode = HCI_EVT_CMD_COMPLETE;
                    resultLen = hci_cmdComplete_evt(1, opcode, HCI_CMD_LE_OPCODE_OGF,
                            sizeof(hci_le_cs_writeCachedRemoteSupportedCap_retParam_t), buff, para);
                }
                break;
                ...
                {
                    CS_HCI_LOG("CS_Set_Default_Setting:%s", hex_to_str(cmdPara,
                            sizeof(hci_le_cs_setDefaultSetting_cmdParam_t)));
                    ...
                    eventCode = HCI_EVT_CMD_COMPLETE;
                    resultLen = hci_cmdComplete_evt(1, opcode, HCI_CMD_LE_OPCODE_OGF,
                            sizeof(hci_le_cs_setDefaultSetting_retParam_t), buff, para);
                }
                break;
                ...
                case HCI_CMD_LE_CS_WRITE_CACHE_REMOTE_FAE_TABLE:
                {
                    ...
                    status = (u8)blc_hci_le_cs_writeCachedRemoteFAE_table ((cmdPara[0] | cmdPara[1]<<8),&cmdPara[2],
                            (hci_le_cs_writeChchedRemoteFAE_retParam_t *)buff);
                    eventCode = HCI_EVT_CMD_COMPLETE;
                    resultLen = hci_cmdComplete_evt(1, opcode, HCI_CMD_LE_OPCODE_OGF,
                            sizeof(hci_le_cs_writeChchedRemoteFAE_retParam_t), buff, para);
                }
                break;
                case HCI_CMD_LE_CS_CREATE_CONFIG:
                {
                    CS_HCI_LOG("CS_Create_Config:%s", hex_to_str(cmdPara,
                            sizeof(hci_le_cs_creatConfig_cmdParam_t)));
                ...
                {
                    CS_HCI_LOG("Set_Procedure_Param:%s", hex_to_str(cmdPara,
                            sizeof(hci_le_cs_setProcedureParame_cmdParam_t)));
                    ...
                    eventCode = HCI_EVT_CMD_COMPLETE;
                    resultLen = hci_cmdComplete_evt(1, opcode, HCI_CMD_LE_OPCODE_OGF,
                            sizeof(hci_le_cs_setProcedureParam_retParam_t), buff, para);
                }
                break;
...
int blc_hci_iso_send_data (u32 h, u8 *iso_load, int data_load_len)
{
    ...
    (void)data_load_len;(void)h;(void)iso_load;
#if 0
    u8 *p = NULL;
    if (((bltHci_txfifo.wptr - bltHci_txfifo.rptr) & 255) < bltHci_txfifo.num)
    {
