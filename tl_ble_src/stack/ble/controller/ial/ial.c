...
ble_sts_t blt_cis_splitSdu2UnframedPdu(ll_cis_conn_t *pCisConn, sdu_packet_t *sdu, u8 *pNumOfCmpPkt)
{
    ...
    log_b16(SL_STACK_CIS_TX_DATA_EN, (SL16_cis0_txSdu_len + ((pCisConn->cisRole) ? (pCisConn->cis_index) : (pCisConn->cis_index - bltCisMng.maxNum_cisMaster))), (u16)sdu->iso_sdu_len);
        #endif
    ...
    for (pn = 0; pn < pCisConn->tx_numSdu2Pdu; pn++) {
        ...
        pRfPdu->llPhysChnPdu.llPduHdr.cisPduHdr.llid   = ((pCisConn->tx_numSdu2Pdu == 1) || (pn == numPdu - 1)) ?
                                                             ISO_LLID_UNFRAMED_PDU_END_FRAGMENT_SDU :
        ...
        log_b16(SL_STACK_CIS_TX_DATA_EN, (SL16_cis0_txSetPldNum + ((pCisConn->cisRole) ? (pCisConn->cis_index) : (pCisConn->cis_index - bltCisMng.maxNum_cisMaster))), (u16)pdu->cis_pdu_number);
        #endif
...
u32 blt_ial_getCisUnframedTimestamp(ll_cis_conn_t *cis_conn, u32 cisSyncRef, iso_evtcnt_t pld)
{
    ...
    tlkapi_send_string_u32s(DBG_IAL_EN, "reassem", blt_debug_hex_2_dec_display(pld), blt_debug_hex_2_dec_display(sduSyncRef), cisSyncRef, clock_time());
...
_attribute_no_inline_
        #endif
    ble_sts_t
    blt_ial_reassembleCisPdu2Sdu(ll_cis_conn_t *cis_conn, iso_rx_evt_t *pIsoRxEvt)
{
    ...
    log_b16(SL_STACK_CIS_RX_DATA_EN, (SL16_cis0_rxProPdu + ((cis_conn->cisRole) ? (cis_conn->cis_index) : (cis_conn->cis_index - bltCisMng.maxNum_cisMaster))), (u16)pIsoRxEvt->curRcvdPldNum); //+cis_conn->cis_index
        #endif
    ...
    if (cis_conn->cis_frame == CIS_UNFRAMED) {
        ...
        log_b16_byte(SL_STACK_CIS_RX_DATA_EN, (SL16_cis_rxPdu2Sdu_st + ((cis_conn->cisRole) ? (cis_conn->cis_index) : (cis_conn->cis_index - bltCisMng.maxNum_cisMaster))), cis_conn->cis_rxSduStatus, type);
        #endif
        ...
        case SDU_STATE_NEW:
        {
            ...
            if (type == UNFRAMED_START) {
                ...
                log_b16(SL_STACK_CIS_RX_DATA_EN, (SL16_cis0_rxSdu_len + ((cis_conn->cisRole) ? (cis_conn->cis_index) : (cis_conn->cis_index - bltCisMng.maxNum_cisMaster))), (u16)sdu->iso_sdu_len);
        #endif
        #if (SLEV_cis0_sdu_cmplt)
                log_event_irq(SL_STACK_CIS_RX_DATA_EN, (SLEV_cis0_sdu_cmplt + ((cis_conn->cisRole) ? (cis_conn->cis_index) : (cis_conn->cis_index - bltCisMng.maxNum_cisMaster))));
        #endif
            }
            ...
            while (remainLen > 0) {
                ...
                case SDU_STATE_NEW:
                {
                    if ((segHdr->sc == 0)) //start/complete
                    {
                        if (gIsoTsEn) {
        #if (ISO_DATA_TIMESTAMP_UNIT_US_EN)
                            sdu->timestamp = pIsoRxEvt->cisRefAP + cis_conn->cis_sync_delay + cis_conn->sdu_int_peer_us + cis_conn->ft_peer * cis_conn->iso_intvl_us - segHdr->time_offset;
        #else
                            sdu->timestamp = pIsoRxEvt->cisRefAP + cis_conn->cis_sync_delay * SYSTEM_TIMER_TICK_1US +
                                             cis_conn->sdu_int_peer_us * SYSTEM_TIMER_TICK_1US + cis_conn->ft_peer * cis_conn->iso_intvl_tick - segHdr->time_offset * SYSTEM_TIMER_TICK_1US;
        #endif
                        }
                        sdu->iso_sdu_len = 0;
                    ...
                    } else // ERROR! continue/End
                    {
                        ...
                        if (!cis_conn->lossFlag) // PDU(Start X, End X, Start X),,,,,,  PDU(Continue)
                        {
                            ...
                            if (gIsoTsEn) {
        #if (ISO_DATA_TIMESTAMP_UNIT_US_EN)
                                sdu->timestamp = pIsoRxEvt->cisRefAP + cis_conn->cis_sync_delay + cis_conn->sdu_int_peer_us + cis_conn->ft_peer * cis_conn->iso_intvl_us;
        #else
                                sdu->timestamp = pIsoRxEvt->cisRefAP + cis_conn->cis_sync_delay * SYSTEM_TIMER_TICK_1US +
...
_attribute_ram_code_
        #endif
    void
    blt_ll_bis_encryptPld(ble_crypt_para_t *pLeCryptCtrl, rf_packet_ll_data_t *pBisPdu, u64 txPayloadCnt)
{
    /*
...
_attribute_ram_code_
        #endif
    ble_sts_t
    blt_bis_splitSdu2UnframedPdu(u16 bisHandle, sdu_packet_t *sdu, u8 *pNumOfCmpPkt)
{
    bis_tx_pdu_t        *pdu;
    rf_packet_ll_data_t *pRfPdu = NULL;
    ...
    if (pBig->framing == 0) //unframed
    {
        ...
        while (offset < sdu->iso_sdu_len) {
            ...
            pdu    = (bis_tx_pdu_t *)(((u32)bltBisPduTxfifo.bis_tx_pdu) + (bis_sel * bltBisPduTxfifo.fifo_num +
                                                                        (pBis->bisPduTxFifoWptr & (bltBisPduTxfifo.mask))) *
                                                                           bltBisPduTxfifo.full_size);
            pRfPdu = &pdu->isoTxPdu;
            //pRfPdu->llPhysChnPdu.llPduHdr.pduHdr.rf_len = 0; //clean
            pRfPdu->llPhysChnPdu.llPduHdr.pduHdr.type      = 0; //clean
        ...
        for (u8 sn = (pBis->txBnIdx + 1); sn <= pBis->numSdu2Pdu; sn++) {
            ...
            //pRfPdu->llPhysChnPdu.llPduHdr.pduHdr.rf_len = 0; //clean
            pRfPdu->llPhysChnPdu.llPduHdr.pduHdr.type      = 0; //clean
            pRfPdu->llPhysChnPdu.llPduHdr.bisPduHdr.llid   = (sn <= 1) ? ISO_LLID_UNFRAMED_PDU_END_FRAGMENT_SDU : ISO_LLID_UNFRAMED_PDU_START_CONTI_FRAGMENT_SDU;
            pRfPdu->llPhysChnPdu.llPduHdr.bisPduHdr.cssn   = 0;
            pRfPdu->llPhysChnPdu.llPduHdr.bisPduHdr.cstf   = 0;
...
_attribute_ram_code_
        #endif
    ble_sts_t
    blt_bis_splitSdu2FramedPdu(u16 bisHandle, u8 *pNumOfCmpPkt)
{
    u16                  remain_len;
...
ll_iso_unframe_type_t blt_ial_getBisUnframedType(ll_bis_t *pBis, bis_rx_pdu_t *pBisPdu, u8 llid)
{
    ...
    if ((pBisPdu->payloadNum) % pBis->numSdu2Pdu == 0) {
        if (pBisPdu->rawData[0] != 0) {      //mark PDU valid
