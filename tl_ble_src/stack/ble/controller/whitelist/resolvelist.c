...
void blt_ll_resolvRefreshRpa(ll_resolv_list_t *pRL)
{
...
ble_sts_t  blc_ll_addDeviceToResolvingList(ida_type_t peerIdAddrType, u8 *peerIdAddr, u8 *peer_irk, u8 *local_irk)
{
    ...
    */
    ...
    if(blRslvLst.addrRlEn && !blt_ll_isResolvingListCommandAllowed()){
        my_dump_str_data(DBG_HCI_CIS_TEST, "task busy, not allowed add", 0, 0);
...
ble_sts_t  blc_hci_le_addDeviceToResolvingList(hci_le_addDeviceResolvinglist_cmdParam_t *cmdParam)
{
    ...
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Add_Device_RL", cmdParam, 7);
    my_dump_str_data(IUT_HCI_LOG_EN, "[PRV] IRK", cmdParam->peer_IRK, 32);
#else
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Add_Device_RL", cmdParam, sizeof(hci_le_addDeviceResolvinglist_cmdParam_t));
#endif
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Remove_Device_RL", cmdParam, sizeof(le_identityAddress_t));
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Clear_RL", 0, 0);
...
ble_sts_t   blc_hci_le_readResolvingListSize(hci_le_readResolvingListSizeCmd_retParam_t *pRetParam)
{
    ...
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Read_RL_Size", pRetParam, 2);
...
{   /*
    ...
    (0x02).
    */
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Read_Peer_RPA", pCmdParam, sizeof(le_identityAddress_t));
...
ble_sts_t blc_hci_le_readLocalResolvableAddress(le_identityAddress_t *pCmdParam, hci_le_readLocalResolvableAddress_retParam_t *pRetParam)
{
    ...
    (0x02).
    */
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Read_local_RPA", pCmdParam, sizeof(le_identityAddress_t));
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Addr_Resolution_Enable", &resolution_en, 1);
...
ble_sts_t  blc_ll_setResolvablePrivateAddressTimeout (u16 rpa_timeout_s)
{
    ...
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_RPA_Timeout", &timeout_s, 2);
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Privacy_Mode", cmdParam, sizeof(hci_le_setPrivacyMode_cmdParam_t));
