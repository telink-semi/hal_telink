...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Scan_Enable", pCmdParam, sizeof(hci_le_setScanEnable_cmdParam_t));
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Scan_Param", pCmdParam, sizeof(hci_le_setScanParam_cmdParam_t));

    return blc_ll_setScanParameter ( pCmdParam->le_scanType, pCmdParam->le_scanInterval, pCmdParam->le_scanWindow,
                                     pCmdParam->ownAddrType, pCmdParam->scanFilterPolicy);
