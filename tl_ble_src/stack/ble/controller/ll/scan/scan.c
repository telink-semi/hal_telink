...
#if (!ESL_RAM_OPTIMIZATION)
_attribute_ram_code_ int blt_prichn_scan_interrupt_task(int flag)
{
...
_attribute_noinline_ void blt_set_scan_default(void)
{
...
_attribute_ram_code_
    #endif
    void
    blt_ll_scanReqBackoff(ll_scn_t *pScanCB, bool rcvRspSts)
{
...
_attribute_ram_code_
    #endif
    int
    blt_ll_addScanRspDevice(u8 type, u8 *mac)
{
...
_attribute_ram_code_ void blt_setScan_cal_chn_phy(int next_chn)
{
    ...
    if (scan_window_hit || next_chn) {
        ...
        if (((bltScn.scanPhy_msk & SCAN_PHY_1M_CODED) == SCAN_PHY_1M_CODED) && ((trng_rand() & 0x01) == 0)) { //LL/DDI/SCN/BV-23-C;;LL/DDI/SCN/BV-62-C
            return;
        }
#endif
        if (bltScn.chn_index == 0 && (IS_EXTENDED_SCAN_VALID)) { //extended_scan
            /* jump to next PHY*/
            do {
...
#if LL_DDI_SCN_BV88C   //EBQ_TEST_EN. This case adds AdvData(variable len) in EXT_ADV_IND, which is RFU. For this case, we set Scan Rx DMA as 40B for receiving the EBQ pkt.
    rf_set_rx_maxlen(40);       //EXT_ADV_IND may contain RFU part, which cause rf_len more than 37.
#else
...
//LL_DDI_SCN_BV88C is for passing the EBQ test case in a noisy environment.
#if (BQB_TEST_EN && SIHUI_FILTER_RSSI) || (LL_DDI_SCN_BV88C) //for debug, remove other device ADV packet
...
#if LL_DDI_SCN_BV88C    //EBQ_TEST_EN    This case adds AdvData(variable len) in EXT_ADV_IND, which is RFU. For this case, we set Scan Rx DMA as 40B for receiving the EBQ pkt.
    if (bltRxPkt.rx_header_tick && (rflen >= 6)) {
#else
    if (bltRxPkt.rx_header_tick && (rflen <= 37) && (rflen >= 6)) {
        ...
        /* ADV_EXT_IND */
        if (advType_mask == TYPE_MASK_EXT_ADV || advType_mask == TYPE_MASK_DECISION_ADV)
        {
...
#endif
    int
    blt_ll_buildPrimaryChannelScanTask(void) //no CONN slot task, so timing is enough, no need ram_code function
{
...
#endif
    void
    blt_set_prichn_scan_start(void) //write in flash to save RamCode, Scan timing not critical
{
...
_attribute_ram_code_ int blt_ll_prichn_scan_insert(void)
{
    if (blmsParam.create_connection == CONNECT_REQ_GOING) {
        //TODO: Timing shortage, should consider this when connection too many, the last CONN_REQ is hard to send
     ...
     * 1M PHY: (37+10)*8 = 376
     * 2M PHY: (37+11)*4 = 192
     * 625us : Because bSlot is 625us.If it is less than 625us,
     * align bSlot and slot. If the scan task occurs with execution time less than 625us,
     * the small probability of post occurring sSlot_tick_start exceeds the 625us of clock_time.
     * bltSche.update - task_rebuild -, there will be a problem.*/
    u32 scan_tick_min = (bltScn.phy_index == PHY_INDEX_CODED ? 1250 : 625) * SYSTEM_TIMER_TICK_1US;
...
_attribute_ram_code_ void blt_ll_prichn_scan_post(void)
{
    if (blc_rf_pa_cb) {
    ...
    u32 clock_tick = clock_time() + (SLOT_PROCESS_MAX_TICK - 30 * SYSTEM_TIMER_TICK_1US);
    if (tick1_exceed_tick2(clock_tick, bltSche.sSlot_tick_start)) {
        int sSlot_diff_num;
        sSlot_diff_num          = (u32)(clock_tick - bltSche.sSlot_tick_start) * SSLOT_TICK_REVERSE + 1;
        ...
        write_dbg32(DBG_SRAM_ADDR + 4, clock_time());
        write_dbg32(DBG_SRAM_ADDR + 8, bltSche.sSlot_tick_start);
        BLMS_ERR_DEBUG(SCH_DEBUG_EN, 0xFF0B0000);
        bltSche.sSlot_idx_next  = 0;
        bltSche.sSlot_tick_next = bltSche.sSlot_tick_start;
    }
}
...
#if (PRICHN_SCAN_SMALL_INTV_WITH_ANOTHER_BIG_INTV_ADV_RESOLVE_EN)
_attribute_ram_code_ int blt_ll_prichn_scan_align_build(void)
{
    #if (ANOTHER_BIG_INTV_EXTENDED_ADV)
    if ((bltAdScn.scanTask_Policy == 0x01) || ((bltAdScn.extAdv_num <= bltAdScn.extAdv_num_thres) && (bltAdScn.extAdv_legacyMode) && (bltAdScn.legadv_int > bltAdScn.legadv_int_thres) && ((bltScn.scanInterval * 2) < bltAdScn.legadv_int)))
    #else
...
#endif
    u32
    blt_ll_calculateScanDuration(u32 scan_tick_left)
{
    u32 scan_duration = scan_tick_left;
