...
_attribute_ram_code_ u8 *blt_ext_scan_searchAcadInfos(u8 *pAcad, int acad_len, data_type_t advType)
{
    u8 *pAcadInfor = pAcad;
...
_attribute_ram_code_ bool blt_ll_process_scan_privacy(ll_resolv_list_t **ppRL, u8 advA_type, u8 *advA_addr, u8 directAdv, u8 initA_type, u8 *initA_addr)
{
    do {
        ...
        if (bltScn.scan_fp_wl && !mon_adv_en)
#else
...
_always_inline bool decisionAdv_resolveTag_handle(u8 testFlag, u8* resolveKey, rf_pkt_decision_adv_t *pDecAdv, u8 extHdrLen){

    /*if the decision adv do not include the resolvable tag, check the testFlag bit3*/
    if( !(pDecAdv->decTypeFlag & DECISION_ADV_RESOLVABLE_TAG)){

        if(testFlag&RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3){
            return TRUE;
        }
    }
//    else{
//        if( (testFlag & (RELEVANT_FIELD_PRESENT_CHECK_PASS_BIT1|RELEVANT_FIELD_PRESENT_CHECK_FAIL_BIT2|RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3)) == RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3){
//            return FALSE;
//        }
//    }

    u8 status = FALSE;
    /* if the decision adv include the resolvable tag:
     * case 1: check pass and the test_flag bit1 is set, the result is pass. other situation is fail.
     * case 2: check fail and the test_flag bit2 is set, the result is pass. other situation is fail.
     */
    if(pDecAdv->decTypeFlag & DECISION_ADV_RESOLVABLE_TAG){
        // decAdvResTag is 6 bytes
        // |---3byte hash---|---3byte prand---|
        if(TRUE != aes_resolve_irk_rpa(resolveKey, (u8*)&pDecAdv->data[extHdrLen])){

            if(testFlag&RELEVANT_FIELD_PRESENT_CHECK_FAIL_BIT2){
                status = TRUE;
            }
        }else{
            if(testFlag&RELEVANT_FIELD_PRESENT_CHECK_PASS_BIT1){
                status = TRUE;
            }
        }
    }
...
_always_inline bool decisionAdv_advMode_handle(u8 testParam_EventType, u8 testFlag, u8 decAdvMode){

    u8 checkStatus = FALSE;

    if( (testFlag & BIT_RNG(1,2)) == 0){
        return FALSE;
    }

    switch(decAdvMode){
        case DECISION_NON_CONNECTABLE_NON_SCANNABLE_UNDIRECTED:
        {
            if(testParam_EventType & BIT(0)){
                checkStatus = TRUE;
            }
        }
        break;

        case DECISION_CONNECTABLE_UNDIRECTED:
        {
            if(testParam_EventType & BIT(1)){
                checkStatus = TRUE;
            }
        }
        break;

        case DECISION_SCANNABLE_UNDIRECTED:
        {
            if(testParam_EventType & BIT(2)){
                checkStatus = TRUE;
            }
        }
        break;
...
_always_inline bool decisionAdv_rssi_handle(s8 minRSSI, s8 maxRSSI, s8 decAdvRSSI, u8 testFlag){

    u8 status = FALSE;

    if(decAdvRSSI > minRSSI && decAdvRSSI < maxRSSI){
...
_always_inline bool decisionAdv_pathLoss_handle(s8 tx_power, s8 decAdvRssi, u8 minLoss, u8 maxLoss, u8 testFlag){

    if(tx_power <= decAdvRssi){
        if(testFlag & RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3){
            return TRUE;
        }else{
            return FALSE;
    ...
    u8 tPathLoss = tx_power - decAdvRssi;

    if(tPathLoss > minLoss && tPathLoss < maxLoss){
        if(testFlag & RELEVANT_FIELD_PRESENT_CHECK_PASS_BIT1){
            return TRUE;
        }
    }else{
...
#define USE_PEER_IRK    0
#define USE_LOCAL_IRQ   1
_always_inline bool decisionAdv_advAddress_handle(u8* testParam_advA, rf_pkt_decision_adv_t * pDecAdv, u8 testFlag){

    if( !(pDecAdv->ext_hdr_flg & EXTHD_BIT_ADVA)){
        if(testFlag & RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3){
            return TRUE;
        }else{
            return FALSE;
    ...
    u8 check = testParam_advA[0];

    u8 address1Type = testParam_advA[1];
    smemcpy(address1, &testParam_advA[2], BLE_ADDR_LEN);

    u8 address2Type = testParam_advA[8];
    smemcpy(address2, &testParam_advA[9], BLE_ADDR_LEN);

    u8 status = FALSE;
    if(pDecAdv->ext_hdr_flg & EXTHD_BIT_ADVA){
        u8 * decAdv = (u8*)&pDecAdv->data[0]; //if advA is exist, the first data is the adv address.

        u8  peer_is_rpa = IS_RESOLVABLE_PRIVATE_ADDR(pDecAdv->txAddr, decAdv);
        ll_resolv_list_t *pRL_match  = NULL;
        if(peer_is_rpa){
            pRL_match = blt_ll_resolve_rpa(0, decAdv, NULL);
        }

        switch(check){
            case 0: //in filter accept list
            {
                if(peer_is_rpa){
                    if(!pRL_match){
                        break;
                    ...
                    if(blt_ll_searchAddrInWhiteListTbl(pRL_match->rlIdAddrType, pRL_match->rlIdAddr)){
                        status = TRUE;
                    }
                }else{
                    if(blt_ll_searchAddrInWhiteListTbl(pDecAdv->txAddr, decAdv)){
                        status = TRUE;
                    }
                }
            }
            ...
            case 1: //same as address 1, ignored address 2.
            {
                if(peer_is_rpa){
                    if(!pRL_match){
                        break;
                    ...
                    if(pRL_match->rlIdAddrType == address1Type && !smemcmp(pRL_match->rlIdAddr, address1, 6)){
                        status = TRUE;
                    }
                }else{
                    if( (pDecAdv->txAddr== address1Type) && !smemcmp(decAdv, address1, 6)){
                        status = TRUE;
                    }
                }
            }
            ...
            case 2: //same as address 1 OR same as address 2.
            {
                if(peer_is_rpa){
                    ...
                    else if(pRL_match->rlIdAddrType == address2Type && !smemcmp(pRL_match->rlIdAddr, address2, 6)){
                        status = TRUE;
                    }
                }else{
                    if( (pDecAdv->txAddr== address1Type) && !smemcmp(decAdv, address1, 6)){
                    ...
                    else if( (pDecAdv->txAddr== address2Type) && !smemcmp(decAdv, address2, 6)){
                        status = TRUE;
                    }
                }
            }
...
//u8 abrLevel, u8* pTestMask, u8* pTarget, u8* pDecAdvData, u8 decisionAdvDataLen, u8 matchLen, u8 testFlag
_attribute_ram_code_sec_noinline_ bool decisionAdv_arbitraryData_handle(abr_param_t abrHandle){

    /* if the decision adv data do not contain the arbitrary data:
     * in this situation, only one pass case: test_flag bit3 is set.
     */
    if(abrHandle.decisionAdvDataLen == 0){
        if(abrHandle.testFlag & RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3){
            return TRUE;
        }else{
            return FALSE;
        }
    ...
    /* here can be sure the decision adv must include arbitrary data
     * case 1: check pass and the test_flag bit1 is set, and the result pass.
     * case 2: check fail, this situation is treated as if it does not contain the relevant field. will check test_flag bit3:
     *         test passes if the decision data does not contain the relevant field.
     */
    u8 matchCnt = 0;
    for(int i=0; i< abrHandle.decisionAdvDataLen; i++){
        if( (abrHandle.pTestMask[i] & abrHandle.pDecAdvData[i]) == abrHandle.pTarget[i] ){
            matchCnt++;
        }else{
            break;
        }
    ...
    u8 status = FALSE;
    switch(abrHandle.abrLevel){
        case ARBITRARY_LEVEL_0: //equal
        {
            /*if any other length, treated as if it does not contain the relevant field,
             *so need to judge RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3
             */
            if(abrHandle.decisionAdvDataLen != abrHandle.matchLen){
                if(abrHandle.testFlag & RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3){
                    return TRUE;
                }else{
                    return FALSE;
            ...
            if(matchCnt == abrHandle.decisionAdvDataLen){//abrHandle.matchLen
                status = TRUE;
            }
        }
        break;

        case ARBITRARY_LEVEL_1: //at least
        {
            /*if less than the value set by host instruction command , treated as if it does not contain the relevant field,
             *so need to judge RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3
             */
            if(abrHandle.decisionAdvDataLen < abrHandle.matchLen){
                if(abrHandle.testFlag & RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3){
            ...
            if(matchCnt == abrHandle.decisionAdvDataLen){
                status = TRUE;
            }
        }
        break;

        case ARBITRARY_LEVEL_2: //at least one and at most
        {
            /*if more than the value set by host instruction command, treated as if it does not contain the relevant field,
             *so need to judge RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3
             *Note:here no need to judge 'abrHandle.decisionAdvDataLen == 0',
             *before code logic can be sure here abrHandle.decisionAdvDataLen must be larger than zero.
             */
            if(abrHandle.decisionAdvDataLen > abrHandle.matchLen){
                if(abrHandle.testFlag & RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3){
                    return TRUE;
            ...
            if(matchCnt == abrHandle.decisionAdvDataLen){//abrHandle.decisionAdvDataLen
                status = TRUE;
            }
        }
        break;
    ...
    if(status == TRUE){
        if(!(abrHandle.testFlag & RELEVANT_FIELD_PRESENT_CHECK_PASS_BIT1)){
            status = FALSE;
        }
    }else{
        if(abrHandle.testFlag & RELEVANT_FIELD_PRESENT_CHECK_FAIL_BIT2){
            return TRUE;
        }
    }
...
volatile u32 A_whileCnt = 0;
_attribute_ram_code_ bool blt_decisionAdvInstructHandle(rf_pkt_ext_adv_t *pExtAdv, u8* raw_pkt)
{
    /*for decision adv, if host don't set instruction, need to ignore decision adv*/
    if(gDecisionInstructTest.group_num == 0){
        return FALSE;
    }

    rf_pkt_decision_adv_t *pDecAdv = (rf_pkt_decision_adv_t *)pExtAdv;

    //first, get the extended header length of ADV_DECISION_IND
    u8 extHdrLen = EXTHD_LEN_3_AUX_PTR; //AuxPtr must be exist.
    if(pDecAdv->ext_hdr_flg & EXTHD_BIT_ADVA){
        extHdrLen += EXTHD_LEN_6_ADVA;
    }
    if(pDecAdv->ext_hdr_flg & EXTHD_BIT_ADI){
        extHdrLen += EXTHD_LEN_2_ADI;
    }
    if(pDecAdv->ext_hdr_flg & EXTHD_BIT_TX_POWER){
        extHdrLen += EXTHD_LEN_1_TX_POWER;
    }

    //second, get the arbitrary data offset. because there may be resolvable tag in the decision data of ADV_DECISION_IND.
    u8 arbitraryDataOft = 0;
    if(pDecAdv->decTypeFlag & DECISION_ADV_RESOLVABLE_TAG){
        arbitraryDataOft += RESOLVABLE_TAG_LEN;
    }

    abr_param_t abr_decision;

    u8* pDecAdvData    = (u8*)&pDecAdv->data[extHdrLen + arbitraryDataOft];
    abr_decision.pDecAdvData = pDecAdvData;

    u8 decisionDataLen = pDecAdv->rf_len - extHdrLen - 2 - arbitraryDataOft;
    abr_decision.decisionAdvDataLen = decisionDataLen;

    u8 testIdx = 0;
    u8 status = FALSE;
    for(testIdx = 0; testIdx < gDecisionInstructTest.num_test; ){
        DBG_CHN11_TOGGLE;
        u8 test_field      = gDecisionInstructTest.pTestData[testIdx].test_field;
        u8 testFlag        = gDecisionInstructTest.pTestData[testIdx].test_flag;
        u8* pTestParamOft0 = &gDecisionInstructTest.pTestData[testIdx].test_param[0];
        u8* pTestParamOft8 = &gDecisionInstructTest.pTestData[testIdx].test_param[8];


        abr_decision.pTestMask = pTestParamOft0;
        abr_decision.pTarget   = pTestParamOft8;
        abr_decision.testFlag  = testFlag;

        if(test_field == TEST_FIELD_RESOLVE_TAG){
            DBG_CHN3_TOGGLE;
            status = decisionAdv_resolveTag_handle(testFlag, pTestParamOft0, pDecAdv, extHdrLen);
        }else if(test_field == TEST_FIELD_ADV_MODE){
            DBG_CHN4_TOGGLE;
            u8 eventType = pTestParamOft0[0];
            status = decisionAdv_advMode_handle(eventType, testFlag, pDecAdv->adv_mode);

        }else if(test_field == TEST_FIELD_RSSI){
            DBG_CHN5_TOGGLE;
            s8 MinRSSI = (s8)pTestParamOft0[0];
            s8 MaxRSSI = (s8)pTestParamOft0[1];
            s8 decAdvRssi = raw_pkt[DMA_RFRX_OFFSET_RSSI(raw_pkt)] - 110;

            status = decisionAdv_rssi_handle(MinRSSI, MaxRSSI, decAdvRssi, testFlag);

        }else if(test_field == TEST_FIELD_PATH_LOSS){
            DBG_CHN6_TOGGLE;
            if(pDecAdv->ext_hdr_flg & EXTHD_BIT_TX_POWER){
                s8 tx_power = pDecAdv->data[extHdrLen-1]; //tx power is the last byte of the extended header
                s8 decAdvRssi = raw_pkt[DMA_RFRX_OFFSET_RSSI(raw_pkt)] - 110;
                u8 minLoss = pTestParamOft0[0];
                u8 maxLoss = pTestParamOft0[1];

                status = decisionAdv_pathLoss_handle(tx_power, decAdvRssi, minLoss, maxLoss, testFlag);
            }

        }else if(test_field == TEST_FIELD_ADV_ADDRESS){
            DBG_CHN7_TOGGLE;
            status = decisionAdv_advAddress_handle(gDecisionInstructTest.pTestData[testIdx].test_param, pDecAdv, testFlag);

        }else if(test_field >= TEST_FIELD_ARBITRARY_DATA_LEN_17 && test_field <= TEST_FIELD_ARBITRARY_DATA_LEN_24){
            abr_decision.abrLevel = ARBITRARY_LEVEL_0;
            abr_decision.matchLen = test_field - TEST_FIELD_ARBITRARY_DATA_LEN_17 + 1;

            status = decisionAdv_arbitraryData_handle(abr_decision);

        }else if(test_field >= TEST_FIELD_ARBITRARY_DATA_LEN_33 && test_field <= TEST_FIELD_ARBITRARY_DATA_LEN_40){
            abr_decision.abrLevel = ARBITRARY_LEVEL_1;
            abr_decision.matchLen = test_field - TEST_FIELD_ARBITRARY_DATA_LEN_33 + 1;

            status = decisionAdv_arbitraryData_handle(abr_decision);

        }else if(test_field >= TEST_FIELD_ARBITRARY_DATA_LEN_49 && test_field <= TEST_FIELD_ARBITRARY_DATA_LEN_56){
            abr_decision.abrLevel = ARBITRARY_LEVEL_2;
            abr_decision.matchLen = test_field - TEST_FIELD_ARBITRARY_DATA_LEN_49 + 1;

            status = decisionAdv_arbitraryData_handle(abr_decision);
        }

        /**
         * 1. A test group shall be classified as passing if all the tests in that group pass
         * 2. A test group shall be classified as fail if any of the tests in that group fail.
         * 3. An ADV_DECISION_IND PDU shall be accepted if any of the test groups pass.
         */
        if(gDecisionInstructTest.num_test == 1){
            break;
        }

        if(status == TRUE){
            testIdx++;
            if(testIdx == gDecisionInstructTest.num_test){
                return status;
            }
            //judge whether the next test is next group, if so, indicate all the tests of the current group have passed.
            if( gDecisionInstructTest.pTestData[testIdx].test_flag&BIT(0)){
                return TRUE; //pass
            }
        }else{
            //only there is one fail(status== FALSE),no need to handle the next test of the same group.
            //just find the next group to handle.
            do{
                testIdx++;
                A_whileCnt++;
                DBG_CHN0_TOGGLE;
            }while( !(gDecisionInstructTest.pTestData[testIdx].test_flag&BIT(0)) );
        }
    }
...
_attribute_ram_code_ int blt_ext_adv_rx_process(u8 *raw_pkt) //primary channel rx processing.
{
    ...
    do {
    #if(LL_FEATURE_ENABLE_DECISION_BASED_ADVERTISING_FILTER)
        if(blmsParam.scanInitEn_union.ext_init_en && bltInit.init_fp == EXT_INIT_FP_ADV_ONLY_DECISION_AND_NO_WL){ //if initial state, and only accept decision PDU
            if(pExtAdv->type != LL_TYPE_ADV_DECISION_IND){
                break;
            }
        }

        ///ADV_DECISION_IND processing
        if(pExtAdv->type == LL_TYPE_ADV_EXT_IND){
            if( (bltScn.scan_filterPolicy & BIT_RNG(2,3)) == 0x0C){ //Only decision PDU can be received
                break;
            }
        }
        ...
        if(pExtAdv->type == LL_TYPE_ADV_DECISION_IND){
            DBG_CHN2_TOGGLE;
            if(blmsParam.scanInitEn_union.ext_scan_en && (bltScn.scan_filterPolicy & BIT_RNG(2,3)) == 0x00){ //No-decision PDU can be received
                break;
            }

            if(FALSE == blt_decisionAdvInstructHandle(pExtAdv, raw_pkt)){
                break;
            }
            DBG_CHN2_TOGGLE;
        }
        ...
        if ( (pExtAdv->type != LL_TYPE_ADV_DECISION_IND) && (pExtAdv->rf_len != pExtAdv->ext_hdr_len + 1) ) { //no "Adv Data"
        ...
        /* Non_Connectable Non_Scannable with/without auxiliary packet */
        if (cur_adv_mode == LL_EXTADV_MODE_NON_CONN_NON_SCAN) {
        ...
        /* Connectable & Scannable */
            ...
            if(pExtAdv->type == LL_TYPE_ADV_DECISION_IND){
                ...
                if ((pExtAdv->ext_hdr_flg & (EXTHD_BIT_ADVA | EXTHD_BIT_TARGETA | EXTHD_BIT_CTE_INFO | EXTHD_BIT_ADI | EXTHD_BIT_AUX_PTR | EXTHD_BIT_SYNC_INFO)) == (EXTHD_BIT_ADI | EXTHD_BIT_AUX_PTR)) {
        ...
        if (more_adv_pkt_exist) {
            ...
            if (aux_irq_distance > BIT(30) || aux_irq_distance < 100 * SYSTEM_TIMER_TICK_1US) {
                ...
                if (available_pauxscn != NULL)                            //find available AUX SCAN
                {
                    ...
                    cur_pauxscn->priAdvType = pExtAdv->type;
...
_attribute_ram_code_ void secChnScn_auxScanRsp_procStart(st_secchn_scn_t *secChnScn)
{
...
_attribute_ram_code_ void secChnScn_auxScanRsp_procPost(st_secchn_scn_t *secChnScn)
{
...
_attribute_ram_code_ int blt_extendedActiveScan_proc(u8 *raw_pkt, u8 *new_pkt) //secChnScn_auxScanRsp_procStart
{
    ...
    while (!(*ph) && (u32)(clock_time() - rx_begin_tick) < scanRsp_ph_timeout_us * SYSTEM_TIMER_TICK_1US)
        ; //150 + pkt(22*8) + 150 + margin(50)

    if (*ph) {
        rx_begin_tick = clock_time();
        while (!HAL_GET_RF_RX_IRQ && (clock_time() - rx_begin_tick) < scanRsp_timeout_us * SYSTEM_TIMER_TICK_1US)
            ; //2200
        ...
        if (RF_BLE_PACKET_VALIDITY_CHECK(new_pkt)) //CRC OK
        {
            ...
            */
            if (MAC_MATCH16(rspAdv16, reqAdv16)) {
                ...
                ///////////////////////////////////////////////////////////////////////////////////////
                /*1. advA. mandatory in aux scan response packet
                ...
                /* 2. TargetA. RFU */
                if (pAuxScanRsp->ext_hdr_flg & EXTHD_BIT_TARGETA) {
                    extHdr_offset += EXTHD_LEN_6_TARGETA;
                }

                /* 3. CTE Info. RFU */
                if (pAuxScanRsp->ext_hdr_flg & EXTHD_BIT_CTE_INFO) {
                    extHdr_offset += EXTHD_LEN_1_CTE;
                }
                ...
                /* 6. Sync Info. RFU */
                if (pAuxScanRsp->ext_hdr_flg & EXTHD_BIT_SYNC_INFO) {
...
_attribute_ram_code_ int irq_scan_rx_secondary_channel(void)
{
    ...
    if (bltRxPkt.rx_header_tick) {
        ...
        if (bltScn.initiate_going == EXT_INITIATE_GOING) {
    ...
    /* 20: sSlot align may introduce 20uS error, 30uS is a safe margin*/
    #if (LL_FEATURE_ENABLE_PERIODIC_ADVERTISING_WITH_RESPONSES_SCANNER)
            extern int PAWR_SYNC_PPM;
            u32        diff_margin = (blt_pSecChnScn->tolerance_peer_us + 30 + 220 + PAWR_SYNC_PPM * 2) * SYSTEM_TIMER_TICK_1US;
    #else
            ...
            if (pExtAdv->ext_hdr_len != 0) {
                ...
                if (pExtAdv->ext_hdr_flg & EXTHD_BIT_ADVA) {
#if (!ESL_RAM_OPTIMIZATION)
                    if (blt_pSecChnScn->prichn_advA_exist){
                        //This field is reserved for future use if the corresponding field in the superior PDU of this PDU is present
                    } else {
                        //This field is optional if the corresponding field in the superior PDU of this PDU is not present
                        if (aux_adv_1st_pkt) {
                ...
                if (pExtAdv->ext_hdr_flg & EXTHD_BIT_TARGETA) {
#if (!ESL_RAM_OPTIMIZATION)
                    if (blt_pSecChnScn->prichn_targetA_exist){
                        //This field is reserved for future use if the corresponding field in the superior PDU of this PDU is present
                    } else {
                        //This field is mandatory if the corresponding field in the superior PDU of this PDU is not present
                        if (aux_adv_1st_pkt) {
    ...
    /* 3. CTE info */
...
#if LL_DDI_SCN_BV88C        //TODO: There is no ADI in AUX_SCAN_RSP, but ADI in AUX_CHAIN_IND in this case. So put a workaround here currently for passing the case. confirmed by @qiuwei/@jiakai 20250403
#else
                        if (adi_info != blt_pSecChnScn->scan_adi) { //all secondary channel data should keep same ADI
                ...
                /* 5. Aux Ptr */
                if (pExtAdv->ext_hdr_flg & EXTHD_BIT_AUX_PTR) {
                    if (blt_pSecChnScn->scan_advMode == LL_EXTADV_MODE_NON_CONN_NON_SCAN) {
            ...
            if (blt_pSecChnScn->scan_advMode == LL_EXTADV_MODE_CONN) //Connectable
            {
                 ...
                 */
                if (pExtAdv->ext_hdr_flg & (EXTHD_BIT_ADVA | EXTHD_BIT_ADI)) {
