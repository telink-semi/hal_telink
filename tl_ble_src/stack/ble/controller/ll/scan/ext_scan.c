...
_attribute_aligned_(4)  ll_rx_pkt_callback_t    ll_secchn_initPkt_cb = NULL;

_attribute_aligned_(4)  u8  scan_sec_chn_rx_fifo[SCAN_SECCHN_RXFIFO_SIZE * SCAN_SECCHN_RXFIFO_NUM]; //extended scan secondary channel RX FIFO


_attribute_aligned_(4)  ll_extscn_t     bltExtScn;




_attribute_aligned_(4)  st_secchn_scn_t         secChnScn_tbl[TSKNUM_SECCHN_SCAN];
_attribute_aligned_(4)  st_secchn_scn_t         *blt_pSecChnScn = NULL;
_attribute_aligned_(4)  auxscn_common_para_t    auxScnCmnParam;



_attribute_aligned_(4)  aux_scn_fut_task_t      AuxScnFutTask; //future task
...
_attribute_aligned_(4)  u8  extadv_pda_rpt_hold_data_buf[TSKNUM_SECCHN_SCAN][EXTADV_PDA_RPT_DATA_HOLD_MAX_LEN];  //228*number
...
ble_sts_t blc_hci_le_setExtScanParam (hci_le_setExtScanParam_cmdParam_t* pCmdParam)
{
    ...
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Ext_Scan_Param", pCmdParam, 3 + 5*phy_cnt);
    ...
    if( pCmdParam->scan_PHYs == SCAN_PHY_1M)
    {
        ...
        return blc_ll_setExtScanParam(pCmdParam->ownAddress_type,      pCmdParam->scan_filter_policy,       SCAN_PHY_1M, \
                                      pCmdParam->scanCfg[0].scan_type, pCmdParam->scanCfg[0].scan_interval, pCmdParam->scanCfg[0].scan_window, \
                                      0,                               0,                                   0);
    }
    else if(pCmdParam->scan_PHYs == SCAN_PHY_CODED ){
        my_dump_str_data(DBG_EXTSCAN_LOGIC, "Coded PHY", 0, 0);
        return blc_ll_setExtScanParam(pCmdParam->ownAddress_type,      pCmdParam->scan_filter_policy,       SCAN_PHY_CODED, \
                                      0,                               0,                                   0,  \
                                      pCmdParam->scanCfg[0].scan_type, pCmdParam->scanCfg[0].scan_interval, pCmdParam->scanCfg[0].scan_window);
    }
    else if(pCmdParam->scan_PHYs == SCAN_PHY_1M_CODED ){
        my_dump_str_data(DBG_EXTSCAN_LOGIC, "1M Coded PHY", 0, 0);
        return blc_ll_setExtScanParam(pCmdParam->ownAddress_type,      pCmdParam->scan_filter_policy,       SCAN_PHY_1M_CODED, \
                                      pCmdParam->scanCfg[0].scan_type, pCmdParam->scanCfg[0].scan_interval, pCmdParam->scanCfg[0].scan_window, \
                                      pCmdParam->scanCfg[1].scan_type, pCmdParam->scanCfg[1].scan_interval, pCmdParam->scanCfg[1].scan_window);
    }
    else{
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Ext_Scan_Enable", pCmdParam, sizeof(hci_le_setExtScanEnable_cmdParam_t));
...
_attribute_ram_code_
int blt_ext_scan_interrupt_task (int flag, void* p)
{
...
_attribute_ram_code_
int blt_ll_procAuxiliaryScanTask (int flag)
{
...
void blt_aux_scan_start (int aux_idx)
{
    ...
    else
    {
        ...
        if(blc_rf_pa_cb){   blc_rf_pa_cb(PA_TYPE_RX_ON); } //QW OK
...
_attribute_ram_code_
void blt_release_secchn_scan(st_secchn_scn_t *cur_pauxscn, u8 aux_idx)
{
    u32 r = irq_disable();
...
_attribute_ram_code_
void blt_add_aux_scan_future_task(u8 llTask_idx, u8 llTask_offset, u32 tick_start, u32 tick_end)
{
...
_attribute_ram_code_
bool blt_remove_aux_scan_future_task(u8 llTask_idx)
{
...
_attribute_ram_code_
int blt_check_aux_scan_future_task(u32 tick_available)
{
    int i;
...
extAdvRptEvt_t  extAdv_truncatedEvt[TSKNUM_SECCHN_SCAN];
...
#endif
    {
        ...
        else
        {
            ...
            if(old_hold_data_len || new_hold_data_len){
                ...
                if(sec_chn_flag == 0){BLMS_ERR_DEBUG(DBG_EXTSCAN_LOGIC, 0xFE0F0000 | old_hold_data_len<<8 | new_hold_data_len);  }
            ...
            if(sec_chn_flag){ // Extended with auxiliary packet
                ...
                if(cur_pauxscn->total_targetA_exist){
                    pExtAdvInfo->direct_address_type = blt_extscan_convert_direct_adr_type(cur_pauxscn->record_direct_adrType, \
                                                        cur_pauxscn->record_direct_addr, cur_pauxscn->direct_rpa_resolve_fail);
...
//              if(new_hold_data_len || old_hold_data_len)
                ...
                if(cur_pPrichnScn->direct_flag){
                    ...
                    pExtAdvInfo->direct_address_type = blt_extscan_convert_direct_adr_type(direct_adrType, \
                                                pExtAdvInfo->direct_address, cur_pPrichnScn->directA_rpa_resolve_fail);
                }
...
_attribute_noinline_
int blt_extScan_timeoutEvt(void)
{
...
_attribute_noinline_
#endif
int blt_ll_procExtScan_periodExpired(void){
