...
extern _attribute_aligned_(4) ll_scn_t bltScn;
    ...
    #define SCNALIGN_FIFO_NUM 4 //scan interval no less than 20mS
#else
    #error "SCNALIGN FIFO NUM error!!!"
#endif
...
{
    //Attention: if not 4B aligned, task scheduler will collapse. SO variable must 4B aligned above !!!
    sch_task_t scnAlign_fifo[SCNALIGN_FIFO_NUM];
} ll_scn_align_t;

extern _attribute_aligned_(4) ll_scn_align_t bltAlignScn;
...
#define SCAN_SECCHN_RXFIFO_SIZE 96 //must be 16*n for both Eagle/Kite/Vulture
#define SCAN_SECCHN_RXFIFO_NUM  2  //must be 2^n //the number of chain packet is large---EBQ
#else
...
#if (!ESL_RAM_OPTIMIZATION)
void blt_setScan_enter_manual_rx(void); //set primary channel scan step 2: enter manual RX
#endif                                  //(!ESL_RAM_OPTIMIZATION)
...
#define SCANRX_FALG_PAWR                   BIT(6) //Above value need to be stored in raw_pkt[2] and used in mainloop

#define SCANRX_FLAG_EXTSCAN_TRUNCATED_PEND BIT(8) //6;not store raw_pkt, only used in scan_rx_flag(u16)
#define SCANRX_FLAG_PDASCAN_TRUNCATED_PEND BIT(9) //7;not store raw_pkt, only used in scan_rx_flag(u16)
...
_attribute_aligned_(4) typedef struct
{
    ...
    u8  priAdvType; //just for decision adv
    u8  u16_rsvd0;
...
enum{
    DECISION_ADV_RESOLVABLE_TAG = BIT(0),
};


enum{
    RELEVANT_FIELD_PRESENT_CHECK_PASS_BIT1     = BIT(1),
    RELEVANT_FIELD_PRESENT_CHECK_FAIL_BIT2     = BIT(2),
    RELEVANT_FIELD_NOT_PRESENT_CHECK_PASS_BIT3 = BIT(3),
};


enum{
    DECISION_NON_CONNECTABLE_NON_SCANNABLE_UNDIRECTED = 0x00,
    DECISION_CONNECTABLE_UNDIRECTED = 0x01,
    DECISION_SCANNABLE_UNDIRECTED   = 0x02,
};

enum{
    TEST_FIELD_RESOLVE_TAG  = 0,
    TEST_FIELD_ADV_MODE     = 6,
    TEST_FIELD_RSSI         = 7,
    TEST_FIELD_PATH_LOSS    = 8,
    TEST_FIELD_ADV_ADDRESS  = 9,
    TEST_FIELD_ARBITRARY_DATA_LEN_17 = 17,
    TEST_FIELD_ARBITRARY_DATA_LEN_24 = 24,
    TEST_FIELD_ARBITRARY_DATA_LEN_33 = 33,
    TEST_FIELD_ARBITRARY_DATA_LEN_40 = 40,
    TEST_FIELD_ARBITRARY_DATA_LEN_49 = 49,
    TEST_FIELD_ARBITRARY_DATA_LEN_56 = 56,

    TEST_FIELD_ARBITRARY_DATA_LEN_240 = 240,
};
...
extern dec_ins_t decison_instruct_test[8]; //at least support 8 tests
extern decision_test_t gDecisionInstructTest;
...
_always_inline int blt_leg_scan_checkSleepReady(void)
{
    // Check if the RX FIFO queue is empty (read pointer == write pointer)
    if (scan_priRxFifo.rptr == scan_priRxFifo.wptr) {
        return 1; // Can go to sleep
    }
    return 0;     // There are pending events or RX queue has data, cannot sleep
}

_always_inline int blt_ext_scan_checkSleepReady(void)
{
    // Check if the RX FIFO queue is empty (read pointer == write pointer)
    if (scan_secRxFifo.rptr == scan_secRxFifo.wptr) {
        return 1; // Can go to sleep
    }
    return 0; // There are pending events or RX queue has data, cannot sleep
}
