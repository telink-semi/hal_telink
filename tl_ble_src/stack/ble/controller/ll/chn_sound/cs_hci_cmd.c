...
// check config param same or not with same config ID. Same:return0 ; Not Same:return 1;
u8 check_sameConfig_param(hci_le_cs_creatConfig_cmdParam_t *pConfig, u8 cfgIdx)
{
    cs_config_t                      *pCsCfg = gCsMng.gGlobal_pCsCfg + cfgIdx;
    u8                                buff[sizeof(hci_le_cs_creatConfig_cmdParam_t)];
    hci_le_cs_creatConfig_cmdParam_t *pConfigPast = (hci_le_cs_creatConfig_cmdParam_t *)buff;

    pConfigPast->Connection_Handle    = pCsCfg->aclHandle;
    pConfigPast->Config_ID            = pCsCfg->Config_ID;
    pConfigPast->Create_Context       = pCsCfg->Create_Context;
    pConfigPast->Main_Mode            = pCsCfg->Main_Mode;
    pConfigPast->Sub_Mode             = pCsCfg->Sub_Mode;
    pConfigPast->Main_Mode_Min_Steps  = pCsCfg->Main_Mode_Min_Steps;
    pConfigPast->Main_Mode_Max_Steps  = pCsCfg->Main_Mode_Max_Steps;
    pConfigPast->Main_Mode_Repetition = pCsCfg->Main_Mode_Repetition;
    pConfigPast->Mode_0_Steps         = pCsCfg->Mode_0_Steps;
    pConfigPast->Role                 = pCsCfg->Role;
    pConfigPast->RTT_Type             = pCsCfg->RTT_Type;
    pConfigPast->CS_SYNC_PHY          = pCsCfg->CS_SYNC_PHY;
    smemcpy(pConfigPast->Channel_Map, pCsCfg->Channel_Map, 10);
    pConfigPast->Channel_Map_Repetition  = pCsCfg->Channel_Map_Repetition;
    pConfigPast->ChSel                   = pCsCfg->ChSel;
    pConfigPast->Ch3c_Shape              = pCsCfg->Ch3c_Shape;
    pConfigPast->Ch3c_Jump               = pCsCfg->Ch3c_Jump;
    pConfigPast->Companion_Signal_Enable = pCsCfg->Companion_Signal_Enable;

    tlkapi_send_string_data(0, "config past", (u8 *)&pConfigPast->Connection_Handle, sizeof(hci_le_cs_creatConfig_cmdParam_t));
    tlkapi_send_string_data(0, "config now", (u8 *)&pConfig->Connection_Handle, sizeof(hci_le_cs_creatConfig_cmdParam_t));

    u8 status = smemcmp((u8 *)&pConfigPast->Connection_Handle, (u8 *)&pConfig->Connection_Handle, sizeof(hci_le_cs_creatConfig_cmdParam_t));
 ...
 */
ble_sts_t blt_cs_extractEnableChnMap(u8 *chm, u8 *Filtered_channel, u8 *Filtered_channel_num)
{
    u8 ret          = BLE_SUCCESS;
    u8 chnEnableNum = 0;
    //chm convert to chn map array
    for (unsigned int i = 0; i < 80; i++) {
        if (chm[i / 8] & (1 << (i % 8))) {
            if (i == 0 || i == 1 || i == 23 || i == 24 || i == 25 || i == 77 || i == 78 || i == 79) {
                ret = HCI_ERR_UNSUPPORTED_LMP_PARAM_VAL;
                continue; //immediate return is an option if we don't care about count
            }
    ...
    *Filtered_channel_num = chnEnableNum;
    return ret;
}
...
ble_sts_t blt_cs_checkConfig(hci_le_cs_creatConfig_cmdParam_t *pConfig)
{
    ...
    if ((pConfig->Config_ID > 3) || (pConfig->Create_Context > 1) || (pConfig->Main_Mode > 3) || (pConfig->Main_Mode == 0) || ((pConfig->Sub_Mode > 3) && (pConfig->Sub_Mode != 0xff)) || (pConfig->Sub_Mode == 0)) {
    ...
    if (pConfig->Sub_Mode != 0xFF && ((pConfig->Main_Mode_Max_Steps == 0) || (pConfig->Main_Mode_Min_Steps == 0))) {
        CS_HCI_LOG("[CHK_CFG] main_mode step abnormal:0x%x,0x%x", pConfig->Main_Mode_Max_Steps, pConfig->Main_Mode_Min_Steps);
        return HCI_ERR_UNSUPPORTED_FEATURE_PARAM_VALUE;
    }

    if ((pConfig->Main_Mode_Repetition > 3) || (pConfig->Mode_0_Steps == 0) || (pConfig->Mode_0_Steps > 3) || (pConfig->Role > 1) || (pConfig->RTT_Type > 6) || (pConfig->CS_SYNC_PHY == 0) || (pConfig->CS_SYNC_PHY > 2) || (pConfig->ChSel > 1) || (pConfig->Ch3c_Shape > 1)) {
        CS_HCI_LOG("[CHK_CFG] Role abnormal:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x",
                   pConfig->Main_Mode_Repetition,
                   pConfig->Mode_0_Steps,
                   pConfig->Role,
                   pConfig->RTT_Type,
                   pConfig->CS_SYNC_PHY,
                   pConfig->Ch3c_Shape,
    ...
    if ((pConfig->ChSel && (pConfig->Ch3c_Jump < 2 || pConfig->Ch3c_Jump > 8)) || (pConfig->Companion_Signal_Enable > 1)) {
...
ble_sts_t blc_hci_le_cs_readRemoteSupportedCap(u16 handle)
{
    ...
    if (pCsParam->cs_cap_exchange) {
        pCsParam->cs_cap_req = PROC_CS_CAP_EVT_PENDING;
    }
    /* HCI/CCO/BI-108-C if EBQ only send hci read remote cap command, cs_cap_req = PROC_CS_CAP_SEND_REQ | PROC_CS_CAP_EVT_PENDING
     * will cause here don't send hci complete event -- yuexin
     */
    #if (HCI_CCO_BI_108_C)
        ...
        pCsParam->cs_cap_req = PROC_CS_CAP_SEND_REQ | PROC_CS_CAP_EVT_PENDING;
    }
    ...
    if (!pAcl->llcp_flag.bit.ll_feat_exg_flag) {
        ...
        CS_HCI_LOG("[CAP][R_REM] expect feature exchange");
    }
...
ble_sts_t blc_hci_le_cs_security_enable(u16 connHandle)
{
     ...
     *then the Controller shall return the error code Command Disallowed (0x2F).*/
    if (0 == pAcl->crypt.enable) {
        CS_HCI_LOG("[SEC] enc not complete");
        return HCI_ERR_INSUFFICIENT_SECURITY;
    }
    ...
    if (pCsParam->cs_security_exchange) {
        pCsParam->cs_security_enable = PROC_CS_SEC_EVT_PENDING;
    } else {
        if (pCsParam->cs_security_enable) {
            pCsParam->cs_security_enable |= PROC_CS_SEC_EVT_PENDING;
        } else {
            pCsParam->cs_security_enable = PROC_CS_SEC_SEND_REQ | PROC_CS_SEC_EVT_PENDING; // start or restart cs security start procedure
        }
    }
    #if OS_SUP_EN
    if (blt_os_giveSem_cb) {
...
{
    CS_HCI_LOG("[CMD] CS Set Default Settings:%s", hex_to_str(pSetting, sizeof(hci_le_cs_setDefaultSetting_cmdParam_t)));

    retParam->status            = blc_ll_cs_setDefaultSetting(pSetting->Connection_Handle, pSetting->Role_Enable, pSetting->CS_SYNC_Antenna_Selection, pSetting->Max_TX_Power);
...
ble_sts_t blc_hci_le_cs_readRemoteFAE_table(u16 connHandle)
{
    ...
    if (pCsParam->cs_fae_exchange) {
        pCsParam->cs_fae_req = PROC_CS_FAE_EVT_PENDING;
    } else {
        pCsParam->cs_fae_req = PROC_CS_FAE_SEND_REQ | PROC_CS_FAE_EVT_PENDING; // LL CS FAE REQ
    }
    /*
...
{
    CS_HCI_LOG("[CMD] cs create config:%s", hex_to_str(pConfig, sizeof(hci_le_cs_creatConfig_cmdParam_t)));
    ...
    chn_sound_capabilities_t *pCsRemoteSupCap = &pc->csRemoteSupCap;
    ...
    #if CS_EBQ_TEST
        //LL/CS/CEN/INI/BI-09-C [Reject a Config Request During an Active CS Procedure With the Same Config_ID , Central, Initiator]
        //LL/CS/CEN/INI/BV-04-C [Initiate Configuration Procedure, Central, Initiator] Don't reject Config ID with different param
        if (!check_sameConfig_param(pConfig, cfgIdx)) {
            CS_EBQ_LOG("[CFG][CR8] Config wrong!!! cfg id is exist and with same param:0x%x", cfgIdx);
            return HCI_ERR_CMD_DISALLOWED;
        } else {
            CS_EBQ_LOG("[CFG]Config ok, same Config ID with different param");
        }
    #endif
    }
    ...
    ret = blt_cs_extractEnableChnMap(pConfig->Channel_Map, pCsCfg->filteredChnArray, &pCsCfg->Chn_en_num);
    #if (CS_EBQ_TEST)
    if ((ret != BLE_SUCCESS) || (pCsCfg->Chn_en_num < 15)) {
        CS_EBQ_LOG("[CFG][CR8] chnNum eroro:0x%x, 0x%x", ret, pCsCfg->Chn_en_num);
        return HCI_ERR_INVALID_HCI_CMD_PARAMS;
    }
    #endif
    ...
    if (pCsCfg->Create_Context == 1) {
        ...
        pCsParam->cs_config_req      = PROC_CS_CONFIG_SEND_REQ | PROC_CS_CONFIG_EVT_PENDING;
    } else {
...
ble_sts_t blc_hci_le_cs_removeConfig(u16 connHandle, u8 config_ID)
{
    ...
    cs_param_t   *pCsParam = &pc->csParam;
    cs_config_t  *config   = gCsMng.gGlobal_pCsCfg + cfgIdx;
...
{
    CS_HCI_LOG("[CMD] set procedure param:%s", hex_to_str(pParam, sizeof(hci_le_cs_setProcedureParame_cmdParam_t)));
...
{
    CS_HCI_LOG("[CMD] set procedure enable:%s", hex_to_str(pCmd, sizeof(hci_le_cs_enableProcedure_cmdParam_t)));
    /*If the Host sends this command with a Connection_Handle that does not exist, or the Connection_Handle
     *is not for an ACL the Controller shall return the error code Unknown Connection Identifier (0x02).*/
    ...
    if (cfgIdx == 0xff) {
        ...
        return HCI_ERR_INVALID_HCI_CMD_PARAMS; //HCI_ERR_CMD_DISALLOWED
    }
    ...
    /* case LL/CS/CEN/INI/BV-27-C This part don't need actually */
    #if (0)
    if (pCmd->Enable && ((0 == config->cs_procedure_para_set_en) || config->cs_procedure_en)) {
        CS_EBQ_LOG("[PROC_EN] cs start abnormal:0x%x,0x%x,0x%x", pCmd->Enable, config->cs_procedure_para_set_en, config->cs_procedure_en);
        return HCI_ERR_CMD_DISALLOWED;
    }
    #endif
    ...
    if (pCmd->Enable) {
        ...
        pCsParam->cs_req                    = (PROC_CS_SEND_REQ | PROC_CS_EVT_PENDING);
    } else if (!pCmd->Enable) { //terminate indicate
        pCsParam->cs_terminate_ind        = PROC_CS_TERMINATE_SEND_REQ;
        pCsParam->cs_terminate_error_code = HCI_ERR_REMOTE_USER_TERM_CONN;
    }
    ...
    hci_le_cs_writeCachedRemoteSupportedCap_cmdParam_t *pCS_param)
{
    ...
    smemcpy(pCsRemoteSupCap, &pCS_param->Num_Config_Supported, sizeof(chn_sound_capabilities_t)); //todo  to check len
...
ble_sts_t blc_ll_cs_setDefaultSetting(u16 handle, cs_role_t role_enable, u8 ant_sel, s8 max_tx_power)
{
    ...
    if (((!(role_enable & CS_INITIATOR_ROLE)) && blt_ll_getCsConfigByRole(pc->acl_conHandle, CS_CONFIG_INITIATOR_ROLE)) || ((!(role_enable & CS_REFLECTOR_ROLE)) && blt_ll_getCsConfigByRole(pc->acl_conHandle, CS_CONFIG_REFLECTOR_ROLE))) {
     ...
     * Value (0x11)
     */
    /* HCI/CCO/BI-86-C, HCI/CCO/BI-87-C */
    if (role_enable) {
    ...
    /* HCI/CCO/BI-88-C, TODO: Check 0xFE with BT SIG, 0xFF is ok */
    ...
    //in case bltCsLocalSupportCap.Roles_Supported is only one - either Initiator or Reflector
    //then a role_enable having both CS_INIT_REFL_ROLE needs to be properly limited
    pCsParam->role_enable = role_enable & bltCsLocalSupportCap.Roles_Supported;

    //pcl.c  blt_ll_pclGetRfActualTxPwr
    if (max_tx_power >= ble_rf_max_tx_pwr) {
        pCsParam->Max_TX_Power = ble_rf_max_tx_pwr;
    } //ble_rf_max_tx_pwr:9dbm
    else if (max_tx_power >= 0) {
        pCsParam->Max_TX_Power = max_tx_power;
    } else if (max_tx_power >= -4) {
        pCsParam->Max_TX_Power = -4;
    } else if (max_tx_power >= -8) {
        pCsParam->Max_TX_Power = -8;
    } else if (max_tx_power >= -12) {
        pCsParam->Max_TX_Power = -12;
    } else if (max_tx_power >= -18) {
        pCsParam->Max_TX_Power = -18;
    } else {
        pCsParam->Max_TX_Power = ble_rf_min_tx_pwr;
    } //ble_rf_min_tx_pwr:-23dbm
...
ble_sts_t blc_ll_cs_setProcedureParam(hci_le_cs_setProcedureParame_cmdParam_t *pParam)
{
    ...
    // if it's the first start procedure instance or the last started procedure count is 0, central side should clean the csProcCount
    if (!config->procMaxCountInstant) {
        config->csProcCount  = 0;
        config->procMaxCount = 0;
        CS_HCI_LOG("Clean cs procedure count and max procedure count on central side");
    }
    config->procMaxCount += pParam->Max_Procedure_Count;
    config->procMaxCountInstant           = pParam->Max_Procedure_Count;
    ...
    config->Subevent_Len = min3(TLK_CS_SUBEVENT_MAX_LEN, subevent_len_us, pAclConn->conn_intvl_n_1m25 * 1250 / 3 * 2);

    config->Subevents_Per_Event = 2; //(pAclConn->conn_intvl_n_1m25*1250/3 * 2)/(config->subEvtIntvl_625us*625) -1 ;
        #if (DBG_CS_ONE_SUBEVENT_72CHN)
    config->Subevent_Len        = TLK_CS_SUBEVENT_MAX_LEN;
    config->Subevents_Per_Event = 1;
        #endif

    if (config->Subevents_Per_Event == 1) {
        config->subEvtIntvl_625us = 0;
    } else {
        #if (CS_EBQ_TEST) // if EBQ test , prepare 6.25ms for calculate next subevent step info -- note by yuexin 2024/07/20
        config->subEvtIntvl_625us = (config->Subevent_Len + 6250) / 625;
        #else
        config->subEvtIntvl_625us = (config->Subevent_Len + TLK_T_MES) / 625 + 5; // 5 bSlot for inserting ACL task
        #endif
    }
    config->Event_Interval = (config->subEvtIntvl_625us * config->Subevents_Per_Event) / pAclConn->bSlot_interval + 2;

    if (config->procMaxCountInstant != 1) {
        config->Procedure_Interval = (config->Max_Procedure_Interval + config->Min_Procedure_Interval) / 2;
    } else {
        config->Procedure_Interval = 0;
    }
    #else
    config->Min_Subevent_Len = min2(config->Min_Subevent_Len, TLK_CS_SUBEVENT_MAX_LEN);
    config->Max_Subevent_Len = min2(config->Max_Subevent_Len, TLK_CS_SUBEVENT_MAX_LEN);
    #endif
    ...
    CS_HCI_LOG("[CS_PARAM] Set Proc Param Success");
    return BLE_SUCCESS;
}
