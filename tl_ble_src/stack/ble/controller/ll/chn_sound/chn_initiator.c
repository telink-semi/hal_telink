/********************************************************************************************************
 * @file    chn_initiator.c
 *
 * @brief   This is the source file for BLE SDK
 *
 * @author  BLE GROUP
 * @date    06,2022
 *
 * @par     Copyright (c) 2022, Telink Semiconductor (Shanghai) Co., Ltd.
 *          All rights reserved.
 *
 *          The information contained herein is confidential property of Telink
 *          Semiconductor (Shanghai) Co., Ltd. and is available under the terms
 *          of Commercial License Agreement between Telink Semiconductor (Shanghai)
 *          Co., Ltd. and the licensee or the terms described here-in. This heading
 *          MUST NOT be removed from this file.
 *
 *          Licensee shall not delete, modify or alter (or permit any third party to delete, modify, or
 *          alter) any information contained herein in whole or in part except as expressly authorized
 *          by Telink semiconductor (shanghai) Co., Ltd. Otherwise, licensee shall be solely responsible
 *          for any claim to the extent arising out of or relating to such deletion(s), modification(s)
 *          or alteration(s).
 *
 *          Licensees are granted free, non-transferable use of the information in this
 *          file under Mutual Non-Disclosure Agreement. NO WARRANTY of ANY KIND is provided.
 *
 *******************************************************************************************************/
#include "tl_common.h"
#include "drivers.h"
#include "stack/ble/controller/ble_controller.h"
#if OS_SUP_EN
#include "stack/ble/os_sup/os_sup.h"
#include "stack/ble/os_sup/os_sup_stack.h"
#endif
#if(LL_FEATURE_ENABLE_CHANNEL_SOUNDING_INITIATOR)

_attribute_no_inline_
int blt_cs_initiator_mainloop_task (int flag, void *p){
    (void)flag;(void)p;
    return 0;
}


#if OS_COMPILE_OPTIMIZE_EN
_attribute_ram_code_sec_optimize_o2_
#endif
_attribute_ram_code_
int blt_cs_init_step_stx_start(void){

    blms_state = BLMS_STATE_CS_INIT_TX_S;

    u8 ext_slot_en = 0;
    u8 tx_early_us = 13;

    u8 slipIdx = blt_pCsCfg->slip_stepReadIdx&SLIP_WINDOW_STEP_MSK;
    slip_window_step_t * pslip_window_step = &blt_pCsCfg->slip_window_step[slipIdx];

    u8 chn  = pslip_window_step->step_chnIdx;
    u8 mode = pslip_window_step->step_modeType;
    u32 ac  = pslip_window_step->step_initAA;


    DBG_CS_CHN3_HIGH;
#if (SL01_cs_step_init_0)
    log_task_begin_irq(SL_STACK_CS_TIME_EN, SL01_cs_step_init_0);
#endif

    ble_rf_set_cs_channel(chn);

    u8 tx_settle_us = TX_STL_TIFS_REAL_COMMON;
    #if (HADM_PHASE_CONTINUITY)
        //After setting the frequency point, enable manual fcal to ensure phase continuity.
        u32 tick_fcal_start = 0;
        if(cs_phase_continuity_flag){
            ble_rf_manual_fcal_start();
            tick_fcal_start = clock_time();
        }
    #endif
    rf_ble_set_tx_settle(tx_settle_us);

    if(mode==STEP_MODE_0){
        tx_early_us = CS_RF_TX_1M_PACKET_EARLY_US;//8us(1 byte preamble) + 5us(1M PHY TXLLDLY + TXPATHDLY)

        rf_set_ble_access_code((u8*)&ac);
    }
    else if(mode == STEP_MODE_2){
        tx_early_us = CS_RFRXEN_MODE_EARLY_US;//5us(1M PHY TXLLDLY + TXPATHDLY)
        ext_slot_en = pslip_window_step->extSlot.step_extSlotInit;

        ble_rf_set_tx_modulation_index(RF_MI_P0p00);
        //ble_rf_set_power_level_singletone(RF_POWER_P4p61dBm);//no need, set RF Power in subevent_start already
    }
#if(ADD_CS_MODE1_FUN)
    else if(mode == STEP_MODE_1){
        tx_early_us = CS_RF_TX_1M_PACKET_EARLY_US;//8us(1 byte preamble) + 5us(1M PHY TXLLDLY + TXPATHDLY)
        //ble_rf_set_tx_modulation_index(RF_MI_P0p50);//no need
        reg_rf_ll_irq_list_h = BIT(0);//clear tr_turnaround_irq
    }
#endif
    else{
        BLMS_ERR_DEBUG(DBG_CS_SCH_INIT, 0xBBEE0000);
    }

#if (MULTIPLE_ANTENNA_EN)
    u8 exist_T_SW = (mode == STEP_MODE_2) ? g_T_SW_us : 0;
    u32 tx_tick = blt_pCsCfg->step_expect_tick + exist_T_SW*SYSTEM_TIMER_TICK_1US - (tx_early_us + tx_settle_us) * SYSTEM_TIMER_TICK_1US;//g_T_SW_us
#else
    u32 tx_tick = blt_pCsCfg->step_expect_tick - (tx_early_us + tx_settle_us) * SYSTEM_TIMER_TICK_1US;
#endif



    /****************just for debug start**************************/
    u32 reflAA = pslip_window_step->step_reflAA;
    u8  extSlotRefl = pslip_window_step->extSlot.step_extSlotRefl;(void)extSlotRefl;
    u8  extSlotAll  = pslip_window_step->extSlot.step_extSlotFlag;(void)extSlotAll;
    u32 chnModeIeRe = chn<<24 | mode<<16 | ext_slot_en<<8 | ext_slot_en;
    tlkapi_send_string_u32s(DBG_CS_LOG_SCH_MASK_EN, "stxCntiAArAAIdx", blt_pCsCfg->csStepCount,ac, reflAA, chnModeIeRe);
    /****************just for debug end**************************/

    u8 rf_packet_len = 0;
    if(mode==STEP_MODE_0){
        //1M PHY, 1 byte for preamble extra length
        rf_packet_len = 2 + 4 + 1;//2(preamble) + 4(access address) + 1(4 bit trail), PRMBL_LENGTH_1M
        pkt_CS.dma_len = rf_tx_packet_dma_len(rf_packet_len);
        smemcpy((u8 *)&pkt_CS.accessAddress, (u8 *)&ac, 4);
        pkt_CS.preamble[0] = pkt_CS.preamble[1] = BIT_IS_SET(pkt_CS.accessAddress, 0) ? 0x55 : 0xAA;
        pkt_CS.trailer = BIT_IS_SET(pkt_CS.accessAddress, 31) ? 0xA : 0x5;
        pkt_CS.shift_sequence = 0;
    }
    else if(mode == STEP_MODE_2){

    #if (MULTIPLE_ANTENNA_EN)
        u32 t_pm_us = (ext_slot_en==1) ? (g_T_SW_us+g_t_pm_us)*(g_antennaPathNum+1): (g_T_SW_us+g_t_pm_us)*g_antennaPathNum;
        t_pm_us -= g_T_SW_us;//because RF machine rx will start to receive after T_SW.
        rf_packet_len = CAL_LL_CS_TONE_TX_SIZE(t_pm_us);
    #else
        if(ext_slot_en){
            rf_packet_len = CAL_LL_CS_TONE_TX_SIZE(g_t_pm_us<<1);//(T_PM_us + tone extension_slot)/8
        }
        else{
            rf_packet_len = CAL_LL_CS_TONE_TX_SIZE(g_t_pm_us);//T_PM_us/8
        }
    #endif

        pkt_CS.dma_len = rf_tx_packet_dma_len(rf_packet_len);
    }
#if(ADD_CS_MODE1_FUN)
    else if(mode == STEP_MODE_1){
        //1M PHY, RTT AA Only, 1 byte for preamble extra length
        rf_packet_len = 2 + 4 + 1;//2(preamble) + 4(access address) + 1(4 bit trail), PRMBL_LENGTH_1M
        pkt_CS.dma_len = rf_tx_packet_dma_len(rf_packet_len);
        smemcpy((u8 *)&pkt_CS.accessAddress, (u8 *)&ac, 4);
        pkt_CS.preamble[0] = pkt_CS.preamble[1] = BIT_IS_SET(pkt_CS.accessAddress, 0) ? 0x55 : 0xAA;
        pkt_CS.trailer = BIT_IS_SET(pkt_CS.accessAddress, 31) ? 0xA : 0x5;
        pkt_CS.shift_sequence = 0;
    }
#endif

    u32 tick_now = clock_time();
    if(tick1_exceed_tick2(tick_now, tx_tick)){
        write_dbg32(0x0018, tick_now);
        write_dbg32(0x001C, tx_tick);
        u32 diff = (u32)(tick_now - tx_tick)/SYSTEM_TIMER_TICK_1US;

        tlkapi_send_string_u32s(0, "cs start", diff, blt_pCsCfg->step_expect_tick, blt_pCsCfg->tick_expect_csSubevent, mode);
        BLMS_ERR_DEBUG(DBG_CS_SCH_INIT, 0xBBff0100);
    }

#if (HADM_PHASE_CONTINUITY)
    if(cs_phase_continuity_flag){//as close as possible to FSM, thus can save some time.
        while(!tick1_exceed_tick2(clock_time(), tick_fcal_start + RF_FCAL_MANUAL_START2DONE_TIME_US * SYSTEM_TIMER_TICK_1US)){}
        ble_rf_manual_fcal_done();
    }
#endif
    rf_start_fsm(FSM_STX, (void *)&pkt_CS, tx_tick);

    //Consider mode0 and mode2, the time is not the same
//  systimer_set_irq_capture(tx_tick + (blt_pCsCfg->t_synu_us + tx_early_us + tx_settle_us + TLK_TM_DELAY)*SYSTEM_TIMER_TICK_1US); //8 us = 1preamble extra
    systick_irq_trigger = SYS_IRQ_TRIG_CS_INIT_SRX;
    return 0;
}

#if OS_COMPILE_OPTIMIZE_EN
_attribute_ram_code_sec_optimize_o2_
#endif
_attribute_ram_code_
int blt_cs_init_step_srx(void){

    blms_state = BLMS_STATE_CS_INIT_RX_S;


    u8 ext_slot_en = 0;
    u16 iq_sample_number = 0;
    u8 start_point = 0;
    u8 rx_early_us = 20;
    u32 rx_tick = 0;
    u32 first_timeout = 0;
    rf_hadm_iq_sample_mode_e cs_rx_mode = RF_HADM_IQ_SAMPLE_SYNC_MODE;

    u8 slipIdx = blt_pCsCfg->slip_stepReadIdx&SLIP_WINDOW_STEP_MSK;
    slip_window_step_t * pslip_window_step = &blt_pCsCfg->slip_window_step[slipIdx];

    u8 mode = pslip_window_step->step_modeType;
    u32 ac = pslip_window_step->step_reflAA;

    u8 rx_settle_us = RX_SETTLE_US;
    #if (HADM_PHASE_CONTINUITY)
        if(mode==STEP_MODE_0){
            if(!cs_phase_continuity_flag && !blt_pCsCfg->phaseContin_config_flag){
                ble_rf_cs_get_tx_cali_value(&tx_cs_cali);
            }
        }

        rx_settle_us = cs_phase_continuity_flag?CS_RF_RX_SETTLE_US:RX_SETTLE_US;
    #endif
    rf_ble_set_rx_settle(rx_settle_us);



    pCsRxAddr = cs_rx_fifo.p_base + (cs_rx_fifo.wptr & cs_rx_fifo.mask) * cs_rx_fifo.size;
    ble_rf_set_rx_dma(pCsRxAddr, cs_rx_fifo.size_div_16);

    if(mode==STEP_MODE_0){
        /**iq_sample_number: 0.25us unit (4MHz) ---> 0.25us * 320 = 80us ---> T_FM
         * iq_start_point: sync mode, sampling starts at (start_point + 1) * 0.125us after sync
         *      (111 + 1) * 0.125us = 14us = 4 + 10 ---> Trailer + T_GD
         */
        iq_sample_number = 320;
        start_point = 111;

        rf_set_ble_access_code((u8*)&ac);

        rx_early_us = 20;
        //1st rx timeout: rx_early_us + RX_SETTLE_US + T_SY + T_RD + margin_15us
        //                rx_early_us + RX_SETTLE_US + 44   + 5    + 15         = rx_early_us + RX_SETTLE_US + 64
        first_timeout = rx_early_us + rx_settle_us + 64;
        //Theoretical rx point = tick_step_start + T_SY + T_RD + T_IP1 = tick_step_start + 44 + 5 + T_IP1 = tick_step_start + T_FCS + T_IP1 + 49
        rx_tick = blt_pCsCfg->step_expect_tick + (g_T_IP1_us + 49 - rx_early_us - rx_settle_us) * SYSTEM_TIMER_TICK_1US;

    }
    else if(mode == STEP_MODE_2){
        //todo check with lijing
        ble_rf_set_power_off_singletone();
        ble_rf_set_tx_modulation_index(RF_MI_P0p50);
        //ble_rf_agc_disable();
        start_point = 6;
        cs_rx_mode = RF_HADM_IQ_SAMPLE_RXEN_MODE;
        ext_slot_en = pslip_window_step->extSlot.step_extSlotRefl;

    #if(MULTIPLE_ANTENNA_EN)
        u32 t_pm_us = (ext_slot_en==1) ? (g_T_SW_us+g_t_pm_us)*(g_antennaPathNum+1): (g_T_SW_us+g_t_pm_us)*g_antennaPathNum;
        t_pm_us -= g_T_SW_us;//because RF machine rx will start to receive after T_SW.
        iq_sample_number = (t_pm_us + CS_RFRXEN_MODE_EARLY_US) << 2;
    #else
        iq_sample_number = ((ext_slot_en?  (g_t_pm_us<<1): g_t_pm_us) + CS_RFRXEN_MODE_EARLY_US) << 2;
    #endif

        rx_early_us = CS_RFRXEN_MODE_EARLY_US;//5us(1M PHY RXPATHDLY) RXEN MODE
        //1st rx timeout: RX_SETTLE_US + rx_early_us + T_PM + extension_slot + margin_25us
        //                RX_SETTLE_US + rx_early_us + T_PM + T_PM           + 25           = RX_SETTLE_US + rx_early_us + (T_PM_us << 1) + 25
    #if(MULTIPLE_ANTENNA_EN)
        u32 realPm_extPm_us = (g_antennaPathNum+1)*(g_T_SW_us+g_t_pm_us);
        first_timeout = rx_settle_us + rx_early_us + realPm_extPm_us + 25;
        rx_tick =   blt_pCsCfg->step_expect_tick + (realPm_extPm_us + 5 + g_T_IP2_us + g_T_SW_us - rx_early_us - rx_settle_us)*SYSTEM_TIMER_TICK_1US;//5--RD(ramp down)
    #else
        first_timeout = rx_settle_us + rx_early_us + (g_t_pm_us << 1) + 25;
        rx_tick =   blt_pCsCfg->step_expect_tick + ((g_t_pm_us<<1) + 5 + g_T_IP2_us - rx_early_us - rx_settle_us)* SYSTEM_TIMER_TICK_1US;
    #endif
    }
#if(ADD_CS_MODE1_FUN)
    else if(mode == STEP_MODE_1){
        u32 tick_tx_done = clock_time();
        //ble_rf_agc_disable();
        start_point = 1;
        cs_rx_mode = RF_HADM_IQ_SAMPLE_RXEN_MODE;
        /**iq_sample_number: 0.25us unit (4MHz) ---> 0.25us * 1 = 0.25us
         * iq_start_point: rx_en mode, sampling starts at 0.25us+start_point*0.125us after settle
         *      0.25us + 1*0.125us = 0.375us
         */
        rx_early_us = CS_RFRXEN_MODE_EARLY_US + CS_RF_RX_1M_EXTRA_PREAMBLE_US;//5us(1M PHY RXPATHDLY) RXEN MODE + 1 byte for preamble extra length
        u8 rx_extend_us = CS_RF_RX_1M_WINDOW_EXTEND_US;// ensure correct Sync
        iq_sample_number = (rx_early_us + CS_1M_PACKET_AA_ONLY_US + rx_extend_us) << 2;

        rf_set_ble_access_code((u8*)&ac);//must

        //1st rx timeout: rx_settle_us + rx_early_us + CS_1M_PACKET_AA_ONLY_US + rx_extend_us  + margin_25us
        //          =     rx_settle_us + rx_early_us + CS_1M_PACKET_AA_ONLY_US + rx_extend_us  + 25
        first_timeout = rx_settle_us + rx_early_us + CS_1M_PACKET_AA_ONLY_US + rx_extend_us  + 25;
        rx_tick = blt_pCsCfg->step_expect_tick + (CS_1M_PACKET_AA_ONLY_US + 5 + g_T_IP1_us - rx_early_us - rx_settle_us) * SYSTEM_TIMER_TICK_1US;

        while((!(reg_rf_ll_irq_list_h & BIT(0))) && (!tick1_exceed_tick2(clock_time(), tick_tx_done + 15 * SYSTEM_TIMER_TICK_1US))){
        }
        reg_rf_ll_irq_list_h = BIT(0);//clear tr_turnaround_irq
        cs_tick_tx_on = rf_hadm_get_timestamp();//now this time obtained is tx_on
    }
#endif

    ble_rf_channel_sounding_iq_sample_config(iq_sample_number, start_point, cs_rx_mode);//iq_sample_number cannot set to 0

    u32 clock_now= clock_time();
    if(tick1_exceed_tick2(clock_now, rx_tick)){
        write_dbg32(0x0018, clock_now);
        write_dbg32(0x001C, rx_tick);
        BLMS_ERR_DEBUG(DBG_CS_SCH_INIT, 0xBBff0200);
    }
    rf_start_fsm(FSM_SRX, NULL, rx_tick);
    rf_set_1st_rx_timeout(first_timeout);
    blt_pCsCfg->step_rx_flag = 0;
    systick_irq_trigger = SYS_IRQ_TRIG_CS_STEP_POST;

    DBG_CS_CHN3_TOGGLE;DBG_CS_CHN3_TOGGLE;

    return 0;
}

#if OS_COMPILE_OPTIMIZE_EN
_attribute_ram_code_sec_optimize_o2_
#endif
_attribute_ram_code_
int blt_cs_init_step_post(void){

    u8 slipIdx = blt_pCsCfg->slip_stepReadIdx&SLIP_WINDOW_STEP_MSK;
    slip_window_step_t * pslip_window_step = &blt_pCsCfg->slip_window_step[slipIdx];

    u8 se_end = pslip_window_step->subeventEndFlag;


    #if (SL01_cs_step_init_0)
        log_task_end_irq(SL_STACK_CS_TIME_EN, SL01_cs_step_init_0);
    #endif
    DBG_CS_CHN3_LOW;


    blms_state = BLMS_STATE_CS_INIT_E;
    u8 mode = pslip_window_step->step_modeType;
    u8 chn  = pslip_window_step->step_chnIdx;
    u8 rx_ext_slot_en = pslip_window_step->extSlot.step_extSlotRefl;
    u32 reflAA = pslip_window_step->step_reflAA;

    rf_set_tx_rx_off();
    STOP_RF_STATE_MACHINE;
    //ble_rf_agc_enable();//review lijing


    blt_pCsCfg->csStepCount ++;
    blt_pCsCfg->slip_stepReadIdx ++;

    ////////// for CS Subevent Result Event start //////////
    pCsRxAddr[2] = BLT_CS_INITIATOR_FLAG;
    if(mode == STEP_MODE_0){
        pCsRxAddr[2] |= BLT_CS_MODE_0_FLAG;

        if(!blt_pCsCfg->step_rx_flag){
            //set to the default value
            pCsRxAddr[0] = 0x6c;
            pCsRxAddr[1] = 0x06;
        }
    }
    else if(mode == STEP_MODE_1){
        pCsRxAddr[2] |= BLT_CS_MODE_1_FLAG;

        u8 cs_tx_on_tick[4] = {U32_TO_BYTES(cs_tick_tx_on)};
        smemcpy(pCsRxAddr + DMA_CS_RFRX_OFFSET_TX_ON_TSTAMP_4BYTE(pCsRxAddr), cs_tx_on_tick, 4);

        u8 cs_rx_AA[4] = {U32_TO_BYTES(reflAA)};
        smemcpy(pCsRxAddr + DMA_CS_RFRX_OFFSET_RX_ACCESS_ADDRESS_4BYTE(pCsRxAddr), cs_rx_AA, 4);
    }
    else if(mode == STEP_MODE_2){
        pCsRxAddr[2] |= BLT_CS_MODE_2_FLAG;

        u8 cs_proc_start_tick[4] = {U32_TO_BYTES(blt_pCsCfg->tick_proc_start)};
        smemcpy(pCsRxAddr + DMA_CS_RFRX_OFFSET_TICK_CS_PROC_START_4BYTE(pCsRxAddr), cs_proc_start_tick, 4);

        pCsRxAddr[DMA_CS_RFRX_OFFSET_T_SW_LOW4BIT(pCsRxAddr)] = g_T_SW_us;
        pCsRxAddr[DMA_CS_RFRX_OFFSET_ACI_HIGH4BIT(pCsRxAddr)] = (pCsRxAddr[DMA_CS_RFRX_OFFSET_T_SW_LOW4BIT(pCsRxAddr)] | (blt_pCsCfg->aci << 4));
    }
    else if(mode == STEP_MODE_3){
        pCsRxAddr[2] |= BLT_CS_MODE_3_FLAG;
    }
    if(blt_pCsCfg->step_rx_flag){
        pCsRxAddr[2] |= BLT_CS_MODE_RX_FLAG;
    }
    if(rx_ext_slot_en){
        pCsRxAddr[2] |= BLT_CS_STEP_TONE_EXTENSION_SLOT_FLAG;
    }

    pCsRxAddr[3] = chn;

    pCsRxAddr[DMA_CS_RFRX_OFFSET_CONN_HANDLE(pCsRxAddr)] = U16_LO(blt_pCsCfg->aclHandle);
    pCsRxAddr[DMA_CS_RFRX_OFFSET_CONFIG_ID_LOW4BIT(pCsRxAddr)] = blt_pCsCfg->Config_ID;
    pCsRxAddr[DMA_CS_RFRX_OFFSET_NUM_ANTENNA_PATHS_HIGH4BIT(pCsRxAddr)] = (pCsRxAddr[DMA_CS_RFRX_OFFSET_CONFIG_ID_LOW4BIT(pCsRxAddr)] | (g_antennaPathNum << 4));//N_AP = 1~4
    pCsRxAddr[DMA_CS_RFRX_OFFSET_START_ACL_CONN_EVENT_2BYTE(pCsRxAddr)] = U16_LO(blt_pCsCfg->connEventCount);
    pCsRxAddr[DMA_CS_RFRX_OFFSET_START_ACL_CONN_EVENT_2BYTE(pCsRxAddr) + 1] = U16_HI(blt_pCsCfg->connEventCount);
    pCsRxAddr[DMA_CS_RFRX_OFFSET_PROCEDURE_COUNTER_2BYTE(pCsRxAddr)] = U16_LO(blt_pCsCfg->csProcCount);
    pCsRxAddr[DMA_CS_RFRX_OFFSET_PROCEDURE_COUNTER_2BYTE(pCsRxAddr) + 1] = U16_HI(blt_pCsCfg->csProcCount);
    pCsRxAddr[DMA_CS_RFRX_OFFSET_PROCEDURE_DONE_STATUS(pCsRxAddr)] = ((se_end && blt_pCsCfg->proc_end_flag) ? 0 : 1);
    pCsRxAddr[DMA_CS_RFRX_OFFSET_SUBEVENT_DONE_STATUS(pCsRxAddr)] = (se_end ? 0 : 1);
    pCsRxAddr[DMA_CS_RFRX_OFFSET_RX_AGC_GAIN(pCsRxAddr)] = cs_rx_agc_gain;

    cs_rx_fifo.wptr++;
#if OS_SUP_EN //todo:
    if(blt_os_semCountIncrementIrq_cb)
    {
        blt_os_semCountIncrementIrq_cb();
    }
#endif
    #if (DBG_CS_DATA_EN)
        if((u8)(cs_rx_fifo.wptr - cs_rx_fifo.rptr) > cs_rx_fifo.num){
            tlkapi_send_string_u8s(DBG_CS_DATA_EN, "cs_rx_fifo overflow", mode, cs_rx_fifo.wptr, cs_rx_fifo.rptr, cs_rx_fifo.num);
            BLMS_ERR_DEBUG(DBG_CS_DATA_EN, 0x55550002);
        }
    #endif
    ////////// for CS Subevent Result Event end //////////

    if(se_end){
        blt_cs_subevent_post(blt_pCsCfg->proc_end_flag ? 0 : 1);
    }
    else{


        if(mode==STEP_MODE_0){
            /**1M PHY, receive succeed Air Timing
             *      Access_Address + Trailer + T_GD + T_FM + T_RD
             *      timestamp           4       10      80      5 = 99
             *
             * 1M PHY, IQ Sample Rx Path Delay Timing
             *      RXPATHDLY
             *          5   //RXEN MODE
             *          12  //SYNC MODE
             *
             *  Total time consumption = 99 - RXPATHDLY = 94    //RXEN MODE
             *  Total time consumption = 99 - RXPATHDLY = 87    //SYNC MODE
             */

//          if(blt_pCsCfg->step_rx_flag){
//
//              tlkapi_send_string_u32s(1, "stx post", blt_pCsCfg->step_expect_tick, hal_rf_get_rx_timestamp() + (87 + g_T_FCS_us) * SYSTEM_TIMER_TICK_1US,hal_rf_get_rx_timestamp(),0);
//              blt_pCsCfg->step_expect_tick = hal_rf_get_rx_timestamp() + (87 + g_T_FCS_us) * SYSTEM_TIMER_TICK_1US ;//SYNC MODE
//          }
//          else
            {
                /**1M PHY, receive lose Air Timing
                 *      T_SY + T_RD +   T_IP1 +     T_SY + T_GD + T_FM + T_RD  + T_FCS
                 *      44      5       g_T_IP1_us  44      10      80      5  + g_T_FCS_us
                 *
                 *  Total time consumption = T_FCS_us + T_IP1_us + 188
                 */
                blt_pCsCfg->step_expect_tick = blt_pCsCfg->step_expect_tick + (g_T_FCS_us + g_T_IP1_us + 188) * SYSTEM_TIMER_TICK_1US;

                systick_irq_trigger = SYS_IRQ_TRIG_CS_INIT_TX_START;
            }
        }
        else if(mode ==STEP_MODE_2){

            /**
             *      T_FCS + T_PM + extension_slot + T_RD +  T_IP2 + T_PM + extension_slot + T_RD
             *      T_FCS   T_PM        T_PM        5       T_IP2   T_PM        T_PM        5
             *
             *  Total time consumption = T_FCS_us + T_PM_us*4 + T_IP2_us + 10
             */
        #if(MULTIPLE_ANTENNA_EN)
            u32 t_pm_us = (g_T_SW_us+g_t_pm_us)*(g_antennaPathNum+1);
            blt_pCsCfg->step_expect_tick = blt_pCsCfg->step_expect_tick + (t_pm_us*2 + g_T_IP2_us + 10 + g_T_FCS_us) * SYSTEM_TIMER_TICK_1US;
        #else
            blt_pCsCfg->step_expect_tick = blt_pCsCfg->step_expect_tick + ((g_t_pm_us << 2) + g_T_IP2_us + 10 + g_T_FCS_us) * SYSTEM_TIMER_TICK_1US;
        #endif
        }
#if(ADD_CS_MODE1_FUN)
    else if(mode ==STEP_MODE_1){
        /**
         *      T_SY + T_RD + T_IP1 + T_SY +  T_RD  +   T_FCS
         *      T_SY    5     T_IP2   T_SY      5   +   T_FCS
         *
         *  Total time consumption = T_FCS_us + T_SY*2 + T_IP1_us + 10
         */
        blt_pCsCfg->step_expect_tick = blt_pCsCfg->step_expect_tick + ((CS_1M_PACKET_AA_ONLY_US<<1) + g_T_IP1_us + 10 + g_T_FCS_us) * SYSTEM_TIMER_TICK_1US;
    }
#endif






        blt_cs_init_step_stx_start();

    }


    return 0;
}

#if OS_COMPILE_OPTIMIZE_EN
_attribute_ram_code_sec_optimize_o2_
#endif
_attribute_ram_code_
int blt_cs_initiator_rx(void){
    blt_pCsCfg->step_rx_flag = 1;

    u8 mode = blt_pCsCfg->slip_window_step[blt_pCsCfg->slip_stepReadIdx&SLIP_WINDOW_STEP_MSK].step_modeType;
    if(mode == STEP_MODE_0){
        if(!blt_pCsCfg->mode0_rx_flag){
            blt_pCsCfg->mode0_rx_flag = 1;

            #if (HADM_PHASE_CONTINUITY)
                if(!cs_phase_continuity_flag && !blt_pCsCfg->phaseContin_config_flag){
                    ble_rf_cs_get_rx_cali_value(&rx_cs_cali);
                }
            #endif
            ble_rf_agc_disable();
        }
    }

    cs_rx_agc_gain = rf_get_gain_lat_value();

#if (SLVE_CS_RX_IRQ)
    log_event_irq(SL_STACK_CS_TIME_EN, SLVE_CS_RX_IRQ);
#endif

    HAL_CLEAR_RF_RX_IRQ;

    #if (HADM_PHASE_CONTINUITY)//mode 0
        ///phase continuity affect distance accuracy of mode 1.need to solve this issue.But now there is not enough time to debug.todo
        ///So mode 1 not use phase continuity setting temporally and thus can keep same accuracy as the 9.30 SDK.
        if( (blt_pCsCfg->Sub_Mode == SUBMODE_TYPE_MODE_UNUSED) && (blt_pCsCfg->Main_Mode != STEP_MODE_1 )){
            if(!cs_phase_continuity_flag && blt_pCsCfg->mode0_rx_flag && !blt_pCsCfg->phaseContin_config_flag){
                ble_rf_cs_phase_continuity_en();//50us
                blt_pCsCfg->phaseContin_config_flag = 1;
            }
        }
    #endif

    return 0;
}


#if OS_COMPILE_OPTIMIZE_EN
_attribute_ram_code_sec_optimize_o2_
#endif
_attribute_ram_code_
int blt_cs_initiator_irq_task (int flag)
{
    if(flag == FLAG_CS_STEP_INIT_STX_START){
        blt_cs_init_step_stx_start();
    }
    else if(flag == FLAG_CS_STEP_INIT_SRX_START){
        blt_cs_init_step_srx();
    }
    else if(flag == FLAG_CS_STEP_RX){
        blt_cs_initiator_rx();
    }
    else if(flag == FLAG_STEP_POST){
        blt_cs_init_step_post();
    }

    return 0;
}

ble_sts_t   blc_ll_initCsInitiatorModule(void){
    ll_cs_initiator_irq_task_cb = blt_cs_initiator_irq_task;

    return 0;
}



#endif


#if (DBG_CS_SUBEVENT_ENABLE)
_attribute_ram_code_ u32 ble_cs_initiator_mode0_test(u32 tick_step_start, u32 csAccessAddr, u8 csChannel, u8 csPHY, u8 T_FCS_us, u8 T_IP1_us)
{
    DBG_CHN7_HIGH;// Phase 1 start

    DBG_CHN5_HIGH;//test configuration consumption time start
    ble_rf_set_cs_channel(csChannel);
    #if (HADM_PHASE_CONTINUITY)
        //After setting the frequency point, enable manual fcal to ensure phase continuity.
        u32 tick_fcal_start = 0;
        if(cs_phase_continuity_flag){
            ble_rf_manual_fcal_start();
            tick_fcal_start = clock_time();
        }
    #endif

    //ble_rf_set_power_level_singletone(RF_POWER_P4p61dBm);//no need, set RF Power in subevent_start already
    DBG_CHN5_LOW;//test configuration consumption time post

    u8 tx_early_us = CS_RF_TX_1M_PACKET_EARLY_US;//8us(1 byte preamble) + 5us(1M PHY TXLLDLY + TXPATHDLY)
    #if (HADM_PHASE_CONTINUITY)
        u8 tx_settle_us;
        if(cs_phase_continuity_flag){
            tx_settle_us = CS_RF_TX_SETTLE_US;
        }
        else{
            tx_settle_us = TX_STL_TIFS_REAL_COMMON;
        }
    #else
        u8 tx_settle_us = TX_STL_TIFS_REAL_COMMON;
    #endif
    u32 tx_tick = tick_step_start + (T_FCS_us - tx_early_us - tx_settle_us) * SYSTEM_TIMER_TICK_1US;
    rf_ble_set_tx_settle(tx_settle_us);

    #if (HADM_PHASE_CONTINUITY)
        if(cs_phase_continuity_flag){
            while(!tick1_exceed_tick2(clock_time(), tick_fcal_start + RF_FCAL_MANUAL_START2DONE_TIME_US * SYSTEM_TIMER_TICK_1US)){}
            ble_rf_manual_fcal_done();
        }
    #endif

    rf_start_fsm(FSM_STX, (void *)&pkt_CS, tx_tick);

    //1M PHY, 1 byte for preamble extra length
    u8 rf_packet_len = 2 + 4 + 1;//2(preamble) + 4(access address) + 1(4 bit trail), PRMBL_LENGTH_1M

    pkt_CS.dma_len = rf_tx_packet_dma_len(rf_packet_len);

    smemcpy((u8 *)&pkt_CS.accessAddress, (u8 *)&csAccessAddr, 4);

    pkt_CS.preamble[0] = pkt_CS.preamble[1] = BIT_IS_SET(pkt_CS.accessAddress, 0) ? 0x55 : 0xAA;
    pkt_CS.trailer = BIT_IS_SET(pkt_CS.accessAddress, 31) ? 0xA : 0x5;
    pkt_CS.shift_sequence = 0;

//  tlkapi_send_string_data(DBG_CS_INITIATOR_TIMING, "pkt_CS_tx", &pkt_CS.dma_len, rf_packet_len + 4);

    while(!HAL_GET_RF_TX_IRQ){
        if(usr_irq_handler_cb){usr_irq_handler_cb();}
    }

    #if (HADM_PHASE_CONTINUITY)
        if(!cs_phase_continuity_flag){
            if(HAL_GET_RF_TX_IRQ){
                ble_rf_cs_get_tx_cali_value(&tx_cs_cali);
            }
        }
    #endif

    u32 tick_tx_done = clock_time();
    DBG_CHN7_LOW;// Phase 1 end

    reg_rf_irq_status = FLD_RF_IRQ_TX | FLD_RF_IRQ_CMD_DONE;

    DBG_CHN5_HIGH;//test configuration consumption time start
    rf_set_ble_access_code((u8*)&csAccessAddr);
    ble_rf_set_rx_dma(cs_rx_buff, DMA_CS_RFRX_MAX_DMA_LEN>>4);

    /**iq_sample_number: 0.25us unit (4MHz) ---> 0.25us * 320 = 80us ---> T_FM
     * iq_start_point: sync mode, sampling starts at (start_point + 1) * 0.125us after sync
     *      (111 + 1) * 0.125us = 14us = 4 + 10 ---> Trailer + T_GD
     */
    ble_rf_channel_sounding_iq_sample_config(320, 111, RF_HADM_IQ_SAMPLE_SYNC_MODE);//iq_sample_number cannot set to 0

    #if (HADM_PHASE_CONTINUITY)
        u8 rx_settle_us;
        if(cs_phase_continuity_flag){
            rx_settle_us = CS_RF_RX_SETTLE_US;
        }
        else{
            rx_settle_us = RX_SETTLE_US;
        }
    #else
        u8 rx_settle_us = RX_SETTLE_US;
    #endif

    rf_ble_set_rx_settle(rx_settle_us);

    u8 rx_early_us = 20;
    //Theoretical rx point = tick_step_start + T_FCS + T_SY + T_RD + T_IP1 = tick_step_start + T_FCS + 44 + 5 + T_IP1 = tick_step_start + T_FCS + T_IP1 + 49
    u32 rx_tick = tick_step_start + (T_FCS_us + T_IP1_us + 49 - rx_early_us - rx_settle_us) * SYSTEM_TIMER_TICK_1US;
    DBG_CHN5_LOW;//test configuration consumption time post

    rf_start_fsm(FSM_SRX, NULL, rx_tick);
    //1st rx timeout: rx_early_us + rx_settle_us + T_SY + T_RD + margin_15us
    //                rx_early_us + rx_settle_us + 44   + 5    + 15         = rx_early_us + rx_settle_us + 64
    rf_set_1st_rx_timeout(rx_early_us + rx_settle_us + 64);

    DBG_CHN7_HIGH;// Phase 2 start

    reg_rf_irq_status = FLD_RF_IRQ_CMD_DONE;

    u8 ret = 0;
    while(!tick1_exceed_tick2(clock_time(), tick_tx_done + 1000 * SYSTEM_TIMER_TICK_1US)){
        if(HAL_GET_RF_RX_IRQ){
            ret = 1;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_TIMEOUT)){
            ret = 2;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_FIFO_FULL)){
            ret = 3;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_CRC_2)){
            ret = 4;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_CMD_DONE)){
            ret = 5;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_FSM_TIMEOUT)){
            ret = 6;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_DR)){
            ret = 7;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_FIRST_TIMEOUT)){
            ret = 8;
        }
        if(ret){
            break;
        }
        if(usr_irq_handler_cb){usr_irq_handler_cb();}
    }

    u32 tick_rx_done = clock_time();

    if(HAL_GET_RF_RX_IRQ){
        if(!cs_mode0_rx_flag){
            cs_mode0_rx_flag = 1;

            #if (HADM_PHASE_CONTINUITY)
                if(!cs_phase_continuity_flag){
                    ble_rf_cs_get_rx_cali_value(&rx_cs_cali);
                }
            #endif

            ble_rf_agc_disable();
        }
    }
    cs_rx_agc_gain = rf_get_gain_lat_value();
    DBG_CHN7_LOW;// Phase 2 end

    u32 tick_rx_timestamp = hal_rf_get_rx_timestamp();

    if(ret){
        tlkapi_send_string_u32s(DBG_CS_INITIATOR_TIMING, "initiator_mode0_break", ret , tick_rx_timestamp / SYSTEM_TIMER_TICK_1US, tick_rx_done / SYSTEM_TIMER_TICK_1US, (tick_rx_done - tick_rx_timestamp) / SYSTEM_TIMER_TICK_1US);

        tlkapi_send_string_data(DBG_CS_INITIATOR_TIMING, "initiator_rx_data", cs_rx_buff, 16);
//      tlkapi_send_string_data(DBG_CS_INITIATOR_TIMING, "initiator_rx_data", cs_rx_buff, 256);

    #if (0)
        tlkapi_send_string_u32s(DBG_CS_INITIATOR_TIMING, "DMA_data1", (u32)DMA_CS_RFRX_DMA_LEN(cs_rx_buff), (u32)DMA_CS_RFRX_IQ_DATA_LEN(cs_rx_buff), (u32)DMA_CS_RFRX_OFFSET_TIME_STAMP(cs_rx_buff), (u32)DMA_CS_RFRX_OFFSET_FREQ_OFFSET(cs_rx_buff));
        tlkapi_send_string_u32s(DBG_CS_INITIATOR_TIMING, "DMA_data2", (u32)DMA_CS_RFRX_OFFSET_RSSI(cs_rx_buff), (u32)DMA_CS_RFRX_OFFSET_STATUS(cs_rx_buff), (u32)DMA_CS_RFRX_OFFSET_IQ_START_TSTAMP(cs_rx_buff), 0);

        u8* raw_pkt = cs_rx_buff;
        tlkapi_send_string_u32s(DBG_CS_INITIATOR_TIMING, "DMA_data3", raw_pkt[DMA_CS_RFRX_OFFSET_FREQ_OFFSET(raw_pkt)], raw_pkt[DMA_CS_RFRX_OFFSET_FREQ_OFFSET(raw_pkt)+1], raw_pkt[DMA_CS_RFRX_OFFSET_RSSI(raw_pkt)], raw_pkt[DMA_CS_RFRX_OFFSET_STATUS(raw_pkt)]);

        u8* raw_pkt_temp;

        u32 temp_TIME_STAMP;
        raw_pkt_temp = raw_pkt + DMA_CS_RFRX_OFFSET_TIME_STAMP(raw_pkt);
        BYTE_TO_UINT32(temp_TIME_STAMP, raw_pkt_temp);

        u32 temp_IQ_START_TSTAMP;
        raw_pkt_temp = raw_pkt + DMA_CS_RFRX_OFFSET_IQ_START_TSTAMP(raw_pkt);
        BYTE_TO_UINT32(temp_IQ_START_TSTAMP, raw_pkt_temp);

        /**
         * rx freq offset = pkt_fdc x bit_rate x 4000 / 256 / 2 / 3.14159
         * 1M PHY, 1M_bit_rate = 1000, 1M_FO = pkt_fdc x 2487
         * 2M PHY, 2M_bit_rate = 2000, 2M_FO = pkt_fdc x 4974
         */
        s32 rx_freq_offset = ((raw_pkt[DMA_CS_RFRX_OFFSET_FREQ_OFFSET(raw_pkt) + 1] & 0x07) << 8) | raw_pkt[DMA_CS_RFRX_OFFSET_FREQ_OFFSET(raw_pkt)];
        rx_freq_offset = ((rx_freq_offset > 0x3ff) ? (rx_freq_offset - 0x800) : rx_freq_offset);
        rx_freq_offset *= 2487;//Hz

        tlkapi_send_string_u32s(DBG_CS_INITIATOR_TIMING, "DMA_data4", temp_TIME_STAMP / SYSTEM_TIMER_TICK_1US, temp_IQ_START_TSTAMP / SYSTEM_TIMER_TICK_1US, csChannel, rx_freq_offset);
    #endif
    }

    DBG_CHN7_HIGH;// Phase 3 start

    STOP_RF_STATE_MACHINE;
    reg_rf_irq_status = FLD_RF_IRQ_TX | FLD_RF_IRQ_RX | FLD_RF_IRQ_CMD_DONE | FLD_RF_IRQ_FIRST_TIMEOUT;

    #if (HADM_PHASE_CONTINUITY)
        if(!cs_phase_continuity_flag && cs_mode0_rx_flag){
            ble_rf_cs_phase_continuity_en();
        }
    #endif

    u32 tick_next_step_start;
#if (0)
    if((ret == 1) || (ret == 5)){
        /**1M PHY, receive succeed Air Timing
         *      Access_Address + Trailer + T_GD + T_FM + T_RD
         *      timestamp           4       10      80      5 = 99
         *
         * 1M PHY, IQ Sample Rx Path Delay Timing
         *      RXPATHDLY
         *          5   //RXEN MODE
         *          12  //SYNC MODE
         *
         *  Total time consumption = 99 - RXPATHDLY = 94    //RXEN MODE
         *  Total time consumption = 99 - RXPATHDLY = 87    //SYNC MODE
         */
        tick_next_step_start = tick_rx_timestamp + (87) * SYSTEM_TIMER_TICK_1US;//SYNC MODE
    }
    else
#endif
    {
        /**1M PHY, receive lose Air Timing
         *      T_FCS +     T_SY + T_RD +   T_IP1 +     T_SY + T_GD + T_FM + T_RD
         *      T_FCS       44      5       T_IP1       44      10      80      5
         *
         *  Total time consumption = T_FCS_us + T_IP1_us + 188
         */
        tick_next_step_start = tick_step_start + (T_FCS_us + T_IP1_us + 188) * SYSTEM_TIMER_TICK_1US;
    }

    cs_rx_buff[2] = BLT_CS_INITIATOR_FLAG;
    cs_rx_buff[2] |= BLT_CS_MODE_0_FLAG;
    if((ret == 1) || (ret == 5)){
        cs_rx_buff[2] |= BLT_CS_MODE_RX_FLAG;
    }
    cs_rx_buff[3] = csChannel;

    cs_rx_buff[DMA_CS_RFRX_OFFSET_RX_AGC_GAIN(cs_rx_buff)] = cs_rx_agc_gain;

    blt_ll_csRxFifoUpdate();

    DBG_CHN7_LOW;// Phase 3 end

    return tick_next_step_start;
}

_attribute_ram_code_ u32 ble_cs_initiator_mode1_test(u32 tick_step_start, u32 csAccessAddr, u8 csChannel, u8 csPHY, u8 T_FCS_us, u8 T_IP1_us)
{
    DBG_CHN7_HIGH;// Phase 1 start

    DBG_CHN5_HIGH;//test configuration consumption time start
    ble_rf_set_cs_channel(csChannel);
    #if (HADM_PHASE_CONTINUITY)
        //After setting the frequency point, enable manual fcal to ensure phase continuity.
        u32 tick_fcal_start = 0;
        if(cs_phase_continuity_flag){
            ble_rf_manual_fcal_start();
            tick_fcal_start = clock_time();
        }
    #endif

//  ble_rf_set_tx_modulation_index(RF_MI_P0p50);//no need
//  ble_rf_set_power_level_singletone(RF_POWER_P4p61dBm);//no need, set RF Power in subevent_start already

    u8 tx_early_us = CS_RF_TX_1M_PACKET_EARLY_US;//8us(1 byte preamble) + 5us(1M PHY TXLLDLY + TXPATHDLY)
    #if (HADM_PHASE_CONTINUITY)
        u8 tx_settle_us;
        if(cs_phase_continuity_flag){
            tx_settle_us = CS_RF_TX_SETTLE_US;
        }
        else{
            tx_settle_us = TX_STL_TIFS_REAL_COMMON;
        }
    #else
        u8 tx_settle_us = TX_STL_TIFS_REAL_COMMON;
    #endif
    u32 tx_tick = tick_step_start + (T_FCS_us - tx_early_us - tx_settle_us) * SYSTEM_TIMER_TICK_1US;
    rf_ble_set_tx_settle(tx_settle_us);
    DBG_CHN5_LOW;//test configuration consumption time post

    reg_rf_ll_irq_list_h = BIT(0);//clear tr_turnaround_irq

    #if (HADM_PHASE_CONTINUITY)
        if(cs_phase_continuity_flag){
            while(!tick1_exceed_tick2(clock_time(), tick_fcal_start + RF_FCAL_MANUAL_START2DONE_TIME_US * SYSTEM_TIMER_TICK_1US)){}
            ble_rf_manual_fcal_done();
        }
    #endif
    rf_start_fsm(FSM_STX, (void *)&pkt_CS, tx_tick);

    //1M PHY, RTT AA Only, 1 byte for preamble extra length
    u8 rf_packet_len = 2 + 4 + 1;//2(preamble) + 4(access address) + 1(4 bit trail), PRMBL_LENGTH_1M
    pkt_CS.dma_len = rf_tx_packet_dma_len(rf_packet_len);
    smemcpy((u8 *)&pkt_CS.accessAddress, (u8 *)&csAccessAddr, 4);
    pkt_CS.preamble[0] = pkt_CS.preamble[1] = BIT_IS_SET(pkt_CS.accessAddress, 0) ? 0x55 : 0xAA;
    pkt_CS.trailer = BIT_IS_SET(pkt_CS.accessAddress, 31) ? 0xA : 0x5;
    pkt_CS.shift_sequence = 0;

    while(!HAL_GET_RF_TX_IRQ){
        if(usr_irq_handler_cb){usr_irq_handler_cb();}
    }
    u32 tick_tx_done = clock_time();
    reg_rf_irq_status = FLD_RF_IRQ_TX | FLD_RF_IRQ_CMD_DONE;
    DBG_CHN7_LOW;// Phase 1 end

    while(!(reg_rf_ll_irq_list_h & BIT(0))){
    }
    reg_rf_ll_irq_list_h = BIT(0);//clear tr_turnaround_irq
    u32 tick_cs_tx_on = rf_hadm_get_timestamp();//now this time obtained is tx_on

    u32 tick_tx_en = reg_rf_tr_turnaround_pos_time;//now this time obtained is tx_en, only for debug

    DBG_CHN7_HIGH;// Phase 2 start

    DBG_CHN5_HIGH;//test configuration consumption time start
    //ble_rf_agc_disable();

    rf_set_ble_access_code((u8*)&csAccessAddr);//must
    ble_rf_set_rx_dma(cs_rx_buff, DMA_CS_RFRX_MAX_DMA_LEN>>4);

    /**iq_sample_number: 0.25us unit (4MHz) ---> 0.25us * 1 = 0.25us
     * iq_start_point: rx_en mode, sampling starts at 0.25us+start_point*0.125us after settle
     *      0.25us + 1*0.125us = 0.375us
     */
    u8 rx_early_us = CS_RFRXEN_MODE_EARLY_US + CS_RF_RX_1M_EXTRA_PREAMBLE_US;//5us(1M PHY RXPATHDLY) RXEN MODE + 1 byte for preamble extra length
    u8 rx_extend_us = CS_RF_RX_1M_WINDOW_EXTEND_US;// ensure correct Sync

    u16 iq_sample_number = (rx_early_us + CS_1M_PACKET_AA_ONLY_US + rx_extend_us) << 2;
    ble_rf_channel_sounding_iq_sample_config(iq_sample_number, 1, RF_HADM_IQ_SAMPLE_RXEN_MODE);//iq_sample_number cannot set to 0
    DBG_CHN5_LOW;//test configuration consumption time post

    #if (HADM_PHASE_CONTINUITY)
        u8 rx_settle_us;
        if(cs_phase_continuity_flag){
            rx_settle_us = CS_RF_RX_SETTLE_US;
        }
        else{
            rx_settle_us = RX_SETTLE_US;
        }
    #else
        u8 rx_settle_us = RX_SETTLE_US;
    #endif
    rf_ble_set_rx_settle(rx_settle_us);

    u32 rx_tick = tick_step_start + (T_FCS_us + CS_1M_PACKET_AA_ONLY_US + 5 + T_IP1_us - rx_early_us - rx_settle_us) * SYSTEM_TIMER_TICK_1US;
    rf_start_fsm(FSM_SRX, NULL, rx_tick);
    //1st rx timeout: rx_settle_us + rx_early_us + CS_1M_PACKET_AA_ONLY_US + rx_extend_us + margin_25us
    //          =     rx_settle_us + rx_early_us + CS_1M_PACKET_AA_ONLY_US + rx_extend_us + 25
    rf_set_1st_rx_timeout(rx_settle_us + rx_early_us + CS_1M_PACKET_AA_ONLY_US + rx_extend_us + 25);
    reg_rf_irq_status = FLD_RF_IRQ_CMD_DONE;//clear

    u8 ret = 0;
    while(!tick1_exceed_tick2(clock_time(), tick_step_start + 1000 * SYSTEM_TIMER_TICK_1US)){
        if(HAL_GET_RF_RX_IRQ){
            ret = 1;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_TIMEOUT)){
            ret = 2;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_FIFO_FULL)){
            ret = 3;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_CRC_2)){
            ret = 4;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_CMD_DONE)){
            ret = 5;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_FSM_TIMEOUT)){
            ret = 6;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_DR)){
            ret = 7;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_FIRST_TIMEOUT)){
            ret = 8;
        }
        if(ret){
            break;
        }
        if(usr_irq_handler_cb){usr_irq_handler_cb();}
    }
    DBG_CHN7_LOW;// Phase 2 end
    u32 tick_rx_done = clock_time();
    u32 tick_rx_timestamp = hal_rf_get_rx_timestamp();
    u32 iq_start_timestamp = rf_hadm_get_iq_start_timestamp();
    cs_rx_agc_gain = rf_get_gain_lat_value();

    if(ret){
        tlkapi_send_string_u32s(DBG_CS_INITIATOR_TIMING, "initiator_mode1_break", ret , tick_tx_en / SYSTEM_TIMER_TICK_1US, tick_cs_tx_on / SYSTEM_TIMER_TICK_1US, iq_start_timestamp / SYSTEM_TIMER_TICK_1US);
        tlkapi_send_string_data(DBG_CS_INITIATOR_TIMING, "initiator_rx_data", cs_rx_buff, 16);
    }

    DBG_CHN7_HIGH;// Phase 3 start
    u32 tick_next_step_start;

    rf_set_tx_rx_off();
    STOP_RF_STATE_MACHINE;
    reg_rf_irq_status = FLD_RF_IRQ_TX | FLD_RF_IRQ_RX | FLD_RF_IRQ_CMD_DONE | FLD_RF_IRQ_FIRST_TIMEOUT;

    //ble_rf_agc_enable();

    /**
     *      T_FCS + T_SY + T_RD + T_IP1 + T_SY +  T_RD
     *      T_FCS   T_SY    5     T_IP2   T_SY      5
     *
     *  Total time consumption = T_FCS_us + T_SY*2 + T_IP1_us + 10
     */
    tick_next_step_start = tick_step_start + (T_FCS_us + (CS_1M_PACKET_AA_ONLY_US << 1) + T_IP1_us + 10) * SYSTEM_TIMER_TICK_1US;

    cs_rx_buff[2] = BLT_CS_INITIATOR_FLAG;
    cs_rx_buff[2] |= BLT_CS_MODE_1_FLAG;
    if((ret == 1) || (ret == 5)){
        cs_rx_buff[2] |= BLT_CS_MODE_RX_FLAG;
    }

    cs_rx_buff[3] = csChannel;

    u8 cs_tx_on_tick[4] = {U32_TO_BYTES(tick_cs_tx_on)};
    smemcpy(cs_rx_buff + DMA_CS_RFRX_OFFSET_TX_ON_TSTAMP_4BYTE(cs_rx_buff), cs_tx_on_tick, 4);

    u8 cs_rx_AA[4] = {U32_TO_BYTES(csAccessAddr)};
    smemcpy(cs_rx_buff + DMA_CS_RFRX_OFFSET_RX_ACCESS_ADDRESS_4BYTE(cs_rx_buff), cs_rx_AA, 4);

    cs_rx_buff[DMA_CS_RFRX_OFFSET_RX_AGC_GAIN(cs_rx_buff)] = cs_rx_agc_gain;

    blt_ll_csRxFifoUpdate();

    DBG_CHN7_LOW;// Phase 3 end

    return tick_next_step_start;
}

_attribute_ram_code_ u32 ble_cs_initiator_mode2_test(u32 tick_step_start, u32 csAccessAddr, u8 csChannel, u8 csPHY, u8 T_FCS_us, u8 T_PM_us, u8 T_IP2_us, u8 CS_DRBG)
{
    DBG_CHN7_HIGH;// Phase 1 start

    DBG_CHN5_HIGH;//test configuration consumption time start
    ble_rf_set_cs_channel(csChannel);
//  rf_set_ble_access_code((u8*)&csAccessAddr);//no need
    #if (HADM_PHASE_CONTINUITY)
        //After setting the frequency point, enable manual fcal to ensure phase continuity.
        u32 tick_fcal_start = 0;
        if(cs_phase_continuity_flag){
            ble_rf_manual_fcal_start();
            tick_fcal_start = clock_time();
        }
    #endif

    ble_rf_set_rx_dma(cs_rx_buff, DMA_CS_RFRX_MAX_DMA_LEN>>4);

    ble_rf_set_tx_modulation_index(RF_MI_P0p00);
//  ble_rf_set_power_level_singletone(RF_POWER_P4p61dBm);//no need, set RF Power in subevent_start already

    u8 tx_early_us = CS_RF_TX_1M_TONE_EARLY_US;//5us(1M PHY TXLLDLY + TXPATHDLY)
    #if (HADM_PHASE_CONTINUITY)
        u8 tx_settle_us;
        if(cs_phase_continuity_flag){
            tx_settle_us = CS_RF_TX_SETTLE_US;
        }
        else{
            tx_settle_us = TX_STL_TIFS_REAL_COMMON;
        }
    #else
        u8 tx_settle_us = TX_STL_TIFS_REAL_COMMON;
    #endif
    u32 tx_tick = tick_step_start + (T_FCS_us - tx_early_us - tx_settle_us) * SYSTEM_TIMER_TICK_1US;
    rf_ble_set_tx_settle(tx_settle_us);
    DBG_CHN5_LOW;//test configuration consumption time post

    u8 rf_packet_len;
    if(CS_DRBG & BIT(1)){//tone extension present
        rf_packet_len = CAL_LL_CS_TONE_TX_SIZE(T_PM_us * 2);//(T_PM_us + tone extension_slot)/8
    }
    else{
        rf_packet_len = CAL_LL_CS_TONE_TX_SIZE(T_PM_us);//T_PM_us/8
    }
    pkt_CS.dma_len = rf_tx_packet_dma_len(rf_packet_len);

    #if (HADM_PHASE_CONTINUITY)
        if(cs_phase_continuity_flag){
            while(!tick1_exceed_tick2(clock_time(), tick_fcal_start + RF_FCAL_MANUAL_START2DONE_TIME_US * SYSTEM_TIMER_TICK_1US)){}
            ble_rf_manual_fcal_done();
        }
    #endif

    rf_start_fsm(FSM_STX, (void *)&pkt_CS, tx_tick);

    while(!HAL_GET_RF_TX_IRQ){
        if(usr_irq_handler_cb){usr_irq_handler_cb();}
    }
    u32 tick_tx_done = clock_time();
    reg_rf_irq_status = FLD_RF_IRQ_TX | FLD_RF_IRQ_CMD_DONE;

    ble_rf_set_power_off_singletone();
    ble_rf_set_tx_modulation_index(RF_MI_P0p50);

    DBG_CHN7_LOW;// Phase 1 end


    DBG_CHN7_HIGH;// Phase 2 start

    DBG_CHN5_HIGH;//test configuration consumption time start
    //ble_rf_agc_disable();

    /**iq_sample_number: 0.25us unit (4MHz) ---> 0.25us * 1 = 0.25us
     * iq_start_point: rx_en mode, sampling starts at 0.25us+start_point*0.125us after settle
     *      0.25us + 6*0.125us = 1us
     */
    u8 rx_early_us = CS_RFRXEN_MODE_EARLY_US;//5us(1M PHY RXPATHDLY) RXEN MODE

    u8 t_rx_PM;
    if(CS_DRBG & BIT(0)){//tone extension present
        t_rx_PM = T_PM_us * 2;
    }
    else{
        t_rx_PM = T_PM_us;
    }

    u16 iq_sample_number = (t_rx_PM + rx_early_us) << 2;
    ble_rf_channel_sounding_iq_sample_config(iq_sample_number, 6, RF_HADM_IQ_SAMPLE_RXEN_MODE);//iq_sample_number cannot set to 0
    DBG_CHN5_LOW;//test configuration consumption time post

    #if (HADM_PHASE_CONTINUITY)
        u8 rx_settle_us;
        if(cs_phase_continuity_flag){
            rx_settle_us = CS_RF_RX_SETTLE_US;
        }
        else{
            rx_settle_us = RX_SETTLE_US;
        }
    #else
        u8 rx_settle_us = RX_SETTLE_US;
    #endif
    rf_ble_set_rx_settle(rx_settle_us);

    u32 rx_tick = tick_step_start + (T_FCS_us + (T_PM_us << 1) + 5 + T_IP2_us - rx_early_us - rx_settle_us) * SYSTEM_TIMER_TICK_1US;
    rf_start_fsm(FSM_SRX, NULL, rx_tick);
    //1st rx timeout: rx_settle_us + rx_early_us + T_PM + extension_slot + margin_25us
    //                rx_settle_us + rx_early_us + T_PM + T_PM           + 25           = rx_settle_us + rx_early_us + (T_PM_us << 1) + 25
    rf_set_1st_rx_timeout(rx_settle_us + rx_early_us + (T_PM_us << 1) + 25);
    reg_rf_irq_status = FLD_RF_IRQ_CMD_DONE;

    u8 ret = 0;
    while(!tick1_exceed_tick2(clock_time(), tick_step_start + 1000 * SYSTEM_TIMER_TICK_1US)){
        if(HAL_GET_RF_RX_IRQ){
            ret = 1;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_TIMEOUT)){
            ret = 2;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_FIFO_FULL)){
            ret = 3;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_CRC_2)){
            ret = 4;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_CMD_DONE)){
            ret = 5;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_FSM_TIMEOUT)){
            ret = 6;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_RX_DR)){
            ret = 7;
        }
        if((reg_rf_irq_status & FLD_RF_IRQ_FIRST_TIMEOUT)){
            ret = 8;
        }
        if(ret){
            break;
        }
        if(usr_irq_handler_cb){usr_irq_handler_cb();}
    }
    u32 tick_rx_done = clock_time();
    u32 tick_rx_timestamp = hal_rf_get_rx_timestamp();//TODO
    cs_rx_agc_gain = rf_get_gain_lat_value();
    DBG_CHN7_LOW;// Phase 2 end

    if(ret){
        tlkapi_send_string_u32s(DBG_CS_INITIATOR_TIMING, "initiator_mode2_break", ret , tick_rx_timestamp / SYSTEM_TIMER_TICK_1US, tick_rx_done / SYSTEM_TIMER_TICK_1US, (tick_rx_done - tick_rx_timestamp) / SYSTEM_TIMER_TICK_1US);

        tlkapi_send_string_data(DBG_CS_INITIATOR_TIMING, "initiator_rx_data", cs_rx_buff, 16);
//      tlkapi_send_string_data(DBG_CS_INITIATOR_TIMING, "initiator_rx_data", cs_rx_buff, 256);
    }

    DBG_CHN7_HIGH;// Phase 3 start
    u32 tick_next_step_start;

    rf_set_tx_rx_off();
    STOP_RF_STATE_MACHINE;
    reg_rf_irq_status = FLD_RF_IRQ_TX | FLD_RF_IRQ_RX | FLD_RF_IRQ_CMD_DONE | FLD_RF_IRQ_FIRST_TIMEOUT;

    //ble_rf_agc_enable();

    /**
     *      T_FCS + T_PM + extension_slot + T_RD +  T_IP2 + T_PM + extension_slot + T_RD
     *      T_FCS   T_PM        T_PM        5       T_IP2   T_PM        T_PM        5
     *
     *  Total time consumption = T_FCS_us + T_PM_us*4 + T_IP2_us + 10
     */
    tick_next_step_start = tick_step_start + (T_FCS_us + (T_PM_us << 2) + T_IP2_us + 10) * SYSTEM_TIMER_TICK_1US;

    cs_rx_buff[2] = BLT_CS_INITIATOR_FLAG;
    cs_rx_buff[2] |= BLT_CS_MODE_2_FLAG;
    if((ret == 1) || (ret == 5)){
        cs_rx_buff[2] |= BLT_CS_MODE_RX_FLAG;
    }
    if(t_rx_PM == T_PM_us * 2){
        cs_rx_buff[2] |= BLT_CS_STEP_TONE_EXTENSION_SLOT_FLAG;
    }

    cs_rx_buff[3] = csChannel;

    cs_rx_buff[DMA_CS_RFRX_OFFSET_RX_AGC_GAIN(cs_rx_buff)] = cs_rx_agc_gain;

    blt_ll_csRxFifoUpdate();

    DBG_CHN7_LOW;// Phase 3 end

    return tick_next_step_start;
}
#endif
