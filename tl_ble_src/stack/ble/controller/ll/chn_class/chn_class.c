 ...
 * @return     none
 */
_attribute_noinline_ void blc_ll_initChnClass_feature(void)
{
...
static inline ll_chnc_cb_t *blt_ll_findAvailableChnc(u16 connIdx)
{
    (void)connIdx;
...
_attribute_no_inline_
    #endif
    ble_sts_t
    blt_ll_chnclassControlPduProc(st_ll_conn_t *pAclConn, u8 opcode, u8 *pLlCtrlPkt)
{
...
_attribute_ram_code_ int blt_ll_chnclassInterruptTask(int flag, void *p)
{
...
_attribute_no_inline_
    #endif
    int
    blt_ll_chnclassMainloopTask(int flag, void *p)
{
    //int conn_idx = flag & FLAG_SCHEDULE_TASK_IDX_MASK;
...
_attribute_no_inline_ void blt_ll_chnclassPreChnStsProc(u8 pChm[5])
{
    assert(pChm != NULL);
 ...
 */
    void
    blt_ll_chnClassInitDftParamsAftConnect(st_ll_conn_t *pAclConn) //called by blms_connect_common in IRQ
{
...
_attribute_no_inline_ void blt_ll_chnClassSendChnRptIndProc(st_ll_conn_t *pAclConn)
{
...
_attribute_no_inline_ void blt_ll_chnClassSendChnStsIndProc(st_ll_conn_t *pAclConn)
{
    ll_chnc_cb_t *pChncCb = pAclConn->pChncCb;
    assert(pChncCb != NULL);
