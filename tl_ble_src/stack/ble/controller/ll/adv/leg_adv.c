...
_attribute_ram_code_ int blt_leg_adv_interrupt_task(int flag, void *p)
{
...
#endif
    int
    blt_leg_adv_mainloop_task(int flag)
{
    if (flag == FLAG_MODULE_RESET) {
        ...
        if ((bltLegAdv.terminated_event == 1) && (blmsParam.legadv_en_strategy == LEG_ADV_EN_STRATEGY_3)) {
            bltLegAdv.terminated_event = 0;
            ...
            pEvt->advHandle                    = 0xFF; //invalid

            pEvt->status            = BLE_SUCCESS;
            pEvt->connHandle        = 0xFFFF;          //invalid
            pEvt->num_compExtAdvEvt = 0x00;
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_Set_Adv_Data", data, len);
...
_attribute_noinline_
    ble_sts_t
    blc_ll_setAdvEnable(adv_en_t adv_enable)
{
    /* core_5.3
    ...
    */
...
#endif
#if ZIGBEE_LEG_ADVERTISING_TERMINATED_EVT_ENABLE
    if (blmsParam.max_slave_num || blmsParam.legadv_en_strategy) {
#else
    ...
    #if OS_SUP_EN
                /* Fixed an issue where the system timer interrupt was not closed when the ble task was empty,
                     * resulting in a program execution exception.
                     */
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_Set_Adv_Enable", &adv_enable, 1);
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_Set_Adv_Param", pCmdParam, sizeof(hci_le_setAdvParam_cmdParam_t));
    ...
    ble_sts_t status = blc_ll_setAdvParam(pCmdParam->intervalMin, pCmdParam->intervalMax, pCmdParam->advType, pCmdParam->ownAddrType, pCmdParam->peerAddrType, pCmdParam->peerAddr, pCmdParam->advChannelMap, pCmdParam->advFilterPolicy);
    ...
    /* step 2, network privacy ignore IDA process */
                                    ...
                                    if (is_connect_req) { //CONNECT_REQ
                                        ...
                                        //if(ll_adv_2_slave_cb)  //to save RamCode
                                        {
                                            ...
                                            {
    #if ZIGBEE_LEG_ADVERTISING_TERMINATED_EVT_ENABLE
                                                //If the slave callback for LL advertising is not active
                                                if ((!ll_adv_2_slave_cb) && (blmsParam.legadv_en_strategy == LEG_ADV_EN_STRATEGY_3)) {
                                                    bltLegAdv.terminated_event = 1; // Received connect patch
                                                    result                     = 1; //succeed
                                                }
    #endif
                                            } else
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_Set_ScanRspData", data, len);
...
_attribute_ram_code_ int blt_ll_buildLegacyAdvTask(void)
{
#if (PRICHN_SCAN_SMALL_INTV_WITH_ANOTHER_BIG_INTV_ADV_RESOLVE_EN && !ANOTHER_BIG_INTV_EXTENDED_ADV)
