...
void blt_adv_ext_ind_fill_advA_targetA(void)
{
...
_attribute_ram_code_
void blt_aux_adv_ind_fill_advA_targetA(void)
{
...
_attribute_ram_code_
static void blt_extadv_rpa_update_process(void)
{
...
static inline int find_auxadv_sSlot_idx(void){
    ...
    u32 random = (bltFutTask.number+1)* 250 + rand()%500;
...
_attribute_ram_code_  //TODO later after BQB
void  blt_updateExtAdvSet(st_ext_adv_t *cur_pextadv)
{
    ...
    if(cur_pextadv->legacy_adv){
        ...
        if( cur_pextadv->cur_advMode == LL_EXTADV_MODE_NON_CONN_NON_SCAN)
        {
            int advA_exist = 0;
...
_attribute_ram_code_
int blt_auxadv_send(int slotTask_idx)
{
                        ...
                        if(is_connect_req){ //AUX_CONNECT_REQ
                            ...
                            /* rxAddr & targetA process for "AUX_CONNECT_RSP" */
                                ...
                                if(blt_pextadv->asyncAdvIndex)
                                {
                                    ...
                                    memset(pkt_aux_conn_rsp.advA,0,12);
                                    for(u8 index=0;index<LL_MAX_ACL_CONN_NUM;index++)
                                    {
                                        if(blms[index].connState == CONN_STATUS_ESTABLISH && !blms[index].async_lea_link)
                                        {
                                            aclApTick = (blms[index].ap_tick_mark + 2*blms[index].conn_intvl_tick  - tx_begin_tick)%asyncCtrl.aclIntervalTick;
                                            pkt_aux_conn_rsp.advA[0] = U32_BYTE0(aclApTick|1);
                                            pkt_aux_conn_rsp.advA[1] = U32_BYTE1(aclApTick);
                                            pkt_aux_conn_rsp.advA[2] = U32_BYTE2(aclApTick);
                                            pkt_aux_conn_rsp.advA[3] = U32_BYTE3(aclApTick);
                                        }
                                    }
                                    ...
                                    for(u8 index = bltCisMng.maxNum_cisMaster; index< bltCisMng.maxNum_cisConn; index++)
                                    {
                                        ...
                                        if(pCisConn->cis_occupied && pCisConn->cis_expect_tick)
                                        {
                                            cisAPTick = (pCisConn->cis_expect_tick+2*pCisConn->iso_intvl_tick- tx_begin_tick)%(asyncCtrl.aclIntervalTick);
                                            pkt_aux_conn_rsp.advA[4] = U32_BYTE0(cisAPTick|1);
                                            pkt_aux_conn_rsp.advA[5] = U32_BYTE1(cisAPTick);
                                            pkt_aux_conn_rsp.targetA[0] = U32_BYTE2(cisAPTick);
                                            pkt_aux_conn_rsp.targetA[1] = U32_BYTE3(cisAPTick);

                                            cisSyncDelayTick = pCisConn->cis_sync_delay*SYSTEM_TIMER_TICK_1US;
                                            pkt_aux_conn_rsp.targetA[2] = U32_BYTE0(cisSyncDelayTick|1);
                                            pkt_aux_conn_rsp.targetA[3] = U32_BYTE1(cisSyncDelayTick);
                                            pkt_aux_conn_rsp.targetA[4] = U32_BYTE2(cisSyncDelayTick);
                                            pkt_aux_conn_rsp.targetA[5] = U32_BYTE3(cisSyncDelayTick);
                                        }
                                    }
                                }
                                #endif
...
//      DBG_C HN10_TOGGLE;
    ...
    if(blt_pextadv->asyncAdvIndex)
    {
        rf_set_ble_channel (blc_asyncAdv_channel[bltAdv.advChn_idx]);
    }
    else
    {
...
_attribute_ram_code_
int blt_extadv_start(int slotTask_idx)
{
    DBG_CHN1_HIGH;
    DBG_SIHUI_CHN1_HIGH;
...
//  DBG_TIANXIANG_CHN2_LOW;
    if(blc_rf_pa_cb){   blc_rf_pa_cb(PA_TYPE_OFF); } //here not need
...
#if 0
_attribute_ram_code_
u8 blt_ll_auxAdvInd_sSlotResetProc(void)
{
    future_task_e *pFutTask = NULL;

    for(int i=0; i< bltFutTask.number; i++){
        pFutTask = &bltFutTask.task_tbl[i];

        //here only process AUX_ADV_IND task
        if(!(pFutTask->task_flg == TSKFLG_AUX_ADV)){ //future task may include scan task later. although now not include.
            continue;
        }

        if(tick1_exceed_tick2(bltSche.sSlot_tick_start, pFutTask->tick_s)){

            u8 aux_idx = pFutTask->task_oft - TSKOFT_AUX_ADV;
            st_ext_adv_t *cur_pextadv = (st_ext_adv_t *)(global_pextadv + aux_idx);

            cur_pextadv->aux_adv_pending = 0;
            blt_sche_removeTaskMask(TSKMSK_AUX_ADV_0 << aux_idx);

            blt_remove_future_task(pFutTask->task_oft);
        }
    }
    return 0;
}
...
_attribute_ram_code_
void blt_add_future_task(u8 llTask_flag, u8 llTask_offset, u32 tick_start, u32 tick_end)
{
...
_attribute_ram_code_
bool blt_remove_future_task(u8 llTask_offset)
{
...
_attribute_ram_code_
#endif
void blt_extAdv_scanReqRevEvt(void){
...
if(advSet_terminate_status || blt_state == BLS_LINK_STATE_CONN){

    if(advSet_terminate_status){  //not connection created
        //
    }

    //TODO: can not send event in IRQ !!!
    if(hci_le_eventMask & HCI_LE_EVT_MASK_EXTENDED_ADVERTISING_SET_TERMINATED){
