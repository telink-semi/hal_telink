...
{
#if (LL_ASYNC_LEA_EN && SDK_RELEASE_CHECK_EN)
    STATIC_ASSERT_FILE(ADV_SET_PARAM_LENGTH == (sizeof(st_ext_adv_t)-4), ext_adv);
#else
    ...
    for(int i=0;i<bltExtA.maxNum_advSets; i++){  //clear ADV handle
        ...
        blt_extadv_clear_adv_set_param(cur_pextadv);
            ...
            cur_pextadv->bSlot_mark_extadv = rand()&0x0f;
        #else
...
int blt_ext_adv_interrupt_task (int flag, void*p)
{
    ...
    if(flag & FLAG_SCHEDULE_START){
        ...
        blt_auxadv_send(adv_idx);
            ...
            if(tgtTskFlg == TSKFLG_BIG_BCST && (global_pBigBcst+pTgtTsk->scheTask_idx)->big_sc_mask){
                my_dump_str_data(0, "[ext_adv]abandon, bis_SC proc", &bltPri.csctvAbandonCnt[curSchTaskOft], 2)
                return 0;
            }
        #endif
    }
...
_attribute_noinline_
int blt_ext_adv_mainloop_task (int flag, void *p)
{
...
ble_sts_t   blc_ll_setExtAdvParam_v2(  u8 adv_handle,                  advEvtProp_type_t adv_evt_prop,  u32 pri_advInter_min,       u32 pri_advInter_max,
                                       ...
                                       le_codedPhy_option pri_adv_codePhy_option,    le_codedPhy_option sec_adv_codePhy_option)
{
...
ble_sts_t   blc_hci_le_setExtAdvParam( hci_le_setExtAdvParam_cmdParam_t *pCmdParam, u8 *pTxPower)
{
    ...
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Ext_Adv_Param", pCmdParam, sizeof(hci_le_setExtAdvParam_cmdParam_t));
...
//      }
    ...
    return blc_ll_setExtAdvParam( pCmdParam->adv_handle,     pCmdParam->advEvt_props, pCmdParam->pri_advIntMin[0] | pCmdParam->pri_advIntMin[1]<<8 , pCmdParam->pri_advIntMax[0] | pCmdParam->pri_advIntMax[1]<<8,
                                  pCmdParam->pri_advChnMap,  pCmdParam->ownAddrType,   pCmdParam->peerAddrType,                                       pCmdParam->peerAddr,
                                  pCmdParam->advFilterPolicy, pCmdParam->adv_tx_pow,   pCmdParam->pri_adv_phy,                                        pCmdParam->sec_adv_max_skip,
                                  pCmdParam->sec_adv_phy,    pCmdParam->adv_sid,       pCmdParam->scan_req_notify_en);
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Ext_Adv_Data", pCmdParam, pCmdParam->length + 4);
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Ext_ScanRsp_Data", pCmdParam, pCmdParam->length + 4);
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Ext_Adv_Enable", pCmdParam, sizeof(hci_le_setExtAdvEn_cmdParam_t));
...
_attribute_noinline_
void blt_extadv_clear_adv_set_param(st_ext_adv_t *cur_pextadv)
{
...
#if(LL_ASYNC_LEA_EN)
void blc_ll_asyncSetPrivateAdvChannel (u16 advHandle,u8 chn0,u8 chn1,u8 chn2)
{
    st_ext_adv_t *cur_pextadv;
    for(int i=0;i<bltExtA.maxNum_advSets; i++)
    {
        cur_pextadv = (st_ext_adv_t *)(global_pextadv + i);
        if( cur_pextadv->adv_handle == advHandle )
        {  //existing ADV set match
            cur_pextadv->asyncAdvIndex = 1;
        }
    }
