...
_attribute_aligned_(4)  ll_past_ctrl_handler_t  ll_past_ctrl_handler = NULL;
...
static inline
ll_past_cb_t* blt_ll_findAvailablePast(u16 connIdx)
{
    (void)connIdx;
...
int         blt_ll_pastInterruptTask (int flag, void*p)
{
...
#endif
int         blt_ll_pastMainloopTask (int flag)
{
                ...
                if(pPdaSync->sync_establish == SYNC_ESTABLISHED_BY_PAST){
                    ...
                    if(hci_le_eventMask & HCI_LE_EVT_MASK_PERIODIC_ADVERTISING_SYNC_TRANSFER_RECEIVED){
                        ...
                        if(pPdaSync->pawr_acad_check){
                            ...
                            hci_le_periodicAdvSyncTransferRcvd_evt_V2 (status, pc->acl_conHandle,
                                pPastCb->perServiceData, pPastCb->perSyncHandle, pPdaSync->pda_id.sid,
                                pPdaSync->adrIdType, pPdaSync->pda_id.addr, pPdaSync->pda_rx.pda_phy, //pda_phy's type: refer to 'le_phy_type_t'
                                pPdaSync->pda_interval, pPdaSync->sca, &pPdaSync->pawr_acadInfo);
                        }else
                    #endif
                        {
                            hci_le_periodicAdvSyncTransferRcvd_evt (status, pc->acl_conHandle,
                                pPastCb->perServiceData, pPastCb->perSyncHandle, pPdaSync->pda_id.sid,
                                pPdaSync->adrIdType, pPdaSync->pda_id.addr, pPdaSync->pda_rx.pda_phy, //pda_phy's type: refer to 'le_phy_type_t'
                                pPdaSync->pda_interval, pPdaSync->sca);
                        }
                    }
...
_attribute_ram_code_
#endif
int blt_ll_pastGetRcvdCEt(u8 *raw_pkt) //called by acl_rx_irq
{
    ll_past_cb_t *pPastCb = blms_pconn->pPastCb;
    if (pPastCb == NULL) return 0;
...
int blt_ll_pastSendSyncIndProc(st_ll_conn_t *pAclConn)
{
    ...
    if(pPastCb->pastSendPending && !pAclConn->ll_enc_busy && (!pAclConn->ll_rsp_timeout_tick && pAclConn->curCEsyncAP)){
        ...
        if(pPastCb->perSyncSrc == PAST_SYNC_SRC_SCAN){ //By ext_scan to get syncInfo
            ...
            if(i == bltExtA.maxNum_advSets){ //Not found
                ...
                if(cur_pPerdadv->num_subevents){
                    ...
                    pPerdSyncWrInd->pawrTimingInfo.num_subevent = cur_pPerdadv->num_subevents;
                    pPerdSyncWrInd->pawrTimingInfo.rsp_AA = cur_pPerdadv->responseAA;
                    pPerdSyncWrInd->pawrTimingInfo.rsp_slot_delay = cur_pPerdadv->response_slot_delay;
                    pPerdSyncWrInd->pawrTimingInfo.rsp_slot_spacing = cur_pPerdadv->response_slot_spacing;
                    pPerdSyncWrInd->pawrTimingInfo.subevent_intvl = cur_pPerdadv->subevent_interval;
                }
            #endif
        ...
        u16 CEref = conn_inst_mark + 6 + pAclConn->conn_latency;
        u32 refAclTime = conn_tick_mark + (6 + pAclConn->conn_latency) * pAclConn->conn_intvl_tick;
        ...
        if((u32)((refAclTime + pAclConn->sSlot_allocNum*SSLOT_TICK_NUM + SLOT_PROCESS_MAX_TICK) - PEaAnchorPoint) < BIT(31)){
            jumpPrdNum = 1 + ((refAclTime + pAclConn->sSlot_allocNum*SSLOT_TICK_NUM + \
                    SLOT_PROCESS_MAX_TICK) - PEaAnchorPoint)/(pSyncInfo->itvl *1250 * SYSTEM_TIMER_TICK_1US);
            ...
            jumpPrdNum = (PEaAnchorPoint - (refAclTime + pAclConn->sSlot_allocNum*SSLOT_TICK_NUM + \
                    SLOT_PROCESS_MAX_TICK))/(pPerdSyncInd->syncInfo.itvl *1250 * SYSTEM_TIMER_TICK_1US);
            PEa = pPerdSyncInd->lastPaEvtCnt - jumpPrdNum;
            PEaAnchorPoint -= jumpPrdNum * (pSyncInfo->itvl *1250 * SYSTEM_TIMER_TICK_1US);
            my_dump_str_data(DBG_LL_PAST_EN, "PEa-", &PEa, 2);
        }
...
_attribute_ram_code_
#endif
void blt_ll_pastInitDftParamsAftConnect(st_ll_conn_t* pAclConn) //called by blms_connect_common in IRQ
{
...
_attribute_no_inline_
#endif
ble_sts_t   blt_ll_pastControlPduProc(st_ll_conn_t* pAclConn, u8 opcode, u8 *pLlCtrlPkt)
{
        ...
        if(advA_is_rpa){ //RPA
            ...
            if(pRL_match && pRL_match->dev_iden_valid){  //resolving pass, have a RL entry
                ...
                advAdrIdType =  pRL_match->rlIdAddrType == BLE_ADDR_PUBLIC ? \
                                       OWN_ADDRESS_RESOLVE_PRIVATE_PUBLIC : OWN_ADDRESS_RESOLVE_PRIVATE_RANDOM; //TODO: rename MICRO
...
//              bltPdaSync.pdA_cacheNum ++;
        ...
        if(opcode == LL_PERIODIC_SYNC_WR_IND){
            ...
            rf_pkt_ll_periodicSyncWrInd_t * tmpPerdSyncInd = (rf_pkt_ll_periodicSyncWrInd_t *)pPerdSyncInd;
         ...
         */
        u8 status = blc_ll_periodicAdvertisingCreateSync (options, cur_pAdv->sid,
                            cur_pAdv->adrType, cur_pAdv->addr, pPastCb->pastSkip,
                            pPastCb->pastSyncTimeout, pPastCb->pastSyncCteType);
