...
_attribute_ram_code_ int blt_ll_linkAllPostTask(sch_task_t *pTsk_cur, sch_task_t *pStart_schTsk, int task_size, int start_idx, int task_num_max)
{
    for (int i = start_idx; i < task_num_max; i++) {
        sch_task_t *pCurrTask = (sch_task_t *)((u8 *)pStart_schTsk + i * task_size);
        pTsk_cur->next        = pCurrTask;
        ...
        bltPri.csctvAbandonCnt[pCurrTask->scheTask_oft] = 0;

        u8 insertTskFlg = pCurrTask->scheTask_flg & TSKFLG_VALID_MASK;
...
_attribute_ram_code_
    u8
    blt_ll_resolve_insertSchTskConflict(sch_task_t *pInsertTsk, sch_task_t *pTgtTsk)
{
 ...
 */
_attribute_ram_code_ int blt_ll_addTask2ExistLinklist(sch_task_t *pStart_schTsk, int task_num_max)
{
    return blt_ll_addTask2ExistLinklistWithTaskSize(pStart_schTsk, sizeof(sch_task_t), task_num_max);
}
 ...
 * other value: indicate the fail task number
 */
_attribute_ram_code_ int blt_ll_addTask2ExistLinklistWithTaskSize(sch_task_t *pStart_schTsk, int task_size, int task_num_max)
{
    ...
    for (int i = 0; i < task_num_max; i++) {
        pTsk_cur = (sch_task_t *)((u8 *)pStart_schTsk + i * task_size);
        //locate pLeft
        ...
        while (pExtLkTsk_next->end < pTsk_cur->begin) {
            ...
            if (pExtLkTsk_next == NULL) {
                blt_ll_linkAllPostTask(pExtLkTsk_left, pStart_schTsk, task_size, i, task_num_max);
...
_attribute_ram_code_ int blt_ll_addTask2AbandonTaskLinklist(sch_task_t *pStart_schTsk, int task_num)
{
    (void)pStart_schTsk;
    (void)task_num;
    return 0;
...
#endif
    static inline void
    blt_ll_cal_sslot_diff_next(void)
{
...
_attribute_ram_code_ void blt_ll_noTask_build_proc(void)
{
...
_attribute_ram_code_sec_optimize_o2_
    ...
    blt_ll_buildTaskLinkList(void) //blt_ll_combine_linklayer_task
{
...
_attribute_ram_code_sec_optimize_o2_
    ...
    blt_ll_updateScheduler(void) //blt_ll_buildTaskLinkList
{
    int task_rebuild = 0;
...
/* ctx/crx_start preset ctx/crx_post generation time point (system timer capture value preset),
        ...
        if (debug_diff > 5 * SYSTEM_TIMER_TICK_1S || debug_diff < 0) {
            ...
            write_dbg32(DBG_SRAM_ADDR + 12, debug_diff);
...
#endif
    void
    blt_ll_proc_bSlot_idx_overflow(void)
{
...
#endif
    void
    blt_ll_irq_startScheduler(void) //write in flash to save RamCode
{
...
#endif
    int
    blt_ll_mainloop_startScheduler(void)
{
    systick_irq_trigger = SYS_IRQ_TRIG_SCHE_START;

    /**
     * If irq_mask is set and irq is not enable, the irq status will be set, when
     * stimer is at the capture value.
     * Move the irq_mask enable form `blc_ll_initBasicMCU` to here, to avoid the
     * `blc_sdk_irq_handler` will call `irq_system_timer` before stimer irq is enable.
     */
...
#endif
    void
    blt_ll_procStateChange(void) //This function is called in IRQ, but can write in Flash to save SRAM
{
    int update_en = 0;
...
_attribute_ram_code_ void blt_ll_calculate_sSlot_next(u32 next_tick)
{
#if (SCH_DEBUG_EN)
...
_attribute_ram_code_ void blt_ll_incSchedulerTaskPriority(u8 task_offset, int inc)
{
...
_attribute_ram_code_ void blt_ll_incSchedulerTaskCalPriority(u8 task_offset, int inc)
{
