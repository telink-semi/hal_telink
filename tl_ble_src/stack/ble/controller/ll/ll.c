...
void blc_ll_initStandby_module (u8 *public_adr)
{
    ...
    blc_ll_initFastSettle(1, 1);
            ...
            #error "BLT_ERR_PROCESS can not be  ERR_TRIGGER_CODE_STUCK !!!"
             ...
             DEBUG_YAFEI_GPIO_ENABLE || DEBUG_TIANXIANG_GPIO_ENABLE)
...
void blc_ll_setCustomizedAdvertisingScanningChannel (u8 chn0, u8 chn1, u8 chn2)
{
    if(blc_adv_chn_ext_sel & ALLOW_LEG_CHN_SETTING){
        ...
        tlkapi_printf(1,"leg chn %d-%d-%d ", chn0, chn1, chn2);
    }
    ...
    if(blc_adv_chn_ext_sel & ALLOW_EXT_CHN_SETTING){
        ...
        tlkapi_printf(1,"ext chn %d-%d-%d ", chn0, chn1, chn2);
    }
}
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Random_Addr", randomAddr, 6);
...
ble_sts_t       blc_hci_reset(void)
{
    ...
    while(!clock_time_exceed(begin_tick, 30));  //30 uS
...
ble_sts_t   blc_hci_le_getLocalSupportedFeatures(hci_le_readLocSupFeature_retParam_t* pRetParam)
{
    ...
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Read_Local_Sup_Feature", pRetParam->le_features, 8);
...
ble_sts_t blc_ll_setHostChannel(u8 * pChm)
{
    ...
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Set_Host_Chn", pChm, 5);
...
init_err_t  blt_contr_checkControllerInitialization(void)
{
    ...
    if (blmsParam.max_master_num && (sys_clk.cclk < 48)) {
        // temporary version
        return SYS_CLK_ERR_NOT_SUPPORTED; //This next version will be optimized with fast settle
    }
         ...
         if ((blmsParam.max_master_num != 0) && (blmsParam.max_slave_num != 1)) {
              ...
              return SYS_CLK_ERR_NOT_SUPPORTED;
         }
         else if(sys_clk.cclk < 24)
         {
...
_attribute_ram_code_sec_noinline_
void blt_ll_addr_set_peer_address(u8 rpa_resolve_ok, u8 peer_adrType, u8 *peer_addr)
{
#if (LL_FEATURE_ENABLE_PRIVACY)
...
//Get RF calibration values
    void    blc_ll_initFastSettle(u8 tx_fast_en,u8 rx_fast_en)
    {
        if(tx_fast_en || rx_fast_en)
        {
            ...
            {
                rf_set_ble_channel (i); //here, fast_settle.tx_fast_en can not be enable !!!
                ...
                fast_settle.cal_tbl[i] = get_rf_hpmc_cal_val();
            }
        ...
        fast_settle.tx_fast_en = tx_fast_en;
        fast_settle.rx_fast_en = rx_fast_en;
    }
#endif
...
///////////////////////////////////////////////////////////////////////
     ...
     */
    _attribute_ram_code_
    void    blt_ll_set_tx_power_by_strategy(rf_tx_power_strategy_t pwrCtrl_flag, u8 pwrCtrl_power){
        (void)pwrCtrl_flag;
        (void)pwrCtrl_power;
    }
#else
 ...
 */
_attribute_ram_code_
void    blt_ll_set_power_level_index(u8 index)
{
