...
_attribute_ble_data_retention_ u8 gBisTermIndPdu[10 + 4] = {
    U32_BYTE0(rf_tx_packet_dma_len(6)),
    U32_BYTE1(rf_tx_packet_dma_len(6)),
    U32_BYTE2(rf_tx_packet_dma_len(6)),
    U32_BYTE3(rf_tx_packet_dma_len(6)), //DMA length
    3,
    4,                                  //Header
...
_attribute_ble_data_retention_ u8 gBisChmIndPdu[14 + 4] = {
    U32_BYTE0(rf_tx_packet_dma_len(10)),
    U32_BYTE1(rf_tx_packet_dma_len(10)),
    U32_BYTE2(rf_tx_packet_dma_len(10)),
    U32_BYTE3(rf_tx_packet_dma_len(10)), //DMA length
    3,
    8,                                   //Header: LLID(2 bits)  CSSN(3 bits)    CSTF(1 bit) RFU(2 bits) Length(8 bits)
...
_attribute_ram_code_ int blt_ll_buildBigBcstSchedulerLinklist(void)
{
    int            intvl_jump_big;
    s32            sSlot_start_big;
...
_attribute_ram_code_
    #endif
    static int
    blt_ll_triggerBigSlotTask(void)
{
...
_attribute_no_inline_
    #endif
    int
    blt_ial_bisBroadcast_tx_process(ll_bis_t *pBis)
{
    /******************************* Bis broadcast ******************************************************************/
    sdu_packet_t *sdu;
    u8            numOfcmpPkt = 0;
    ...
    for (int i = 0; i < bltBisMng.curNum_bigBcst; i++) {
        ...
        if (big->big_create_cmp_evt) {
            ...
            if ((hci_le_eventMask & HCI_LE_EVT_MASK_CREATE_BIG_COMPLETE)) {
                hci_le_createBigComplete_evt(evt_status, big->big_handle, (u8 *)&big->big_sync_delay_us, (u8 *)&big->transLatency_us, big->curBisPhy, big->nse, big->bn, big->pto, big->irc, big->max_pdu, big->iso_itvl, big->bis_cnt, big->bis_handle);
            }
...
_attribute_no_inline_
    #endif
    int
    blt_ll_ctrlBigBcstChClassUpd(unsigned char *pChm)
{
    for (int i = 0; i < bltBisMng.curNum_bigBcst; i++) {
...
_attribute_ram_code_ int blt_big_bcst_interrupt_task(int flag, void *p)
{
...
_attribute_no_inline_
    #endif
    int
    blt_big_bcst_mainloop_task(int flag, void *p)
{
...
_attribute_noinline_ void blt_ll_reset_big_bcst(void)
{
    ll_big_bcst_t *pBigBcst = NULL;
    ll_bis_t      *cur_pBis = NULL;
    //Clear BIG Bcst concerned global variables
...
static bool blt_ll_isBigParamsValid_V2(hci_le_createBigParamsV2_t *pParam)
{
    const u8  MAX_BIG_HANDLE        = 0xEF;
    const u8  MAX_ADV_HANDLE        = 0xEF;
    const u8  MIN_NUM_BIS           = 0x01;
...
static bool blt_ll_isBigTestParamsValid_V2(hci_le_createBigParamsTestV2_t *pParam)
{
    const u8  MAX_BIG_HANDLE   = 0xEF;
    const u8  MAX_ADV_HANDLE   = 0xEF;
    const u8  MIN_NUM_BIS      = 0x01;
...
ble_sts_t blc_hci_le_createBigParams(hci_le_createBigParams_t *pCmdParam)
{
    ...
    */
    ...
    /*When a version of this command is issued that does not include all the parameters,
     *  the following values shall be used for any missing parameters*/
    latest_pBigBcst->code_rates = 0x03;
    latest_pBigBcst->hdt_rates = 0x1f;
    latest_pBigBcst->hdt_mic_length = 0x02;
    ...
    if (latest_pBigBcst->packing == BIS_UNFRAMED) {
         ...
         */
        bn = ((latest_pBigBcst->max_sdu + latest_pBigBcst->max_pdu - 1) / latest_pBigBcst->max_pdu) * (latest_pBigBcst->iso_itvl / latest_pBigBcst->sdu_intvl);
    ...
    if (latest_pBigBcst->packing == BIS_UNFRAMED) {
        latest_pBigBcst->pto = ((((pCmdParam->max_trans_lat * 1000 - latest_pBigBcst->big_sync_delay_us + latest_pBigBcst->sdu_intvl) /
                                ...
                                1) /
                               (latest_pBigBcst->nse / latest_pBigBcst->bn - latest_pBigBcst->irc);
    } else {
        /*
    ...
    latest_pBigBcst->sSlot_duration_big = (big_iso_pdu_length_total_us + ctrl_pdu_length_us + SLOT_PROCESS_MAX_US + latest_pBigBcst->bis_distance_schToAir_us + TLK_TM_DELAY) * SSLOT_US_REVERSE + 1;
...
ble_sts_t blc_hci_le_createBigParams_V2(hci_le_createBigParamsV2_t *pCmdParam)
{
     ...
     * return the error code Unsupported Feature or Parameter Value(0x11).
     */
    if (!blt_ll_isBigParamsValid_V2(pCmdParam)) {
        return HCI_ERR_UNSUPPORTED_FEATURE_PARAM_VALUE;
    }
    /*
     * If the Controller cannot create all BISes of the BIG or if Num_BIS exceeds the
    ...
    */
     ...
     * Unsupported Feature or Parameter Value (0x11).
     */
    if (!(pCmdParam->phy & 0x17) || (pCmdParam->phy & 0xE8)) {
        return HCI_ERR_UNSUPPORTED_FEATURE_PARAM_VALUE;
    }

    /*If the Host sets, in the Coded_Rates or HDT_Rates parameter, a bit for a rate that
    ...
    if ((pCmdParam->hdt_rates | HDT_PHY_RATES_SUPPORTED) != HDT_PHY_RATES_SUPPORTED) {
        return HCI_ERR_UNSUPPORTED_FEATURE_PARAM_VALUE;
    }
    if ((pCmdParam->code_rates | CODED_PHY_RATES_SUPPORTED) != CODED_PHY_RATES_SUPPORTED) {
        return HCI_ERR_UNSUPPORTED_FEATURE_PARAM_VALUE;
    }
    ...
    latest_pBigBcst->max_pdu = min(251, latest_pBigBcst->max_sdu);
    #endif
    latest_pBigBcst->phy        = pCmdParam->phy & 0x17;
    latest_pBigBcst->packing    = pCmdParam->packing; //0x00 Sequential, 0x01 Interleaved
    latest_pBigBcst->framing    = pCmdParam->framing; //0x00 Unframed,   0x01 Framed
    latest_pBigBcst->encryption = pCmdParam->enc;     //0x00 Unencrypted, 0x01 Encrypted
    ...
    latest_pBigBcst->code_rates     = pCmdParam->code_rates;
    latest_pBigBcst->hdt_rates      = pCmdParam->hdt_rates;
    latest_pBigBcst->hdt_mic_length = pCmdParam->mic_length;

    u8 micLen = pCmdParam->enc ? 4 : 0;//todo hdt
    ...
    if (latest_pBigBcst->phy & PHY_PREFER_1M) {
        ...
        tx_max_us          = (latest_pBigBcst->max_pdu + micLen + 11) * 4;                                                //mic 4 len
        ctrl_pdu_length_us = (ctrl_pdu_len_max + micLen + 11) * 4;                                                        //mic 4 len
    } else if (latest_pBigBcst->phy & PHY_PREFER_HDT) {
        curBisPhy          = BLE_PHY_HDT;//todo hdt
        tx_max_us          = (latest_pBigBcst->max_pdu + micLen + 11) * 4;                                                //mic 4 len
        ctrl_pdu_length_us = (ctrl_pdu_len_max + micLen + 11) * 4;                                                        //mic 4 len
    } else {
        curBisPhy          = BLE_PHY_CODED;//todo hdt                                                                     //dft: LE_CODED_S8
        tx_max_us          = 376 + ((latest_pBigBcst->max_pdu + micLen) * 64 + 43) * (coding_ind == LE_CODED_S8 ? 8 : 2); //mic 4 len
        ctrl_pdu_length_us = 376 + ((ctrl_pdu_len_max + micLen) * 64 + 43) * (coding_ind == LE_CODED_S8 ? 8 : 2);         //mic 4 len
    }
...
ble_sts_t blc_hci_le_createBigParamsTest_V2(hci_le_createBigParamsTestV2_t *pCmdParam)
{
    ...
    * return the error code Unknown Advertising Identifier (0x42)
    ...
    if (latest_pBigBcst->phy & PHY_PREFER_1M) {
        ...
        tx_max_us          = (latest_pBigBcst->max_pdu + micLen + 11) * 4;                       //mic 4 len
        ctrl_pdu_length_us = (ctrl_pdu_len_max + micLen + 11) * 4;                               //mic 4 len
    } else if (latest_pBigBcst->phy & PHY_PREFER_HDT) {//todo hdt
        curBisPhy          = BLE_PHY_HDT;
        tx_max_us          = (latest_pBigBcst->max_pdu + micLen + 11) * 4;                       //mic 4 len
        ctrl_pdu_length_us = (ctrl_pdu_len_max + micLen + 11) * 4;                               //mic 4 len
    } else {//todo hdt
        curBisPhy          = BLE_PHY_CODED;                                                      //dft: CODED_PHY_PREFER_S8
...
ble_sts_t blc_hci_le_terminateBig(hci_le_terminateBigParams_t *pCmdParam)
{
     ...
     */
    if (ll_big_sync_mlp_task_cb && (ll_big_sync_mlp_task_cb(FLAG_BIG_SYNC_HANDLE_SEARCH, (void *)(&pCmdParam->big_handle)) != BIG_HANDLE_INVALID)) { //blt_ll_findExistingBigSyncByBigHdl
        return HCI_ERR_CMD_DISALLOWED;
    }
    /*
...
_attribute_ram_code_ static int blt_ll_find_next_bisBcst_subevent(int start_idx)
{
    int i = 100; //important
...
_attribute_ram_code_ int blt_bigBcst_start(int slotTask_idx)
{
    DBG_CHN6_HIGH;
    #if (SL01_big_bcst)
    ...
    for (int i = 0; i < bltBisMng.maxNum_bisBcst; i++) {
        ...
        for (int j = 1; j <= cur_pBis->nse; j++) { //NSE = 5,   35-40us,  NSE=16,   71us
            cur_pBis->subEventChnIdx[j - 1] = blt_ll_generateNextChannel(&blt_pBigBcst->chnParam, blt_pBigBcst->bigEventCnt, cur_pBis->chnIdentifier, j);
        }
        cur_pBis->ctrlSubEventChnIdx = blt_ll_generateNextChannel(&blt_pBigBcst->chnParam, blt_pBigBcst->bigEventCnt, blt_pBigBcst->chnIdentifier, 1);
    }
...
_attribute_ram_code_ int blt_bigBcst_post(void)
{
    ...
    for (int i = 0; i < blt_pBigBcst->bis_cnt; i++) {
        if (blt_pBigBcst->bis_alloc_msk & BIT(i)) {
            ...
            if ((pBis->pBisTestParam != NULL) && (pBis->pBisTestParam->isoTestMode == ISO_TEST_TRANSMIT_MODE) && (!pBis->pBisTestParam->tranMode.isoTestSendTick)) {
...
_attribute_ram_code_ static void blt_ll_prepareBigCtrlData(void)
{
...
_attribute_ram_code_ static u8 *blt_ll_prepareBisData(ll_bis_t *pBis, bool big_sc_start)
{
    if (!big_sc_start) {
        ...
        if (pBis->bisCryptCtrl.enable) {
            blt_ll_setAesCcmPara(0, pBis->bisCryptCtrl.sk, pBis->bisCryptCtrl.nonce.iv, 0xc3, curSendBisPldCtnr, 0, 0);
        }
    #endif
        ...
        for (int i = (pBis->bisPduTxFifoRptr & bltBisPduTxfifo.mask); i != (pBis->bisPduTxFifoWptr & bltBisPduTxfifo.mask); i++, i &= bltBisPduTxfifo.mask) {
            ...
            if (curSendBisPldCtnr == pBisTxPduPkt->payloadNumber) {
                ...
                break;
            } else if (curSendBisPldCtnr < pBisTxPduPkt->payloadNumber) { //It indicates that the intermediate HCI ISO DATA PDU is missing
...
_attribute_ram_code_ int blt_bisBcst_tx_start(void)
{
    DBG_CHN4_HIGH;
...
_attribute_ram_code_ int blt_bisBcst_tx_post(void)
{
...
_attribute_ram_code_ int blt_ll_perdAdvAcadUpdateBigInfo(int prdadv_idx)
{
    DBG_CHN5_TOGGLE;
...
ble_sts_t blt_ll_pushBisData(u16 connHandle, iso_pb_flag_t PB_Flag, u8 TS_Flag, u32 time_stamp, u16 seqnum, u16 total_len, u16 cur_len, u8 *pData)
{
    ...
    if (!pBis->bn) {
        tlkapi_send_string_u8s(HOST_HCI_ERR_LOG_EN, "HCI ERROR, bis data not allowed", connHandle, pBis->bn, 0, 0);
...
_attribute_ram_code_
        #endif
    ble_sts_t
    blc_hci_le_pushBisData(iso_data_packet_t *pIsoDatPkt)
{
