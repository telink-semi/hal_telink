...
_attribute_ram_code_ int bigBuildTsk_confilctACL_proc(u8 aclTask_idx)
{
...
_attribute_ram_code_ int blt_ll_buildBigSyncSchedulerLinklist(void)
{
    int            intvl_jump_big;
    s32            sSlot_start_big;
...
_attribute_ram_code_
        #endif
    void
    blt_bisSync_rx_procUpdateReq(u8 *raw_pkt)
{
...
_attribute_ram_code_
        #endif
    void
    blt_bisSync_slotgap_procUpdateReq(void)
{
    ll_big_sync_t *pBigSync = NULL;
    for (int i = 0; i < bltBisMng.maxNum_bigSync; i++) {
...
_attribute_ram_code_
        #endif
    int
    blt_ll_bisSync_chnm_term_update(ll_big_sync_t *pBigSync)
{
...
_attribute_no_inline_
    #endif
    void
    blt_ll_bisSync_sdu_out_loop(ll_bis_t *pBis)
{
    ...
    blt_ll_bigSyncMainloop(void)
{
    ...
    for (int i = 0; i < bltBisMng.maxNum_bigSync; i++) {
        ...
        } else if (pBigSync->irq_bigSyncEvt == 1) { //notify host the establish(or terminate, use same evt) evt
            pBigSync->irq_bigSyncEvt = 0;

            if (hci_le_eventMask & HCI_LE_EVT_MASK_BIG_SYNC_ESTABLISHED) {
                hci_le_bigSyncEstablished_evt(pBigSync->bigSyncEvtStatus, pBigSync->big_handle, (u8 *)&pBigSync->transLatency_us, pBigSync->nse, pBigSync->bn, pBigSync->pto, pBigSync->irc, pBigSync->max_pdu, pBigSync->iso_itvl, pBigSync->sync_bis_num, pBigSync->bis_handle);
            }

            if (pBigSync->bigSyncEvtStatus) {   //0: success; others: failure, should destroy BISes and BIG's control block
...
_attribute_ram_code_ int irq_big_sync_rx(void)
{
    HAL_CLEAR_RF_RX_IRQ;
    ...
    if (blms_state == BLMS_STATE_BSYNC_S) {
        ...
        if (bltRxPkt.rx_header_tick) {
            ...
            if (!blt_pBigSync->bisSyncRxTick) {
                if (blms_state == BLMS_STATE_BSYNC_S) { //BIS Sync used only
                    blt_pBigSync->bisSyncRxTick = bltRxPkt.rx_header_tick;

                    blt_pBigSync->bSync_expect_tick    = blt_pBigSync->bisSyncRxTick;
                    blt_pBigSync->bigs_1st_expect_tick = (blt_pBigSync->bisSyncRxTick - (blt_pBis->bisSubEventCnt - 1) * blt_pBis->sub_interval_tick) - (blt_pBis->bisSyncIdx - 1) * blt_pBis->bis_spacing_tick + blt_pBigSync->bigs_ap_offset_us * SYSTEM_TIMER_TICK_1US;
                    ...
                    if ((diff > (BYNC_FIRST_EARLY_US + 30) * SYSTEM_TIMER_TICK_1US || diff < (BYNC_FIRST_EARLY_US - 30) * SYSTEM_TIMER_TICK_1US) || (blt_pBigSync->big_state == BIG_SYNCHRONIZING)) {
                        DBG_CHN0_TOGGLE;
                        DBG_CHN0_TOGGLE;
                        DBG_FANQH_CHN0_TOGGLE;
                        DBG_FANQH_CHN0_TOGGLE;


                        if (blt_pBigSync->big_state == BIG_SYNCHRONIZING) { // delete the 30/300us tolerance
            ...
            if (blt_pBigSync->big_sc_flg & BIG_SC_RDY2RCV_BIGCTRL) {
                ...
                if (rcvdLlid == 0x03 && rcvdCstf == 0) {                            /* Big ctrl temp buffer 16B: special */
    ...
    #endif
                    } else { //same pkt received, skip rx pkt
...
_attribute_ram_code_
    init_err_t
    blt_bis_insertRxPdu(ll_bis_t *pBisSync)
{
...
_attribute_ram_code_
    init_err_t
    blt_bis_deletePdu(bis_rx_pdu_chain_t *bisRxPduChain)
{
    u32           r          = irq_disable();
...
_attribute_ram_code_ void blt_ll_bis_duration_jump(ll_bis_t *pBis)
{
...
_attribute_ram_code_ int blt_big_sync_interrupt_task(int flag, void *p)
{
    ...
    if (flag & FLAG_SCHEDULE_BIGSYNC_START) {
        ...
        if (bltPri.csctvAbandonCnt[curSchTaskOft] >= 5) {
            tlkapi_send_string_data(0, "[big_bcst]consecutive abandon count", &bltPri.csctvAbandonCnt[curSchTaskOft], 2) return 1; /* 1:conflict resolved; 0: insert task failed */
        }
    }
    return 0;
}

_attribute_noinline_ int blt_big_sync_mainloop_task(int flag, void *p)
{
...
_attribute_noinline_ void blt_ll_reset_big_sync(void)
{
...
_attribute_ram_code_ static int blt_ll_find_next_bisSync_subevent(int start_idx)
{
...
_attribute_ram_code_ int blt_bigSync_start(int slotTask_idx)
{
    DBG_FANQH_CHN3_HIGH;
    #if (SL01_big_sync)
    ...
    for (int i = 0; i < blt_pBigSync->sync_bis_num; i++) {
        ...
        for (int j = 1; j <= pBis->nse; j++) { //NSE = 5,   35-40us,  NSE=16,   71us
            pBis->subEventChnIdx[j - 1] = blt_ll_generateNextChannel(&blt_pBigSync->chnParam, blt_pBigSync->bigEventCnt, pBis->chnIdentifier, j);
        }

        pBis->ctrlSubEventChnIdx = blt_ll_generateNextChannel(&blt_pBigSync->chnParam, blt_pBigSync->bigEventCnt, blt_pBigSync->chnIdentifier, 1);
    }
...
_attribute_ram_code_ int blt_bigSync_post(void)
{
...
_attribute_ram_code_ int blt_bisSync_rx_start(void)
{
    //  DBG_C HN4_HIGH;
    #if (SL01_bsync0)
    ...
    } //Turn on PA ~100us in advance with RF packet head
    ...
    if (blt_pBis->bisCryptCtrl.enable) {
        ...
        blt_ll_setAesCcmPara(1, blt_pBis->bisCryptCtrl.sk, blt_pBis->bisCryptCtrl.nonce.iv, 0xc3, 0, payloadNum, 0);
    }
    #endif
...
_attribute_ram_code_ int blt_bisSync_rx_post(void)
{
    ...
    } // turn off PA ~20us after RF done
...
ble_sts_t blc_hci_le_bigCreateSync(hci_le_bigCreateSyncParams_t *pCmdParam)
{
    ...
    //todo 39 bits
    u32 bisPldCnt = pBigInfo->bisPldCnt39Framing1[0] | (pBigInfo->bisPldCnt39Framing1[1] << 8) | (pBigInfo->bisPldCnt39Framing1[2] << 16) | (pBigInfo->bisPldCnt39Framing1[3] << 24);
    ...
    latest_pBigSync->transLatency_us = latest_pBigSync->big_sync_delay_us +
                                       ((latest_pBigSync->pto * (latest_pBigSync->nse / latest_pBigSync->bn - latest_pBigSync->irc)) + 1) * (latest_pBigSync->iso_itvl * 1250) + latest_pBigSync->sdu_intvl;
    ...
    latest_pBigSync->sSlot_duration_big = (big_iso_pdu_length_total_us + rx_max_big_ctrl_us + latest_pBigSync->bigSyncToleranceTime) * SSLOT_US_REVERSE + 1 + SLOT_PROCESS_MAX_SSLOT_NUM + BYNC_FIRST_EARLY_SLOT_NUM;
...
_attribute_no_inline_
    #endif
    int
    blt_bisSync_process(ll_bis_t *pBis)
{
    /******************************* Bis Sync ******************************************************************/
