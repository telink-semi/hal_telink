...
//      ll_big_bcst_t *pBigBcst;
        if((pBis = blt_ll_findBisByHandle(iso_connHandle)) != NULL){
             ...
             * If the Host issues this command before an SDU has been transmitted by the
                ...
                (0x0C).
             */
            tlkapi_send_string_u32s(0, "read_iso_tx_sync",pBis->tx_first_pdu,0,0,0 )
            if(pBis->tx_first_pdu==1){
                pRetParam->status = HCI_ERR_CMD_DISALLOWED;
                return HCI_ERR_CMD_DISALLOWED;
            }
...
_attribute_ram_code_
#endif
ble_sts_t blt_iso_process_sdu_in_data(sdu_packet_t *iso_sdu, iso_pb_flag_t PB_Flag, u8 TS_Flag, u32 time_stamp, u16 seqnum, u16 total_len, u16 cur_len, u8 *pData)
{
