...
int blt_cis_conn_interrupt_task (int flag, void *p)
{
    if(flag == FLAG_IRQ_RX){
        irq_cis_rx();
    }
...
#endif
int blt_cis_conn_mainloop_task (int flag, void *p)
{
    ...
    if(sduCisMng.out_fifo_b){
        ...
        for(int i=0; i < bltCisMng.maxNum_cisConn; i++){
            ...
            for(int j=0; j < sduCisMng.out_fifo_num; j++){
                ...
                my_dump_str_u32s(0my_dump_str_u32s(0, "0xAAAA0000", pSduOut, &pSduOut->isoHandle, pSduOut->isoHandle, pCisConn->cis_connHandle);
            }
        }
    }
#endif
...
ble_sts_t blc_ll_initCisTxFifo(u8 *pTxbuf, int fifo_size, int fifo_number)
{
    ...
    my_dump_str_u32s(STACK_DUMP_EN, "CIS TX PDU buffer", fifo_size, cis_fifo_size, fifo_number, 0);
...
ble_sts_t blc_ll_initCisRxFifo(u8 *pRxbuf, int fifo_size, int fifo_number)
{
    ...
    my_dump_str_u32s(STACK_DUMP_EN, "CIS RX PDU buffer", pRxbuf, fifo_size, fifo_number, 0);
...
void blc_ll_initCisSduBuffer(u8 *in_fifo, int in_fifo_size, u8 in_fifo_num, u8 *out_fifo, int out_fifo_size, u8 out_fifo_num)
{
    ...
    my_dump_str_u32s(STACK_DUMP_EN, "CIS SDU buffer", sduCisMng.max_in_fifo_size, in_fifo_num, sduCisMng.max_out_fifo_size, out_fifo_num);
}
...
int         blt_cis_connect_common(st_ll_conn_t* pAclConn, ll_cis_conn_t *pCisConn)
{
...
void blt_cis_establish_common(ll_cis_conn_t *pCisConn)
{
    ...
    if((pCisConn->cis_frame==CIS_UNFRAMED) && (pCisConn->bn_loca) && (pCisConn->tx_numSdu2Pdu == 0)){
        my_dump_str_u32s(DBG_CIS_PARAM, "[CIS][PAR] tx numSdu2Pdu ERR", pCisConn->bn_loca, pCisConn->sdu_int_loca_us, pCisConn->iso_intvl_us, 0);
        BLMS_ERR_DEBUG(DBG_CIS_LOGIC, 0x99C50000);
    }
    if((pCisConn->cis_frame==CIS_UNFRAMED) && (pCisConn->bn_peer) && (pCisConn->rx_numSdu2Pdu == 0)){
        my_dump_str_u32s(DBG_CIS_PARAM, "[CIS][PAR] rx numSdu2Pdu ERR", pCisConn->bn_peer, pCisConn->sdu_int_peer_us, pCisConn->iso_intvl_us, 0);
        BLMS_ERR_DEBUG(DBG_CIS_LOGIC, 0x99C50000);
    }

    my_dump_str_u32s(DBG_CIS_PARAM, "[CIS][PAR] numSdu2Pdu", pCisConn->tx_numSdu2Pdu, pCisConn->rx_numSdu2Pdu, 0, 0);
#endif
}
...
_attribute_ram_code_ int blt_ll_cis_start_common_1   (ll_cis_conn_t *pCisConn)
{
    ...
    //DBG_C HN10_HIGH;
    u8 cis_cur_chn = blt_ll_generateNextChannel(pChnParam, pCisConn->cisEventCnt,
            pCisConn->chnIdentifier, pCisConn->cisSubEventCnt);
    //DBG_C HN10_LOW;
...
_attribute_ram_code_ int blt_ll_cis_start_common_2   (ll_cis_conn_t *pCisConn)
{
    ...
    {
        blt_ll_setAesCcmPara(!blt_pCisConn->cisRole, blt_pCisConn->crypt.sk, blt_pCisConn->crypt.nonce.iv, 0xa3,\
                blt_pCisConn->cisSendPldNum, blt_pCisConn->cisRcvdPldNum, 0);
    }
#endif
    ...
    if(pCisConn->cisSubEventCnt == 1){  //first sub_event of current CIS event
        ...
        #if (SL16_cis0_evtcnt)
            log_b16_irq(SL_STACK_CIS_BASIC_TIMING_EN, (SL16_cis0_evtcnt+((pCisConn->cisRole)?
                    (pCisConn->cis_index): (pCisConn->cis_index - bltCisMng.maxNum_cisMaster))) , (u16)pCisConn->cisEventCnt);
        #endif
    }
...
void blt_ll_cis_start_jump (ll_cis_conn_t *pCisConn, int jump_num)
{
    ...
    if(pCisConn->bn_peer){
        ...
        log_b16_irq(SL_STACK_CIS_RX_DATA_EN, SL16_cis0_rxPldNum+((pCisConn->cisRole)?
                    (cisConn_param.blt_cis_sel): (cisConn_param.blt_cis_sel - bltCisMng.maxNum_cisMaster)),
...
//  }
    ...
    if(localBN )
    {
        if(pCisConn->cisEventCnt >= localFT){
            ...
            if(pCisConn->cisSendPldNum <= last_tx_ft_pduNum){
                ...
                log_b16_irq(SL_STACK_CIS_TX_DATA_EN,  SL16_cis0_txCurSendPldNum+((pCisConn->cisRole)?
                        (pCisConn->cis_index): (pCisConn->cis_index - bltCisMng.maxNum_cisMaster)),   (u16)pCisConn->cisSendPldNum);//+pCisConn->cis_index
            #endif
    ...
    my_dump_str_u32s(DBG_CIS_TX_DATA_FLOW_EN|DBG_CIS_RX_DATA_FLOW_EN, "Tx&Rx pld ft",
            pCisConn->cisEventCnt,pCisConn->cisSendPldNum, pCisConn->cisRcvdPldNum, 0);
}
...
_attribute_ram_code_ u8* blt_ll_cis_prepare_data_common (ll_cis_conn_t *pCisConn)
{
    ...
    if(pCisConn->conState == CONN_STATUS_ESTABLISH && localBN){
        ...
        while(pCisConn->cisPduTxFifoWptr != pCisConn->cisPduTxFifoRptr){
            ...
            if(pCisTxPduPkt->cis_pdu_number < pCisConn->cisSendPldNum){
                //my_dump_str_u32s(0, "PDU 2", pCisTxPduPkt->cis_pdu_number, pCisConn->cisSendPldNum, 0, 0);
                ...
                my_dump_str_u32s(DBG_CIS_TX_DATA_FLOW_EN, "[cis][tx] preData", pCisTxPduPkt->cis_pdu_number, pCisConn->cisPduTxFifoRptr, pCisConn->cisPduTxFifoWptr,pCisConn->cisEventCnt);
    ...
    if(send_data_pkt)
    {
        ...
        if(pCurrCisPdu->llPhysChnPdu.llPduHdr.cisPduHdr.rf_len)
        {
            ...
            log_b16_irq(SL_STACK_CIS_TX_DATA_EN,  SL16_cis0_txPrePldNum+((pCisConn->cisRole)?
                    (pCisConn->cis_index): (pCisConn->cis_index - bltCisMng.maxNum_cisMaster)),   (u16)pCisConn->cisSendPldNum);
        #endif
    }
    else
    {
...
_attribute_ram_code_
int blt_cis_pushRxEvtInfoToFifo(ll_cis_conn_t *pCisConn, rf_packet_ll_data_t* pIsoRxRawPkt, u8 jump_num)
{
    ...
    my_dump_str_u32s(DBG_CIS_RX_DATA, "[CIS RX] pushRxEvt", blt_debug_hex_2_dec_display(pCisConn->cisEventCnt),pIsoRxEvt->curRcvdPldNum, pIsoRxEvt->null_flag,pCisConn->cisSubEventCnt);
    ...
    if((pIsoRxEvt->curRcvdPldNum % pCisConn->rx_numSdu2Pdu == 0) || (pIsoRxEvt->null_flag))
    {
        ...
        if(pCisConn->cisRole == CIS_ROLE_SLAVE)
        {
            ...
            pIsoRxEvt->cisRefAP = (pCisConn->cis_expect_tick - (subEventOffset)*pCisConn->sub_intvl_tick\
                                            - (pCisConn->cisEventCnt - pIsoRxEvt->curRcvdPldNum / pCisConn->bn_peer) * pCisConn->iso_intvl_tick)/SYSTEM_TIMER_TICK_1US ;
#else
            // unit:System tick
            pIsoRxEvt->cisRefAP = (pCisConn->cis_expect_tick - (subEventOffset)*pCisConn->sub_intvl_tick\
                                            - (pCisConn->cisEventCnt - pIsoRxEvt->curRcvdPldNum / pCisConn->bn_peer) * pCisConn->iso_intvl_tick);
#endif
            my_dump_str_u32s(0, "cisEvnt",blt_debug_hex_2_dec_display(pIsoRxEvt->curRcvdPldNum),
                                            blt_debug_hex_2_dec_display(pCisConn->cisEventCnt),
                                            blt_debug_hex_2_dec_display(pCisConn->cisSubEventCnt),
                                            blt_debug_hex_2_dec_display(pCisConn->bn_peer));

            my_dump_str_u32s(0, "cisAef", blt_debug_hex_2_dec_display(pIsoRxEvt->cisRefAP),
                                            blt_debug_hex_2_dec_display(pCisConn->cis_expect_tick),
                                            blt_debug_hex_2_dec_display(pCisConn->sub_intvl_tick),
                                            blt_debug_hex_2_dec_display(pCisConn->iso_intvl_tick));
        }
        else
        {

        #if(ISO_DATA_TIMESTAMP_UNIT_US_EN)
            pIsoRxEvt->cisRefAP = (pCisConn->cis_expect_tick - (pCisConn->cisSubEventCnt-1)*pCisConn->sub_intvl_tick
                                            - (pCisConn->cisEventCnt - pIsoRxEvt->curRcvdPldNum / pCisConn->bn_peer) * pCisConn->iso_intvl_tick)/SYSTEM_TIMER_TICK_1US;///SYSTEM_TIMER_TICK_1US ;
        #else
            pIsoRxEvt->cisRefAP = (pCisConn->cis_expect_tick - (subEventOffset)*pCisConn->sub_intvl_tick
                                            - (pCisConn->cisEventCnt - pIsoRxEvt->curRcvdPldNum / pCisConn->bn_peer) * pCisConn->iso_intvl_tick);///SYSTEM_TIMER_TICK_1US ;
        #endif

            my_dump_str_u32s(0, "cisEvnt1",blt_debug_hex_2_dec_display(pCisConn->cis_connHandle),
                                            blt_debug_hex_2_dec_display(pIsoRxEvt->curRcvdPldNum),
                                            blt_debug_hex_2_dec_display(pCisConn->cis_expect_tick),
                                            blt_debug_hex_2_dec_display(pIsoRxEvt->cisRefAP)
                                            );


            my_dump_str_u32s(0, "cisEvnt2",blt_debug_hex_2_dec_display(pCisConn->cis_connHandle),
                                                blt_debug_hex_2_dec_display(pCisConn->cisEventCnt),
                                                blt_debug_hex_2_dec_display(pIsoRxEvt->curRcvdPldNum),
                                                blt_debug_hex_2_dec_display(pCisConn->iso_intvl_tick)
                                                );
    ...
    if(pCisConn->conState == CONN_STATUS_COMPLETE){
        ...
        if(pCisConn->cis_receive_packet){
            ...
            if(pCisConn->cisFlowFlg == CIS_FLOW_CIS_SYNCING){
                ...
                //my_dump_str_u32s(0, "cis conn 7", pCisConn->cis_index, pCisConn->cis_connHandle, pCisConn, 0);
...
_attribute_noinline_
ble_sts_t       blt_ll_cis_disconnect(ll_cis_conn_t * pCisConn, u8 reason)
{
    ...
    if(blt_llmsPushLlCtrlPkt(acl_handle, LL_CIS_TERMINATE_IND, temp_buff))
    {   //push terminate packet OK
...
_attribute_ram_code_ int irq_cis_rx(void)
{
    ...
    if(bltRxPkt.rx_header_tick)
    {
        ...
        if(!blt_pCisConn->txNullPduFlag)
        {
            if(blt_pCisConn->bn_loca)  //if BN == 0, means local device has no CIS Data PDU to send
            {
                ...
                if(blt_pCisConn->locl_sn != peerCurNESN)
                {
                    ...
                    my_dump_str_u32s(DBG_CIS_TX_DATA, "rx pdu ack", blt_pCisConn->cisSendPldNum, blt_pCisConn->locl_sn, peerCurNESN, 0 );
                ...
                #if(SL16_cis0_txCurSendPldNum)
                    log_b16_irq(SL_STACK_CIS_TX_DATA_EN,  SL16_cis0_txCurSendPldNum+((blt_pCisConn->cisRole)?
                            (blt_pCisConn->cis_index): (blt_pCisConn->cis_index - bltCisMng.maxNum_cisMaster)),   (u16)blt_pCisConn->cisSendPldNum);//+blt_pCisConn->cis_index
                #endif
                    ...
                    if(!blt_pCisConn->insertEmtpy_flg){
                        if(blt_pCisConn->cisPduTxFifoWptr != blt_pCisConn->cisPduTxFifoRptr){
                            ...
                            my_dump_str_u32s(0, "rx ack",blt_pCisConn->cisEventCnt, blt_pCisConn->cisSubEventCnt, blt_pCisConn->cisPduTxFifoWptr, blt_pCisConn->cisPduTxFifoRptr);
                        }
                        else{
                            my_dump_str_u32s(DBG_CIS_TX_DATA, "PDU buf ERR", blt_pCisConn->cisPduTxFifoWptr, blt_pCisConn->cisPduTxFifoRptr, blt_pCisConn->cisEventCnt, blt_pCisConn->cisSubEventCnt);
        ...
        if(!peerCurNPI && blt_pCisConn->bn_peer && (peerCurSN == blt_pCisConn->locl_nesn))
        {
            ...
            //////////////////////////////////////////////////////////////////////////////////////////////
            my_dump_str_u32s(DBG_CIS_RX_DATA, "[CIS RX] IRQ RX", blt_pCisConn->cisEventCnt, blt_pCisConn->cisRcvdPldNum,blt_pCisConn->cis_rx_stream_start,  rf_len);
            ...
            next_buffer = blt_cis_pushRxEvtInfoToFifo(blt_pCisConn, pCisRawPkt, 0); //must before RX PDU number update
            ...
            #endif
                {
                    ...
                    #if (SL16_cis0_rxPldNum)
                    log_b16_irq(SL_STACK_CIS_RX_DATA_EN, SL16_cis0_rxPldNum+((blt_pCisConn->cisRole)?
                                (cisConn_param.blt_cis_sel): (cisConn_param.blt_cis_sel - bltCisMng.maxNum_cisMaster)),
                                (iso_evtcnt_t)blt_pCisConn->cisRcvdPldNum);
                    #endif
                }
                ...
                if((blt_pCisConn->cisRcvdPldNum / blt_pCisConn->bn_peer) > blt_pCisConn->cisEventCnt){
                    ...
                    my_dump_str_u32s(0, "Clean Peer cie", blt_pCisConn->cisEventCnt, blt_pCisConn->cisSubEventCnt, blt_pCisConn->peer_cie, blt_pCisConn->local_cie);
                }
            #endif
...
//          my_dump_str_u32s(0, "cie", blt_pCisConn->cisEventCnt, blt_pCisConn->cisSubEventCnt, blt_pCisConn->peer_cie, blt_pCisConn->local_cie);
        }
    #endif
...
_attribute_ram_code_
#endif
int blt_cis_update_chn_map (int trigger_tick, void *p)
{
    st_ll_conn_t *pAclConn = (st_ll_conn_t *)p;
    for(int i=0; i<bltCisMng.maxNum_cisConn; i++){
        if(pAclConn->cisEstablish_msk & BIT(i)){
...
#if(HCI_SEND_NUM_OF_CMP_AFT_ACK)
_attribute_ram_code_
void blt_cis_mark_numOfcmpEvt_status(ll_cis_conn_t * pCisConn, int numOfcmpPkt)
{
    /* CIS use: send numOfCmpEvt after data transmitted OK or flushed */
...
_attribute_no_inline_
#endif
int         blt_cis_tx_loop(ll_cis_conn_t * pCisConn)
{
...
_attribute_no_inline_
#endif
int         blt_cis_release_iso_data_buffer(ll_cis_conn_t * pCisConn)
{
    int numberPkt = 0;
    while(pCisConn->cisNocpRptr != pCisConn->cisNocpWptr){
...
int blt_cis_rx_event_loop(void)
{
    ...
    while (bltCisRxEvt.rptr != bltCisRxEvt.wptr)
    {
        ...
        if (pIsoRxEvt->payloadLen && pCisConn->crypt.enable){
             ...
             */
            DBG_FANQH_CHN4_HIGH;
            u8 st = aes_ll_ccm_decryption(&pIsoRxEvt->pCurrIsoRxPdu->llPhysChnPdu, pCisConn->cisRole == CIS_ROLE_MASTER ? 0 : 1,
                                                CRYPT_NONCE_TYPE_CIS, pLeCryptCtrl);
            DBG_FANQH_CHN4_LOW;
            aes_enc_dec_busy = 0;
...
_attribute_no_inline_
#endif
void blt_cis_sdu_out_loop(ll_cis_conn_t *pCisConn)
{
    ...
    while(pCisConn->cisSduOut_wptr != pCisConn->cisSduOut_rptr)
    {
        ...
        tlkapi_send_string_u8s(DBG_CIS_RX_DATA, "[CIS RX] pop sduOut", pCisConn->cisSduOut_wptr, pCisConn->cisSduOut_rptr, 0, 0);
            ...
            sdu_packet_t* sdu = (sdu_packet_t*)(pCisConn->cis_sduOutBuf + sduCisMng.max_out_fifo_size * (pCisConn->cisSduOut_rptr&(sduCisMng.out_fifo_mask)));
            /*
            +鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�+鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�+鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�+鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�+鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�+鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�+鈹�鈹�鈹�鈹�+鈹�鈹�鈹�鈹�+鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�鈹�+
             ...
             */
                ...
                my_dump_str_u32s(0, "sdu timestamp", pCisConn->cis_ap_tick , sdu->pkt_seq_num, sdu->timestamp ,0);
            #endif
...
_attribute_noinline_
ll_cis_conn_t* blt_isCisEstablished_by_handle(u16 handle)
{
...
_attribute_noinline_
ll_cis_conn_t* blt_isCisAllocated_by_handle(u16 handle)
{
    if(handle >= BLT_CIS_HANDLE && handle < (BLT_CIS_HANDLE + bltCisMng.maxNum_cisConn)){
        u8 cis_idx = handle & BLT_CIS_IDX_MSK;
        ll_cis_conn_t * pCisConn = (ll_cis_conn_t *) (global_pCisConn + cis_idx);
...
ble_sts_t blc_ll_cis_iso_receive_mode(u16 connHandle, itest_payload_type_t pdu_type)
{
    ...
    my_dump_str_u32s(DBG_ISO_TEST_EN,"iso set rec mode",isoTest->isoTestMode,0,0,0);
...
_attribute_no_inline_
#endif
void blt_cis_test_process(ll_cis_conn_t *pCisConn)
{
...
_attribute_noinline_
int blt_cis_cmd_process_task (int opcode, void *pCmd, void *pRet)
{
...
ble_sts_t   blc_ll_setupCisDataPath(u16 cis_handle, dat_path_dir_t dir, dat_path_id_t id, u8 cid_assignNum, u16 cidcompId, u16 cid_vendorDef,
...
//  }
    ...
    if(pCisConn){
        ...
        if(dir == Data_Dir_Input){ /* input data path */
            //my_dump_str_u32s(0, "DEBUG DATA IN", pCisConn->bn_loca, pCisConn->cis_dapth_setup, 0, 0);
            if(!pCisConn->bn_loca){
                return HCI_ERR_CMD_DISALLOWED;
            }
...
ble_sts_t   blc_hci_le_setupCisDataPath(hci_le_setupIsoDataPath_cmdParam_t *pCmdPara, hci_le_setupIsoDataPath_retParam_t *pRetParam)
{
    ...
    pRetParam->status = blc_ll_setupCisDataPath(pCmdPara->conn_handle, pCmdPara->data_path_dir, pCmdPara->data_path_id,
...
ble_sts_t blt_ll_pushCisData(u16 connHandle, iso_pb_flag_t PB_Flag, u8 TS_Flag, u32 time_stamp, u16 seqnum, u16 total_len, u16 cur_len, u8 *pData)
{
...
#if (IUT_HCI_LOG_EN)
    my_dump_str_u32s(IUT_HCI_LOG_EN, "HCI ISO info 2", connHandle, PB_Flag, total_len, cur_len);
    ...
    if(pCisConn == NULL || !pCisConn->cis_established){
        my_dump_str_u32s(HOST_HCI_ERR_LOG_EN, "HCI ERROR, CIS not establish", connHandle, 0, 0, 0);
    ...
    if(!pCisConn->max_sdu_loca || !pCisConn->max_pdu_loca || !pCisConn->bn_loca){
        tlkapi_send_string_u8s(HOST_HCI_ERR_LOG_EN, "HCI ERROR, cis data not allowed", \
                pCisConn->max_sdu_loca, pCisConn->max_pdu_loca, pCisConn->bn_loca, pCisConn->max_sdu_loca);
        BLMS_ERR_DEBUG(HOST_HCI_ERR_LOG_EN, 0x99C80000);
        return LL_ERR_INVALID_PARAMETER;
    }
    ...
    if( !(PB_Flag & 1) && total_len > sduCisMng.max_in_fifo_size){
        my_dump_str_u32s(HOST_HCI_ERR_LOG_EN, "HCI ERROR, iso data too long", total_len, sduCisMng.max_in_fifo_size, pCisConn->max_sdu_loca, 0);
        BLMS_ERR_DEBUG(HOST_HCI_ERR_LOG_EN, 0x99C80000);
...
_attribute_ram_code_
#endif
ble_sts_t blc_hci_le_pushCisData(iso_data_packet_t *pIsoDatPkt)
{
    ...
    else // HCI_ISO_SDU_FIRST_FRAG   HCI_ISO_SDU_COMPLETE
    {
        ...
        if(pIsoDatPkt->ts)
        {
            ...
            if(pIsoDatPkt->pb == HCI_ISO_SDU_COMPLETE && (pIso_load->iso_sdu_len + 8) != pIsoDatPkt->iso_dat_len){
                my_dump_str_u32s(HOST_HCI_ERR_LOG_EN, "HCI ERROR, ISO data length not match ISO SDU length", \
...
void blc_cis_get_tx_point(u8 role)
{
    if(role == 1)
    {
        while(1)
        {
            ...
            if((clock_tick-blt_pCigMst->cigm_trigger_tick+10*1000*SYSTEM_TIMER_TICK_1US)<300*SYSTEM_TIMER_TICK_1US\
             ||(blt_pCigMst->cigm_trigger_tick+10*1000*SYSTEM_TIMER_TICK_1US-clock_tick)<300*SYSTEM_TIMER_TICK_1US )
            {
                break;
            }
        }
    ...
    else if(role == 0)
    {
        ...
        {
            u32 clock_tick = clock_time();
            if((clock_tick-blt_pCisSlv->cigs_expect_tick+10*1000*SYSTEM_TIMER_TICK_1US)<300*SYSTEM_TIMER_TICK_1US\
             ||(blt_pCisSlv->cigs_expect_tick+10*1000*SYSTEM_TIMER_TICK_1US-clock_tick)<300*SYSTEM_TIMER_TICK_1US )
            {
                break;
            }
        }
