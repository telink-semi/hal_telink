...
ble_sts_t   blc_hci_le_setCigParams(hci_le_setCigParam_cmdParam_t* pCmdParam, hci_le_setCigParam_retParam_t* pRetParam)
{
    ...
    u32 max_trans_lat_peer = pCmdParam->max_trans_lat_m2s * 1000;
        ...
        tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] cis cnt,pack,frame,acl int", pCmdParam->cis_count, pCmdParam->packing, pCmdParam->framing, \
                                                                          blt_debug_hex_2_dec_display(aclmMster_intvl_us));

        tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] SDU int, max_tran_lat",      blt_debug_hex_2_dec_display(pCigMaster->sdu_int_loca), \
                                                                          blt_debug_hex_2_dec_display(pCigMaster->sdu_int_peer), \
                                                                          blt_debug_hex_2_dec_display(max_trans_lat_loca), \
                                                                          blt_debug_hex_2_dec_display(max_trans_lat_peer));
    #endif
    ...
    tlkapi_send_string_u32s(DBG_SET_CIG_PARAMS , "[CISC][PAR] iso interval cal",     blt_debug_hex_2_dec_display(isoIntvlUs_min), \
                                                                  blt_debug_hex_2_dec_display(isoIntvlUs_max), \
                                                                  blt_debug_hex_2_dec_display(cgSyDly_ft_IsoInt_loca), \
                                                                  blt_debug_hex_2_dec_display(cgSyDly_ft_IsoInt_peer));
    ...
    for(int cis_idx = 0; cis_idx< bltCisMng.maxNum_cisMaster; cis_idx++){
        ...
        if( pCisConn->cis_occupied == 0){  //available
            ...
            if(pCigMaster->cig_frame == CIS_UNFRAMED){
                ...
                tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] sdu, pdu",     blt_debug_hex_2_dec_display(pCisConn->max_sdu_loca), \
                                                                    blt_debug_hex_2_dec_display(pCisConn->max_sdu_peer), \
                                                                    blt_debug_hex_2_dec_display(pCisConn->max_pdu_loca), \
                                                                    blt_debug_hex_2_dec_display(pCisConn->max_pdu_peer));
                ...
                tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] split num, bn min",  pCisConn->sdu_split_pdu_loca, \
                                                                             pCisConn->sdu_split_pdu_peer, \
                                                                             bnLoca_min, \
                                                                             bnPeer_min);
                ...
                //for(int i=0; i<isoIntvlTra_cnt; i++ )
                {
                    ...
                    if(pCisConn->max_sdu_loca){
                        ...
                        tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] local F, ceilF, PDU",      ceil(F), ceil(F)*5, \
                                                                                     blt_debug_hex_2_dec_display(ceil(F*pCisConn->max_sdu_loca)), \
                                                                                     blt_debug_hex_2_dec_display(iso_data_loca));
#else
                        u8 extra_loca_byte;
                    ...
                    if(pCisConn->max_sdu_peer){
                        ...
                        tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] peer F, ceilF, PDU",  ceil(F), ceil(F)*5, \
                                                                                     blt_debug_hex_2_dec_display(ceil(F*pCisConn->max_sdu_peer)), \
                                                                                     blt_debug_hex_2_dec_display(iso_data_peer));
#else
                        u8 extra_peer_byte;
            ...
            #if (DBG_CIS_CENTRAL_PARAM)
                tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] mptm_mpts, se_length",     blt_debug_hex_2_dec_display(mptm_tifs_mpts), \
                                                                        blt_debug_hex_2_dec_display(se_length_us[new_cis_cnt]), \
...
//          }
            ...
            tlkapi_send_string_u32s(DBG_SET_CIG_PARAMS, "[CISC][PAR] ISO interval traverse",   traverse_cnt, \
                                                                            blt_debug_hex_2_dec_display(isoIntvlUs_traverse), \
                                                                            blt_debug_hex_2_dec_display(cig_task_us), \
                                                                            nse_cig_traverse);
                ...
                for(int k=1; k<=4; k++){
                    ...
                    if(cur_nse < cig_nse_thres){
                        ...
                        if(cigSyDly_min + 1500 < isoIntvlUs_traverse){
                            ...
                            tlkapi_send_string_u32s(DBG_SET_CIG_PARAMS, "[CISC][PAR] unframed, allocate OK",    blt_debug_hex_2_dec_display(isoIntvlUs_traverse), \
                                                                                        blt_debug_hex_2_dec_display(cig_task_us), \
                                                                                        nse_final_use, \
                                                                                        blt_debug_hex_2_dec_display(cur_task_us));
                            tlkapi_send_string_u32s(DBG_SET_CIG_PARAMS, "[CISC][PAR] ft, trns_latncy",     ftLoca_max, \
                                                                                        ftpeer_max, \
                                                                                        blt_debug_hex_2_dec_display(trnsp_lat_loca), \
                                                                                        blt_debug_hex_2_dec_display(trnsp_lat_peer));
        ...
        tlkapi_send_string_u32s(DBG_SET_CIG_PARAMS, "[CISC][PAR] ISO interval traverse",   traverse_cnt, \
                                                                        blt_debug_hex_2_dec_display(isoIntvlUs_traverse), \
                                                                        blt_debug_hex_2_dec_display(cig_task_us), \
                                                                        blt_debug_hex_2_dec_display(nse_cig_traverse));
            ...
            for(int k=1; k<=4; k++){
                ...
                if(cur_nse < cig_nse_thres){
                    ...
                    if(cigSyDly_min + 1500 < isoIntvlUs_traverse){
                        ...
                        (void)trnsp_lat_loca; //remove compiler warning
                        (void)trnsp_lat_peer; //remove compiler warning
                        tlkapi_send_string_u32s(DBG_SET_CIG_PARAMS, "[CISC][PAR] framed, allocate OK",    blt_debug_hex_2_dec_display(isoIntvlUs_traverse), \
                                                                                    blt_debug_hex_2_dec_display(cig_task_us), \
                                                                                    nse_final_use, \
                                                                                    blt_debug_hex_2_dec_display(cur_task_us));
                        tlkapi_send_string_u32s(DBG_SET_CIG_PARAMS, "[CISC][PAR] ft, trns_latncy",     ftLoca_max, \
...
//          pCisConn->bn_peer = ;
        ...
        tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] iso int, nse, pdu",    \
                                                            blt_debug_hex_2_dec_display(pCigMaster->cism_intvl_us), \
                                                            pCisConn->nse, \
                                                            blt_debug_hex_2_dec_display(pCisConn->max_pdu_loca), \
                                                            blt_debug_hex_2_dec_display(pCisConn->max_pdu_peer));
...
ble_sts_t   blc_hci_le_setCigParamsTest(hci_le_setCigParamTest_cmdParam_t* pCmdParam,   hci_le_setCigParam_retParam_t* pRetParam)
{
    ...
    */
    ...
    ///// find available CIS connection for current CIG ///////////////////
    #if (DBG_CIS_CENTRAL_PARAM)
        tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] cis cnt, iso int, sdu int",    \
                                                                    pCmdParam->cis_count, \
                                                                    blt_debug_hex_2_dec_display(pCmdParam->iso_intvl*1250), \
                                                                    blt_debug_hex_2_dec_display(pCigMaster->sdu_int_loca), \
                                                                    blt_debug_hex_2_dec_display(pCigMaster->sdu_int_peer));
...
ble_sts_t   blc_hci_le_setCigParamsULL(hci_le_setCigParamTest_cmdParam_t* pCmdParam,    hci_le_setCigParam_retParam_t* pRetParam)
{
    ...
    */
    ...
    /* check BN, NSE, FT */
    //FT must equal to 1
    if(pCmdParam->ft_m2s != 1 || pCmdParam->ft_m2s != 1){
        pRetParam->status = HCI_ERR_INVALID_HCI_CMD_PARAMS;
...
ble_sts_t   blc_hci_le_createCis(hci_le_CreateCisParams_t* pCisPara)
{
    ...
    */
    ...
    for(i=0; i<pCigMaster->cism_set_cnt; i++){
        ...
        tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] cis timing 1",   blt_debug_hex_2_dec_display(pCisConn->MPTM_TIFS_MPTS), \
                                                                     blt_debug_hex_2_dec_display(pCisConn->se_length_us), \
                                                                     pCisConn->nse, \
                                                                     blt_debug_hex_2_dec_display(pCisConn->cis_task_us));
    }
 ...
 */
_attribute_no_inline_
ble_sts_t   blt_ll_createCisCancel(ll_cis_conn_t *pCisConn)
{
...
_attribute_no_inline_
ble_sts_t   blc_hci_le_removeCig(u8 cigId, hci_le_removeCig_retParam_t* pRetParam)
{
...
_attribute_noinline_
void blt_ll_cism_check_other_cis_create(ll_cis_conn_t * pCisConn, ll_cig_mst_t *pCigMaster)
{
...
_attribute_noinline_
int         blt_ll_cis_master_cis_establish(ll_cis_conn_t * pCisConn, ll_cig_mst_t *pCigMaster, ble_sts_t status)
{
    ...
    if(hci_le_eventMask & HCI_LE_EVT_MASK_CIS_ESTABLISHED){
        hci_le_cisEstablished_evt(status, pCisConn->cis_connHandle, (u8*)&pCigMaster->cigm_sync_delay,
                                 (u8*)&pCisConn->cis_sync_delay, (u8*)&pCisConn->transLaty_m2s,
                                 (u8*)&pCisConn->transLaty_s2m, pCisConn->curCisPhy, pCisConn->curCisPhy,
                                 pCisConn->nse, pCisConn->bn_loca, pCisConn->bn_peer, pCisConn->ft_loca,
                                 pCisConn->ft_peer, pCisConn->max_pdu_loca, pCisConn->max_pdu_peer,
                                 pCigMaster->cism_isoIntvl);
    }
...
bool        blt_ll_sendCisReq(st_ll_conn_t *pAclConn, ll_cis_conn_t *pCisConn, ll_cig_mst_t *pCigMaster)
{
    ...
    #if 1
            tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] SDU PDU",  pCisConn->max_sdu_loca, \
                                                                pCisConn->max_sdu_peer, \
                                                                pCisConn->max_pdu_loca, \
                                                                pCisConn->max_pdu_peer);
    #endif
    ...
    tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] cis timing 2",  blt_debug_hex_2_dec_display(pCisConn->MPTM_TIFS_MPTS), \
                                                                blt_debug_hex_2_dec_display(pCisConn->se_length_us), \
                                                                blt_debug_hex_2_dec_display(pCisConn->cis_maxPossible_us),
                                                                blt_debug_hex_2_dec_display(oftMaxUs_spec));
    ...
    if(pCigMaster->cism_estab_cnt == 0)
    {
        ...
        tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] cig timing", blt_debug_hex_2_dec_display(pCigMaster->task_total_us), \
                                                                 blt_debug_hex_2_dec_display(cigm_maxPossible_us), \
                                                                 blt_debug_hex_2_dec_display(iso_use_us),
                                                                 blt_debug_hex_2_dec_display(cigm_idle_us));
        ...
        tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] cig idle, range", blt_debug_hex_2_dec_display(temp_cigm_idle_1), \
                                                                    blt_debug_hex_2_dec_display(temp_cigm_idle_2), \
                                                                    blt_debug_hex_2_dec_display(offset_min_add_margin),
 ...
 */
    ...
    else
    {
        ...
        if(oftMinUs > oftMaxUs_spec){
            tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] offset min ERROR",  blt_debug_hex_2_dec_display(oftMinUs), \
                                                                            blt_debug_hex_2_dec_display(oftMaxUs_spec), \
                                                                            blt_debug_hex_2_dec_display(pCisConn->cis_maxPossible_us), \
                                                                            0);
    ...
    tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] ap_distan_us 1", pCisConn->cism_alloc_position, \
                                                                   blt_debug_hex_2_dec_display(pCisConn->cig_ap_distan_us), \
                                                                   0, 0);
    ...
    if(status){     //always consider that: push TX FIFO may not success
        ...
        pAclConn->ll_rsp_timeout_tick = clock_time() | 1;
            ...
            tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] pdu, nse, sub_int",    blt_debug_hex_2_dec_display(pCisConn->max_pdu_loca), \
                                                                            blt_debug_hex_2_dec_display(pCisConn->max_pdu_peer), \
                                                                            pCisConn->nse, \
                                                                            blt_debug_hex_2_dec_display(pCisConn->sub_intvl_us));
            tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] bn ft", pCisConn->bn_loca, pCisConn->bn_peer, pCisConn->ft_loca, pCisConn->ft_peer);

            tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] iso int, OFFSET, acl_evt",     blt_debug_hex_2_dec_display(pCigMaster->cism_intvl_us), \
                                                                                    blt_debug_hex_2_dec_display(pCisConn->own_cisOffsetMin_us), \
                                                                                    blt_debug_hex_2_dec_display(pCisConn->own_cisOffsetMax_us), \
                                                                                    blt_debug_hex_2_dec_display(pCtrlCisReq->connEventCnt));
        #endif
    }
...
bool        blt_ll_sendCisInd(st_ll_conn_t *pAclConn, ll_cis_conn_t *pCisConn, ll_cig_mst_t *pCigMaster)
{
    ...
    //peerCisOffset_diff
    tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] ap_distan_us 2", blt_debug_hex_2_dec_display(0), \
                                                                 blt_debug_hex_2_dec_display(pCisConn->cisOffset_use), \
                                                                 blt_debug_hex_2_dec_display(pCisConn->cisOffset_use - pCisConn->own_cisOffsetMin_us), \
                                                                 blt_debug_hex_2_dec_display(pCisConn->cig_ap_distan_us));
    ...
    if((u16)(eventCnt_past + 5) < BIT(14)){
        entcnt_change = 1;
        tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CIS][TIM] evtcnt change, instant not enough", blt_debug_hex_2_dec_display(eventCnt_now), \
                                                                                        blt_debug_hex_2_dec_display(cisConn_param.cis_1st_anchor_evtCnt), \
                                                                                        tick_now, cisConn_param.cis_1st_anchor_tick);
    }
    if(!entcnt_change && tick1_exceed_tick2(tick_safe, cisConn_param.cis_1st_anchor_tick)){
        entcnt_change = 1;
        tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] evtcnt change, 80ms not enough",    blt_debug_hex_2_dec_display(eventCnt_now), \
                                                                                        blt_debug_hex_2_dec_display(cisConn_param.cis_1st_anchor_evtCnt), \
                                                                                        tick_now, cisConn_param.cis_1st_anchor_tick);
    }
    ...
    if(entcnt_change){ //
        ...
        tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] evtcnt change", blt_debug_hex_2_dec_display(acl_evt_inc), \
                                                                    blt_debug_hex_2_dec_display(cisConn_param.cis_1st_anchor_evtCnt), 0, 0);
    }
#endif
    ...
    if(status){
        ...
        blt_cis_connect_common(pAclConn, pCisConn);
            ...
            tlkapi_send_string_u32s(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] cis ind, OFFSET", blt_debug_hex_2_dec_display(pCisConn->cisOffset_use), \
                                                                          blt_debug_hex_2_dec_display(pCigMaster->cigm_sync_delay), \
                                                                          blt_debug_hex_2_dec_display(pCisConn->cis_sync_delay), \
                                                                          blt_debug_hex_2_dec_display(cisConn_param.cis_1st_anchor_evtCnt));
        #endif
...
ble_sts_t   blt_ll_cis_master_control_pdu_process(st_ll_conn_t* pAclConn, u8 opcode, u8 *pLlCtrlPkt)
{
    ...
    if(opcode == LL_CIS_RSP) //only master receive this opcode
    {
        ...
        tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] cis rsp OFFSET", blt_debug_hex_2_dec_display(pCisConn->peer_cisOffsetMin_us), \
                                                               blt_debug_hex_2_dec_display(pCisConn->peer_cisOffsetMax_us), \
                                                               blt_debug_hex_2_dec_display(pCisRsp->connEventCnt), \
                                                               pCisConn->align_with_acl);
            ...
            if(peer_oftMin_us > peer_oftMax_us){
                ...
                tlkapi_send_string_data(DBG_CIS_1ST_AP_TIMING_EN, "[CISC][TIM] ERROR, offset min bigger than max", 0, 0);
                    ...
                    if(peer_oftMin_us > pCisConn->iso_intvl_us){
                        ...
                        tlkapi_send_string_u32s(DBG_CIS_CENTRAL_PARAM, "[CISC][PAR] offset after adjust", blt_debug_hex_2_dec_display(peer_oftMin_us), \
                                                                                      blt_debug_hex_2_dec_display(peer_oftMax_us), \
                                                                                      0, 0);
...
_attribute_ram_code_
static int  blt_ll_find_next_cis_mst_subevent (int start_idx)
{
    ...
    if(blt_pCigMst->first_ap_update){
        ...
        if(blt_pCigMst->tx_trigger_diff_tick > (IRQ_CTX_DELAY_US+20+2)*SYSTEM_TIMER_TICK_1US || \
           blt_pCigMst->tx_trigger_diff_tick < (IRQ_CTX_DELAY_US-2)*SYSTEM_TIMER_TICK_1US){
            ...
            tlkapi_send_string_u32s(DBG_CIS_MASTER_IRQ_TIMING_DUMPLOG, "tx trigger tick err", blt_pCigMst->cigm_expect_tick, bltSche.sSlot_tick_irq,\
                                                                           blt_pCigMst->tx_trigger_diff_tick, 0);
...
_attribute_ram_code_
int         blt_ctx_post(ll_cis_conn_t *pCisConn)
{
...
_attribute_ram_code_
int blt_cig_mst_calculate_timing(ll_cig_mst_t *pCigMaster, int type)
{
