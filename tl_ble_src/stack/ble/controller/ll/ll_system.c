...
_attribute_ble_data_retention_ ll_task_callback_0_t ll_cs_rawData_process_cb     = NULL;
_attribute_ble_data_retention_ ll_task_callback_0_t ll_cs_hci_subevent_report_cb = NULL;


//Monitoring Advertisers
_attribute_ble_data_retention_ ll_task_callback_2_t ll_mon_adv_mlp_task_cb = NULL;
...
_attribute_ram_code_sec_optimize_o2_
    ...
    blc_sdk_irq_handler(void) //blt_ll_updateScheduler //irq_system_timer
{
    ...
    if (src_rf & FLD_RF_IRQ_RX) {
        ...
        if (blms_state == BLMS_STATE_PRICHN_SCAN_S) {
            ...
            if (ll_cs_reflector_irq_task_cb) {
                DBG_CS_CHN13_TOGGLE;
                ll_cs_reflector_irq_task_cb(FLAG_CS_STEP_RX); //blt_cs_reflector_irq_task  blt_cs_refl_rx
            }
        }
#endif
#if (LL_RSSI_SNIFFER_MASTER_ENABLE)
    ...
    if (src_rf & BLMS_FLG_RF_CONN_DONE) {
        ...
        if (blms_state & (BLMS_STATE_BTX_S | BLMS_STATE_BRX_S)) {
            ...
            blt_pm_setWfiWakeupTick(clock_time() + 50 * SYSTEM_TIMER_TICK_1US);
#endif
...
#endif
#if (LL_FEATURE_ENABLE_CHANNEL_SOUNDING)
        else if (blms_state & (BLMS_STATE_CS_INIT_TX_S | BLMS_STATE_CS_INIT_RX_S | BLMS_STATE_CS_REFL_STEP_S))
        {
            DBG_CS_CHN12_TOGGLE;
            blmsParam.stimer_irq_process_en = 1;
        }
...
#if(LL_FEATURE_ENABLE_CHANNEL_SOUNDING && (MCU_CORE_TYPE == MCU_CORE_B92) && HARDWARE_CHANNEL_SOUNDING_SUPPORT_EN)
        //if cs procedure is ranging, baseband reset will affect phase continuity between Subevents,Because modem reset will cause CS phase continuity related registers to be lost.
        //Jaguar Linklayer reset will affect modem reset.So when cs procedure is ranging, prohibited do baseband reset
        //Tercel Linklayer reset and Modem reset can do separately.

        if(blmsParam.cs_procedure_busy == 1){
            /*
             * During the cs procedure, if the tx state is not finished and the state machine is forced to stop in advance,
             * it may lead to a tx exception, and then disable dma chn can solve the problem. This method cannot solve the S2/S8 mode.
             */
...
#endif
        {
            //This action is to fix tx dma exception caused by force stopping the state machine when RF machine is Tx. reset linklayer can fix it.
...
#endif
    _attribute_ram_code_ void
    irq_system_timer(void)
{
    if (systick_irq_trigger == SYS_IRQ_TRIG_NEW_TASK) {
        #if (SCH_DEBUG_EN)
            /**
             * @brief Sync with Ronglu and Qiuwei. Here are some conditions that will lead to a specific exception.
             *
             * When the following conditions are met:
             * 1. In BLE/Thread/ZigBee triple modes(or other multi-modes), and the BLE controller is not enabled.
             * 2. The Stimer is at the capture value, but the stimer_irq is not enabled.
             * 3. Other RF_irq from ZigBee trigger the rf_irq to call the blc_sdk_irq_handler function.
             *
             * Then, the irq_system_timer function will be called. Meanwhile, the bltSche.pTask_next pointer points to NULL,
             * which will cause an exception.
             */
            if (NULL == bltSche.pTask_next) {
                BLMS_ERR_DEBUG(SCH_DEBUG_EN, 0xFF020000);
                return;
            }
        #endif
    ...
    #if (BLMS_PM_ENABLE)
        if (blmsPm.pm_entered) {
            tick_margin = 500 * SYSTEM_TIMER_TICK_1US;
        }
    #endif
        ...
        if (slotTask_flg == TSKFLG_ACL_MASTER || slotTask_flg == TSKFLG_ACL_SLAVE) {    //BTX start or BRX start
...
#endif
#if LL_FEATURE_SUPPORT_LE_EXTENDED_SCANNING
    ...
    } else                                                              //post task
    {
        ...
        else if (systick_irq_trigger & (SYS_IRQ_TRIG_CTX_START | SYS_IRQ_TRIG_CRX_START | SYS_IRQ_TRIG_CTX_POST | SYS_IRQ_TRIG_CRX_POST | SYS_IRQ_TRIG_BIS_TX_START | SYS_IRQ_TRIG_BIS_TX_POST | SYS_IRQ_TRIG_BIS_RX_START | SYS_IRQ_TRIG_BIS_RX_POST)) {
...
_attribute_ram_code_ int blc_pm_handler(void)
{
    /* sleep_allowed is IRQ variable, need check again;  bltSche.task_mask can be guaranteed by  sleep_allowed, no need check */
    if (ll_module_pm_cb &&
        blmsPm.sleep_mask != PM_SLEEP_DISABLE && ((blmsPm.sleep_taskMask & bltSche.task_mask) == bltSche.task_mask) && blmsPm.sleep_allowed && !blmsParam.state_chng && !tlkapi_debug_isBusy()) {
...
#if (TIFS_VARIATION_WORKAROUND_MLP_CODE_IN_RAM || ESL_CURRENT_OPTIMIZATION)
_attribute_ram_code_
#else
_attribute_no_inline_
#endif
    void
    blc_sdk_main_loop(void)
{
#if (LL_RSSI_SNIFFER_MODE_ENABLE)
    /* Feeding dog here is only used for the RSSI/CS Sniffer SDK, watch dog has been enabled at the very beginning of cstartup.S
     * Other SDKS are not affected
     */
    ...
    if (ll_module_pm_cb && blmsParam.sdk_mainloop_flg && !blmsParam.state_chng && !blt_isOsSupEnable() && !tlkapi_debug_isBusy()) {
        if (blmsParam.rf_fsm_busy) {
#if (ESL_CURRENT_OPTIMIZATION) //  , MCU stall must enable, and the next brunch has issue.
            // Check if entering WFI (Wait For Interrupt) mode is allowed, based on the value of blmsPm.wfi_mask
            if (1) { // If wfi_mask is not equal to WFI_DISABLE, it indicates that entering WFI mode is allowed

                // Define a flag variable to indicate whether entering WFI mode is allowed
                unsigned char wfi_enter_en = 0;

                u32 r = irq_disable();

                // Check if the following conditions are all met:
                // 1. The read pointer and write pointer of the ACL receive FIFO (blt_rxfifo) are equal, indicating that the FIFO is empty
                // 2. The read pointer and write pointer of the scan receive FIFO (scan_priRxFifo) are equal, indicating that the FIFO is empty
                // 3. No connection event, disconnection event, connection update event, or PHY update event masks are set
                if (blt_leg_scan_checkSleepReady() && blt_acl_checkSleepReady() && blt_ext_scan_checkSleepReady()) {
                    // If all the above conditions are met, allow entering WFI mode
                    wfi_enter_en = 1;
                }

                irq_restore(r);

                // If entering WFI mode is allowed, call the function to enter WFI mode
                if (wfi_enter_en && blmsParam.rf_fsm_busy) {
                    core_entry_wfi_mode();
                }
            }
#else
            if (blmsPm.wfi_mask != WFI_DISABLE) { // If wfi_mask is not equal to WFI_DISABLE, it indicates that entering WFI mode is allowed
                ...
                // Check if the BLE is in peripheral state (BLMS_STATE_BRX_S)
                if (blms_state == BLMS_STATE_BRX_S && (blmsPm.wfi_mask & WFI_ACL_PERIPHR)) {
                    wfi_enter_en = 1;
                } else {
                    wfi_enter_en = 0; //Others not supported
                }

                u32 tick_margin = 20 * SYSTEM_TIMER_TICK_1US;
                if (!tick1_exceed_tick2(blmsPm.wfi_wakeup_tick, clock_time() + tick_margin)) {
                    wfi_enter_en = 0;
                }
                ...
                // If entering WFI mode is allowed, call the function to enter WFI mode
                if (wfi_enter_en) {
                    u32         wfi_wakeup_tick = (u32)(blmsPm.wfi_wakeup_tick - clock_time());
                    extern void blc_pm_WfiDelayWakeup_begin(u32 tick);
                    blc_pm_WfiDelayWakeup_begin(wfi_wakeup_tick);
                    core_entry_wfi_mode();
                    extern void blc_pm_WfiDelayWakeup_end(void);
                    blc_pm_WfiDelayWakeup_end();
                }
            }
#endif
        } else {
            if (blmsPm.sleep_mask != PM_SLEEP_DISABLE && ((blmsPm.sleep_taskMask & bltSche.task_mask) == bltSche.task_mask) && blmsPm.sleep_allowed) {
                int sleep_enter_en = 0;
                u32 r              = irq_disable();


                if (blmsPm.sleep_allowed && !blmsPm.slave_no_sleep &&
                    blt_leg_scan_checkSleepReady() && blt_acl_checkSleepReady() && blt_ext_scan_checkSleepReady()) {
        #if ACL_SLAVE_PM_LATENCY_EN
                    u32 tick_margin = (blmsPm.pTask_wakeup->scheTask_flg == TSKFLG_ACL_SLAVE ? 10 : PM_MIN_SLEEP_US) * SYSTEM_TIMER_TICK_1US;
                    if (tick1_exceed_tick2(blmsPm.next_task_tick, clock_time() + tick_margin))
        #else
    ...
    if (ll_module_pm_cb && blmsParam.sdk_mainloop_flg &&
        ...
        #else
    if (ll_module_pm_cb && blmsParam.sdk_mainloop_flg &&
        blmsPm.sleep_mask != PM_SLEEP_DISABLE && ((blmsPm.sleep_taskMask & bltSche.task_mask) == bltSche.task_mask) && blmsPm.sleep_allowed && !blmsParam.state_chng && !blt_isOsSupEnable() && !tlkapi_debug_isBusy())
        #endif
    {
        int sleep_enter_en = 0;
        u32 r              = irq_disable();
    ...
    #endif //#if (MCU_STALL_EN)

#endif     //#if (BLMS_PM_ENABLE)

           /*
    * This blmsParam.sdk_mainloop_flg indicates that the operation MUST be executed after (ll_module_pm_cb).
    *  The reason is that if it is placed before PM, the system will quickly enter sleep mode,
    *  causing the main_idle_loop and other application-layer codes to fail to execute.
    * */
    blmsParam.sdk_mainloop_flg = 1; // must
}
