...
typedef struct __attribute__((packed)){
    ...
    //u8    ext_hdr_len     :6;
    //u8    adv_mode        :2;
    u8  extHdrLen_advMode;

    u8  ext_hdr_flg;

    u8  advA[6];
    u8  targetA[6];
}rf_pkt_aux_conn_rsp_2_t;
    ...
    if(flag == FLAG_SCHEDULE_EXTINIT_CHECK_CONNRSP){
        ...
        while(1)
        {
            if(HAL_GET_RF_RX_IRQ)
            {
                ...
                {
                    rf_pkt_aux_conn_rsp_2_t * pConnRsp = (rf_pkt_aux_conn_rsp_2_t *) (glb_temp_rx_buff + DMA_RFRX_LEN_HW_INFO);
                    if(pConnRsp->rf_len == 14 && pConnRsp->extHdrLen_advMode == 13 && pConnRsp->ext_hdr_flg == (EXTHD_BIT_ADVA | EXTHD_BIT_TARGETA)){
                    #if (LL_ASYNC_LEA_EN)
                        if(bltScn.asyncScanIndex)
                        {
                            ...
                            aclApTick = (u32)pConnRsp->advA[0]\
                                       |(u32)pConnRsp->advA[1]<<8\
                                       |(u32)pConnRsp->advA[2]<<16\
                                       |(u32)pConnRsp->advA[3]<<24;

                            cisAPTick = (u32)pConnRsp->advA[4]\
                                           |(u32)pConnRsp->advA[5]<<8\
                                           |(u32)pConnRsp->targetA[0]<<16\
                                           |(u32)pConnRsp->targetA[1]<<24;

                            cisSyncDelayTick = (u32)pConnRsp->targetA[2]\
                                              |(u32)pConnRsp->targetA[3]<<8\
                                              |(u32)pConnRsp->targetA[4]<<16\
                                              |(u32)pConnRsp->targetA[5]<<24;
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Extended_Create_Connection", pCmdParam, sizeof(hci_le_ext_createConn_cmdParam_t));
