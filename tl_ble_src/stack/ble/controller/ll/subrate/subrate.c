...
ble_sts_t   blt_ll_subrate_process_subrate_req(st_ll_conn_t* pAclConn, u8 *pLlCtrlPkt)
{
        ...
        else
        {
            ...
            if((pAcptPara->valid)&&((max_latency_req > pAcptPara->factor_max) || (timeout_req > pAcptPara->subrate_timeout) ||
                    (factor_max_req<pAcptPara->factor_min) || (factor_min_req > pAcptPara->factor_max)
                    ||((pAclConn->conn_intvl_n_1m25*1250*factor_min_req*(max_latency_req+1))*2 >= timeout_req*10000)))
            {
                ret = HCI_ERR_UNSUPPORTED_FEATURE_PARAM_VALUE;
            }
        }
...
ble_sts_t   blt_ll_subrate_control_pdu_process(st_ll_conn_t* pAclConn, u8 opcode, u8 *pLlCtrlPkt)
{
    if(opcode == LL_SUBRATE_IND){
            ...
            if((factor==0) || (factor>500) || (continue_num>=factor)
                    || (latency>0x1F3)|| (timeout<0x0a) || (timeout>0xc80) )
...
//              blt_llms_rejectInd(pAclConn->acl_conHandle, LL_SUBRATE_IND, HCI_ERR_UNACCEPTABLE_CONN_PARAMETERS, 1);
                ...
                if((pAclConn->factor != factor) || (pAclConn->conti_num != continue_num) ||
                        (pAclConn->per_latency != latency) || (pAclConn->conn_timeout!= timeout*10000*SYSTEM_TIMER_TICK_1US))
                { //conn para change
                ...
                if(pAclConn->subrate_flag.bit.subrate_update_flag){
                    ...
                    my_dump_str_u32s(DBG_SUBRATE_EN, "LL_SUBRATE_IND", pAclConn->factor,pAclConn->conti_num,
                                                                pAclConn->subrateBaseEvent, pAclConn->conn_timeout);
...
bool blt_ll_sendSubrateInd(u16 connHandle){
    ...
    if(!blt_ll_isEncryptionBusy(connHandle) && !(pAcl->conn_update_union.update_mark &(CONN_UPDATE_CMD|CONN_UPDATE_PENDING|CONN_UPDATE_NEARBY))
            )//&& (blt_ll_getRealTxFifoNumber(connHandle)==0)
    {
        ...
        if(blt_llmsPushLlCtrlPkt(connHandle, LL_SUBRATE_IND, dat)==TRUE)
        {
            ...
            if((pAcl->factor != pAcl->factor_next) || (pAcl->conti_num != pAcl->conti_num_next) ||
                    (pAcl->per_latency != pAcl->per_latency_next) || (pAcl->conn_timeout!=pAcl->subrate_timeout_next*10000*SYSTEM_TIMER_TICK_1US))
            { //conn para change
...
bool blt_hci_checkSubrateParams(u16 factor_min, u16 factor_max, u16 max_latency, u16 continue_num, u16 subrate_timeout){
    ...
    if((factor_min==0) || (factor_min>factor_max) || (factor_min>500) || (factor_max>500) || (max_latency>0x1F3)\
            || (continue_num>=factor_max) || (subrate_timeout<0x0A) || (subrate_timeout>0x0c80)|| (factor_max*(max_latency+1) > 500)){
        return FALSE;
    }
    return TRUE;
}
...
_attribute_ram_code_
u16 blt_ll_subrate_calNextEvent(st_ll_conn_t* pc, u16 e, u16 factor, u16 *pBase){
