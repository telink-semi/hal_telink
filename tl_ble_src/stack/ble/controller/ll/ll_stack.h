 ...
 * @return     none
 */
void blc_ll_setCustomTxPowerEnable(u8 en, rf_power_level_index_e tx_pwr_1m, rf_power_level_index_e tx_pwr_2m, rf_power_level_index_e tx_pwr_coded);
...
enum
{
    ...
    FLAG_MON_ADV_DATA_REPORT_LEGADV = BIT(20),
    FLAG_MON_ADV_DATA_REPORT_EXTADV = BIT(19),
...
#define BLMS_STATE_CS_REFL_STEP_TX_S BIT(17) //TODO yuexin 2024/07/08
...
_attribute_aligned_(4) typedef struct
{
    ...
    u32 fsu_en            : 1; //Frame Space Update
    u32 reserved1         : 31;
...
extern ll_task_callback_2_t ll_mon_adv_mlp_task_cb;
...
// todo: in master branch,can't change this,use a paramter to control, just when it's cs task, set to 10, don't influence other module
#ifndef MAX_CONFLICT_NUM
    #define MAX_CONFLICT_NUM 10
#endif
...
_attribute_aligned_(4) typedef struct __attribute__((packed)) sch_task_t
{
    ...
    u8                                         pawr_last_subevt;
    u8                                         pawr_subevt_idx;
    struct __attribute__((packed)) sch_task_t *next;
} sch_task_t;

_attribute_aligned_(4) typedef struct __attribute__((packed)) cs_sch_task_t
{
    sch_task_t task;
    u32        cs_subevent_seqNum; //u8 is ok
    u16        cs_procCnt;
    u8         cs_oft;
    u8         rsvd8;
} cs_sch_task_t;
...
#define TICKS_ABS_2_SSLOT_ABS(ticks_abs) ((SYSTICK_NUM_PER_US == 16) ?                                               \
                                              (u32)(((u64)(ticks_abs - bltSche.sSlot_tick_start) * 2 + 313) / 625) : \
                                              (u32)(((u64)(ticks_abs - bltSche.sSlot_tick_start) * 4 + 938) / 1875))
...
int blt_ll_addTask2ExistLinklistWithTaskSize(sch_task_t *pStart_schTsk, int task_size, int task_num_max);
...
#endif
    static inline void
    blt_sche_addTaskMask(u64 tskmsk)
{
...
#endif
    static inline void
    blt_sche_removeTaskMask(u64 tskmsk)
{
    bltSche.task_mask &= ~(tskmsk);
}
...
#endif
    static inline void
    blt_sche_enableTask(u64 tskmsk)
{
...
#endif
    static inline void
    blt_sche_disableTask(u64 tskmsk)
{
    bltSche.task_en &= ~(tskmsk);
}
...
#endif
    static inline void
    blt_sche_setSystemIrqTrigger(u32 trigger)
{
...
_attribute_aligned_(4) typedef struct
{
    ...
    u32 appWakeup_tick;
        ...
        u32     z_suspend_exitLatencyTick; //used for OS zephyr pm
        u32     z_deepret_exitLatencyTick; //used for OS zephyr pm
    #endif
...
typedef void (*ll_module_wfi_callback_t)(le_phy_type_t);
extern ll_module_wfi_callback_t ll_module_wfi_cb;
int                             blt_sleep_process(void);

#define blt_pm_setWfiWakeupTick(tick)  \
    {                                  \
        blmsPm.wfi_wakeup_tick = tick; \
    }
