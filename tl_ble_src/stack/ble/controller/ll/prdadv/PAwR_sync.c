...
u8* blt_search_eslGroupID_fromAdvData(u8* pAdvData, int advDataLen, data_type_t advType)
{
...
_attribute_ram_code_
u8  blt_search_eslID_fromLTVdata(esl_TLV_t* pLTVdata, int LTV_len, u8 eslID)
{
 ...
 */
ble_sts_t   blc_hci_le_setPAwRsync_rspData( u16 sync_handle, u16 req_pdaEvtCnt, u8 req_subEvtCnt, u8 rsp_subEvtCnt, u8 rsp_slotIdx,
                                            u8 rspDataLen,  u8* pRspData)
{
...
//  pPdaRspDataCtrl->pdaRspDataCtrl.req_subevt_idx  = req_subEvtCnt;
    ...
    my_dump_str_u32s(DBG_PAwR_SYNC_LOGIC, "set rsp data", pPdaRspDataCtrl->auxSyncSubevtRsp_header.ext_hdr_len,pPdaRspDataCtrl->auxSyncSubevtRsp_header.rf_len,\
                     rspDataLen, pPdaRspDataCtrl->auxSyncSubevtRsp_header.dma_len);
    ...
    blt_add_aux_scan_future_task(pSecChn->scnIndex, pSecChn->scnIndex + TSKOFT_PAWRS_RSP,\
                                 rspSlot_expectTick, rspSlot_expectTick + pPdaRspDataCtrl->pdaRspDataCtrl.rsp_slot_duration_us * SYSTEM_TIMER_TICK_1US);
...
_attribute_ram_code_
int blt_ll_PAwRsync_rspSlotTxProc(int flag)
{
...
//  rf_set_tx_rx_off();
    ...
    u32 tx_begin_tick = blt_pSecChnScn->pawr_rsp_tick + RSP_SLOT_EARLY_TICK - (TX_PATH_DELAY_1M + bltPHYs.tx_stl_adv)*SYSTEM_TIMER_TICK_1US - extraPreamble*bltPHYs.own_oneByte_us;
...
static inline int blt_ll_PAwR_AFH_Phy_chnMap_proc(void) //adapted channel hopping //blt_pda_start_common_1
{
    ...
    int pawrEvt_jump_num = 0; //pawrEvt_jump_num
     ...
     * 5 slot margin can handle 3.125 mS timing shit, and maximum 10 slot 6.25mS error no risk for pawrEvt_jump_num */

    pawrEvt_jump_num = (bltSche.bSlot_idx_irq_real + 5 - blt_pPda->bSlot_mark_prdadv)/blt_pPda->bSlot_prdadv_itvl;


    u16 tmpPdaEvtCnt = blt_pPda->paEvtCnt + pawrEvt_jump_num;

    if(tmpPdaEvtCnt != blt_pPdAsync->prePawrEvtCnt){

        blt_pPdAsync->prePawrEvtCnt = blt_pPda->paEvtCnt = tmpPdaEvtCnt;

        blt_pPdAsync->preSubevtIdx = 0;
        blt_pPda->bSlot_mark_prdadv += (blt_pPda->bSlot_prdadv_itvl*pawrEvt_jump_num);
        blt_pPdAsync->sSlot_mark_prdadv += (blt_pPda->bSlot_prdadv_itvl<<5);
    }
...
_attribute_ram_code_
void blt_ll_PAwRsync_build_task(void)
{
    u32 i,j;
    ...
    for(i=0; i<bltPawrSync.maxNum_pawrSync; i++)
    {
        if( bltSche.task_mask & (TSKMSK_PAWRS_SUB_0<<i) )
        {
            ...
            sSlot_subevent_task = sSlot_pawrEvt_start_task + int_jump_pawrSubevtTsk*pPdA_sync->subevtIntvl_sSlot;
            ...
            for(int k=0; k < pPdA_sync->pdaSyncSubevtCtrl.sync_subevt_num; k++){
            ...
            if((sSlot_pawrEvt_start_task + (*pPdA_sync->pSubevtOfBuild&0x7F)*pPdA_sync->subevtIntvl_sSlot) < bltSche.sSlot_idx_next){
            ...
            for(j=0;j<PAWR_SYNC_FIFONUM;j++){
                ...
                pCur_schTask->begin = nextSyncSubevtOffset_sSlot;
...
_attribute_ram_code_
void blt_ll_PAwRsync_subevtStart(u8 index)
{
...
_attribute_ram_code_
void blt_ll_PAwRsync_subevtPost(void)
{
...
_attribute_ram_code_
u8  blt_ll_PAwRsync_auxConnInd_proc(rf_pkt_ext_adv_t* pAuxConnReq)
{

    rf_pkt_adv_rx_t * pAdvRx = (rf_pkt_adv_rx_t *)(pAuxConnReq);

    if(smemcmp(pAdvRx->advA, pkt_Adv.advA, BLE_ADDR_LEN)){
    ...
    blt_quick_tx_prepare(FSM_STX, (void*)&pkt_aux_conn_rsp, pAdvRx->rf_len); //when 48M, diff is 31us.
        ...
        */
        do{
            ...
            */
            ...
            pkt_aux_conn_rsp.txAddr = BLE_ADDR_PUBLIC;
            smemcpy(pkt_aux_conn_rsp.advA, bltMac.macAddress_public, BLE_ADDR_LEN);
...
_attribute_ram_code_
u8 blt_ll_PAwRsync_auxSyncSubevtInd_Proc(rf_pkt_ext_adv_t * pExtAdv, u8* pExtHdrOffset)
{
...
_attribute_ram_code_
int blt_pawr_sync_sub_interrupt_task (int flag, void *p0, void* p1)
{
    int index = flag & FLAG_SCHEDULE_TASK_IDX_MASK;
