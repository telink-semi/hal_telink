...
#error "PAwR task number size error"
    ...
    for (int i = 0; i < num_periodic_adv; i++) {
        ...
        for (int j = 0; j < 15; j++) {
            ...
            cur_pPerdadv->pdaSubevtDataCtrl[j].subevent_idx = 0xFF; //invalid
        }
...
{
    if (pSubeventData == NULL || subeventDataLenMax == 0 || subeventDataLenMax > 251 || subeventDataCnt > 128) {
        return;
    }
...
_attribute_ram_code_ int blt_pawra_rsp_interrupt_task(int flag, void *p)
{
...
_attribute_ram_code_ int blt_pawra_rsp_start(int index)
{
    DBG_CHN9_HIGH;
...
_attribute_ram_code_ int blt_pawra_rsp_post(void)
{
...
_attribute_ram_code_ int blt_pawra_rsp_slot_start(void)
{
    DBG_CHN10_HIGH;
    DBG_CHN11_HIGH;
...
_attribute_ram_code_ int blt_pawra_rsp_slot_post(void)
{
    DBG_CHN10_HIGH;
...
_attribute_ram_code_ int irq_pawra_rsp_slot_rx(void)
{
    u8 *raw_pkt = ble_curr_rx_dma_buff; //or cisConn_param.cis_rx_dma_buff
...
_attribute_ram_code_ int blt_pawra_rsp_task_insert(int index)
{
    /*
    ...
    if (subevtEnd == 0xFF || subevtEnd == currSubevt || subevtStart == subevtEnd) {
        ...
        pPerdadv->subeventData_validPaEvt =
            (subevtStart == subevtEnd || pPerdadv->paSubEventCnt == pPerdadv->num_subevents) ? blt_pPda->paEvtCnt : (blt_pPda->paEvtCnt - 1); //attention!!!
...
_attribute_ram_code_ int blt_pawra_rsp_sch_build(void)
{
    st_prd_adv_t *cur_pPerdadv = NULL;
    ...
    for (int i = 0; i < bltFutTask.number; i++) {
        ...
        if (pFutTask->task_flg == TSKFLG_PAWRA_RSP) {
            ...
            if (tick1_exceed_tick2(bltSche.sSlot_tick_next, pFutTask->tick_s)) { //timing passed
                ...
                my_dump_str_u32s(0, "abandon pawr_rsp task: time passed", bltSche.sSlot_tick_next, pFutTask->tick_s, pFutTask->task_oft,
                                 bltPri.csctvAbandonCnt[pFutTask->task_oft]);
            } else {
...
_attribute_ram_code_ int blt_pawra_task_conflic(sch_task_t *pTgtTsk, u8 curSchTaskOft)
{
...
_attribute_ram_code_ int blt_pawra_subx_interrupt_task(int flag, void *p)
{
    int index = flag & FLAG_SCHEDULE_TASK_IDX_MASK;
...
_attribute_ram_code_ int blt_pawra_sub0_mark(int index)
{
    ...
    blt_pPerdadv->sSlot_mark_end_subx =
        blt_pPerdadv->sSlot_mark_subx + blt_pPerdadv->sSlot_subevent_itvl * (blt_pPerdadv->num_subevents - 1) + blt_pPda->sSlot_duration_pda + 10; //margin 220us.
...
_attribute_ram_code_ int blt_pawra_sub_prepare_connect(int index)
{
...
_attribute_ram_code_ int blt_pawra_subx_start(int index)
{
    DBG_CHN4_HIGH;
...
/* PHY switch, do not consider S2 */
    ...
    if (blt_pPerdadv->acad_chaged) {
        ...
        u32 pkt_periodic_us         = blt_phy_getRfPacketTime_us(pkt_periodic.rf_len, blt_pPda->pda_phy, bltPHYs.cur_own_CI);
        u32 pkt_periodic_targetTick = tx_begin_tick + (bltPHYs.tx_stl_adv + pkt_periodic_us + 100) * SYSTEM_TIMER_TICK_1US;

#if (ADV_DURATION_STALL_EN)
        cpu_stall_WakeUp_By_RF_SystemTick(IRQ_ZB_RT, FLD_RF_IRQ_TX, 0);
#else
        while (!HAL_GET_RF_TX_IRQ && tick1_exceed_tick2(pkt_periodic_targetTick, clock_time())) { //wait for TX finish
...
_attribute_ram_code_ int blt_pawra_subx_advdata_update(void)
{
...
_attribute_ram_code_ int blt_pawra_subx_sch_build(void)
{
    u16           int_jump_task;
...
_attribute_noinline_ static void blt_reset_pawra(void)
{
...
_attribute_noinline_ int blt_ll_procPeriodicAdvRspReportEvent(void)
{
    u8 *raw_pkt = NULL;
...
_attribute_noinline_ int blt_pawra_mainloop_task(int flag, void *p)
{
...
static bool blt_ll_advPeriodicChkRspDataItvl(st_prd_adv_t *cur_pPerdadv, u8 subevtDataLen, u16 subevtItvl)
{
    ...
    u16 firstPkt_extHdrLen = (cte_en ? EXTHD_LEN_1_CTE : 0) + (cur_pPerdadv->txPower_en_len ? EXTHD_LEN_1_TX_POWER : 0) + (cur_pPerdadv->include_ADI_flag ? EXTHD_LEN_2_ADI : 0) +
                             9 + /* ACAD for channel map update */ +1 /* Extended Header Flags */;
...
hci_le_setPeriodicAdvParamV2_retParam_t blc_ll_setPeriodicAdvParam_v2(adv_handle_t adv_handle, u16 advInter_min, u16 advInter_max, perd_adv_prop_t property, u8 numSubevents,
                                                                      u8 subeventInterval, u8 responseSlotDelay, u8 responseSlotSpace, u8 numResponseSlots)
{
    ...
    if (numSubevents != 0) {
        ...
        cur_pPerdadv->sSlot_duration_rsp =
            TICKS_DUR_2_SSLOT_DUR(cur_pPerdadv->response_slot_spacing * cur_pPerdadv->num_response_slots * SYSTEM_TIMER_TICK_125US + SLOT_PROCESS_MAX_TICK) + 1;
...
hci_le_setPeriodicAdvParamV2_retParam_t blc_hci_le_setPeriodicAdvParam_v2(hci_le_setPeriodicAdvParamV2_cmdParam_t *pCmdParam)
{
    ...
    return blc_ll_setPeriodicAdvParam_v2(pCmdParam->adv_handle, pCmdParam->advInter_min, pCmdParam->advInter_max, pCmdParam->property, pCmdParam->numSubevents,
                                         pCmdParam->subeventInterval, pCmdParam->responseSlotDelay, pCmdParam->responseSlotSpace, pCmdParam->numResponseSlots);
}
...
ble_sts_t blc_ll_extended_createConnection_v2(adv_handle_t adv_handle, u8 subevent, init_fp_t filter_policy, own_addr_type_t ownAdrType, u8 peerAdrType, u8 *peerAddr,
                                              init_phy_t init_phys, scan_inter_t scanInter_0, scan_wind_t scanWindow_0, conn_inter_t conn_min_0, conn_inter_t conn_max_0,
                                              conn_tm_t timeout_0, scan_inter_t scanInter_1, scan_wind_t scanWindow_1, conn_inter_t conn_min_1, conn_inter_t conn_max_1,
                                              conn_tm_t timeout_1, scan_inter_t scanInter_2, scan_wind_t scanWindow_2, conn_inter_t conn_min_2, conn_inter_t conn_max_2,
                                              conn_tm_t timeout_2)
{
...
ble_sts_t blc_hci_le_extended_createConnection_v2(hci_le_ext_createConnV2_cmdParam_t *pCmdParam)
{
    ...
    return blc_ll_extended_createConnection_v2(pCmdParam->adv_handle, pCmdParam->subevent, pCmdParam->fp, pCmdParam->ownAddr_type, pCmdParam->peerAddr_type, pCmdParam->peer_addr,
                                               pCmdParam->init_PHYs, pCmdParam->initCfg[0].scan_inter, pCmdParam->initCfg[0].scan_wind, pCmdParam->initCfg[0].conn_min,
                                               pCmdParam->initCfg[0].conn_max, pCmdParam->initCfg[0].timeout, pCmdParam->initCfg[1].scan_inter, pCmdParam->initCfg[1].scan_wind,
                                               pCmdParam->initCfg[1].conn_min, pCmdParam->initCfg[1].conn_max, pCmdParam->initCfg[1].timeout, pCmdParam->initCfg[2].scan_inter,
                                               pCmdParam->initCfg[2].scan_wind, pCmdParam->initCfg[2].conn_min, pCmdParam->initCfg[2].conn_max, pCmdParam->initCfg[2].timeout);
}
