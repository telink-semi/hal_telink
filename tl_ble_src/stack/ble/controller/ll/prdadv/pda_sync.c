...
_attribute_data_retention_ _attribute_aligned_(4) ll_prdadv_sync_t bltPdaSync;

    #if (PDA_SYNC_TIMING_ADJUST_EN)
_attribute_data_retention_ _attribute_aligned_(4) pda_syncTiming_t pda_sync_timingAdjust[TSKNUM_PDA_SYNC];
    #endif


//_attribute_aligned_(4)    extadv_id_t             pdaList_tbl[PERDADV_LIST_SIZE]; //periodic ADV list table
_attribute_data_retention_ _attribute_aligned_(4) pda_list_t pdaList_tbl[PERDADV_LIST_SIZE];   //periodic ADV list table

_attribute_data_retention_ _attribute_aligned_(4) pda_cache_t pdaCache_tbl[PERDADV_CACHE_NUM]; //periodic ADV device table, overwrite old one if overflow

_attribute_data_retention_ _attribute_aligned_(4) st_pda_sync_t pdAsync_tbl[TSKNUM_PDA_SYNC];
_attribute_data_retention_ _attribute_aligned_(4) st_pda_sync_t *blt_pPdAsync = NULL;


    #define PDA_TRUNCATED_EVT_SIZE 8
_attribute_data_retention_ le_periodAdvReportEvt_t pdaScan_truncatedEvt[TSKNUM_PDA_SYNC];
...
_attribute_noinline_ void blc_ll_initPeriodicAdvertisingSynchronization_module(void)
{
    ...
    bltPdaSync.pdaSync_timeout_us = 10 * 5000 * 1000;
...
_attribute_noinline_ void blt_reset_pda_sync(void)
{
...
_attribute_noinline_
    ble_sts_t
    blc_ll_periodicAdvertisingCreateSync(option_msk_t options, u8 adv_sid, u8 adv_adrType, u8 *adv_addr, u16 skip, sync_tm_t sync_timeout, u8 sync_cte_type)
{
    ...
    for (i = 0; i < end_idx; i++) {
        ...
        for (j = 0; j < PERDADV_CACHE_NUM; j++) {
            ...
            //remove blt_ll_searchAddrInWhiteListTbl(pPdA_cache->pda_dev_id.adrType, pPdA_cache->pda_dev_id.addr)). confirmed with yafei.
...
_attribute_no_inline_
    ble_sts_t
    blc_hci_le_periodic_advertising_create_sync(hci_le_periodicAdvCreateSync_cmdParam_t *cmdPara)
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_Periodic_Adv_Create_Sync", cmdPara, sizeof(hci_le_periodicAdvCreateSync_cmdParam_t));
    return blc_ll_periodicAdvertisingCreateSync(cmdPara->Options, cmdPara->Advertising_SID, cmdPara->Advertiser_Address_Type, cmdPara->Advertiser_Address, cmdPara->Skip, cmdPara->Sync_Timeout, cmdPara->Sync_CTE_Type);
}

_attribute_noinline_ int blt_pda_syncing_finish(void) //1. createSync_cancel;2. createSync_timeout
{
    int finish_valid = 0;
...
_attribute_noinline_ void blt_pda_sync_check_timeout(void)
{
...
_attribute_noinline_
    ble_sts_t
    blc_ll_periodicAdvertisingCreateSyncCancel(void)
{
    //HCI/GEV/BV-03-C [Disallow Mixing Legacy and Extended Scanning Commands]
    if (IS_LEGACY_SCAN_VALID) {
        return HCI_ERR_CMD_DISALLOWED;
...
_attribute_ram_code_ void blc_ll_pdaRxFifoPacketProc(st_secchn_scn_t *pSecChnScn)
{
    u32 r = irq_disable();
...
_attribute_noinline_
    ble_sts_t
    blc_ll_periodicAdvertisingTerminateSync(u16 sync_handle)
{
    //HCI/GEV/BV-03-C [Disallow Mixing Legacy and Extended Scanning Commands]
    if (IS_LEGACY_SCAN_VALID) {
        return HCI_ERR_CMD_DISALLOWED;
    ...
    blt_pda_sync_main_loop(void)
{
    ...
    #if (LL_FEATURE_ENABLE_PERIODIC_ADVERTISING_WITH_RESPONSES_SCANNER)
                if (pPdA_sync->pawr_acad_check) {
                    hci_le_periodicAdvSyncEstablished_evt_v2(status, sync_handle, pPdA_sync->pda_id.sid, pPdA_sync->pda_id.adrType, pPdA_sync->pda_id.addr, pPdA_sync->pda_rx.pda_phy, pPdA_sync->pda_interval, pPdA_sync->sca, pPdA_sync->pawr_acadInfo.num_subevent, pPdA_sync->pawr_acadInfo.subevent_intvl, pPdA_sync->pawr_acadInfo.rsp_slot_delay, pPdA_sync->pawr_acadInfo.rsp_slot_spacing);
                } else
    #endif
                {
                    hci_le_periodicAdvSyncEstablished_evt(status, sync_handle, pPdA_sync->pda_id.sid, pPdA_sync->pda_id.adrType, pPdA_sync->pda_id.addr, pPdA_sync->pda_rx.pda_phy, pPdA_sync->pda_interval, pPdA_sync->sca);
                }
            }
...
_attribute_ram_code_ void blt_ll_pda_sync_sslot_reset(void)
{
    st_pda_sync_t   *pPdA_sync;
...
_attribute_ram_code_ int blt_ll_period_bisAcad_process(u8 *bigInfor)
{
    ...
    if (!pBigInfor_para->creating_bisSync_flag) {
        ...
        pBigInfor_para->sSlot_idx_Rx     = bltSche.sSlot_idx_irq_real + (bltRxPkt.rx_header_tick - bltSche.sSlot_tick_irq_real) * SSLOT_TICK_REVERSE;
        ;                                                                  //SYSTEM_TIMER_TICK_625US
...
_attribute_ram_code_ int blt_ll_period_chmUptAcad_process(u8 *pChmUptAcad)
{
...
_attribute_ram_code_ int blt_pda_sync_interrupt_task(int flag, void *p)
{
    ...
    if (flag & FLAG_SCHEDULE_START) {
        ...
        if (bltPri.csctvAbandonCnt[curSchTaskOft] >= 5) {
            my_dump_str_data(0, "[per_sync]consecutive abandon count", &bltPri.csctvAbandonCnt[curSchTaskOft], 2) return 1; /* 1:conflict resolved; 0: insert task failed */
        }
            ...
            my_dump_str_data(0, "[per_sync]abandon, bis_SC proc", &bltPri.csctvAbandonCnt[curSchTaskOft], 2) return 0;
        }
    #endif
    }
...
_attribute_noinline_ int blt_pda_sync_mainloop_task(int flag)
{
...
_attribute_ram_code_ void blt_pda_sync_analyze_prdadv_info(st_pda_sync_t *pPdA_sync, pda_cache_t *pPdA_cache)
{
    if (1) {
        ...
        u32        distance_us = 0;
        extern u32 offsetUsForLargeInterval;
        if (offsetUsForLargeInterval != 0) {
            distance_us              = offsetUsForLargeInterval;
            offsetUsForLargeInterval = 0;
        } else {
            distance_us = pPdA_cache->sncInf.syncPktOffset * (pPdA_cache->sncInf.offsetUnit == EXT_ADV_PDU_SYNC_OFFSET_UNITS_300_US ? 300 : 30);
...
_attribute_ram_code_ void blt_pda_sync_build_task(void)
{
    u32 i, j;
...
_attribute_ram_code_ void blt_pda_sync_start(u8 index)
{
    ...
    if (blt_pPdAsync->sync_state == SYNC_STATE_IDLE ||
        ...
        static int mark = 0;
        if (!mark) {
            my_dump_str_u32s(DBG_PDA_SYNC_TIMING, "first timing", blt_pSecChnScn->tolerance_peer_us, blt_pPdAsync->sync_early_set_us, bltPHYs.prmb_ac_us, 0);
        }
        mark = 1;
    }
...
_attribute_ram_code_ void blt_pda_sync_post(void)
{
    #if (LL_FEATURE_ENABLE_LE_AOA_AOD)
    ...
    if (blt_pPdAsync->sync_state == SYNC_STATE_SYNCING) {
        if (auxScnCmnParam.rx_received) {
            ...
            //lihaojie 20241224
            // use u64, because it will overflow for larger interval, 1250 = 1000 * 5 / 4, so it is more accurate to div 1000
            u64 tolerance                  = blt_pPdAsync->max_err_us_per_second * blt_pPdAsync->pda_interval * 5 / 4 / 1000;
            blt_pPdAsync->tolerance_pda_us = (u16)tolerance;
            ...
            blt_pPda->bSlot_mark_prdadv = bltSche.bSlot_idx_irq_real + (n_sSlot >> 5) - blt_pPda->bSlot_prdadv_itvl;
    #if (LL_FEATURE_ENABLE_PERIODIC_ADVERTISING_WITH_RESPONSES_SCANNER)
...
_attribute_ram_code_ void blt_pda_sync_rx(rf_pkt_ext_adv_t *pExtAdv)
{
    //u8 * raw_pkt;
...
int blt_ll_bigInfoAdvReport(unsigned char *raw_pkt)
{
    ...
    if (blt_isSyncHandleValid(syncHandle) == TRUE) {
        ...
        if (hci_le_eventMask_2 & HCI_LE_EVT_MASK_2_BIGINFO_ADVERTISING_REPORT) {
            ...
            hci_le_BigInfoAdvReport_evt(syncHandle, pBigInfo->numBis, pBigInfo->nse, pBigInfo->isoItvl, pBigInfo->bn, pBigInfo->pto, pBigInfo->irc, pBigInfo->maxPdu, sduItvl, pBigInfo->maxSdu, phy, framing, encrypt);
        }
...
_attribute_no_inline_ int blt_pda_sync_prdadv_data_report(void)
{
    u8 *pHolDataBuf = NULL;
...
_attribute_noinline_ int blt_ll_procPdaScanTruncatedPend(void)
{
    if (!(hci_le_eventMask & HCI_LE_EVT_MASK_PERIODIC_ADVERTISING_REPORT)) {
        return 0;
    }
...
_attribute_noinline_
    ble_sts_t
    blc_ll_periodicAdvertisingReceiveEnable(u16 sync_handle, sync_adv_rcv_en_msk enable)
{
    //HCI/GEV/BV-03-C [Disallow Mixing Legacy and Extended Scanning Commands]
    if (IS_LEGACY_SCAN_VALID) {
        return HCI_ERR_CMD_DISALLOWED;
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_Set_Periodic_Adv_Receive_Enable", cmdPara, sizeof(hci_le_setPeriodicAdvReceiveEnCmdParams_t));
