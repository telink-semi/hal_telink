...
_attribute_noinline_ void blt_reset_prd_adv(void)
{
    st_prd_adv_t *cur_pPerdadv;
...
_attribute_ram_code_ int blt_prd_adv_interrupt_task(int flag, void *p)
{
...
_attribute_noinline_ int blt_prd_adv_mainloop_task(int flag, void *p)
{
    int           index        = flag & FLAG_SCHEDULE_TASK_IDX_MASK;
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_Set_Periodic_Advertising_Parameters", pCmdParam, sizeof(hci_le_setPeriodicAdvParam_cmdParam_t));
    /* validate intervals */
...
void blt_prdadv_task_begin(st_prd_adv_t *cur_pPerdadv)
{
    ...
    //2.use trng_rand() to avoid different advertise set to use same mark.
    ...
    p_curPda->bSlot_mark_prdadv = (blmsParam.sche_run_flag ? bltSche.bSlot_idx_next : 0) + (trng_rand() & 0x1f); //TODO: 22-02-11 0x0f
    ...
    if (p_curPda->bSlot_prdadv_itvl * 625 > 2457300 && cur_pextadv->advInt_max * 625 > 2457300) {
        ...
        if (prdadv_bSlotMark > extadv_bSlotMark) {
            ...
            prdadv_bSlotMark = extadv_bSlotMark + extIntvlNum * extadv_advIntMax + (extadv_advIntMax >> 2) + (trng_rand() & 0x1f);

        } else {
            prdadv_bSlotMark = extadv_bSlotMark + (extadv_advIntMax >> 2) + (trng_rand() & 0x1f);
        }
    ...
    //  p_curPda->paEvtCnt = (trng_rand()&20) | 1;
...
_attribute_ram_code_ st_prd_adv_t *blt_ll_search_existing_perdAdv_index_by_advHandle(u8 adv_handle)
{
    st_prd_adv_t *cur_pPerdadv;
    for (int i = 0; i < bltPrdA.maxNum_perdAdv; i++) {
        cur_pPerdadv = (st_prd_adv_t *)(global_pPerdadv + i);
...
_attribute_ram_code_ int blt_ll_buildPerdAdvSchedulerLinklist(void)
{
...
_attribute_ram_code_ int blt_ll_aux_syncinfo_update(int extadv_index)
{
...
_attribute_ram_code_ void blt_prdadv_updatePram(st_prd_adv_t *cur_pPerdadv) //irq used optim latter
{
...
_attribute_ram_code_ int blt_prdadv_start(int slotTask_idx)
{
    DBG_CHN0_HIGH;
    ...
    u8  periodic_coded_phy_ind = bltPHYs.cur_own_CI;
