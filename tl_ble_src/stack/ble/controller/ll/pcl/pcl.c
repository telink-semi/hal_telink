...
static inline
ll_pcl_cb_t* blt_ll_findAvailablePcl(u16 connIdx)
{
    (void)connIdx;
...
int         blt_ll_pclInterruptTask(int flag, void*p)
{
...
//  u16 connHandle = conn_idx | BLM_CONN_HANDLE;
    ...
    switch(flag){
        ...
        case FLAG_PCL_MONITORING_ACL_RX_RSSI:{
            ...
            break;
        case FLAG_PCL_MONITORING_CIS_RX_RSSI:{ //Notice: called in cis rx IRQ, blt_pCisConn can be used here
            /* record cis_rx rssi */
...
#endif
int         blt_ll_pclMainloopTask(int flag, void *p)
{
...
static s8   blt_ll_pclGetRfActualTxPwr(s8 txPwr, bool needComp)
{
    ...
    else if (txPwr >=   1)  {  txPwr =   1;  }
    else if (txPwr >=   0)  {  txPwr =   0;  }
    else if (txPwr >=  -4)  {  txPwr =  -4;  }
    else if (txPwr >=  -8)  {  txPwr =  -8;  }
    else if (txPwr >= -12)  {  txPwr = -12;  }
    else if (txPwr >= -18)  {  txPwr = -18;  }
    else                    {  txPwr = -23;  }
...
static inline
void      blt_ll_pclRecordAclRcvdRSSI(st_ll_conn_t *pAclConn, s8 currRcvdRssi) //Used in ACL rx IRQ handler
{
    /* read register value */
...
static inline
void      blt_ll_pclRecordCisRcvdRSSI(ll_cis_conn_t *pCisConn, s8 currRcvdRssi) //Used in ACL rx IRQ handler
{
    /* read register value */
    currRcvdRssi = rf_get_rssi();
...
ble_sts_t blt_ll_pclSendReqProc(u16 connHandle, u8 phy, s8 delta, s8 txPwr)
{
...
_attribute_noinline_
ble_sts_t blt_ll_pclSendRspProc(u16 connHandle, u8 limitInfo, s8 delta, s8 txPwr, u8 apr)
{
...
_attribute_noinline_
ble_sts_t blt_ll_pclSendChgIndProc(u16 connHandle, u8 phy, u8 limitInfo, s8 delta, s8 txPwr, bool phychg)
{
...
_attribute_no_inline_
#endif
ble_sts_t blt_ll_pclControlPduProc(st_ll_conn_t* pAclConn, u8 opcode, u8 *pLlCtrlPkt)
{
    ll_pcl_cb_t *pPclCb = pAclConn->pPclCb;
    ...
    if(opcode == LL_POWER_CONTROL_REQ){
        ...
        if(pPclCb->pwrRptLocal && delta != 0){
            if(hci_le_eventMask_2 & HCI_LE_EVT_MASK_2_TRANSMIT_POWER_REPORTING){
                ...
                hci_le_transmitPwrRpting_evt(BLE_SUCCESS, pAclConn->acl_conHandle, LL_PWR_CTRL_RPTING_REASON_LOCAL, \
                        pPclCb->pc_reqPhy, txPwr, limitInfo, delta);
            }
        }
        ...
        if(pPclCb->rdRemoteTxPwr){
            ...
            if(hci_le_eventMask_2 & HCI_LE_EVT_MASK_2_TRANSMIT_POWER_REPORTING){
                ...
                hci_le_transmitPwrRpting_evt(BLE_SUCCESS, pAclConn->acl_conHandle, LL_PWR_CTRL_RPTING_READ_REMOTE, \
                        pPclCb->pc_reqPhy, pPcRsp->txPwr, pPcRsp->limitInfo, pPcRsp->delta);
            }
        }
            ...
            if(hci_le_eventMask_2 & HCI_LE_EVT_MASK_2_TRANSMIT_POWER_REPORTING){
                ...
                hci_le_transmitPwrRpting_evt(BLE_SUCCESS, pAclConn->acl_conHandle, LL_PWR_CTRL_RPTING_REASON_LOCAL, \
                        pPclCb->pc_reqPhy, pPcRsp->txPwr, pPcRsp->limitInfo, pPcRsp->txPwr - pPclCb->peerTxPwrLvl);
            }
        }
        ...
        if(pPclCb->pwrRptRemote && (pPclCb->peerTxPwrLvl != pPchgRsp->txPwr)){
            ...
            if(hci_le_eventMask_2 & HCI_LE_EVT_MASK_2_TRANSMIT_POWER_REPORTING)
            {
                ...
                hci_le_transmitPwrRpting_evt(BLE_SUCCESS, pAclConn->acl_conHandle, LL_PWR_CTRL_RPTING_REASON_REMOTE, \
                        pPclCb->pc_reqPhy, pPchgRsp->txPwr, pPchgRsp->limitInfo, pPchgRsp->delta);
            }
        }
    ...
    if(oldPhyTxPwr == adjNewPhyTxLvl){
        ...
        if(pPclCb->pwrRptLocal){
            ...
            if(hci_le_eventMask_2 & HCI_LE_EVT_MASK_2_TRANSMIT_POWER_REPORTING){
                ...
                hci_le_transmitPwrRpting_evt(BLE_SUCCESS, pc->acl_conHandle, LL_PWR_CTRL_RPTING_REASON_LOCAL, \
                    currTxPwrPhy, adjNewPhyTxLvl, limitInfo, delta);
            }
        }
    }
...
_attribute_ram_code_
#endif
void      blt_ll_pclInitParamsAftAclConnect(st_ll_conn_t *pAclConn) //called by blms_connect_common in IRQ
{
...
_attribute_ram_code_
#endif
void      blt_ll_pclAutoInitiateReqProc(st_ll_conn_t *pAclConn, ll_cis_conn_t *pCisConn) //called in
{
    ...
    if(pCisConn != NULL){//cis phy
        currRcvdRssi = pCisConn->rssi;
            ...
            if(lastCisRssi != pCisConn->rssi)my_dump_str_data(DBG_LL_PCL_EN, "RSSI[CIS]", &pCisConn->rssi, 1);
        ...
        currRcvdRssi = pAclConn->rssi;
            ...
            if(lastAclRssi != pAclConn->rssi)my_dump_str_data(DBG_LL_PCL_EN, "RSSI[ACL]", &pAclConn->rssi, 1);
...
_attribute_ram_code_
#endif
void      blt_ll_pclPathLossMonitorProc(st_ll_conn_t *pAclConn) //called in ACL_xxx irq
{
    ll_pcl_cb_t *pPclCb = pAclConn->pPclCb;
    if (pPclCb == NULL) return;
...
_attribute_ram_code_
#endif
void      blt_ll_pclPwrChgIndAftPhyUpt(st_ll_conn_t *pAclConn) //called in irq: blms_start_common_1
{
...
_attribute_ram_code_
#endif
void      blt_ll_pclPwrChgIndAftCisEst(st_ll_conn_t *pAclConn, ll_cis_conn_t * pCisConn) //called in irq: blt_ll_cis_post_common
{
...
_attribute_no_inline_
#endif
void      blt_ll_pclInitParamsAftCisConnect(st_ll_conn_t *pAclConn, ll_cis_conn_t * pCisConn) //called in loop: blt_cis_connect_common
{
#if (LL_FEATURE_ENABLE_CONNECTED_ISO)
    assert(pAclConn != NULL);
    assert(pCisConn != NULL);
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "@HCI_Read_RSSI", 0, 0);
