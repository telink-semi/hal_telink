...
_attribute_noinline_ int blt_ll_acl_sniffer_main_loop_data(u16 snifHandle, u8 *raw_pkt)
{
...
_attribute_no_inline_ void blt_ll_acl_sniffer_mainloop(void)
{
...
_attribute_ram_code_ int blt_sniffer_start(int conn_idx)
{
    ...
    if (blmsPm.slave_idx_calib == 0xFF) {
        ...
        #if (LL_CS_SNIFFER_MODE_ENABLE)
    blms_pconn->ap_tick_mark = sniffer_conn.c->connExpectTime;
        #endif
...
_attribute_ram_code_ int blt_sniffer_post(void)
{
    ...
    if (blms_post_common_1(blms_pconn)) { // return 1: terminate happens
        ...
        #if (LL_CS_SNIFFER_MODE_ENABLE)
            blms_pconn->ap_tick_mark = sniffer_conn.c->tick_1st_rx;
        #endif
