...
_attribute_ram_code_ int blt_sniffer_mst_start(int conn_idx)
{
    ...
    blms_pconn->ap_tick_mark = blm_pconn->connExpectTime;
        #endif
...
_attribute_ram_code_ int blt_sniffer_mst_post(void)
{
    ...
    if (blms_post_common_1(blms_pconn)) { // return 1: terminate happens
        ...
        #if (LL_CS_SNIFFER_MODE_ENABLE)
            blms_pconn->ap_tick_mark = blm_pconn->tick_1st_rx;
        #endif
...
_attribute_no_inline_ void blt_ll_acl_sniffer_mst_mainloop(void)
{
...
_attribute_noinline_ int blt_acl_sniffer_mst_mainloop_task(int flag)
{
...
_attribute_noinline_ void blc_ll_initAclSnifferMst_module(void)
{
