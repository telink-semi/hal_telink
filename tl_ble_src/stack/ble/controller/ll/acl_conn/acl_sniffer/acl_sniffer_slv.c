...
_attribute_ram_code_ int blt_sniffer_slv_post(void)
{
    ...
    if (blms_post_common_1(blms_pconn)) { // return 1: terminate happens
        ...
        #if (LL_CS_SNIFFER_MODE_ENABLE)
            blms_pconn->ap_tick_mark = bls_pconn->tick_1st_rx;
        #endif
...
int blc_ll_updateAclSnifferSlvSync(u8 *cmd)
{
    ...
    if (pc_conn->connState == CONN_STATUS_ESTABLISH && !pc_conn->acl_sniffer_sync_creating) {
        if (ps_conn->brx_pkt_miss < 4) {
            if ((pc_conn->conn_intvl_n_1m25 == param->sync_intvl_n_1m25) && (pc_conn->conn_timeout == param->sync_conn_timeout) && (!smemcmp(pc_conn->acl_chnParam.chmTbl, param->sync_conn_chm, 5)) && (pc_conn->conn_chn_hop == param->sync_conn_hop) && (pc_conn->aclAccessAddr == param->sync_accessAddr) && (pc_conn->aclCrcInit == param->sync_crcInit) && (pc_conn->conn_chnsel == param->sync_conn_chnSel) && (pc_conn->connPhyCtrl.conn_cur_phy == (param->sync_CI_phy & 0x0F)) && (pc_conn->connPhyCtrl.conn_cur_CI == (param->sync_CI_phy & 0xF0 >> 4))) {
...
int blc_ll_updateAclSnifferSlvSync_v2(u8 *cmd, u32 latencyTime)
{
    ...
    if (param->slave_index >= LL_MAX_ACL_PER_NUM || param->slave_connInterval < 6 || param->slave_connInterval > 3200 || param->slave_connTimeout < 10 || param->slave_connTimeout > 3200 || (param->slave_connHop & 0x1f) < 5 || (param->slave_connHop & 0x1f) > 16) {
        return SNIFFER_PARAMETER_INVALID;
    }
        ...
        if (param->slave_status == SLAVE_STATUS_CONNECTION_UPDATE) {
            ...
            if (pc_conn->connState == CONN_STATUS_ESTABLISH && !pc_conn->acl_sniffer_sync_creating) {
                if (ps_conn->brx_pkt_miss < 4) {
                    ...
                    if ((pc_conn->conn_intvl_n_1m25 == param->slave_connInterval) && (pc_conn->conn_timeout == (param->slave_connTimeout * 10 * SYSTEM_TIMER_TICK_1MS)) && (!smemcmp(pc_conn->acl_chnParam.chmTbl, param->slave_connChannelMap, 5)) && (pc_conn->conn_chn_hop == param->slave_connHop) && (pc_conn->aclAccessAddr == aclAccess_addr) && (pc_conn->aclCrcInit == aclCrc_init) && (pc_conn->conn_offset_next == param->slave_winOffset)) {
                        pc_conn->acl_sniffer_sync_update_ignore = 1;
                        irq_restore(r);
                        return BLE_SUCCESS;
                    }
...
int blc_ll_updateAclSnifferSlvSync_v3(u8 *cmd)
{
    ...
    if (pc_conn->connState == CONN_STATUS_ESTABLISH && !pc_conn->acl_sniffer_sync_creating) {
        if (ps_conn->brx_pkt_miss < 4) {
            if ((pc_conn->conn_intvl_n_1m25 == param->slave_connInterval) && (pc_conn->conn_timeout == sync_conn_timeout) && (!smemcmp(pc_conn->acl_chnParam.chmTbl, param->slave_connChannelMap, 5)) && (pc_conn->conn_chn_hop == param->slave_connHop) && (pc_conn->aclAccessAddr == sync_accessAddr) && (pc_conn->aclCrcInit == sync_crcInit) && (pc_conn->conn_chnsel == sync_conn_chnSel) && (pc_conn->connPhyCtrl.conn_cur_phy == (sync_CI_phy & 0x0F)) && (pc_conn->connPhyCtrl.conn_cur_CI == (sync_CI_phy & 0xF0 >> 4))) {
                pc_conn->acl_sniffer_sync_update_ignore = 1;
                irq_restore(r);
                return BLE_SUCCESS;
            }
...
_attribute_ram_code_ int blt_ll_sniffer_slv_seek_anchor(u8 *cmd)
{
    ...
    if (blms_pconn->connState == CONN_STATUS_ESTABLISH && !blms_pconn->acl_sniffer_sync_creating) {
        if (bls_pconn->brx_pkt_miss < 4) {
            if ((blms_pconn->conn_intvl_n_1m25 == param->sync_intvl_n_1m25) && (blms_pconn->conn_timeout == param->sync_conn_timeout) && (!smemcmp(blms_pconn->acl_chnParam.chmTbl, param->sync_conn_chm, 5)) && (blms_pconn->conn_chn_hop == param->sync_conn_hop) && (blms_pconn->aclAccessAddr == param->sync_accessAddr) && (blms_pconn->aclCrcInit == param->sync_crcInit) && (blms_pconn->conn_chnsel == param->sync_conn_chnSel) && (blms_pconn->connPhyCtrl.conn_cur_phy == (param->sync_CI_phy & 0x0F)) && (blms_pconn->connPhyCtrl.conn_cur_CI == (param->sync_CI_phy & 0xF0 >> 4))) {
...
_attribute_no_inline_ void blt_ll_acl_sniffer_slv_mainloop(void)
{
...
_attribute_noinline_ int blt_acl_sniffer_slv_mainloop_task(int flag)
{
...
_attribute_noinline_ void blc_ll_initAclSnifferSlv_module(void)
{
