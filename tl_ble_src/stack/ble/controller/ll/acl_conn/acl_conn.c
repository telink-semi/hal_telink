...
__attribute__((weak))
_attribute_ram_code_ void blt_debug_acl_conn_start(u16 idx)
{
    (void)idx;
...
__attribute__((weak))
_attribute_ram_code_ void blt_debug_acl_conn_post(u16 idx)
{
    (void)idx;
...
#endif
_attribute_ram_code_ int blms_start_pre_process(int conn_idx)
{
...
_attribute_ram_code_sec_optimize_o2_
#endif
_attribute_ram_code_ int blms_start_common_1(st_ll_conn_t *pc)
{
...
//      blt_restore_aes_ccm_para(pc);
    ...
    my_dump_str_u32s(0, "brx", blt_debug_hex_2_dec_display(pc->conn_inst),  blt_debug_hex_2_dec_display(pc->crypt.enc_pno),
            blt_debug_hex_2_dec_display(pc->crypt.dec_pno),reg_ccm_control);
#endif
    ...
    rf_ble_set_tx_wait(RF_TX_WAIT_MIN_VALUE); //only involved in BTX/BRX/RX2TX
            ...
            if(cte_conn_switchPattern[blms_conn_sel].cte_rx_mode_en){
                ...
                if(rx_header & BIT(5)){//CP
                        ...
                        if(blms_pconn->connPhyCtrl.conn_cur_phy == BLE_PHY_1M){
                            rf_ble_set_tx_settle(TX_STL_AUTO_MODE_1M + 8);
                        }
                        else if(blms_pconn->connPhyCtrl.conn_cur_phy == BLE_PHY_2M){
                            rf_ble_set_tx_settle(TX_STL_AUTO_MODE_2M + 1);
                        }
                    #else
                        rf_ble_set_tx_settle(TX_STL_AUTO_MODE_1M + 8);
                    #endif
...
_attribute_ram_code_ int irq_acl_conn_tx(void)
{
    ...
    if(blms_pconn->aclRole == ACL_ROLE_CENTRAL && aclConn_param.txPktCnt == 0){
            ...
            rf_ble_csem_set_tx_rx_settle(1, tx_stl_auto_mode[blms_pconn->connPhyCtrl.conn_cur_phy], 0);
        #else
            rf_ble_set_tx_settle(TX_STL_AUTO_MODE_1M);
            rf_ble_csem_set_tx_rx_settle(1, TX_STL_AUTO_MODE_1M, 0);
        #endif
...
bool blt_llms_pushTxfifo(u16 connHandle, u8 *p)
{
    if(BLC_LL_LOG_EN || (stkLog_mask & STK_LOG_LL_TX)){
        u8 LLID=p[0] & 0x03;
        if(LLID==1){tlkapi_send_string_data(BLC_LL_LOG_EN || (stkLog_mask & STK_LOG_LL_TX), "[LL][TX] L2CAP Continuation Fragment/Empty", p+2, p[1]);}
        else if(LLID==2){tlkapi_send_string_data(BLC_LL_LOG_EN || (stkLog_mask & STK_LOG_LL_TX), "[LL][TX] Start Fragment/Complete", p+2, p[1]);}
        else if(LLID==3){tlkapi_send_string_data(BLC_LL_LOG_EN || (stkLog_mask & STK_LOG_LL_TX), "[LL][TX] Packet", p+2, p[1]);}
    }
...
ble_sts_t blc_ll_disconnect (u16 connHandle, u8 reason)
{
    ...
    u8 const cmd_length_array[38] =  {  12,  8,  2, 23, 13,  1,  1,  2,  9,  9,
                                         1,  1,  6,  2,  9, 24, 24,  3,  1,  1,
...
//                                   };
    ...
    //todo add len fanqh
    u8 const cmd_length_array[58] =  {  12,  8,  2, 23, 13,  1,  1,  2,  9,  9,
                                         1,  1,  6,  2,  9, 24, 24,  3,  1,  1,
                                         9,  9,  3,  3,  5,  3,  2,  2, 35,  2,
                                         2, 36,  9, 16,  4,  4,  5,  5, 11, 11,
                                         4, 11, 43, 0, 0,
                                         21, 26, 26, 28, 2, 29, 22, 19, 3,
                                         1,73, 13,21,//channel sounding
                                     };
#elif (BLUETOOTH_VER == BLUETOOTH_VER_5_4)

    u8 const cmd_length_array[43] =  {  12,  8,  2, 23, 13,  1,  1,  2,  9,  9,
                                         1,  1,  6,  2,  9, 24, 24,  3,  1,  1,
                                         9,  9,  3,  3,  5,  3,  2,  2, 35,  2,
                                         2, 36,  9, 16,  4,  4,  5,  5, 11, 11,
                                         4, 11, 43,
...
//                              }
            ...
            else if((opcode >= LL_CS_SEC_RSP) && (opcode <= LL_CS_SEC_REQ)){
...
int blt_ll_procConnectionEvent(u16 connHandle, st_ll_conn_t *pc)
{
    ...
    if (pc->irq_event1_union.connect_evt)
    {
        ...
        { // blms_gap_conn_complete_handler  //32M M/S take T: 140us/245us
        ...
        if((hci_le_eventMask & HCI_LE_EVT_MASK_ENHANCED_CONNECTION_COMPLETE)
            ...
            if(hci_le_eventMask_2 & HCI_LE_EVT_MASK_2_ENHANCED_CONNECTION_COMPLETE_V2){
                hci_le_enhancedConnectionComplete_evt_v2( pc->create_conn_status, connHandle, pc->aclRole, peer_addressType, pc->conn_peerAddr, localRpa, peerRpa,
                                                          pc->conn_intvl_n_1m25, pc->conn_latency, pc->conn_timeout/(10*SYSTEM_TIMER_TICK_1MS), pc->conn_sca, 0x00, 0xffff);
            }else{//HCI_LE_EVT_MASK_ENHANCED_CONNECTION_COMPLETE
                hci_le_enhancedConnectionComplete_evt( pc->create_conn_status, connHandle, pc->aclRole, peer_addressType, pc->conn_peerAddr, localRpa, peerRpa,
                                                          pc->conn_intvl_n_1m25, pc->conn_latency, pc->conn_timeout/(10*SYSTEM_TIMER_TICK_1MS), pc->conn_sca);
            }
        ...
        else if(hci_le_eventMask & HCI_LE_EVT_MASK_CONNECTION_COMPLETE)
        {
            if(!(pc->irq_event1_union.connect_evt & ENHANCED_CONN_FLAG_ALL)){
                ...
                hci_le_connectionComplete_evt(BLE_SUCCESS, connHandle,  pc->aclRole, pc->conn_peerPktA_type, pc->conn_peerPktA,
                                              pc->conn_intvl_n_1m25, pc->conn_latency, pc->conn_timeout/(10*SYSTEM_TIMER_TICK_1MS), pc->conn_sca);
            }
        }
...
_attribute_no_inline_
#endif
void blt_acl_conn_rx_fifo_proc(void)
{
    int sum;
        ...
        if(pAclConn->aclRole == ACL_ROLE_CENTRAL){  //Master
            ...
            if(blmhostChnClassUpt.hostMapUptCmdPending & BIT(conn_idx)){
                ...
                if(pAclConn->establish_evt){
                    ...
                    hci_tlk_connectionEstablish_evt(BLE_SUCCESS, connHandle,  pAclConn->aclRole, pAclConn->conn_peerPktA_type, pAclConn->conn_peerPktA,
                                                  pAclConn->conn_intvl_n_1m25, pAclConn->conn_latency, pAclConn->conn_timeout/(10*SYSTEM_TIMER_TICK_1MS), pAclConn->conn_sca);
