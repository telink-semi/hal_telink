...
int blt_aclc_check_init(void)
{
        ...
        if(aclMas_param.timCus_en){
            ...
            my_dump_str_u8s(DBG_CUSTOM_ACLC_TIMING, "[CUS CEN] bSlot number", aclMas_param.bSlot_number[0],
                                                                               aclMas_param.bSlot_number[1],
                                                                               aclMas_param.bSlot_number[2],
                                                                               aclMas_param.bSlot_number[3])

            my_dump_str_u8s(DBG_CUSTOM_ACLC_TIMING, "[CUS CEN] bSlot offset", aclMas_param.bSlot_offset_pos0[0],
                                                                               aclMas_param.bSlot_offset_pos0[1],
                                                                               aclMas_param.bSlot_offset_pos0[2],
                                                                               aclMas_param.bSlot_offset_pos0[3])
        }
    #endif
...
_attribute_ram_code_
u32 blt_ll_calMasterInitiateTiming(int tim_type, st_llm_conn_t  *pm)
{
...
_attribute_ram_code_
u32 blt_ll_calCenTaskConcentratedInitTiming(int tim_type, st_llm_conn_t  *pm)
{
    /*attention: can not use blm_pconn / blm_pconn / blms_conn_sel
     * in this function, cause conn_update_cmd will call it in main_loop */
    u32 btx_slot = 0;
    ...
    if(blmsParam.cur_master_num == 0){
        ...
        my_dump_str_u32s(DBG_CUSTOM_ACLC_TIMING, "[CUS CEN] cal timing, 1st new", aclMas_param.aclc_idx_conn | pm->init_pos_idx<<8 | aclMas_param.cur_bslotOft_diffMod<<16,
                                                                            0, 0, pm->init_pos_msk);
    }
    else{
        ...
        if(allocate_result)
        {
            ...
            my_dump_str_u32s(DBG_CUSTOM_ACLC_TIMING, "[CUS CEN] cal timing, second", aclMas_param.aclc_idx_conn | pm->init_pos_idx<<8 | aclMas_param.cur_bslotOft_diffMod<<16,
                    allocate_result, pos_offset_num, pm->init_pos_msk);
        }
    }
...
bool blt_ll_calConnUpdateTiming(u8 conn_idx, st_ll_conn_t *pc, st_llm_conn_t  *pm, rf_packet_connect_upd_req_t *pUpdate)
{
...
int blt_btx_start (int conn_idx, void *p)
{
    ...
    if(1)
    {
        ...
        rf_start_fsm(FSM_BTX, tx_buff, tick);
            ...
            #if(LL_FEATURE_ENABLE_LE_2M_PHY || LL_FEATURE_ENABLE_LE_CODED_PHY)
                rf_ble_set_tx_settle(tx_stl_btx_1st_pkt[blms_pconn->connPhyCtrl.conn_cur_phy]);
            #else
                rf_ble_set_tx_settle(TX_STL_BTX_1ST_PKT_SET_1M);
            #endif
        #endif
