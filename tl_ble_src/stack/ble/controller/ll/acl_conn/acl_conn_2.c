...
{
    if (max_master_num > LL_MAX_ACL_CEN_NUM) {
        return LL_ERR_INVALID_PARAMETER;
    } else {
        blmsParam.max_master_num = max_master_num;
    }

    if (max_slave_num > LL_MAX_ACL_PER_NUM) {
        return LL_ERR_INVALID_PARAMETER;
    } else {
...
int blt_ll_checkAclInit(void)
{
    ...
    if (bltempParam.ll_aclTxSlvFifo_set) {
        if (blt_s_txfifo.logic_num == 0 || blt_s_txfifo.size == 0) { //eagle:if(blt_s_txfifo.logic_num == 0 || blt_s_txfifo.size == 0)
            return LL_ACL_TX_BUF_PARAM_INVALID;
        }
#if (MCU_HARDWARE_TX_FIFO_4K_BYTES_LIMITATION)                       //only B91 have this limitation
...
#endif
    u8
    blt_ll_getMaxAclCentralNumber(void)
{
...
_attribute_noinline_ void blt_ll_authPayloadTimeoutExpiredHandler(u16 connHandle)
{
    ...
    if (hci_eventMaskPage2 & HCI_EVT_MASK_AUTH_PAYLOAD_TIMEOUT_EXPIRED) {
        hci_le_authPayloadTimeoutExpired_evt(connHandle);
    }
}

_attribute_noinline_ void blt_ll_processFeatureExchange(st_ll_conn_t *pAclConn)
{
    /*
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_Read_Chn_Map", &connHandle, 2);
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_LTK_Request_Negative_Reply", 0, 0);
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] LE_Start_Encryption", 0, 0);
...
{
    my_dump_str_data(IUT_HCI_LOG_EN, "[HCI][CMD] Start_Encryption V2", 0, 0);
...
ble_sts_t blc_hci_le_readMaxDataLength_V2(hci_le_readMaxDataLengthV2_cmdParam_t *pCmdParam, hci_le_readMaxDataLengthV2_retParam_t *pRetParam)
{
    ...
    pRetParam->support_max_tx_payload_per_packet         = 1;
    pRetParam->support_max_tx_payload_per_packet         = 1;
    pRetParam->support_max_tx_octets                     = aclConn_param.maxTxOct;
    pRetParam->support_max_tx_time                       = LL_PACKET_OCTET_TIME(aclConn_param.maxTxOct);
    pRetParam->support_max_rx_octets                     = aclConn_param.maxRxOct;
    pRetParam->support_max_rx_time                       = LL_PACKET_OCTET_TIME(aclConn_param.maxRxOct);
    if(pCmdParam->phy){
        if(pCmdParam->phy  == BLE_PHY_HDT){
            pRetParam->support_max_tx_octets             = hdt_dftParam.MaxTxOct;
            pRetParam->support_max_tx_time               = LL_PACKET_OCTET_TIME(hdt_dftParam.MaxTxOct);
            pRetParam->support_max_rx_octets             = hdt_dftParam.MaxRxOct;
            pRetParam->support_max_rx_time               = LL_PACKET_OCTET_TIME(hdt_dftParam.MaxRxOct);
            pRetParam->support_max_tx_payload_per_packet = hdt_dftParam.max_tx_payload_window_size;
            pRetParam->support_max_tx_payload_per_packet = hdt_dftParam.max_rx_payload_window_size;
        }
    }
    else{
        if (blmsParam.phy_hdt_en){
            pRetParam->support_max_tx_octets             = max(aclConn_param.maxTxOct,hdt_dftParam.MaxTxOct);
            pRetParam->support_max_rx_octets             = max(aclConn_param.maxRxOct,hdt_dftParam.MaxRxOct);
            pRetParam->support_max_tx_time               = max(LL_PACKET_OCTET_TIME(aclConn_param.maxTxOct),LL_PACKET_OCTET_TIME(hdt_dftParam.MaxTxOct));
            pRetParam->support_max_tx_time               = max(LL_PACKET_OCTET_TIME(aclConn_param.maxRxOct),LL_PACKET_OCTET_TIME(hdt_dftParam.MaxRxOct));
        }
    }
    return BLE_SUCCESS;
}
...
_attribute_noinline_
    ble_sts_t
    blt_ll_exchangeDataLength(u16 connHandle, u8 opcode, u16 maxTxOct)
{
#if (LL_FEATURE_ENABLE_LE_DATA_LENGTH_EXTENSION)
    u8                   conn_idx  = (connHandle & CONN_IDX_MASK);
    st_ll_conn_t        *pc        = (st_ll_conn_t *)&blms[conn_idx];
...
_attribute_noinline_ void blc_ll_dataLenAutoExgDisable(u16 connHandle)
{
#if (LL_FEATURE_ENABLE_LE_DATA_LENGTH_EXTENSION)
    u8                   conn_idx  = (connHandle & CONN_IDX_MASK);
    st_ll_conn_t        *pc        = (st_ll_conn_t *)&blms[conn_idx];
...
_attribute_noinline_ void blt_ll_procDlePending(u16 connHandle)
{
#if (LL_FEATURE_ENABLE_LE_DATA_LENGTH_EXTENSION)
    u8                   conn_idx  = (connHandle & CONN_IDX_MASK);
    st_ll_conn_t        *pc        = (st_ll_conn_t *)&blms[conn_idx];
...
#endif
    ble_sts_t
    blc_hci_receiveHostACLData(hci_acl_data_pkt_t *pAclDatPkt)
{
...
#endif
    void *
    blt_ll_getAclConnPtr(u16 connHandle)
{
...
#endif
    u32
    blc_ll_getConnectionStartTick(u16 connHandle)
{
...
#endif
    ble_sts_t
    blt_ll_isAclHandleOutOfRange(u16 connHandle)
{
...
ble_sts_t blt_ll_isAclhdlInvalid(u16 connHandle)
{
    ...
    extern bool ble_host_conn_is_connected(uint16_t conn_handle);
    bool flag = ble_host_conn_is_connected(connHandle);
    return flag == true ? BLE_SUCCESS:HCI_ERR_UNKNOWN_CONN_ID;
#else
    connHandle &= ~HANDLE_STK_FLAG;
...
int blc_ll_getCurrentCentralRoleNumber(void)
{
...
_attribute_noinline_ void blt_ll_rspTimeoutLoopEvt(u16 connHandle)
{
    u8            idx = connHandle & CONN_IDX_MASK;
    st_ll_conn_t *pc  = (st_ll_conn_t *)&blms[idx];
...
#endif
    int
    blt_ll_isEncryptionBusy(u16 connHandle)
{
    u8            idx = connHandle & CONN_IDX_MASK;
    st_ll_conn_t *pc  = (st_ll_conn_t *)&blms[idx];
...
#endif
    u8
    blt_ll_push_hold_data(u16 connHandle)
{
    u8            idx = connHandle & CONN_IDX_MASK;
    st_ll_conn_t *pc  = (st_ll_conn_t *)&blms[idx];
...
_attribute_ram_code_ void blt_save_aes_ccm_para(st_ll_conn_t *pc)
{
...
{
    u8 idx                    = connHandle & CONN_IDX_MASK;
    ll_acl_recentAvgRSSI[idx] = 0;
    ll_acl_recentRSSI[idx]    = 0;
}
...
_attribute_ram_code_
    #endif
    bool
    blt_acl_pushCacheTxfifo(u16 connHandle, u8 *p)
{
    //  DBG_C HN4_TOGGLE;
...
_attribute_ram_code_
    #endif
    void
    blt_acl_cache_tx_fifo_to_hw_tx_fifo(void)
{
    //  DBG_C HN6_TOGGLE;
