...
_attribute_noinline_ void blc_ll_initAclPeriphrRole_module(void)
{
...
_attribute_ram_code_sec_optimize_o2_
    ...
    blt_acl_slave_interrupt_task(int flag, void *p)
{
...
_attribute_noinline_ int blt_acl_slave_mainloop_task(int flag, void *p)
{
    int conn_idx = flag & FLAG_SCHEDULE_TASK_IDX_MASK;
...
_attribute_ram_code_ bool blt_s_connect(rf_packet_connect_t *pInit, bool aux_conn)
{
    ...
    if (pInit->interval < 6 || pInit->interval > 3200 || pInit->winSize < 1 || pInit->winSize > 8 || pInit->winSize >= pInit->interval || pInit->timeout < 10 || pInit->timeout > 3200 /*  || pInit->winOffset > pInit->interval               \ */
...
_attribute_ram_code_sec_optimize_o2_
    #endif
    _attribute_ram_code_ int
    blt_brx_start(int conn_idx, void *p)
{
    ...
    if (1) {
        ...
        if (blms_pconn->connPhyCtrl.conn_cur_phy == BLE_PHY_CODED && blms_pconn->connPhyCtrl.conn_cur_CI == LE_CODED_S8) {
            rf_ble_csem_set_tx_rx_settle(0, tx_stl_auto_mode[blms_pconn->connPhyCtrl.conn_cur_phy + 1], RX_SETTLE_US);
        } else {
        ...
        // Calculate reception completion tick, including max time based on conn.  + 50us.50us is the margin
        u32 rxCompleteTick = blt_llms_get_connEffectiveMaxRxTime_by_connIdx(conn_idx) + 50 * SYSTEM_TIMER_TICK_1US;
        // Set WFI wakeup tick based on expected conn. time + reception completion tick.
        blt_pm_setWfiWakeupTick(bls_pconn->connExpectTime + rxCompleteTick);
    #endif
        ...
        rf_txWait_txSettle_handle(ACL_ROLE_PERIPHERAL);
    #endif
...
_attribute_ram_code_ int blt_brx_post(void)
{
    ...
    if (result == 1) { // return 1: terminate happens
        ...
        if (bls_pconn->tick_1st_rx) {
            ...
            if (blms_pconn->sync_timing) {
                ...
                int n_sSlot                  = (bls_pconn->conn_start_time - bltSche.sSlot_tick_irq_real) * SSLOT_TICK_REVERSE;
                bls_pconn->sSlot_mark_conn   = bltSche.sSlot_idx_irq_real + n_sSlot - bls_pconn->sSlot_interval;
                blms_pconn->bSlot_mark_conn  = bltSche.bSlot_idx_irq_real + (bls_pconn->tick_mark_1strx - bltSche.bSlot_tick_irq_real)/SYSTEM_TIMER_TICK_625US ;
...
_attribute_noinline_ int blt_acl_slave_clear_sleep_latency(u8 conn_idx)
{
    #if (BLMS_PM_ENABLE)
    ...
    #if (BLMS_PM_ENABLE)
_attribute_ram_code_ void blt_brx_timing_init(void)
{
...
_attribute_ram_code_ void blt_brx_timing_update(void)
{
    u8 cur_cal_synced            = 0;
...
_attribute_ram_code_ int blt_ll_build_acl_slave_schedule(void)
{
    u32 i, j;
...
_attribute_ram_code_
        #endif
    void
    blt_acl_slave_rx_procUpdateReq(u8 *raw_pkt)
{
    ...
    */
    if (llid == 3 && (blms_pconn->crypt.enable ? (rf_len == 9 || rf_len == 12 || rf_len == 16 || rf_len == 15) : (rf_len == 5 || rf_len == 8 || rf_len == 12 || rf_len == 11))) {
...
_attribute_ram_code_
        #endif
    void
    blt_acl_slave_tx_procUpdateReq(void)
{
...
_attribute_ram_code_
        #endif
    void
    blt_acl_slave_slotgap_procUpdateReq(void)
{
    for (int i = 0; i < LL_MAX_ACL_PER_NUM; i++) {
...
_attribute_ram_code_
        #endif
    int
    blt_ll_conn_chn_phy_update(st_ll_conn_t *pc, u8 *pkt)
{
    int st = 0;
    ...
    else if (pll->opcode == LL_PHY_UPDATE_IND_V2) {
        if (pll->rf_len != 10) {
            ps->errFlag |= SLV_FLAG_LEN_ERR;
            ps->unknownType = LL_PHY_UPDATE_IND_V2;
            return st;
        }

        rf_pkt_ll_phy_update_ind_v2_t *pUpdt = (rf_pkt_ll_phy_update_ind_v2_t *)pkt;
