...
_attribute_ram_code_ signal_fifo_t *blc_ll_get_llmic_param(void)
{
    return &ble_llmic_signal;
}

_attribute_ram_code_ void blt_ll_set_llmic_status(llmic_ble_sigl_e ble_signal)
{
...
_attribute_ram_code_ void blt_ll_check_llmic_status(void)
{
    bltLlmic.occupy_cur_task = 0;
...
_attribute_ram_code_ void blt_llmic_updateNextTask(void)
{
...
_attribute_ram_code_ int blt_llmic_build_extadv_task(void)
{
    #if (BLE_LLMIC_CONCURRENT_EN)
...
_attribute_ram_code_ int blt_llmic_extadv_start(int slotTask_idx)
{
    DBG_CHN1_HIGH;
...
_attribute_ram_code_ int blt_llmic_build_acl_slave_schedule(void)
{
    u32 i, j;
...
_attribute_ram_code_ int blt_llmic_quick_brx(int conn_idx)
{
