...
ble_sts_t blt_phyTest_setTestEnd(u8 *pkt_num)
{
    ...
    if (blt_phyTest.cmd == PHY_TEST_CMD_HDT_RX) {

        if (!blmsParam.phytest_en) { //must set phy mode
            //blc_phy_setHdtPhyTestEnable(BLC_PHYTEST_ENABLE);
        }

        HDT_HCI_LOG("hdt phy_test end,pkts or blocks = %x", blt_phyTest.pkts);
        ...
        HDT_HCI_LOG("phy_test end,pkts = %x", blt_phyTest.pkts);
...
_attribute_ram_code_ void blt_hdtPhyTest_main_loop(void)
{
    #define PHY_CMD_HDT_TX        4
    #define PHY_CMD_HDT_RX        5
    #define PHY_CMD_END_V2        6

    if(blt_phyTest.cmd == PHY_CMD_HDT_TX) {
    ...
    } else if (blt_phyTest.cmd == PHY_CMD_HDT_RX) {

    }else if (blt_phyTest.cmd == PHY_CMD_END_V2) {

    }
    #undef PHY_CMD_HDT_TX
    #undef PHY_CMD_HDT_RX
    #undef PHY_CMD_END_V2
}
