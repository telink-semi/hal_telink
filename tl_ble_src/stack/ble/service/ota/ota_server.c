...
_attribute_no_inline_ void blt_ota_reset(void)
{
 ...
 * @return     none
 */
_attribute_no_inline_ void blc_ota_initOtaServer_module(void)
{
    blotaSvr.otaInit = 1;
...
{
    int param_valid = 0;
#if ((MCU_CORE_TYPE == MCU_CORE_B92) || (MCU_CORE_TYPE == MCU_CORE_B91))  //ronglu
...
#elif ((MCU_CORE_TYPE == MCU_CORE_TL721X) || (MCU_CORE_TYPE == MCU_CORE_TL321X))  //ronglu
         if(firmware_size_k > (boot_addr>>10))  //   /1024&/2  ==  >>11
         {
            return SERVICE_ERR_INVALID_PARAMETER;
         }
//todo
//        unsigned int blc_flash_size =1 << (((ble_flash_read_mid() & 0x00ff0000) >> 16) - 10 + 5);
//
//        if(blc_flash_size > firmware_size_k)
//        {
             param_valid = 1;
//        }

#else
    #warning "Please check the size supported by the MCU"
#endif
...
_attribute_no_inline_
    ble_sts_t
    blc_ota_setOtaProcessTimeout(int timeout_second)
{
...
_attribute_no_inline_
    ble_sts_t
    blc_ota_setOtaDataPacketTimeout(int timeout_second)
{
...
_attribute_no_inline_
    ble_sts_t
    blc_ota_setOtaScheduleIndication_by_pduNum(int pdu_num)
{
...
_attribute_no_inline_
    ble_sts_t
    blc_ota_setOtaScheduleIndication_by_FirmwareSize(int fw_size)
{
    /* can only select one from PDU number & FW size */
...
_attribute_no_inline_ int ota_save_data(u32 flash_addr, int len, u8 *data)
{
...
_attribute_no_inline_ void blt_ota_setResult(int next_step, int result)
{
...
_attribute_no_inline_ void blt_ota_procTimeout(void)
{
 ...
 */
_attribute_no_inline_ int blt_ota_server_terminate(u16 connHandle)
{
...
_attribute_no_inline_ void blt_ota_procOtaResultFeedback(void)
{
...
_attribute_no_inline_ void blt_ota_procOtaFinish(void)
{
...
_attribute_no_inline_
    ble_sts_t
    blt_ota_pushSchedulerIndication(void)
{
...
_attribute_no_inline_ int blt_proc_ota_server(void)
{
