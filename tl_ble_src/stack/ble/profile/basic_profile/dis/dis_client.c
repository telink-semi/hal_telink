...
struct blc_dis_client_ctrl dis_client_ctrl = {
    .process = {
        .pNext = NULL,
        .id = DIS_CLIENT,
        .usedAclRole = 0,
...
static void blt_disc_displayInfo(u16 connHandle, struct blc_dis_client* client)
{
    ...
    if(client->manufacturerNameHdl){
        BLT_DIS_LOG("Manufacturer Name:[handle:0x%x value:%.*s]",
                client->manufacturerNameHdl, client->manufacturerNameLen, client->manufacturerName
        );
    }

    if(client->modelNumberHdl){
        BLT_DIS_LOG("Model Number:[handle:0x%x value:%.*s]",
                client->modelNumberHdl, client->modelNumberLen, client->modelNumber
        );
    }

    if(client->serialNumberHdl){
        BLT_DIS_LOG("Serial Number:[handle:0x%x value:%.*s]",
                client->serialNumberHdl, client->serialNumberLen, client->serialNumber
        );
    }

    if(client->hardwareRevisionHdl){
        BLT_DIS_LOG("Hardware Revision:[handle:0x%x value:%.*s]",
                client->hardwareRevisionHdl, client->hardwareRevisionLen, client->hardwareRevision
        );
    }

    if(client->firmwareRevisionHdl){
        BLT_DIS_LOG("Firmware Revision:[handle:0x%x value:%.*s]",
                client->firmwareRevisionHdl, client->firmwareRevisionLen, client->firmwareRevision
        );
    }

    if(client->softwareRevisionHdl){
        BLT_DIS_LOG("Software Revision:[handle:0x%x value:%.*s]",
                client->softwareRevisionHdl, client->softwareRevisionLen, client->softwareRevision
        );
    }

    if(client->systemIdHdl){
        BLT_DIS_LOG("System ID:[handle:0x%x, manufacturer:0x%02x%02x%02x%02x%02x, OUI:0x%02x%02x%02x]",
                client->systemIdHdl, client->systemId.manufacturer[4], client->systemId.manufacturer[3], client->systemId.manufacturer[2],
                client->systemId.manufacturer[1], client->systemId.manufacturer[0],
                client->systemId.oui[2], client->systemId.oui[1], client->systemId.oui[0]
        );
    }

    if(client->IEEEDataListHdl){
        BLT_DIS_LOG("IEEE 11073-20601 Regulatory Certification Data List:[handle:0x%x, value is %s]",
                client->IEEEDataListHdl, hex_to_str(client->IEEEDataList, client->IEEEDataListLen)
        );
    }
    ...
    if(client->PnPIDHdl){
        ...
        BLT_DIS_LOG("PnP ID:[handle:0x%x, %s:%d(0x%x) Product Id:%d, Product Version:%d]",
                ...
                client->PnPID.vid, client->PnPID.vid, client->PnPID.pid, client->PnPID.ver
        );
    }

    if(client->udiForMedicalDevicesHdl){
        BLT_DIS_LOG("UDI for Medical Devices:[handle:0x%x, value is %s]",
                client->udiForMedicalDevicesHdl, hex_to_str(client->udiForMedicalDevices, client->udiForMedicalDevicesLen)
        );
    }
