...
#define BLT_PRF_CHECK_NULL_PTR1(ptr1) \
    if (CHECK_PTR_NUL(ptr1))          \
    return PRF_COMMON_ERR_INPUT_NULL
#define BLT_PRF_CHECK_NULL_PTR2(ptr1, ptr2)         \
    if (CHECK_PTR_NUL(ptr1) || CHECK_PTR_NUL(ptr2)) \
    return PRF_COMMON_ERR_INPUT_NULL
#define BLT_PRF_CHECK_NULL_PTR3(ptr1, ptr2, ptr3)                          \
...
typedef struct
{
    u8                      clientRdySdp;
    u8                      reconnFlag;
    struct blc_prf_process *currSvcNodeId;
