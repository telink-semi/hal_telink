...
int32_t ltv_pack(const struct ltv_data *ltv[], void *outData)
{
    uint8_t *p = outData;
    while (*ltv != NULL) {
        U8_TO_STREAM(p, (*ltv)->length);
        U8_TO_STREAM(p, (*ltv)->type);
        memcpy(p, (*ltv)->value, (*ltv)->length - 1);
        p += (*ltv)->length - 1;

        ltv++;
    }

    return (p - (uint8_t *) outData);
}
#endif
...
bool blc_advGetFlags(u8 *advData, u16 len, u8 *flags)
{
    ...
    u8 *buff = blc_adv_getAdvTypeInformation(advData, len, DT_FLAGS, &flagsLen);
...
{
    u8   uuidLen = 0;
    u16 *pUuid = (u16 *) blc_adv_getAdvTypeInformation(advData, len, DT_INCOMPLETE_LIST_16BIT_SERVICE_UUID, &uuidLen);

    for (int i = 0; i < (uuidLen >> 1); i++) {
        // tlkapi_printf(1, "Service pUuid is %x, uuid is %x", *pUuid, uuid);
        if (*pUuid == uuid) {
            return true;
        }
        pUuid++;
    }
    pUuid = (u16 *) blc_adv_getAdvTypeInformation(advData, len, DT_COMPLETE_LIST_16BIT_SERVICE_UUID, &uuidLen);
...
{
    u8   uuidLen = 0;
    u16 *pUuid = (u16 *) blc_adv_getAdvTypeInformation(advData, len, DT_APPEARANCE, &uuidLen);

    for (int i = 0; i < (uuidLen >> 1); i++) {
        // tlkapi_printf(1, "Appearance pUuid is %x, uuid is %x", *pUuid, uuid);
        if (*pUuid == uuid) {
            return true;
        }
        pUuid++;
