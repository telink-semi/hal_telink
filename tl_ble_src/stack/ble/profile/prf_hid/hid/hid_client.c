...
struct blc_hid_client_ctrl hid_client_ctrl = {
    .process = {
        .pNext = NULL,
        .id = HID_CLIENT,
        .usedAclRole = 0,
...
static void blt_hidc_displayInfo(u16 connHandle, struct blc_hid_client* client)
{
    ...
    {
        BLT_HID_LOG("Protocol Mode:[Handle:0x%x, Properties:0x%x, %s]", client->protocolModeHdl, client->protocolModeProp,
                client->protocolMode == BOOT_PROTOCOL_MODE ? "Boot Protocol Mode":
                client->protocolMode == REPORT_PROTOCOL_MODE ? "Report Protocol Mode" : "Reserved for Future Use");
    }
    if(client->bootKeyboardInputReportHdl)
    {
        BLT_HID_LOG("Boot Keyboard Input Report:[Handle:0x%x, Properties:0x%x, value:%s]",
                client->bootKeyboardInputReportHdl, client->bootKeyboardInputReportProp,
    ...
    if(client->bootKeyboardOutputReportHdl)
    {
        BLT_HID_LOG("Boot Keyboard Output Report:[Handle:0x%x, Properties:0x%x, value:%s]",
                client->bootKeyboardOutputReportHdl, client->bootKeyboardOutputReportProp,
    ...
    if(client->bootMouseInputReportHdl)
    {
        BLT_HID_LOG("Boot Mouse Input Report:[Handle:0x%x, Properties:0x%x, value:%s]",
                client->bootMouseInputReportHdl, client->bootMouseInputReportProp,
    ...
    if(client->HIDInformationHdl)
    {
        BLT_HID_LOG("HID Information:[Handle:0x%x], Properties:0x%x, HID Version: %x, Country Count: %d, Remote Wake:%s, Normally Connectable :%s",
                client->HIDInformationHdl, client->HIDInformationProp,
                client->HIDInformation.bcdHID, client->HIDInformation.bCountCode, client->HIDInformation.remoteWake? "Supp": "Nosupp",
    ...
    for(int i=0; i<client->reportCount; i++) {
        BLT_HID_LOG("index:%d, Handle:0x%x, Properties:0x%x, CCC Handle:0x%x, Reference Handle:0x%x", i,
                client->reportCharInfo[i].attrHandle, client->reportCharInfo[i].properties, client->reportCharInfo[i].cccHandle,
                client->reportCharInfo[i].reportReferenceHandle);

        u8 type = client->reportCharInfo[i].reportReferenceValue.reportType;
        BLT_HID_LOG("report ID is %d, type is %s", client->reportCharInfo[i].reportReferenceValue.reportId,
                type == HID_REPORT_TYPE_INPUT ? "Input": type == HID_REPORT_TYPE_OUTPUT ? "Output" :
                type == HID_REPORT_TYPE_FEATURE ? "Feature" : "RFU"
        );
    }
...
static int blc_hidc_writeReportValue(u16 connHandle, struct hid_reportReferenceValue *reference,
        u8* value, u16 valueLen, prf_write_cb_t writeCb, bool withoutRsp)
{
...
{
    if(report == NULL || reportLen == NULL)     return PRF_COMMON_ERR_INPUT_NULL;
