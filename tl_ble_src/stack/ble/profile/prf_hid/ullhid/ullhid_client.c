...
struct blc_ullhid_client_ctrl ullhid_client_ctrl = {
    .process = {
        .pNext = NULL,
        .id = ULLHID_CLIENT,
        .usedAclRole = 0,
...
static void blt_ullhidc_dataInput(u16 connHandle, u16 attHdl, u8 *val, u16 valLen)
{
    ...
    else if(*val == ULL_HID_OPCODE_SELECT_HYBRID_MODE)
    {
        ...
        if(blt_ullhid_checkSelectHybridMode(&client->properties, client->propertiesLen,
                pHybrid->suppInterval, &pHybrid->indices[0], indicesSize) != ATT_SUCCESS)
        {
            return ;
        }
...
void blc_ullhid_displayProperties(struct blc_ullhid_properties_format* properties, u16 propertiesLen)
{
    ...
    {
        BLT_ULLHID_LOG("[%d] Report ID:%02d, Type:%s, PowerSavingConfirmation:%s, Repetition:%s", i, properties->hybridModeReport[i].reportID,
                properties->hybridModeReport[i].reportType == ULL_HID_REPORT_TYPE_INPUT? "Input": "Output",
