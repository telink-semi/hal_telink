...
#else
#define ULLHID_SELECT_HYBRID_MODE_HEAD_SIZE         (5+2)

struct ullhid_hybrid_mode_iso_report_enable
{
    u8 index            : 3;
    u8 rfu              : 3;
    u8 cfmEnable        : 1;
...
struct ullhid_select_hybrid_mode_format
{
    ...
    struct ullhid_hybrid_mode_iso_report_enable indices[ULL_HID_HYBRID_MODE_ULL_REPORT_COUNT];
} __attribute__((packed));
#endif
