...
static int blt_aicsc_dataInput(blc_aics_client_t* client, u16 attHdl, u8 *val, u16 valLen)
{
    if(client->audioInStateHdl == attHdl)
    {
        ...
        BLT_AICS_LOG("INFO: Audio Input State, handle[0x%x], gainSetting[%d], Mute[%d], GainMode[%d], changeCount[%d]", attHdl,
                client->audioInState.gainSetting, client->audioInState.mute, client->audioInState.gainMode, client->audioInState.changeCnt
        );
...
void blt_aicsc_micpFoundServiceEnd(u16 connHandle)
{
    ...
    for(int i=0; i<micp->aicsClientCnt; i++) {
        ...
        BLT_AICS_LOG("  INFO: audio input state Handle[0x%x] gainSet[%d] mute[%d] gainMode[%d] changeCnt[%d]", client->audioInStateHdl, client->audioInState.gainSetting,
                client->audioInState.mute, client->audioInState.gainMode, client->audioInState.changeCnt);
        BLT_AICS_LOG("  INFO: gain setting properties Handle[0x%x] gainSettingUnits[%d] gainSettingMinimum[%d] gainSettingMaximum[%d]", client->gainSettingPropertiesHdl,
                client->gainSettingProperties.gainSettingUnits, client->gainSettingProperties.gainSettingMinimum, client->gainSettingProperties.gainSettingMaximum);
...
int blc_aicsc_writeSetGainSettingByVcpIndex(u16 connHandle, int index, s8 gainSetting)
{
    return blt_aicsc_writeCtrlPoint(
            connHandle, SERVICE_UUID_VOLUME_CONTROL, index,
            AICS_OPCODE_SET_GAIN_SETTING, gainSetting
            );
}
...
{
    return blt_aicsc_writeCtrlPoint(
            connHandle, SERVICE_UUID_MICROPHONE_CONTROL, index,
            AICS_OPCODE_SET_GAIN_SETTING, gainSetting
            );
}
...
int blc_aicsc_vcpSetManualGainMode(u16 connHandle, int index)
{
    return blt_aicsc_writeCtrlPoint(
            connHandle, SERVICE_UUID_VOLUME_CONTROL, index,
            AICS_OPCODE_SET_MANUAL_GAIN_MODE, 0
            );
}
...
int blc_aicsc_vcpSetAutoGainMode(u16 connHandle, int index)
{
    return blt_aicsc_writeCtrlPoint(
            connHandle, SERVICE_UUID_VOLUME_CONTROL, index,
            AICS_OPCODE_SET_AUTOMATIC_GAIN_MODE, 0
            );
}
