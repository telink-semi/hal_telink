...
void blt_vocsc_foundServiceEnd(u16 connHandle)
{
    ...
    for (int i = 0; i < vcp->vocsClientCnt; i++) {
        ...
        BLT_VOCS_LOG("  INFO: volume offset state Handle[0x%x] volumeOffset[%d] changeCounter[%d]", client->volOffStateHdl, client->volOffsetState.volOffset, client->volOffsetState.changeCnt);
...
static int blt_vocsc_sendInfoEvt(u16 connHandle, u16 attHdl, u8 *val, u16 valLen)
{
    ...
    for (int i = 0; i < vcp->vocsClientCnt; i++) {
        ...
        if (vocsc->volOffStateHdl == attHdl) {
            ...
            BLT_VOCS_LOG("INFO: index[%d], volumeOffsetState, handle[0x%x], volume_offset[%d], changeCount[%d]", i, attHdl, vocsc->volOffsetState.volOffset, vocsc->volOffsetState.changeCnt);
