...
typedef struct{ //Event ID: AUDIO_EVT_GTBSS_BEARER_SIGNAL_STRENGTH_REPORTING_INTERVAL
    u8 interval;
} blc_gtbss_bearerSignalStrengthReportingIntervalEvt_t;

typedef struct{ //Event ID: AUDIO_EVT_GTBSS_CALL_CONTROL_POINT_ACCEPT
    u8 callIndex;
} blc_gtbss_callControlPointAcceptEvt_t;

typedef struct{ //Event ID: AUDIO_EVT_GTBSS_CALL_CONTROL_POINT_TERMINATE
    u8 callIndex;
} blc_gtbss_callControlPointTerminateEvt_t;

typedef struct{ //Event ID: AUDIO_EVT_GTBSS_CALL_CONTROL_POINT_LOCAL_HOLD
    u8 callIndex;
} blc_gtbss_callControlPointLocalHoldEvt_t;

typedef struct{ //Event ID: AUDIO_EVT_GTBSS_CALL_CONTROL_POINT_LOCAL_RETRIEVE
    u8 callIndex;
} blc_gtbss_callControlPointLocalRetrieveEvt_t;

typedef struct{ //Event ID: AUDIO_EVT_GTBSS_CALL_CONTROL_POINT_ORIGINATE
    u16 uriLen;
    u8 uri[];
} blc_gtbss_callControlPointOriginateEvt_t;

typedef struct{ //Event ID: AUDIO_EVT_GTBSS_CALL_CONTROL_POINT_JOIN
    u16 callIndexNum;
    u8 callIndexes[];
