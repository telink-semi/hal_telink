...
static att_err_t blt_basss_dealAddSource(u16 connHandle, u8* value, u16 valueLen)
{
    ...
    *pRcvStateLen = (u32)pRcvStateSub - (u32)pRcvStateTemp;
    ...
    if(ntfFlag)     blc_basss_notifyRecvState(connHandle, stateIndex+1);
...
//      pRcvStateSub->bisSync = 0x00;
    ...
    *pRcvStateLen = (u32)pRcvStateSub - (u32)pRcvState;
    ...
    if(ntfFlag) blc_basss_notifyRecvState(connHandle, pModifySrc->parameter.srcId);
...
static const basss_dealOpcode basssFun[] = {
    blt_basss_dealRemoteScanStopped, blt_basss_dealRemoteScanStarted, blt_basss_dealAddSource, blt_basss_dealModifySource,
    blt_basss_dealSetBroadcastCode, blt_basss_dealRemoveSource
};
