...
static void blt_pacsc_dataInput(u16 connHandle, u16 attHdl, u8 *val, u16 valLen)
{
    ...
    if(attHdl == client->sinkAudioLcaHdl) {
        ...
        STREAM_TO_U32(client->sinkAudioLca, val); return ;
    }
    else if(attHdl == client->srcAudioLcaHdl) {
        if(valLen != sizeof(client->srcAudioLca)) {
            return ;
        }
        STREAM_TO_U32(client->srcAudioLca, val); return ;
    }
        ...
        STREAM_TO_U16(client->avaAudioCtx.avaSrcCtx, val); return ;
    }
        ...
        STREAM_TO_U16(client->supAudioCtx.supSrcCtx, val); return ;
    }
...
static void blt_pacsc_displayInfo(u16 connHandle, blc_pacs_client_t* client)
{
    ...
    BLT_PACS_LOG("audio locations:sink[handle: 0x%x value 0x%x] source[handle: 0x%x value 0x%x]", client->sinkAudioLcaHdl, client->sinkAudioLca,
                    client->srcAudioLcaHdl, client->srcAudioLca);
    BLT_PACS_LOG("audio contexts:ava[handle:0x%x sink:0x%x src:0x%x] supp[handle: 0x%x sink:0x%x src:0x%x]",
            client->avaAudioCtxHdl, client->avaAudioCtx.avaSinkCtx, client->avaAudioCtx.avaSrcCtx,
            client->supAudioCtxHdl, client->supAudioCtx.supSinkCtx, client->supAudioCtx.supSrcCtx);
...
#define BLT_PACSC_CHECK_PARAM(connHandle, attrHandle)       if(blt_ll_isAclHandleOutOfRange(connHandle)) return AUDIO_ERR_CONNHANDLE_INVALID;   \
                                                            blc_pacs_client_t* client = blt_pacsc_getClientInst(connHandle); \
...
{
    blc_pacs_client_t *client = blt_pacsc_getClientInst(connHandle);
    if(!client || client->sinkAudioLcaHdl == 0 || client->sinkPacRcdNum == 0)           return -1;
    for(int i=0; i<client->sinkPacRcdNum; i++)
    {
