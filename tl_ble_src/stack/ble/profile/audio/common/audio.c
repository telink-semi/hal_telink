...
_attribute_ble_data_retention_ static struct gap_hciEventNode leAudioHciEventCallBack = {
...
void blc_audio_initialModule(prf_evt_cb_t evtCb)
{
    ...
    if (pm_check_info) {
        blc_prf_initialModule(evtCb, hostMallocBuffer, HOST_MALLOC_BUFF_SIZE);
...
int blt_audio_getMetadataParams(u8 metaLen, u8 *pMeta, blc_audio_metadata_parsed_t *pParam)
{
    ...
    while (metaLen > 0) {
        ...
        metaLen -= length + 1;
        if (type == BLC_AUDIO_METATYPE_PREFERRED_CONTEXTS) { // Preferred_Audio_Contexts
            if (length != 3) {
                goto lengthErr;
            }
        ...
        } else if (type == BLC_AUDIO_METATYPE_STREAMING_CONTEXTS) { // Streaming_Audio_Contexts
            if (length != 3) {
                goto lengthErr;
            }
        ...
        } else if (type == BLC_AUDIO_METATYPE_PROGRAM_INFO) { //Program Info
            if (length < 2) {
                goto lengthErr;
            }
        ...
        } else if (type == BLC_AUDIO_METATYPE_LANGUAGE) { //Language
            if (length != 4) {
                goto lengthErr;
            }
        ...
        } else if (type == BLC_AUDIO_METATYPE_CCID_LIST) { //CCID_LIST
            if (length < 2) {
                goto lengthErr;
            }
        ...
        } else if (type == BLC_AUDIO_METATYPE_PARENTAL_RATING) { //Parental rating
            if (length != 2) {
                goto lengthErr;
            }
        ...
        } else if (type == BLC_AUDIO_METATYPE_PROGRAM_INFO_URI) { //Program Info URI
            if (length < 2) {
                goto lengthErr;
            }
        ...
        } else if (type == BLC_AUDIO_METATYPE_EXTENDED_METADATA) { // Extended Metadata
            if (length <= 3) {
                goto lengthErr;
            }
        ...
        } else if (type == BLC_AUDIO_METATYPE_VENDOR_SPECIFIC) { // Vendor_Specific
            if (length <= 3) {
                goto lengthErr;
            }
...
int blt_audio_getCodecSpecCapParam(u8 *pSpecCap, blt_audio_codecSpecCapParam_t *pParam)
{
    ...
    while (tempspecLen != 0) {
        ...
        if (type == BLC_AUDIO_CAPTYPE_SUP_SAMPLE_FREQUENCY && length == 3) { // Supported_Sampling_Frequencies
        ...
        } else if (type == BLC_AUDIO_CAPTYPE_SUP_FRAME_DURATION && length == 2) { // Supported_Frame_Durations
        ...
        } else if (type == BLC_AUDIO_CAPTYPE_SUP_AUDIO_CHN_COUNTS && length == 2) { // Audio_Channel_Counts
        ...
        } else if (type == BLC_AUDIO_CAPTYPE_SUP_OCTETS_PER_CODEC_FRAME && length == 5) { // Supported_Octets_Per_Codec_Frame
        ...
        } else if (type == BLC_AUDIO_CAPTYPE_SUP_MAX_CODEC_FRAMES_PER_SDU && length == 2) { // Supported_Max_Codec_Frames_Per_SDU
...
int blt_audio_getCodecSpecCfgParam(u8 *pSpecCfg, blc_audio_codecSpecCfgParsed_t *pParam)
{
    ...
    while (tempspecLen > 0) {
        ...
        if (type == BLC_AUDIO_CAPTYPE_CFG_SAMPLE_FREQUENCY && length == 2) { // Sampling_Frequency
            ...
            pParam->fieldExistFlg |= BLC_AUDIO_SAMPLE_FREQUENCY_MASK;
        } else if (type == BLC_AUDIO_CAPTYPE_CFG_FRAME_DURATION && length == 2) {      // Frame_Duration
            ...
            pParam->fieldExistFlg |= BLC_AUDIO_FRAME_DURATION_MASK;
        } else if (type == BLC_AUDIO_CAPTYPE_CFG_CHANNELS_ALLOCATION && length == 5) { // Audio_Channel_Allocation
            ...
            pParam->fieldExistFlg |= BLC_AUDIO_AUDIO_CHANNEL_MASK;
        } else if (type == BLC_AUDIO_CAPTYPE_CFG_OCTETS_PER_CODEC_FRAME && length == 3) { // Octets_Per_Codec_Frame
            STREAM_TO_U16(pParam->frameOcts, pSpecCfg);
            if (pParam->frameOcts == 0) {
                return AUDIO_ERR_RFU_SUPP_PER_CODEC_FRAME;
            }
            pParam->fieldExistFlg |= BLC_AUDIO_OCTETS_PER_CODEC_FRAME_MASK;
        } else if (type == BLC_AUDIO_CAPTYPE_CFG_CODEC_FRAME_BLCKS_PER_SDU && length == 2) { // Codec_Frame_Blocks_Per_SDU
...
u16 blt_audio_getFrameOctsBySampleAndDuration(u8 audioSampleIdx, u8 durationType)
{
    ...
    const u32 audioFreqCfg[14] = {0, 8000, 11025, 16000, 22050, 24000, 32000, 44100, 48000, 88200, 96000, 176400, 192000, 384000};
