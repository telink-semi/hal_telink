...
//////////////////CoC parameter//////////////
_attribute_ble_data_retention_ static l2cap_coc_control_t sL2capCocCtrl = {0};
...
{
    if (!pCid->connHandle || !pCid->sendTotalLen) {
        return 0;
    }
...
ble_sts_t blc_l2cap_createLeCreditBasedConnect(u16 connHandle)
{
    ...
    if (!pAcl) {
        return L2CAP_ERR_NO_CREATE_COC_HANDLER;
    }
    ...
    if (srcCID == 0) {
        return L2CAP_ERR_ALL_CID_ALLOCATED;
    }
