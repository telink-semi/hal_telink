...
int blt_smp_llEncryptionDone(u16 connHandle, u8 status, u8 enc_enable) //register encryption done event in GAP event callBack
{
    ...
    if (status == BLE_SUCCESS && enc_enable) { //encryption done success
        ...
        if (is_master) {                       //Master
            ...
            pCurBondNum = (u16 *)&smpMStblBondDevice.slave_cur_bondNum[slave_dev_idx];
        }
...
{
    // tlk_printf("blt_smp_push_tx_fifo, 0x%x", connHandle);

    struct ble_host_conn *p_conn = ble_host_conn_find_by_conn_handle(connHandle);
    // if (p_conn == NULL) {
    //  tlk_printf("LE ACL data for unknown connection handle:0x%03x", connHandle);
    //     return false;
    // }
    // tlk_printf("p_conn = %p, smp_pkt data = %d", p_conn, smp_pkt->l2capLen);

    struct ble_host_l2cap_packet p_send_data = {
        .cb          = NULL,
        .data_length = smp_pkt->l2capLen,
        .channel_id  = LE_L2CAP_CID_SMP,
        .p_data      = (u8 *) &smp_pkt->opcode,
    };

    ble_host_hci_send_acl_data(p_conn, &p_send_data);
...
#endif
    void
    blt_smp_proc_pairing_loop(u16 connHandle)
{
    ...
    if (blms_p_sts->tk_status & TK_ST_CONFIRM_PENDING) {
        ...
        #if DUAL_CORE_MODE_ENABLED
            blt_smp_push_tx_fifo(connHandle, (smp2llcap_type_t *)pr);
        #else
...
int bls_smp_llGetLtkReq(u16 connHandle, u8 *random, u16 ediv) //register get LTK function in controller
{
    ...
    if (!memcmp(const_u8_10_zero, randParam, 10)) // EDIV+ RAND are all zero
    {
        ...
        if (pSmpOwn->sc_pairing) {
            ...
            if (pSmpSts->smp_phase_chk & BIT(SMP_OP_PAIRING_DHKEY)) {
            ...
            #if DUAL_CORE_MODE_ENABLED
                extern int ble_host_hci_ltk_request_reply(u16 connHandle, u8 * ltk);
                ble_host_hci_ltk_request_reply(connHandle, smp_Stk);
            #else
                ...
                pSmpSts->smp_phase_chk = PAIRING_PHASE_IDLE;
#if DUAL_CORE_MODE_ENABLED
                extern int ble_host_hci_ltk_request_negative_reply(u16 connHandle);
                ble_host_hci_ltk_request_negative_reply(connHandle);
#else
...
//printf("secure connections back\n");
...
#endif
                {
                    ...
                    if (flash_addr) {
                        ...
                        app_parse_printf("[SMP][LTK] : %s", hex_to_str(load_ltk, 16));
                    #endif
                        tlkapi_send_string_data((stkLog_mask & STK_LOG_SMP_LTK), "[SMP][LTK] ", load_ltk, 16);
                    #if DUAL_CORE_MODE_ENABLED
                        extern int ble_host_hci_ltk_request_reply(u16 connHandle, u8 * ltk);
                        ble_host_hci_ltk_request_reply(connHandle, load_ltk);
                    #else
        ...
        if (flash_addr) {
            ...
            #if DUAL_CORE_MODE_ENABLED
                extern int ble_host_hci_ltk_request_reply(u16 connHandle, u8 * ltk);
                ble_host_hci_ltk_request_reply(connHandle, pSmpOwn->own_ltk);
            #else
    ...
    case SMP_OP_PAIRING_CONFIRM:
    {
        ...
        if (blms_p_own->sc_pairing)                         //secure connection enable
        {
            ...
            /* Calculate Sconfirm
                 ...
                 **/
            //aes_encryption_le in blt_crypto_alg_c1/blt_crypto_alg_s1 need critical data 4B aligned
            blt_crypto_alg_c1(smpOwnPairingConfirm, blms_p_own->pairing_tk, blms_p_own->own_rand, (u8 *)&blms_p_own->pairing_rsp, (u8 *)&blms_p_own->pairing_req, iat, ia, rat, ra);
        }
    ...
    case SMP_OP_PAIRING_REQ:
    {
        if (!smp_master_role)                                         //Slave
        {
            ...
            if (custom_darwin_fmn.pair_req_cb) {
                u8 ret = custom_darwin_fmn.pair_req_cb(connHandle);
            ...
            if (blms_p_own->stk_method == OOB_Authentication) {
                ...
                if (tk_set > 0 && tk_set <= 999999) {
                    ...
                    smemcpy(blms_p_own->pairing_tk, &tk_set, 4);
#else
                    tk_set = trng_rand() & 0xFFFF; // should be "x % 1000000", here "x & 0xFFFF" equal to "x % 65536",
            ...
            #if DUAL_CORE_MODE_ENABLED
                extern void ble_host_set_encryption_busy(u16 connHandle, u8 enc_busy);
                ble_host_set_encryption_busy(connHandle, 1);
            #else
        ...
        if (blms_p_own->sc_pairing) //secure connection enable
        {
            ...
            if (smp_master_role) {
                ...
                /* Checkout Sconfirm
                     ...
                     **/
                //aes_encryption_le in blt_crypto_alg_c1/blt_crypto_alg_s1 need critical data 4B aligned
                blt_crypto_alg_c1(pairing_conf, blms_p_own->pairing_tk, blms_p_peer->peer_pairing_rand, (u8 *)&blms_p_own->pairing_rsp, (u8 *)&blms_p_own->pairing_req, blms_p_own->own_addr_type, blms_p_own->own_conn_addr, blms_p_peer->peer_addr_type, blms_p_peer->peer_conn_addr);
                ...
                if (!memcmp(blms_p_peer->peer_confirm, pairing_conf, 16)) {
                    ...
                    blms_p_sts->smpDistributeKeyOrder = SMP_TRANSPORT_SPECIFIC_KEY_START;
#if DUAL_CORE_MODE_ENABLED
                    struct ble_hci_le_start_encrypt_cp start_enc_cp = {0};
                    start_enc_cp.conn_handle = connHandle;
                    smemcpy(start_enc_cp.ltk, blms_p_peer->peer_ltk, 16);
                ...
                /* Checkout Mconfirm
                     ...
                     **/
                //aes_encryption_le in blt_crypto_alg_c1/blt_crypto_alg_s1 need critical data 4B aligned
                blt_crypto_alg_c1(pairing_conf, blms_p_own->pairing_tk, blms_p_peer->peer_pairing_rand, (u8 *)&blms_p_own->pairing_rsp, (u8 *)&blms_p_own->pairing_req, blms_p_peer->peer_addr_type, blms_p_peer->peer_conn_addr, blms_p_own->own_addr_type, blms_p_own->own_conn_addr);
        ...
        if (smp_master_role) {
...
#endif
            {
                ...
                if (smpMStblBondDevice.isBond_fastSmp & FlAG_BOND) { //reconnection
                    ...
                    tlkapi_send_string_data((stkLog_mask & STK_LOG_SMP_LTK), "[SMP][LTK] ", blms_p_peer->peer_ltk, 16);

                #if defined(GOOGLE_CS_INIT_ROLE_EN) && (GOOGLE_CS_INIT_ROLE_EN == 1)
                    extern void app_parse_printf(const char *format, ...);
                    app_parse_printf("[SMP][LTK] : %s", hex_to_str(blms_p_peer->peer_ltk, 16));
                #endif
...
static void blt_smp_SecureConnInit(pairing_methods_t method)
{
    if (method == LE_Secure_Connection) // if support secure connection feature
    {
        ...
        if(blt_l2cap_getAclRxBufferSize() < 65)
        {
            tlkapi_printf(SMP_DBG_EN, "secure connection need a mtu size large than 65");
        }
    }
}
...
int blt_smp_setSecurityParameters(smp_param_own_t *pSmpOwn, bonding_mode_t bond_mode, pairing_methods_t method, int MITM_en, int OOB_en, io_capability_t ioCapability, int keyPress_en)
{
    /*
    u8 bondingFlag : 2;
    u8 MITM : 1;
...
void blc_smp_setSecurityParameters(bonding_mode_t mode, int MITM_en, pairing_methods_t method, int OOB_en, int keyPress_en, io_capability_t ioCapability)
{
    for (int i = 0; i < (1 + LL_MAX_ACL_PER_NUM); i++) {
        ...
        blt_smp_setSecurityParameters(pSmpOwn,
                                      blt_smpProp[i].bonding_mode,
                                      blt_smpProp[i].pairing_method,
                                      blt_smpProp[i].MITM_protection,
                                      blt_smpProp[i].oob_enable,
                                      blt_smpProp[i].IO_capability,
                                      blt_smpProp[i].keyPress_en);
#endif
    }
...
void blc_smp_setSecurityParameters_central(bonding_mode_t bond_mode, int MITM_en, pairing_methods_t method, int OOB_en, int keyPress_en, io_capability_t ioCapability)
{
    ...
    // conFig all SMP parameters according to application's setting
    blt_smp_setSecurityParameters(pSmpOwn,
                                  blt_smpProp[0].bonding_mode,
                                  blt_smpProp[0].pairing_method,
                                  blt_smpProp[0].MITM_protection,
                                  blt_smpProp[0].oob_enable,
                                  blt_smpProp[0].IO_capability,
                                  blt_smpProp[0].keyPress_en);
#endif
}
...
void blc_smp_setSecurityParameters_periphr_by_device_index(int slave_dev_ind, bonding_mode_t bond_mode, int MITM_en, pairing_methods_t method, int OOB_en, int keyPress_en, io_capability_t ioCapability)
{
    blt_smpProp[slave_dev_ind + 1].IO_capability   = ioCapability;
    blt_smpProp[slave_dev_ind + 1].oob_enable      = OOB_en;
...
ble_sts_t blc_smp_setAclPeripheralParameters(u16 connHandle, smp_method_t expSmpMethod, bonding_mode_t bondable, bool keypress, ecdh_keys_mode_t mode)
{
    ...
    blt_smp_setSecurityParameters(blms_p_own_s,
                                  blt_smpProp[smp_prop_idx].bonding_mode,
                                  blt_smpProp[smp_prop_idx].pairing_method,
                                  blt_smpProp[smp_prop_idx].MITM_protection,
                                  blt_smpProp[smp_prop_idx].oob_enable,
                                  blt_smpProp[smp_prop_idx].IO_capability,
                                  blt_smpProp[smp_prop_idx].keyPress_en);
    ...
    #endif
#if DUAL_CORE_MODE_ENABLED
void blt_att_l2capSmpRxHandler(struct ble_host_conn *p_conn, uint16_t len, uint8_t *p_packet)
{
    uint8_t                tx_buffer[80];
    rf_packet_l2cap_req_t *ptrSMP = (rf_packet_l2cap_req_t *)tx_buffer;
    ptrSMP->l2capLen              = len;
    ptrSMP->chanId                = L2CAP_CID_SMP;
    memcpy(&ptrSMP->opcode, p_packet, len);

    u8 *pr = blt_smp_l2capSmpCmdHandler(p_conn->conn_handle, (u8 *)ptrSMP);

    if (pr) {
        ptrSMP = (rf_packet_l2cap_req_t *)pr;
        ble_host_smp_send_data_sync(p_conn, &ptrSMP->opcode, ptrSMP->l2capLen);
    }
}
#endif
#endif
