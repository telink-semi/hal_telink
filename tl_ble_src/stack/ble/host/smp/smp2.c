...
// refer to BLE Core Specification: Vol 3, Part H, "2.3.5.1 Selecting key generation method" for more information.
...
{
#if DUAL_CORE_MODE_ENABLED
    // remove this code, beacuse dual core mode not support this function
    (void) connHandle;
    return 0;
#else
...
#endif
    void
    blt_smp_procSlaveSecurityRequest(u16 connHandle)
{
#if !DUAL_CORE_MODE_ENABLED
    u8            conn_idx = (connHandle & CONN_IDX_MASK);
...
#else
    extern int ble_host_is_encryption_busy(u16 connHandle);
    if (!ble_host_is_encryption_busy(connHandle) && blc_ll_getConnectionStartTick(connHandle) &&
        clock_time_exceed(blc_ll_getConnectionStartTick(connHandle), blc_SecReq_ctrl.pending_ms * 1000))
#endif
    {
...
#endif
    void
    blt_smp_procCentralPairingRequest(u16 connHandle)
{
...
#endif
    int
    blc_smp_isPairingBusy(u16 connHandle) //this API may be used by user
{
...
#endif
    void
    blt_smp_setPairingBusy(u16 connHandle, u8 busy)
{
...
#endif
    void
    blt_smp_setCertTimeoutTick(u16 conn_handle, u32 t)
{
...
#endif
    void
    blt_smp_certTimeoutLoopEvt(u16 connHandle)
{
...
#endif
    void
    blt_smp_procPairingEnd(u16 connHandle, u8 err_reason) // 0 is OK; other for fail
{
    u8        conn_idx = connHandle & CONN_IDX_MASK;
    ...
    ble_host_set_encryption_busy(connHandle, 0);
#else
...
void blt_smp_saveBondingKey(u16 connHandle)
{
    ...
    if (blms_p_sts->bonding_enable) {
        ...
        app_parse_printf("[SMP][LTK] : %s", hex_to_str(smp_param_save.local_peer_ltk, 16));
    #endif

        tlkapi_send_string_data((stkLog_mask & STK_LOG_SMP_LTK), "[SMP][LTK] ", smp_param_save.local_peer_ltk, 16);
        ...
        smpMStblBondDevice.pairing_status[conn_idx] = (blms_p_own->stk_method == JustWorks) ? Unauthenticated_LTK : (blms_p_own->sc_pairing ? Authenticated_LTK_Secure_Connection : Authenticated_LTK_Legacy_Pairing);
