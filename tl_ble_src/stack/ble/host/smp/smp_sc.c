...
{
    if(!local_oob_data || !local_ecdh_key) return 0;
        ...
        switch(type)
        {
            ...
            case SMP_OP_PAIRING_DHKEY:
            {
                if(smp_master_role){
                    ...
                    blt_crypto_alg_f5 (blms_p_own->macKey, blms_p_own->own_ltk, smp_ecdh_key.sc_sk_dhk_own, blms_p_own->own_rand, blms_p_peer->peer_pairing_rand,
                    ...
                    blt_crypto_alg_f6 (confirm_ea, blms_p_own->macKey,  blms_p_own->own_rand, blms_p_peer->peer_pairing_rand,
                            rb, ioCapA, bd_addr_init, bd_addr_rsp);
        ...
        switch(req->opcode)  //smp4.2 or above
        {
                ...
                if(!smp_master_role){
                    ...
                    u32 flash_addr = blt_smp_searchBondingDevice_by_PeerMacAddr(smp_master_role, slave_dev_idx, blms_p_peer->peer_addr_type, blms_p_peer->peer_conn_addr);
                        if(flash_addr){
                            ...
                            u8 index = blt_smp_getBondingIndex_by_FlashAddr(smp_master_role, slave_dev_idx, flash_addr);
            ...
            case SMP_OP_PAIRING_DHKEY:
            {
                ...
                if(smp_master_role){
                    ...
                    blt_crypto_alg_f6 (confirm_eb, blms_p_own->macKey, blms_p_peer->peer_pairing_rand, blms_p_own->own_rand,
                            ra, ioCapB, bd_addr_rsp, bd_addr_init);
                    ...
                    //MacKey || LTK = f5(DHKey, Na, Nb,A,B)
                    blt_crypto_alg_f5 (blms_p_own->macKey, blms_p_own->own_ltk, smp_ecdh_key.sc_sk_dhk_own, blms_p_peer->peer_pairing_rand, blms_p_own->own_rand,
                            bd_addr_init, bd_addr_rsp);
                    swapN (blms_p_own->own_ltk, 16);
                    ...
                    //Ea = f6(MacKey,Na,Nb,rb,IOcapA,A,B)
                    blt_crypto_alg_f6 (confirm_ea, blms_p_own->macKey, blms_p_peer->peer_pairing_rand, blms_p_own->own_rand,
                            rb, ioCapA, bd_addr_init, bd_addr_rsp);
                    ...
                    else
                    {
                        ...
                        //Eb = f6(MacKey,Nb,Na,ra,IOcapB,B,A)
                        blt_crypto_alg_f6 (blms_p_peer->peer_confirm, blms_p_own->macKey, blms_p_own->own_rand, blms_p_peer->peer_pairing_rand,
                                ra, ioCapB, bd_addr_rsp, bd_addr_init);
