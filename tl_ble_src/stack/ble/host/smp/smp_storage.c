    ...
    if(sector_mark0 == U8_MAX && sector_mark1 == U8_MAX){  //first power on, choose area 0, write mark
        ...
        smp_param_next_start_addr = SMP_PARAM_NV_SEC_ADDR_START;    //kite_vulture-0x7a000;eagle-0xFC000: use area 1
            ...
            if(flash_prot_op_cb){
                flash_prot_op_cb(FLASH_OP_EVT_STACK_SMP_SELECT_PAIRING_INFO_AREA_BEGIN, \
                        smp_param_current_start_addr + FLASH_SMP_MARK_OFFSET, smp_param_current_start_addr + FLASH_SMP_MARK_OFFSET + 1);
            }
        #endif
        ...
        if(invalid_sector_mark != U8_MAX || sector_head != U32_MAX){
            /* special: use select paring info area event */
                ...
                if(flash_prot_op_cb){
                    flash_prot_op_cb(FLASH_OP_EVT_STACK_SMP_SELECT_PAIRING_INFO_AREA_BEGIN, \
                            smp_param_next_start_addr, smp_param_next_start_addr + SMP_PARAM_NV_MAX_LEN);
                }
            #endif
...
u32 blt_smp_searchBondingDevice_by_PeerMacAddr(u8 isMasterRole,u8 perDevIdx, u8 peer_addr_type, u8* peer_addr)
{
    ...
    if(isMasterRole){
        pFlashOffset = (u16*)&smpMStblBondDevice.master_bond_flash_idx[0];
...
u8 blt_smp_getBondingIndex_by_FlashAddr(u8 isCentral, u8 perDevIdx, u32 flash_addr)
{
    int CurBondNum = 0;
    u16 *pFlashOffset = 0;
...
u32 bls_smp_loadLTK_by_EdivRand(u8 perDevIdx, u16 ediv, u8* random, u8 *ltk)
{
    ...
    for(int i=0; i<CurBondNum; i++){
        ...
        if( (ediv == load_ediv) && !memcmp(random, load_rand, 8)){
            ...
            tlkapi_send_string_data(CS_IOP_EN || (stkLog_mask & STK_LOG_SMP_LTK), "[SMP][LTK] ", ltk, 16);
...
void blc_smp_eraseAllBondingInfo(void)
{
        ...
        if(flash_prot_op_cb){
            flash_prot_op_cb(FLASH_OP_EVT_STACK_SMP_DELETE_DEVICE_BEGIN, \
                        SMP_PARAM_NV_ADDR_START, SMP_PARAM_NV_ADDR_START + 2*SMP_PARAM_NV_MAX_LEN);
        }
    #endif
...
int blt_smp_deleteBondingInfo_by_Index(u8 isCentral, u8 perDevIdx, u8 index, bool smp_del_cb)
{
    ...
    if(isCentral){
        ...
        pCurBondNum = (u8*)&smpMStblBondDevice.slave_cur_bondNum[perDevIdx];
        pFlashOffset = (u16*)&smpMStblBondDevice.slave_bond_flash_idx[perDevIdx][0];
    }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
u32 blt_smp_updateBondingInfoToNearestByIndex(u8 isCentral, u8 perDevIdx, u8 index)
{
...
ble_sts_t    blc_smp_setCustomBondingInfoAddress(u16 connHandle, u8 cur_bondNum, u32 bond_flash_start_addr, u16* bond_flash_idx)
{
