...
u16 blt_atts_proc_exchangeMtuReq(attConCb_t *pAttrConCb, attr_pkt_t *attr, u16 attrLen)
{
    ...
    if(pAttrConCb->scid != L2CAP_CID_ATTR_PROTOCOL) { //For EATT channels
        return blt_att_packageErrorRsp(ATT_OP_EXCHANGE_MTU_REQ, ATT_HANDLE_NONE,
                    ATT_ERR_REQ_NOT_SUPPORTED, pAttrConCb->attTxBuff);
    }
...
u16 blt_atts_proc_findByTypeValueReq(attConCb_t *pAttrConCb, attr_pkt_t *attr, u16 attrLen)
{
    ...
    if(!err)
    {
        ...
        while(((handle = blt_atts_findUuidInRange(connHandle, handle, endHandle, ATT_16_UUID_LEN,
                                                attrType, &pAttr, &pGroup)) != ATT_HANDLE_NONE) && maxListCnt)
        {
...
u16 blt_atts_proc_readByTypeReq(attConCb_t *pAttrConCb, attr_pkt_t *attr, u16 attrLen)
{
    ...
    if(!err)
    {
        ...
        else if((err = blt_atts_permissions(connHandle, ATT_PERMISSIONS_READ,
        ...
        else
        {
            ...
            handle ++;
            while(((handle = blt_atts_findUuidInRange(connHandle, handle, endHandle, uuid_len,
                    pAttrData, &pAttr, &pGroup)) != ATT_HANDLE_NONE) && maxListCnt)
            {
                if(blt_atts_permissions(connHandle, ATT_PERMISSIONS_READ,
                                        handle, pAttr->perm) != ATT_SUCCESS)
                {
                    break;
                }
...
u16 blt_atts_proc_read(attConCb_t *pAttrConCb, u16 attrHandle, u16 valueOffset, u8 opcode)
{
    ...
    if(pAttr != NULL)
    {
        if((err = blt_atts_permissions(connHandle, ATT_PERMISSIONS_READ,
    ...
    if(!err)
    {
        ...
        else
        {
            ...
            while(maxListCnt)
            {
                ...
                if((handle = blt_atts_findUuidInRange(connHandle, handle, endHandle, uuid_len,
                        pAttrData, &pAttr, &pGroup)) == ATT_HANDLE_NONE)
                {
                    break;
                }
                if((*pAttr->attrValueLen == attLen) &&
                        (blt_atts_permissions(connHandle, ATT_PERMISSIONS_READ,
                                        handle, pAttr->perm) == ATT_SUCCESS))
                {
                    U16_TO_STREAM(rsp, handle);
                    handle = blt_atts_findServiceGroupEnd(connHandle, handle);
...
u16 blt_atts_proc_write(attConCb_t *pAttrConCb, u8 opcode, u8* pAttrData, u16 attrLen)
{
    ...
    if(!err && ((err = blt_atts_permissions(connHandle, ATT_PERMISSIONS_WRITE,
                                    attrHandle, pAttr->perm)) == ATT_SUCCESS))
    {
...
u16 blt_atts_proc_prepareWriteReq(attConCb_t *pAttrConCb, attr_pkt_t *attr, u16 attrLen)
{
    ...
    else if((err = blt_atts_permissions(connHandle, ATT_PERMISSIONS_WRITE,
                            attrHandle, pAttr->perm)) == ATT_SUCCESS)
    {
...
u16 blt_atts_proc_executeWriteReq(attConCb_t *pAttrConCb, attr_pkt_t *attr, u16 attrLen)
{
    ...
    if(att_pre_write.handle != 0xffff && !err && ((err = blt_atts_permissions(connHandle, ATT_PERMISSIONS_WRITE,
                                    handle, pAttr->perm)) == ATT_SUCCESS))
    {
        if(flags == ATT_EXEC_WRITE_ALL)
        {
...
u16 blt_atts_proc_handleValueCfm(attConCb_t *pAttrConCb, attr_pkt_t *attr, u16 attrLen)
{
    ...
    if(gap_eventMask & GAP_EVT_MASK_GATT_HANDLE_VALUE_CONFIRM){
            ...
            if (custom_darwin_fmn.darwin_fmn_enable) {
                blc_gap_send_event ( GAP_EVT_GATT_HANDLE_VALUE_CONFIRM, (u8 *)&pGap_ms_para->indicate_handle, 2);           //app_host_event_callback
            } else
        #endif
            {
                blc_gap_send_event ( GAP_EVT_GATT_HANDLE_VALUE_CONFIRM, NULL, 0 );          //app_host_event_callback
            }
    }
...
ble_sts_t blc_atts_sendErrResponse(u16 connHandle, u8 reqOpcode, u16 attHdlInErr, u8 ErrorCode)
{
    ...
    return blt_l2cap_pushData_2_controller(connHandle, L2CAP_CID_ATTR_PROTOCOL, &pAttTxPkt->opcode, 1,
                                            pAttTxPkt->data, pAttTxPkt->dataLen);
#else
    u8 format[1];
