...
/* ATT Client concerned */
        ...
        {ATT_OP_EXCHANGE_MTU_RSP, ATT_RESPONSE, sizeof(blt_attr_exchangeMtuRsp_t), blt_attc_proc_exchangeMtuRsp},
        {
            ...
            ATT_RESPONSE,
            sizeof(blt_attr_findInfoRsp_t),
            blt_attc_proc_findInfoRsp,
        },
...
void blt_att_l2capAttRxHandler1(struct ble_host_conn *p_conn, uint16_t len, uint8_t *p_packet)
{
    uint8_t    tx_buffer[512];
    attConCb_t attConCb = {
        .connHandle = p_conn->conn_handle,
        .scid = L2CAP_CID_ATTR_PROTOCOL,
        .attTxBuff = (attr_pkt_t *) tx_buffer,
    };

    u16 pduLen = blt_att_procAttrRxPkt(&attConCb, (attr_pkt_t *) p_packet, len);

    if (pduLen) {
        ble_host_att_send_packet_sync(p_conn, tx_buffer, pduLen);
    }
}
#endif
