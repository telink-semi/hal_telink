...
{
    if( ( (connHandle & BLM_CONN_HANDLE) && (!blc_att_setCentralRxMtuSize(mtu_size)) ) \
           || ( (connHandle & BLS_CONN_HANDLE) && (!blc_att_setPeripheralRxMtuSize(mtu_size)) ))
    {
...
void        blt_att_procMtuExgPending (u16 connHandle)
{
    ...
    if (!blt_ll_isEncryptionBusy(connHandle) && \
        ...
        mtuSizeExcFunc mtuSizeExcProc = pGap_ms_para->mtu_exg_pending == 1 ? \
                                        blc_att_requestMtuSizeExchange : blt_gattc_mtuSizeExchangeReq;
...
//  if(s >= gAttributes[0].attNum) return 0;
    if(s > gAttributes[0].attNum) return sh; // add by yuexin, review by qinghua
    eh = eh < gAttributes[0].attNum ? eh : gAttributes[0].attNum;
    ...
    while(s <= eh)
    {
        ...
        {
            if((*(u16*)pAtt->uuid== DECLARATIONS_UUID_CHARACTERISTIC) || (*(u16*)pAtt->uuid== DECLARATIONS_UUID_SECONDARY_SERVICE) \
                    || (*(u16*)pAtt->uuid== DECLARATIONS_UUID_PRIMARY_SERVICE))
            {
                return (s-1);
            }
        }
    ...
    while(sh <= eh)
    {
        ...
        if(uuid_match(2, pAtt->uuid, attUUID))// attribute type 2bytes
        {
            if(len<= pAtt->attrLen)
            {
                ...
                {
                    if((*(u16*)attUUID == DECLARATIONS_UUID_PRIMARY_SERVICE) || (*(u16*)attUUID == DECLARATIONS_UUID_SECONDARY_SERVICE)\
                            ||(*(u16*)attUUID == DECLARATIONS_UUID_CHARACTERISTIC))
                    {
...
//      pkt_errRsp.errReason = ATT_SUCCESS;
        ...
        if((sh!=0) && (sh<=eh))
        {
            if((attUUID==DECLARATIONS_UUID_PRIMARY_SERVICE) || (attUUID==DECLARATIONS_UUID_SECONDARY_SERVICE)\
                    ||(attUUID==DECLARATIONS_UUID_CHARACTERISTIC))
            {
                u16 total_pkt = 2;
    ...
    case ATT_OP_FIND_INFO_REQ: {
        ...
        u8 format = 1;
        u8 last_uuidLen = 0;
        if(eh > gAttributes[0].attNum) eh = gAttributes[0].attNum;
        while (sh <= eh ){
