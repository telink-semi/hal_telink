...
static int blt_gapc_getMsgPriority(void *node)
{
    ...
    if (elem->state == GAPC_MSG_ANALYSIS) {
        return GAPC_MSG_NONE;
    }
...
static ble_sts_t blt_gapc_creatDiscServiceMsg(blt_gapc_discovery_t *disc)
{
    ...
    return blt_gapc_createSdpMsg(disc, (uuid_t *)(size_t)&disc->list->service->uuid, ATT_HANDLE_START, ATT_HANDLE_MAX, GATT_DISCOVER_PRIMARY, 0, blt_gapc_discServiceCb);
}
...
static u8 blt_gapc_discIncludeCb(u16 connHandle, gatt_attr_t *attr, gattc_sdp_cfg_t *params)
{
    ...
    BLT_GAPC_DEBUG("    found include service, count:%d, UUID: 0x%s, startHandle:%x endHandle:%x", gapDisc->includeSize, uuidFormat(&incl->uuid), incl->startHdl, incl->endHdl);
...
static ble_sts_t blt_gapc_createDiscIncludeMsg(blt_gapc_discovery_t *disc, u16 startHdl, u16 endHdl)
{
    ...
    return blt_gapc_createSdpMsg(disc, NULL, startHdl, endHdl, GATT_DISCOVER_INCLUDE, 0, blt_gapc_discIncludeCb);
}
...
static u8 blt_gapc_discCharCb(u16 connHandle, gatt_attr_t *attr, gattc_sdp_cfg_t *params)
{
    ...
    if (attr == NULL) {
        blt_gapc_freeGapMsgHeader(connHandle);
        if ((gapDisc->lastChar.handle != ATT_HANDLE_NONE && gapDisc->lastChar.handle <= params->endHdl) && (gapDisc->lastChar.subscribeNtf || gapDisc->lastChar.subscribeInd || gapDisc->lastChar.findDecs)) {
    ...
    BLT_GAPC_DEBUG("found characteristic UUID:0x%s, Properties:%x ValueHandle:%x", uuidFormat(&chrc->uuid), chrc->properties, chrc->valueHdl);
...
static ble_sts_t blt_gapc_createDiscChar(blt_gapc_discovery_t *disc, u16 startHdl, u16 endHdl)
{
    ...
    return blt_gapc_createSdpMsg(disc, NULL, startHdl, endHdl, GATT_DISCOVER_CHARACTERISTIC, 0, blt_gapc_discCharCb);
}
...
static ble_sts_t blt_gapc_createDiscDescriptors(blt_gapc_discovery_t *disc, u16 endHdl)
{
    BLT_GAPC_DEBUG("discovery descriptor UUID is 0x%s, startHdl:%x endHdl:%x properties:%x", uuidFormat(disc->lastChar.uuid), disc->lastChar.handle, endHdl, disc->lastChar.properties);

    return blt_gapc_createSdpMsg(disc, disc->lastChar.uuid, disc->lastChar.handle, endHdl, GATT_DISCOVER_DESCRIPTOR, disc->lastChar.properties, blt_gapc_discDescriptorsCb);
}
...
static void blt_gapc_subscribeCccCb(u16 connHandle, u8 err, struct gattc_sub_ccc_cfg *params)
{
    ...
    if (characteristic && characteristic->scfun) {
        characteristic->scfun(connHandle, params->valueHdl, err);
    }
...
static ble_sts_t blt_gapc_createDiscInclChar(blt_gapc_discovery_t *disc, blt_gapc_discAttrInfo_t *info)
{
    ...
    return blt_gapc_createSdpMsg(disc, info->uuid, info->startHandle, info->endHandle, GATT_DISCOVER_CHARACTERISTIC, 0, blt_gapc_discInclCharCb);
}
...
static void blt_gapc_sdpReconnDealChar(u16 connHandle, const blc_gapc_reconnChar_t *reconnChar)
{
    ...
    blc_gapc_charInfo_t charInfo[20] = {0};
    int                 charInfoCnt = 0;
...
void blc_gapc_discoveryOrReconnectService_loop(void)
{
    for (int i = 0; i < GAPC_DISCOVERY_MAX_NUM; i++) {
        if (gGapcDisc[i].state != DISCOVERY_STATE_IDLE) {
            ...
            if(ble_host_conn_is_connected(connHandle)==false)
#else
