        ...
        if (ptrL2cap->chanId == L2CAP_CID_ATTR_PROTOCOL) {                                       //CID ==> ATT
            ...
            if (opcode == ATT_OP_EXCHANGE_MTU_REQ || opcode == ATT_OP_EXCHANGE_MTU_RSP) {        //att opcode m/s role common process
                ...
                if (opcode == ATT_OP_EXCHANGE_MTU_REQ) {
                    rf_packet_att_mtu_t pkt_mtu_rsp = {
  //  spec 4.1 ,  3.4.7.1 Handle Value Notification
                        0x02, // type
...
#endif
    ble_sts_t
    blt_l2cap_pushData_2_controller(u16 connHandle, u16 cid, u8 *format, int format_len, u8 *pDate, int data_len)
{
#if DUAL_CORE_MODE_ENABLED
    (void)format;
    (void)format_len;
    tlkapi_printf(1,"blt_l2cap_pushData_2_controller");
    u8 buffer[255];
    memcpy(buffer, format, format_len);
    memcpy(buffer + format_len, pDate, data_len);
    extern int ble_host_send_att_package(uint16_t conn_handle, uint16_t cid, const uint8_t *data, uint16_t data_len);
    return ble_host_send_att_package(connHandle,cid,buffer,format_len + data_len);
#else
#if L2CAP_DATA_2_HCI_DATA_BUFFER_ENABLE
...
#endif
    u8
    blt_UpdateParameter_request(u16 connHandle)
{
    if (connHandle & BLS_CONN_HANDLE) {
        gap_s_para_t *pGap_s_para = bls_gap_getSlavePara(connHandle);
...
#endif
    void
    blt_l2cap_processConnParamUpdateReq(u16 connHandle, host_acl_ms_t *pHostAclms)
{
    //if(pHostAclms->l2cap_connParaUpReq_pending)
    {
        ...
        if (BLE_SUCCESS == blc_ll_updateConnection(connHandle, pHostAclms->l2cap_connParaUpdateReq_minInterval, pHostAclms->l2cap_connParaUpdateReq_maxInterval, conn_latency, pHostAclms->l2cap_connParaUpdateReq_timeout, 0, 0xFFFF)) {
